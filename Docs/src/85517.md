...
调用 [setRoomExtraInfo](85516.md#IRTCRoom-setroomextrainfo) 设置房间附加信息结果的回调。
...
| errCode | **SetRoomExtraInfoResult** | 设置房间附加信息的结果，详见 [SetRoomExtraInfoResult](85519.md#setroomextrainforesult) |
...
接收同一房间内，其他用户调用 [setRoomExtraInfo](85516.md#IRTCRoom-setroomextrainfo) 设置的房间附加信息的回调。
...
```
字幕状态发生改变回调。 <br>
当用户调用 [startSubtitle](85516.md#IRTCRoom-startsubtitle) 和 [stopSubtitle](85516.md#IRTCRoom-stopsubtitle) 使字幕状态发生改变或出现错误时，触发该回调。

**传入参数**

| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | **SubtitleState** | 字幕状态。参看 [SubtitleState](85519.md#subtitlestate)。 |
| error_code | **SubtitleErrorCode** | 字幕任务错误码。参看 [SubtitleErrorCode](85519.md#subtitleerrorcode)。 |
...
```
字幕相关内容回调。 <br>
当用户调用 [startSubtitle](85516.md#IRTCRoom-startsubtitle) 后会收到此回调，通知字幕的相关信息。

**传入参数**

| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| subtitles | **const SubtitleMessage*** | 字幕消息内容。参看 [SubtitleMessage](85519.md#subtitlemessage)。 |
| cnt | **int** | 字幕消息个数。 |
...
| source_type | **DataMessageSourceType** | SEI 消息类型，自 3.52.1 版本后固定为 `0`，自定义消息。参看 [DataMessageSourceType](85519.md#datamessagesourcetype)。 |
...
回调公共流中包含的数据信息。
通过 [startPlayPublicStream](85516.md#IRTCVideo-startplaypublicstream) 开始播放公共流后，可以通过本回调获取发送端发送的非SEI消息。
...
| source_type | **DataMessageSourceType** | 数据消息来源，参看 [DataMessageSourceType](85519.md#datamessagesourcetype)。 |
...
```
本地代理状态发生改变回调。调用 [setLocalProxy](85516.md#IRTCVideo-setlocalproxy) 设置本地代理后，SDK 会触发此回调，返回代理连接的状态。  <br>

**传入参数**

| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| local_proxy_type | **LocalProxyType** | 本地代理类型。参看 [LocalProxyType](85519.md#localproxytype) 。 |
| local_proxy_state | **LocalProxyState** | 本地代理状态。参看 [LocalProxyState](85519.md#localproxystate)。 |
| local_proxy_error | **LocalProxyError** | 本地代理错误。参看 [LocalProxyError](85519.md#localproxyerror)。 |
