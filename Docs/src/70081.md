...
当用户调用 [startSubtitle](70080#RTCRoom-startsubtitle) 和 [stopSubtitle](70080#RTCRoom-stopsubtitle) 使字幕状态发生改变或字幕任务出现错误时，触发该回调。
...
调用 [openWithCustomSource](70080#IMediaPlayer-openwithcustomsource) 接口播放用户传入的内存音频数据时，会触发此回调，用户需要写入音频数据。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| remoteStreamKey | RemoteStreamKey | 远端流信息，参看 [RemoteStreamKey](70083#RemoteStreamKey) |
| frameInfo | VideoFrameInfo | 视频帧信息，参看 [VideoFrameInfo](70083#VideoFrameInfo) |
...
| --- | --- | --- |
| streamIndex | StreamIndex | 流属性，参看 [StreamIndex](70083#streamindex-2) |
| frameInfo | VideoFrameInfo | 视频信息，参看 [VideoFrameInfo](70083#VideoFrameInfo) |


**注意**

对于采集到的本地视频帧，你可以调用 [setLocalVideoCanvas](70080#RTCVideo-setlocalvideocanvas) 或 [setLocalVideoSink](70080#RTCVideo-setlocalvideosink) 在本地渲染。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | int | <br>当前 SDK 与信令服务器连接状态。 详细定义参见 [ConnectionState](70083#ConnectionState) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| route | AudioRoute | 新的音频播放路由，详见 [AudioRoute](70083#AudioRoute) |


**注意**

插拔音频外设，或调用 [setAudioRoute](70080#RTCVideo-setaudioroute) 都可能触发音频路由切换，详见[移动端设置音频路由](https://www.volcengine.com/docs/6348/117836) 。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamIndex | StreamIndex | 音频流属性, 详见 [StreamIndex](70083#streamindex-2) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| remoteStreamKey | RemoteStreamKey | 远端音频流信息, 详见 [RemoteStreamKey](70083#RemoteStreamKey) |
...
```
收到通过调用 [sendSEIMessage](70080#RTCVideo-sendseimessage) 发送带有 SEI 消息的视频帧时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| remoteStreamKey | RemoteStreamKey | 包含 SEI 发送者的用户名，所在的房间名和媒体流，详见 [RemoteStreamKey](70083#RemoteStreamKey) |
...
在语音通话场景下，本地用户调用 [sendSEIMessage](70080#RTCVideo-sendseimessage) 通过黑帧视频流发送 SEI 数据时，流的发送状态会通过该回调通知远端用户。
...
| --- | --- | --- |
| remoteStreamKey | RemoteStreamKey | 远端流信息，参看 [RemoteStreamKey](70083#RemoteStreamKey)。 |
| event | SEIStreamUpdateEvent | 黑帧视频流状态，参看 [SEIStreamUpdateEvent](70083#SEIStreamUpdateEvent) |
...
| errorCode | int | <br>登录结果<br>详见 [LoginErrorCode](70082#LoginErrorCode)。 |
| elapsed | int | <br>从调用 [login](70080#RTCVideo-login) 接口开始到返回结果所用时长。<br>单位为 ms。 |


**注意**

调用 [login](70080#RTCVideo-login) 后，会收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| reason | int | 用户登出的原因，参看 [LogoutReason](70083#LogoutReason) |


**注意**

在以下两种情况下会收到此回调：调用 [logout](70080#RTCVideo-logout) 接口主动退出；或其他用户以相同 UserId 进行 `login` 导致本地用户被动登出。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| error | int | <br>设置结果<br><ul><li>返回 200，设置成功</li><li>返回其他，设置失败，详见 [UserMessageSendResult](70082#UserMessageSendResult) </li></ul> |


**注意**

调用 [setServerParams](70080#RTCVideo-setserverparams) 后，会收到此回调。
...
| status | int | <br>查询的用户登录状态<br>详见 [UserOnlineStatus](70083#UserOnlineStatus). |


**注意**

必须先调用 [getPeerOnlineStatus](70080#RTCVideo-getpeeronlinestatus)，才能收到此回调。
...
```
收到房间外用户调用 [sendUserMessageOutsideRoom](70080#RTCVideo-sendusermessageoutsideroom) 发来的文本消息时，会收到此回调
...
```
收到房间外用户调用 [sendUserBinaryMessageOutsideRoom](70080#RTCVideo-senduserbinarymessageoutsideroom) 发来的二进制消息时，会收到此回调
...
| error | int | <br>消息发送结果<br>详见 [UserMessageSendResult](70082#UserMessageSendResult)。 |


**注意**

当调用 [sendUserMessageOutsideRoom](70080#RTCVideo-sendusermessageoutsideroom) 或 [sendUserBinaryMessageOutsideRoom](70080#RTCVideo-senduserbinarymessageoutsideroom) 发送消息后，会收到此回调。
...
| error | int | 消息发送结果，详见 [UserMessageSendResult](70082#UserMessageSendResult)。 |
| message | ByteBuffer | 应用服务器收到 HTTP 请求后，在 ACK 中返回的信息。消息不超过 64 KB。 |


**注意**

本回调为异步回调。当调用 [sendServerMessage](70080#RTCVideo-sendservermessage) 或 [sendServerBinaryMessage](70080#RTCVideo-sendserverbinarymessage) 接口发送消息后，会收到此回调。
...
成功调用 [startNetworkDetection](70080#RTCVideo-startnetworkdetection) 接口开始探测后，会在 3s 内首次收到该回调，之后每 2s 收到一次该回调。
...
| quality | int | 探测网络的质量，参看 [NetworkQuality](70083#NetworkQuality)。 |
...
1. 当调用 [stopNetworkDetection](70080#RTCVideo-stopnetworkdetection) 接口停止探测后，会收到一次该回调；
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| reason | NetworkDetectionStopReason | <br>停止探测的原因类型,参考 [NetworkDetectionStopReason](70083#NetworkDetectionStopReason) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| event | RemoteStreamSwitch | 音视频流发生变化的信息。参看 [RemoteStreamSwitch](70083#RemoteStreamSwitch)。 |
...
```
本地未通过 [setPublishFallbackOption](70080#RTCVideo-setpublishfallbackoption) 开启发布性能回退，检测到设备性能不足时，收到此回调。

本地通过 [setPublishFallbackOption](70080#RTCVideo-setpublishfallbackoption) 开启发布性能回退，因设备性能/网络原因，造成发布性能回退/恢复时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| mode | PerformanceAlarmMode | 指示本地是否开启发布回退功能。参看 [PerformanceAlarmMode](70083#PerformanceAlarmMode)<br><ul><li>当发布端未开启发布性能回退时，mode 值为 NORMAL。</li><li>当发布端开启发布性能回退时，mode 值为 SIMULCAST。</li></ul> |
| roomId | String | <ul><li>未开启发布性能回退时，roomId 为空</li><li>开启发布性能回退时，roomId 是告警影响的房间 ID。</li></ul> |
| reason | PerformanceAlarmReason | 告警原因，参看 [PerformanceAlarmReason](70083#PerformanceAlarmReason) |
| data | SourceWantedData | 性能回退相关数据，详见 [SourceWantedData](70083#SourceWantedData)。 |
...
| user | RtcUser | 本地用户信息，详见 [RtcUser](70083#RtcUser) |
| state | FirstFrameSendState | 首帧发送状态，详见 [FirstFrameSendState](70083#FirstFrameSendState) |
...
| --- | --- | --- |
| roomId | String | 音频发布用户所在的房间 ID |
| user | RtcUser | 远端用户信息，详见 [RtcUser](70083#RtcUser) |
| state | FirstFramePlayState | 首帧播放状态，详见 [FirstFramePlayState](70083#FirstFramePlayState) |
...
| deviceType | AudioDeviceType | 设备类型，详见 [AudioDeviceType](70083#AudioDeviceType)。 |
| deviceState | int | 设备状态，详见 [MediaDeviceState](70083#MediaDeviceState)。 |
| deviceError | int | 设备错误类型，详见 [MediaDeviceError](70083#MediaDeviceError)。 |
...
| --- | --- | --- |
| deviceID | String | 设备 ID |
| deviceType | VideoDeviceType | 设备类型，详见 [VideoDeviceType](70083#VideoDeviceType)。 |
| deviceState | int | 设备状态，详见 [MediaDeviceState](70083#MediaDeviceState)。 |
| deviceError | int | 设备错误类型，详见 [MediaDeviceError](70083#MediaDeviceError)。 |
...
| --- | --- | --- |
| deviceID | String | 设备 ID |
| deviceType | AudioDeviceType | 参看 [AudioDeviceType](70083#AudioDeviceType) |
| deviceWarning | int | 参看 [MediaDeviceWarning](70083#MediaDeviceWarning) |
...
| --- | --- | --- |
| deviceID | String | 设备 ID |
| deviceType | VideoDeviceType | 参看 [VideoDeviceType](70083#VideoDeviceType) |
| deviceWarning | int | 参看 [MediaDeviceWarning](70083#MediaDeviceWarning) |
...
该回调由 [startFileRecording](70080#RTCVideo-startfilerecording) 或 [stopFileRecording](70080#RTCVideo-stopfilerecording) 触发。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | StreamIndex | 录制流的流属性，参看 [StreamIndex](70083#streamindex-2) |
| state | RecordingState | 录制状态，参看 [RecordingState](70083#RecordingState) |
| errorCode | RecordingErrorCode | 录制错误码，参看 [RecordingErrorCode](70083#RecordingErrorCode) |
| info | RecordingInfo | 录制文件的详细信息，参看 [RecordingInfo](70083#RecordingInfo) |
...
该回调由 [startFileRecording](70080#RTCVideo-startfilerecording) 触发，录制状态正常时，系统每秒钟都会通过该回调提示录制进度。
...
| --- | --- | --- |
| type | StreamIndex | 录制流的流属性，参看 [StreamIndex](70083#streamindex-2) |
| progress | RecordingProgress | 录制进度，参看 [RecordingProgress](70083#RecordingProgress) |
| info | RecordingInfo | 录制文件的详细信息，参看 [RecordingInfo](70083#RecordingInfo) |
...
```
调用 [startAudioRecording](70080#RTCVideo-startaudiorecording) 或 [stopAudioRecording](70080#RTCVideo-stopaudiorecording) 改变音频文件录制状态时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | AudioRecordingState | 录制状态，参看 [AudioRecordingState](70083#AudioRecordingState) |
| errorCode | AudioRecordingErrorCode | 录制错误码，参看 [AudioRecordingErrorCode](70082#AudioRecordingErrorCode) |
...
调用 [setAudioMixingProgressInterval](70080#IAudioMixingManager-setaudiomixingprogressinterval) 将时间间隔设为大于 0 的值后，或调用 [startAudioMixing](70080#IAudioMixingManager-startaudiomixing) 将 [AudioMixingConfig](70083#AudioMixingConfig) 中的时间间隔设为大于 0 的值后，SDK 会按照设置的时间间隔回调该事件。
...
```
调用 [enableAudioPropertiesReport](70080#RTCVideo-enableaudiopropertiesreport) 后，你会周期性地收到此回调，了解本地音频的相关信息。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audioPropertiesInfos | LocalAudioPropertiesInfo[] | 本地音频信息，详见 [LocalAudioPropertiesInfo](70083#LocalAudioPropertiesInfo)。 |
...
调用 [startAudioPlaybackDeviceTest](70080#IRTCAudioDeviceManager-startaudioplaybackdevicetest) 或 [startAudioDeviceRecordTest](70080#IRTCAudioDeviceManager-startaudiodevicerecordtest)，开始播放音频文件或录音时，将开启该回调。本回调为周期性回调，回调周期由上述接口的 `interval` 参数指定。
...
```
远端用户进房后，本地调用 [enableAudioPropertiesReport](70080#RTCVideo-enableaudiopropertiesreport) ，根据设置的 interval 值，本地会周期性地收到此回调，了解订阅的远端用户的瞬时音频信息。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audioPropertiesInfos | RemoteAudioPropertiesInfo[] | 远端音频信息，其中包含音频流属性、房间 ID、用户 ID ，详见 [RemoteAudioPropertiesInfo](70083#RemoteAudioPropertiesInfo)。 |
...
```
调用 [enableAudioPropertiesReport](70080#RTCVideo-enableaudiopropertiesreport) 后，根据设置的 `AudioPropertiesConfig.interval`，你会周期性地收到此回调，获取房间内的最活跃用户信息。
...
```
音频流同步信息回调。可以通过此回调，在远端用户调用 [sendStreamSyncInfo](70080#RTCVideo-sendstreamsyncinfo) 发送音频流同步消息后，收到远端发送的音频流同步信息。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamKey | RemoteStreamKey | 远端流信息，详见 [RemoteStreamKey](70083#RemoteStreamKey)。 |
| streamType | StreamSycnInfoConfig.SyncInfoStreamType | 媒体流类型，详见 [SyncInfoStreamType](70083#SyncInfoStreamType)。 |
...
调用 [startPushPublicStream](70080#RTCVideo-startpushpublicstream) 接口发布公共流后，启动结果通过此回调方法通知用户。
...
| error | PublicStreamErrorCode | 公共流发布结果状态码。详见 [PublicStreamErrorCode](70082#PublicStreamErrorCode)。 |
...
调用 [startPlayPublicStream](70080#RTCVideo-startplaypublicstream) 接口启动拉公共流功能后，通过此回调收到启动结果和拉流过程中的错误。
...
| --- | --- | --- |
| publicStreamId | String | 公共流 ID。 |
| error | PublicStreamErrorCode | 公共流订阅结果状态码。详见 [PublicStreamErrorCode](70082#PublicStreamErrorCode)。 |
...
调用 [startPlayPublicStream](70080#RTCVideo-startplaypublicstream) 接口启动拉公共流功能后，通过此回调收到公共流中的 SEI 消息。
...
| --- | --- | --- |
| publicStreamId | String | 公共流 ID。 |
| message | ByteBuffer | 收到的 SEI 消息内容。<br>通过调用客户端 [sendSEIMessage](70080#RTCVideo-sendseimessage-2) 插入的 SEI 信息。<br>当公共流中的多路视频流均包含有 SEI 信息：SEI 不互相冲突时，将通过多次回调分别发送；SEI 在同一帧有冲突时，则只有一条流中的 SEI 信息被透传并融合到公共流中。 |
| sourceType | DataMessageSourceType | SEI 消息类型，自 3.52.1 版本后固定为 `0`，自定义消息。参看 [DataMessageSourceType](70083#DataMessageSourceType)。 |
...
调用 [startPlayPublicStream](70080#RTCVideo-startplaypublicstream) 订阅公共流后，通过监听本回调获取公共流中的数据消息，包括调用 Open API 发送的 SEI 消息和音量回调。
...
| sourceType | DataMessageSourceType | 数据消息来源，参看 [DataMessageSourceType](70083#DataMessageSourceType)。 |
...
关于订阅公共流，详见 [startPlayPublicStream](70080#RTCVideo-startplaypublicstream)。
...
| --- | --- | --- |
| publicStreamId | String | 公共流 ID |
| videoFrameInfo | VideoFrameInfo | 视频帧信息，参看 [VideoFrameInfo](70083#VideoFrameInfo) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| result | EchoTestResult | 测试结果，参看 [EchoTestResult](70083#EchoTestResult) |
...
```
调用 [startCloudProxy](70080#RTCVideo-startcloudproxy) 开启云代理，SDK 首次成功连接云代理服务器时，回调此事件。
...
```
首次调用 [getNetworkTimeInfo](70080#RTCVideo-getnetworktimeinfo) 后，SDK 内部启动网络时间同步，同步完成时会触发此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| hardwareEchoDetectionResult | HardwareEchoDetectionResult | 参见 [HardwareEchoDetectionResult](70083#HardwareEchoDetectionResult) |


**注意**

- 通话前调用 [startHardwareEchoDetection](70080#RTCVideo-starthardwareechodetection) 后，将触发本回调返回检测结果。
- 建议在收到检测结果后，调用 [stopHardwareEchoDetection](70080#RTCVideo-stophardwareechodetection) 停止检测，释放对音频设备的占用。
...
```
本地代理状态发生改变回调。调用 [setLocalProxy](70080#RTCVideo-setlocalproxy) 设置本地代理后，SDK 会触发此回调，通知代理连接的状态。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| localProxyType | LocalProxyType | 本地代理类型。参看 [LocalProxyType](70083#LocalProxyType)。 |
| localProxyState | LocalProxyState | 本地代理状态。参看 [LocalProxyState](70083#LocalProxyState)。 |
| localProxyError | LocalProxyError | 本地代理错误。参看 [LocalProxyError](70083#LocalProxyError)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| info | FrameUpdateInfo | RTC 智能决策后合适的帧率和分辨率积（宽*高）。参看 [FrameUpdateInfo](70083#FrameUpdateInfo)。 |
...
| muteState | MuteState | 发送状态，参看 [MuteState](70083#MuteState) |
...
| muteState | MuteState | 视频流的发送状态。参看 [MuteState](70083#MuteState)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| device | AudioPlaybackDevice | 新的音频设备，详见 [AudioPlaybackDevice](70083#AudioPlaybackDevice) |


**注意**

音频设备设置与音频路由策略，详见 [setAudioPlaybackDevice](70080#RTCVideo-setaudioplaybackdevice)
...
| --- | --- | --- |
| deviceID | String | 设备 ID |
| deviceType | int | 设备类型，详见 [MediaDeviceType](70083#MediaDeviceType)。 |
| deviceState | int | 设备状态，详见 [MediaDeviceState](70083#MediaDeviceState)。 |
| deviceError | int | 设备错误类型，详见 [MediaDeviceError](70083#MediaDeviceError)。 |
...
| --- | --- | --- |
| deviceID | String | 设备 ID |
| deviceType | int | 参看 [MediaDeviceType](70083#MediaDeviceType) |
| deviceWarning | int | 参看 [MediaDeviceWarning](70083#MediaDeviceWarning) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| mixId | int | <br>混音 ID<br>使用 [IAudioMixingManager](70080#IAudioMixingManager) 相关接口时传入的唯一 ID。 |
| state | AudioMixingState | <br>混音状态<br>其混音状态可参考： [AudioMixingState](70083#AudioMixingState)。 |
| error | AudioMixingError | <br>错误码<br>详见 [AudioMixingError](70083#AudioMixingError)。 |


**注意**

此回调会被触发的时机汇总如下：
- 当调用 [startAudioMixing](70080#IAudioMixingManager-startaudiomixing) 方法成功后，会触发 state 值为 AUDIO_MIXING_STATE_PLAYING 回调；否则触发 state 值为 AUDIO_MIXING_STATE_FAILED 的回调。
- 当使用相同的 ID 重复调用 [startAudioMixing](70080#IAudioMixingManager-startaudiomixing) 后，后一次会覆盖前一次，且本回调会以 AUDIO_MIXING_STATE_STOPPED 通知前一次混音已停止。
- 当调用 [pauseAudioMixing](70080#IAudioMixingManager-pauseaudiomixing) 方法暂停播放成功后， 会触发 state 值为 AUDIO_MIXING_STATE_PAUSED 回调；否则触发 state 值为 AUDIO_MIXING_STATE_FAILED 的回调。
- 当调用 [resumeAudioMixing](70080#IAudioMixingManager-resumeaudiomixing) 方法恢复播放成功后，会触发 state 值为 AUDIO_MIXING_STATE_PLAYING 回调；否则触发 state 值为 AUDIO_MIXING_STATE_FAILED 的回调。
- 当调用 [stopAudioMixing](70080#IAudioMixingManager-stopaudiomixing) 方法暂停止播放成功后， 会触发 state 值为 AUDIO_MIXING_STATE_STOPPED 回调；否则触发 state 值为 AUDIO_MIXING_STATE_FAILED 的回调。
...
```
调用 [takeLocalSnapshot](70080#RTCVideo-takelocalsnapshot) 截取视频画面时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| taskId | long | 本地截图任务的编号。和 [takeLocalSnapshot](70080#RTCVideo-takelocalsnapshot) 的返回值一致。 |
| streamIndex | StreamIndex | 截图的视频流的属性，参看 [StreamIndex](70083#StreamIndex)。 |
...
```
调用 [takeRemoteSnapshot](70080#RTCVideo-takeremotesnapshot) 截取视频画面时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| taskId | long | 远端截图任务的编号。和 [takeRemoteSnapshot](70080#RTCVideo-takeremotesnapshot) 的返回值一致。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audioFrame | IAudioFrame | 音频帧地址，参看 [IAudioFrame](70083#IAudioFrame)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamKey | RemoteStreamKey | 音频流信息，参看 [RemoteStreamKey](70083#RemoteStreamKey) |
| audioFrame | IAudioFrame | 音频帧地址，参看 [IAudioFrame](70083#IAudioFrame) |
