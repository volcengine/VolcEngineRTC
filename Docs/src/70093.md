...
房间内其他用户调用 [sendRoomBinaryMessage:](70092#ByteRTCRoom-sendroombinarymessage) 发送的广播二进制消息时，收到此回调。
...
当用户调用 [startSubtitle:](70092#ByteRTCRoom-startsubtitle) 和 [stopSubtitle](70092#ByteRTCRoom-stopsubtitle) 使字幕状态发生改变或出现错误时，触发该回调。
...
| code | ByteRTCWarningCode | 警告代码，参看 [ByteRTCWarningCode](70091#ByteRTCWarningCode)。 |
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象。 |
| errorCode | ByteRTCErrorCode | 错误代码，参看 [ByteRTCErrorCode](70091#ByteRTCErrorCode)。 |
...
| engine | ByteRTCVideo * | `ByteRTCVideo` 实例 |
| roomId | NSString * | 房间 ID。 |
| errorCode | NSInteger | 创建房间错误码，具体原因参看 [ByteRTCErrorCode](70091#ByteRTCErrorCode)。 |
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| state | ByteRTCConnectionState | 当前 SDK 与信令服务器的连接状态，详见 [ByteRTCConnectionState](70089#ByteRTCConnectionState)。 |
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| type | ByteRTCNetworkType | SDK 当前的网络连接类型，详见 [ByteRTCNetworkType](70089#ByteRTCNetworkType) |
...
```
房间内的可见用户调用 [startAudioCapture](70092#ByteRTCVideo-startaudiocapture) 开启音频采集时，房间内其他用户会收到此回调。
...
- (void)rtcEngine:(ByteRTCVideo * _Nonnull)engine onUserStopAudioCapture:(NSString* _Nonnull)roomId
              uid:(NSString *_Nonnull)userId;
```
房间内的可见用户调用 [stopAudioCapture](70092#ByteRTCVideo-stopaudiocapture) 关闭音频采集时，房间内其他用户会收到此回调。
...
```
调用 [enableAudioPropertiesReport:](70092#ByteRTCVideo-enableaudiopropertiesreport) 后，根据设置的 interval 值，你会周期性地收到此回调，了解本地音频的相关信息。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| audioPropertiesInfos | NSArray<ByteRTCLocalAudioPropertiesInfo *\> * | 本地音频信息，详见 [ByteRTCLocalAudioPropertiesInfo](70089#ByteRTCLocalAudioPropertiesInfo) 。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| engine | ByteRTCVideo * | 参看 [ByteRTCVideo](70092#ByteRTCVideo)。 |
| volume | int | 音频设备测试播放音量。取值范围：[0,255] |


**注意**

调用 [startAudioPlaybackDeviceTest:interval:](70092#ByteRTCAudioDeviceManager-startaudioplaybackdevicetest-interval) 或 [startAudioDeviceRecordTest:](70092#ByteRTCAudioDeviceManager-startaudiodevicerecordtest)，开始播放音频文件或录音时，将开启该回调。本回调为周期性回调，回调周期由上述接口的 `interval` 参数指定。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | 参看 [ByteRTCVideo](70092#ByteRTCVideo) |
| deviceType | ByteRTCAudioDeviceType | 设备类型，包括麦克风和扬声器，参阅 [ByteRTCAudioDeviceType](70089#ByteRTCAudioDeviceType)。 |
...
```
远端用户进房后，本地调用 [enableAudioPropertiesReport:](70092#ByteRTCVideo-enableaudiopropertiesreport) ，根据设置的 interval 值，本地会周期性地收到此回调，了解订阅的远端用户的瞬时音频信息。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| audioPropertiesInfos | NSArray<ByteRTCRemoteAudioPropertiesInfo *\> * | 远端音频信息，其中包含音频流属性、房间 ID、用户 ID ，详见 [ByteRTCRemoteAudioPropertiesInfo](70089#ByteRTCRemoteAudioPropertiesInfo)。 |
...
```
调用 [enableAudioPropertiesReport:](70092#ByteRTCVideo-enableaudiopropertiesreport) 后，根据设置的 `config.interval`，你会周期性地收到此回调，获取房间内的最活跃用户信息。
...
```
房间内的可见用户调用 [startVideoCapture](70092#ByteRTCVideo-startvideocapture) 开启内部视频采集时，房间中其他用户会收到此回调。
...
| uid | NSString * | 调用 [startVideoCapture](70092#ByteRTCVideo-startvideocapture) 接口的远端用户 ID |
...
- (void)rtcEngine:(ByteRTCVideo * _Nonnull)engine onUserStopVideoCapture:(NSString * _Nonnull)roomId
              uid:(NSString * _Nonnull)uid;
```
房间内的可见用户调用 [stopVideoCapture](70092#ByteRTCVideo-stopvideocapture) 关闭内部视频采集时，房间内其他用户会收到此回调。
...
| frameInfo | ByteRTCVideoFrameInfo * | 视频帧信息，参看 [ByteRTCVideoFrameInfo](70089#ByteRTCVideoFrameInfo) |
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| streamKey | ByteRTCRemoteStreamKey * | 流属性，参看 [ByteRTCRemoteStreamKey](70089#ByteRTCRemoteStreamKey) |
| frameInfo | ByteRTCVideoFrameInfo * | 视频帧信息，参看 [ByteRTCVideoFrameInfo](70089#ByteRTCVideoFrameInfo) |
...
| deviceType | ByteRTCAudioDeviceType | 设备类型，参看 [ByteRTCAudioDeviceType](70089#ByteRTCAudioDeviceType)。 |
| deviceState | ByteRTCMediaDeviceState | 设备状态，参看 [ByteRTCMediaDeviceState](70089#ByteRTCMediaDeviceState)。 |
| deviceError | ByteRTCMediaDeviceError | 设备错误类型，参看 [ByteRTCMediaDeviceError](70089#ByteRTCMediaDeviceError)。 |
...
| engine | ByteRTCVideo * | 参看 [ByteRTCVideo](70092#ByteRTCVideo)。 |
| deviceID | NSString * | 设备 ID |
| deviceType | ByteRTCVideoDeviceType | 设备类型，参看 [ByteRTCVideoDeviceType](70089#ByteRTCVideoDeviceType)。 |
| deviceState | ByteRTCMediaDeviceState | 设备状态，参看 [ByteRTCMediaDeviceState](70089#ByteRTCMediaDeviceState)。 |
| deviceError | ByteRTCMediaDeviceError | 设备错误类型，参看 [ByteRTCMediaDeviceError](70089#ByteRTCMediaDeviceError)。 |
...
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| deviceId | NSString * | 设备 ID |
| deviceType | ByteRTCAudioDeviceType | 参看 [ByteRTCAudioDeviceType](70089#ByteRTCAudioDeviceType) |
| deviceWarning | ByteRTCMediaDeviceWarning | 参看 [ByteRTCMediaDeviceWarning](70089#ByteRTCMediaDeviceWarning) |
...
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| deviceId | NSString * | 设备 ID |
| deviceType | ByteRTCVideoDeviceType | 参看 [ByteRTCVideoDeviceType](70089#ByteRTCVideoDeviceType) |
| deviceWarning | ByteRTCMediaDeviceWarning | 参看 [ByteRTCMediaDeviceWarning](70089#ByteRTCMediaDeviceWarning) |
...
| user | ByteRTCUser * | 本地用户信息，详见 [ByteRTCUser](70089#ByteRTCUser) |
| state | ByteRTCFirstFrameSendState | 首帧发送状态，详见 [ByteRTCFirstFrameSendState](70089#ByteRTCFirstFrameSendState) |
...
| user | ByteRTCUser * | 本地用户信息，参看 [ByteRTCUser](70089#ByteRTCUser) |
| state | ByteRTCFirstFrameSendState | 首帧发送状态，参看 [ByteRTCFirstFrameSendState](70089#ByteRTCFirstFrameSendState) |
...
| engine | ByteRTCVideo * | ByteRTCVideo 对象。 |
| roomId | NSString * | 音频流发布用户所在的房间 ID |
| user | ByteRTCUser * | 音频流来源的用户信息，参看 [ByteRTCUser](70089#ByteRTCUser) |
| state | ByteRTCFirstFramePlayState | 首帧播放状态，参看 [ByteRTCFirstFramePlayState](70089#ByteRTCFirstFramePlayState) |
...
| engine | ByteRTCVideo * | ByteRTCVideo 对象。 |
| roomId | NSString * | 视频流发布用户所在的房间 ID |
| user | ByteRTCUser * | 视频流来源的用户信息，参看 [ByteRTCUser](70089#ByteRTCUser) |
| state | ByteRTCFirstFramePlayState | 首帧播放状态，参看 [ByteRTCFirstFramePlayState](70089#ByteRTCFirstFramePlayState) |
...
| engine | ByteRTCVideo * | ByteRTCVideo 对象。 |
| roomId | NSString * | 屏幕流发布用户所在的房间 ID |
| user | ByteRTCUser * | 屏幕共享流来源的用户信息，参看 [ByteRTCUser](70089#ByteRTCUser) |
| state | ByteRTCFirstFramePlayState | 首帧播放状态，参看 [ByteRTCFirstFramePlayState](70089#ByteRTCFirstFramePlayState) |
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| streamIndex | ByteRTCStreamIndex | 音频流属性, 详见 [ByteRTCStreamIndex](70089#ByteRTCStreamIndex) |
...
```
收到通过调用 [sendSEIMessage:andMessage:andRepeatCount:andCountPerFrame:](70092#ByteRTCVideo-sendseimessage-andmessage-andrepeatcount-andcountperframe) 发送带有 SEI 消息的视频帧时，收到此回调。
...
| remoteStreamKey | ByteRTCRemoteStreamKey * | 包含 SEI 发送者的用户名，所在的房间名和媒体流，参看 [ByteRTCRemoteStreamKey](70089#ByteRTCRemoteStreamKey) |
...
在语音通话场景下，本地用户调用 [sendSEIMessage:andMessage:andRepeatCount:andCountPerFrame:](70092#ByteRTCVideo-sendseimessage-andmessage-andrepeatcount-andcountperframe) 通过黑帧视频流发送 SEI 数据时，流的发送状态会通过该回调通知远端用户。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | 当前 ByteRTCVideo 实例。 |
| remoteStreamKey | ByteRTCRemoteStreamKey * | 远端流信息，参看 [ByteRTCRemoteStreamKey](70089#ByteRTCRemoteStreamKey)。 |
| eventType | ByteRTCSEIStreamEventType | 黑帧视频流状态，参看 [ByteRTCSEIStreamEventType](70089#ByteRTCSEIStreamEventType) |
...
```
音频流同步信息回调。可以通过此回调，在远端用户调用 [sendStreamSyncInfo:config:](70092#ByteRTCVideo-sendstreamsyncinfo-config) 发送音频流同步消息后，收到远端发送的音频流同步信息。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | 当前 ByteRTCVideo 实例。 |
| remoteStreamKey | ByteRTCRemoteStreamKey * | 远端流信息，详见 [ByteRTCRemoteStreamKey](70089#ByteRTCRemoteStreamKey) 。 |
| streamType | ByteRTCSyncInfoStreamType | 媒体流类型，详见 [ByteRTCSyncInfoStreamType](70089#ByteRTCSyncInfoStreamType)。 |
| data | NSData * | 消息内容。 |
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| stats | const ByteRTCSysStats * | cpu 和 memory 使用率信息，详见 [ByteRTCSysStats](70089#ByteRTCSysStats) 数据类型 |
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| state | ByteRTCLocalAudioStreamState | 本地音频设备的当前状态，详见 [ByteRTCLocalAudioStreamState](70089#ByteRTCLocalAudioStreamState) |
| error | ByteRTCLocalAudioStreamError | 本地音频流状态改变时的错误码，详见 [ByteRTCLocalAudioStreamError](70089#ByteRTCLocalAudioStreamError) |
...
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| key | ByteRTCRemoteStreamKey * | 远端流信息, 详见 [ByteRTCRemoteStreamKey](70089#ByteRTCRemoteStreamKey) |
| state | ByteRTCRemoteAudioState | 远端音频流的当前状态，详见 [ByteRTCRemoteAudioState](70089#ByteRTCRemoteAudioState) |
| reason | ByteRTCRemoteAudioStateChangeReason | 远端音频流状态改变的原因，详见 [ByteRTCRemoteAudioStateChangeReason](70089#ByteRTCRemoteAudioStateChangeReason) |
...
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| streamIndex | ByteRTCStreamIndex | 流属性，参看 [ByteRTCStreamIndex](70089#ByteRTCStreamIndex) |
| state | ByteRTCLocalVideoStreamState | 本地视频流的当前状态，参看 [ByteRTCLocalVideoStreamState](70089#ByteRTCLocalVideoStreamState) |
| error | ByteRTCLocalVideoStreamError | 本地视频状态改变时的错误码，参看 [ByteRTCLocalVideoStreamError](70089#ByteRTCLocalVideoStreamError) |
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| streamKey | ByteRTCRemoteStreamKey * | 远端视频流信息，房间、用户 ID、流属性等，参看 [ByteRTCRemoteStreamKey](70089#ByteRTCRemoteStreamKey) |
| state | ByteRTCRemoteVideoState | 远端视频流的当前状态，参看 [ByteRTCRemoteVideoState](70089#ByteRTCRemoteVideoState) |
| reason | ByteRTCRemoteVideoStateChangeReason | 远端视频流状态改变的原因，参看 [ByteRTCRemoteVideoStateChangeReason](70089#ByteRTCRemoteVideoStateChangeReason) |
...
| errorCode | ByteRTCLoginErrorCode | <br>登录结果<br>详见 [ByteRTCLoginErrorCode](70091#ByteRTCLoginErrorCode)。 |
| elapsed | NSInteger | <br>从调用 [login:uid:](70092#ByteRTCVideo-login-uid) 接口开始到返回结果所用时长。<br>单位为 ms。 |


**注意**

调用 [login:uid:](70092#ByteRTCVideo-login-uid) 后，会收到此回调。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| reason | ByteRTCLogoutReason | 用户登出的原因，参看 [ByteRTCLogoutReason](70089#ByteRTCLogoutReason) |


**注意**

在以下两种情况下会收到此回调：调用 [logout](70092#ByteRTCVideo-logout) 接口主动退出；或其他用户以相同 UserId 进行 `login` 导致本地用户被动登出。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| errorCode | NSInteger | <br>设置结果<br><ul><li>返回 200，设置成功</li><li>返回其他，设置失败，详见 [ByteRTCUserMessageSendResult](70091#ByteRTCUserMessageSendResult) </li></ul> |


**注意**

调用 [setServerParams:url:](70092#ByteRTCVideo-setserverparams-url) 后，会收到此回调。
...
| status | ByteRTCUserOnlineStatus | <br>查询的用户登录状态<br>详见 [ByteRTCUserOnlineStatus](70089#ByteRTCUserOnlineStatus). |


**注意**

必须先调用 [getPeerOnlineStatus:](70092#ByteRTCVideo-getpeeronlinestatus)，才能收到此回调。
...
```
收到房间外用户调用 [sendUserMessageOutsideRoom:message:config:](70092#ByteRTCVideo-sendusermessageoutsideroom-message-config) 发来的文本消息时，会收到此回调
...
```
收到房间外用户调用 [sendUserBinaryMessageOutsideRoom:message:config:](70092#ByteRTCVideo-senduserbinarymessageoutsideroom-message-config) 发来的二进制消息时，会收到此回调。
...
| error | ByteRTCUserMessageSendResult | <br>消息发送结果<br>详见 [ByteRTCUserMessageSendResult](70091#ByteRTCUserMessageSendResult)。 |


**注意**

当调用 [sendUserMessageOutsideRoom:message:config:](70092#ByteRTCVideo-sendusermessageoutsideroom-message-config) 或 [sendUserBinaryMessageOutsideRoom:message:config:](70092#ByteRTCVideo-senduserbinarymessageoutsideroom-message-config) 发送消息后，会收到此回调。
...
本回调为异步回调。当调用 [sendServerMessage:](70092#ByteRTCVideo-sendservermessage) 或 [sendServerBinaryMessage:](70092#ByteRTCVideo-sendserverbinarymessage) 接口发送消息后，会收到此回调。
...
成功调用 [startNetworkDetection:uplinkBandwidth:downlink:downlinkBandwidth:](70092#ByteRTCVideo-startnetworkdetection-uplinkbandwidth-downlink-downlinkbandwidth) 接口开始探测后，会在 3s 内首次收到该回调，之后每 2s 收到一次该回调。
...
| quality | ByteRTCNetworkQuality | 探测网络的质量，参看 [ByteRTCNetworkQuality](70089#ByteRTCNetworkQuality)。 |
...
1. 当调用 [stopNetworkDetection](70092#ByteRTCVideo-stopnetworkdetection) 接口停止探测后，会收到一次该回调；
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| errorCode | ByteRTCNetworkDetectionStopReason | <br>停止探测的原因类型,参考 [ByteRTCNetworkDetectionStopReason](70089#ByteRTCNetworkDetectionStopReason) |
...
调用 [setAudioMixingProgressInterval:interval:](70092#ByteRTCAudioMixingManager-setaudiomixingprogressinterval-interval) 将时间间隔设为大于 0 的值后，或调用 [startAudioMixing:filePath:config:](70092#ByteRTCAudioMixingManager-startaudiomixing-filepath-config) 将 [ByteRTCAudioMixingConfig](70089#ByteRTCAudioMixingConfig) 中的时间间隔设为大于 0 的值后，SDK 会按照设置的时间间隔回调该事件。
...
```
本地未通过 [setPublishFallbackOption:](70092#ByteRTCVideo-setpublishfallbackoption) 开启发布性能回退，检测到设备性能不足时，收到此回调。

本地通过 [setPublishFallbackOption:](70092#ByteRTCVideo-setpublishfallbackoption) 开启发布性能回退，因设备性能/网络原因，造成发布性能回退/恢复时，收到此回调。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | `ByteRTCVideo` 实例 |
| mode | ByteRTCPerformanceAlarmMode | 指示本地是否开启发布回退功能。参看 [ByteRTCPerformanceAlarmMode](70089#ByteRTCPerformanceAlarmMode)<br><ul><li>当发布端未开启发布性能回退时，mode 值为 ByteRTCPerformanceAlarmModeNormal。</li><li>当发布端开启发布性能回退时，mode 值为 ByteRTCPerformanceAlarmModeSimulcast。</li></ul> |
| roomId | NSString * | <ul><li>未开启发布性能回退时，roomId 为空</li><li>开启发布性能回退时，roomId 是告警影响的房间 ID。</li></ul> |
| reason | ByteRTCPerformanceAlarmReason | 告警原因，参看 [ByteRTCPerformanceAlarmReason](70089#ByteRTCPerformanceAlarmReason) |
| data | ByteRTCSourceWantedData * | 性能回退相关数据，详见 [ByteRTCSourceWantedData](70089#ByteRTCSourceWantedData)。 |
...
| --- | --- | --- |
| engine | ByteRTCVideo * | 当前 ByteRTCVideo 实例 |
| event | ByteRTCRemoteStreamSwitchEvent * | 流切换信息。详见 [ByteRTCRemoteStreamSwitchEvent](70089#ByteRTCRemoteStreamSwitchEvent)。 |
...
#endif
```
外部采集时，调用 [setOriginalScreenVideoInfo:withOriginalCaptureHeight:](70092#ByteRTCVideo-setoriginalscreenvideoinfo-withoriginalcaptureheight) 设置屏幕或窗口大小改变前的分辨率后，若屏幕采集模式为智能模式，你将收到此回调，根据 RTC 智能决策合适的帧率和分辨率积（宽*高）重新采集。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | 参看 [ByteRTCVideo](70092#ByteRTCVideo) |
| frameUpdateInfo | ByteRTCFrameUpdateInfo *_Nullable | RTC 智能决策后合适的帧率和分辨率积（宽*高）。参看 [ByteRTCFrameUpdateInfo](70089#ByteRTCFrameUpdateInfo)。 |
...
该回调由 [startFileRecording:withRecordingConfig:type:](70092#ByteRTCVideo-startfilerecording-withrecordingconfig-type) 或 [stopFileRecording:](70092#ByteRTCVideo-stopfilerecording) 触发。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| type | ByteRTCStreamIndex | 录制流的流属性，参看 [ByteRTCStreamIndex](70089#ByteRTCStreamIndex) |
| state | ByteRTCRecordingState | 录制状态，参看 [ByteRTCRecordingState](70089#ByteRTCRecordingState) |
| errorCode | ByteRTCRecordingErrorCode | 录制错误码，参看 [ByteRTCRecordingErrorCode](70089#ByteRTCRecordingErrorCode) |
| recordingInfo | ByteRTCRecordingInfo * | 录制文件的详细信息，参看 [ByteRTCRecordingInfo](70089#ByteRTCRecordingInfo) |
...
该回调由 [startFileRecording:withRecordingConfig:type:](70092#ByteRTCVideo-startfilerecording-withrecordingconfig-type) 触发，录制状态正常时，系统每秒钟都会通过该回调提示录制进度。
...
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| type | ByteRTCStreamIndex | 录制流的流属性，参看 [ByteRTCStreamIndex](70089#ByteRTCStreamIndex) |
| process | ByteRTCRecordingProgress * | 录制进度，参看 [ByteRTCRecordingProgress](70089#ByteRTCRecordingProgress) |
| recordingInfo | ByteRTCRecordingInfo * | 录制文件的详细信息，参看 [ByteRTCRecordingInfo](70089#ByteRTCRecordingInfo) |
...
```
调用 [startAudioRecording:](70092#ByteRTCVideo-startaudiorecording) 或者 [stopAudioRecording](70092#ByteRTCVideo-stopaudiorecording) 改变音频文件录制状态时，收到此回调。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | `ByteRTCVideo` 实例 |
| state | ByteRTCAudioRecordingState | 录制状态，参看 [ByteRTCAudioRecordingState](70089#ByteRTCAudioRecordingState) |
| errorCode | ByteRTCAudioRecordingErrorCode | 录制错误码，参看 [ByteRTCAudioRecordingErrorCode](70091#ByteRTCAudioRecordingErrorCode) |
...
调用 [startPushPublicStream:withLayout:](70092#ByteRTCVideo-startpushpublicstream-withlayout)
...
| errorCode | ByteRTCPublicStreamErrorCode | 公共流发布结果状态码。 详见 [ByteRTCPublicStreamErrorCode](70091#ByteRTCPublicStreamErrorCode)。 |
...
调用 [startPlayPublicStream:](70092#ByteRTCVideo-startplaypublicstream) 接口拉公共流后，你会通过此回调收到启动结果和拉流过程中的错误。
...
| errorCode | ByteRTCPublicStreamErrorCode | 公共流订阅结果状态码。 详见 [ByteRTCPublicStreamErrorCode](70091#ByteRTCPublicStreamErrorCode)。 |
...
调用 [startPlayPublicStream:](70092#ByteRTCVideo-startplaypublicstream) 接口启动拉公共流功能后，通过此回调收到公共流中的 SEI 消息。
...
| sourceType | ByteRTCDataMessageSourceType | SEI 消息类型，自 3.52.1 版本后固定为 `0`，自定义消息。参看 [ByteRTCDataMessageSourceType](70089#ByteRTCDataMessageSourceType)。 |
...
通过 [startPlayPublicStream:](70092#ByteRTCVideo-startplaypublicstream) 开始播放公共流后，可以通过本回调获取公共流中的数据消息，包括调用 Open API 发送的 SEI 消息和音量回调。
...
关于订阅公共流，详见 [startPlayPublicStream:](70092#ByteRTCVideo-startplaypublicstream)。
...
```
调用 [startCloudProxy:](70092#ByteRTCVideo-startcloudproxy) 开启云代理，SDK 首次成功连接云代理服务器时，回调此事件。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | `ByteRTCVideo` 实例 |
| result | ByteRTCEchoTestResult | 测试结果，参看 [ByteRTCEchoTestResult](70089#ByteRTCEchoTestResult)。 |
...
```
首次调用 [getNetworkTimeInfo](70092#ByteRTCVideo-getnetworktimeinfo) 后，SDK 内部启动网络时间同步，同步完成时会触发此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| engine | ByteRTCVideo * | [ByteRTCVideo](70092#ByteRTCVideo) 对象 |
...
| --- | --- | --- |
| engine | ByteRTCVideo * | `ByteRTCVideo` 实例 |
| result | ByteRTCHardwareEchoDetectionResult | 参见 [ByteRTCHardwareEchoDetectionResult](70089#ByteRTCHardwareEchoDetectionResult)。 |


**注意**

- 通话前调用 [startHardwareEchoDetection:](70092#ByteRTCVideo-starthardwareechodetection) 后，将触发本回调返回检测结果。
- 建议在收到检测结果后，调用 [stopHardwareEchoDetection](70092#ByteRTCVideo-stophardwareechodetection) 停止检测，释放对音频设备的占用。
...
```
本地代理状态发生改变回调。调用 [setLocalProxy:](70092#ByteRTCVideo-setlocalproxy) 设置本地代理后，SDK 会触发此回调，返回代理连接的状态。
...
| --- | --- | --- |
| engine | ByteRTCVideo * | `ByteRTCVideo` 实例 |
| type | ByteRTCLocalProxyType | 本地代理类型。参看 [ByteRTCLocalProxyType](70089#ByteRTCLocalProxyType)。 |
| state | ByteRTCLocalProxyState | 本地代理状态。参看 [ByteRTCLocalProxyState](70089#ByteRTCLocalProxyState)。 |
| error | ByteRTCLocalProxyError | 本地代理错误。参看 [ByteRTCLocalProxyError](70089#ByteRTCLocalProxyError)。 |
...
| muteState | ByteRTCMuteState | 发送状态，参看 [ByteRTCMuteState](70089#ByteRTCMuteState) |
...
| muteState | ByteRTCMuteState | 视频流的发送状态。参看 [ByteRTCMuteState](70089#ByteRTCMuteState)。 |
...
| engine | ByteRTCVideo * | ByteRTCVideo 实例 |
| deviceID | NSString * | 设备 ID |
| deviceType | ByteRTCMediaDeviceType | 设备类型，参看 [ByteRTCMediaDeviceType](70089#ByteRTCMediaDeviceType)。 |
| deviceState | ByteRTCMediaDeviceState | 设备状态，参看 [ByteRTCMediaDeviceState](70089#ByteRTCMediaDeviceState)。 |
| deviceError | ByteRTCMediaDeviceError | 设备错误类型，参看 [ByteRTCMediaDeviceError](70089#ByteRTCMediaDeviceError)。 |
...
| engine | ByteRTCVideo * | ByteRTCVideo 对象 |
| deviceId | NSString * | 设备 ID |
| deviceType | ByteRTCMediaDeviceType | 参看 [ByteRTCMediaDeviceType](70089#ByteRTCMediaDeviceType) |
| deviceWarning | ByteRTCMediaDeviceWarning | 参看 [ByteRTCMediaDeviceWarning](70089#ByteRTCMediaDeviceWarning) |
...
| --- | --- | --- |
| engine | ByteRTCVideo * | 当前 RTC SDK 对象 |
| mixId | NSInteger | 混音 ID <br>使用 [ByteRTCAudioMixingManager](70092#ByteRTCAudioMixingManager) 相关接口时传入的唯一 ID。 |
| state | ByteRTCAudioMixingState | <br>混音状态<br>其混音状态可参考： [ByteRTCAudioMixingState](70089#ByteRTCAudioMixingState)。 |
| error | ByteRTCAudioMixingError | <br>错误码<br>详见 [ByteRTCAudioMixingError](70089#ByteRTCAudioMixingError)。 |


**注意**

此回调会被触发的时机汇总如下：
- 当调用 [startAudioMixing:filePath:config:](70092#ByteRTCAudioMixingManager-startaudiomixing-filepath-config) 方法成功后，会触发 state 值为 ByteRTCAudioMixingStatePlaying 回调；否则触发 state 值为 ByteRTCAudioMixingStateFailed 的回调。
- 当使用相同的 ID 重复调用 [startAudioMixing:filePath:config:](70092#ByteRTCAudioMixingManager-startaudiomixing-filepath-config) 后，后一次会覆盖前一次，且本回调会以 ByteRTCAudioMixingStateStopped 通知前一次混音已停止。
- 当调用 [pauseAudioMixing:](70092#ByteRTCAudioMixingManager-pauseaudiomixing) 方法暂停播放成功后，会触发 state 值为 ByteRTCAudioMixingStatePaused 回调；否则触发 state 值为 ByteRTCAudioMixingStateFailed 的回调。
- 当调用 [resumeAudioMixing:](70092#ByteRTCAudioMixingManager-resumeaudiomixing) 方法恢复播放成功后，会触发 state 值为 ByteRTCAudioMixingStatePlaying 回调；否则触发 state 值为 ByteRTCAudioMixingStateFailed 的回调。
- 当调用 [stopAudioMixing:](70092#ByteRTCAudioMixingManager-stopaudiomixing) 方法停止播放成功后，会触发 state 值为 ByteRTCAudioMixingStateStopped 回调；否则触发 state 值为 ByteRTCAudioMixingStateFailed 的回调。
...
| event | ByteRTCStreamMixingEvent | 转推直播任务状态，参看 [ByteRTCStreamMixingEvent](70089#ByteRTCStreamMixingEvent)。 |
| taskId | NSString * | 转推直播任务 ID。 |
| Code | ByteRtcTranscoderErrorCode | 转推直播错误码，参看 [ByteRtcTranscoderErrorCode](70091#ByteRtcTranscoderErrorCode)。 |
| mixType | ByteRTCStreamMixingType | 转推直播类型，参看 [ByteRTCStreamMixingType](70089#ByteRTCStreamMixingType)。 |
...
```
调用 [registerLocalEncodedVideoFrameObserver:](70092#ByteRTCVideo-registerlocalencodedvideoframeobserver) 后，SDK 每次使用内部采集，采集到一帧视频帧，或收到一帧外部视频帧时，都会回调该事件。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamIndex | ByteRTCStreamIndex | 本地视频帧类型，参看 [ByteRTCStreamIndex](70089#ByteRTCStreamIndex) |
| frame | ByteRTCEncodedVideoFrame *_Null_unspecified | 本地视频帧信息，参看 [ByteRTCEncodedVideoFrame](70089#ByteRTCEncodedVideoFrame) |
...
```
调用 [registerRemoteEncodedVideoFrameObserver:](70092#ByteRTCVideo-registerremoteencodedvideoframeobserver) 后，SDK 监测到远端编码后视频数据时，触发该回调
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamKey | ByteRTCRemoteStreamKey * | 收到的远端流信息，参看 [ByteRTCRemoteStreamKey](70089#ByteRTCRemoteStreamKey) |
| videoFrame | ByteRTCEncodedVideoFrame * | 收到的远端视频帧信息，参看 [ByteRTCEncodedVideoFrame](70089#ByteRTCEncodedVideoFrame) |
...
调用 [openWithCustomSource:config:](70092#ByteRTCMediaPlayer-openwithcustomsource-config) 接口播放用户传入的内存音频数据时，会触发此回调，用户需要写入音频数据。
...
若 [openWithCustomSource:config:](70092#ByteRTCMediaPlayer-openwithcustomsource-config) 接口调用失败，请在 buffer 和 bufferSize 两个参数中填入 0。 此时 SDK 会停止调用此接口。
...
在调用 [openWithCustomSource:config:](70092#ByteRTCMediaPlayer-openwithcustomsource-config) 接口传入内存音频数据，或者调用 [setPosition:position:](70092#ByteRTCAudioEffectPlayer-setposition-position) 设置了音频数据的起始播放位置后，SDK 会对音频数据进行读取和分析，此时会触发该回调，你需要根据参数中设置的起始读取位置和偏移量进行操作。
...
| whence | ByteRTCMediaPlayerCustomSourceSeekWhence | 音频数据的起始读取位置。参看 [ByteRTCMediaPlayerCustomSourceSeekWhence](70089#ByteRTCMediaPlayerCustomSourceSeekWhence) |
