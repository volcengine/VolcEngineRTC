...
- 若未取得当前设备的麦克风权限，调用该方法后会触发 [onAudioDeviceStateChanged](70096#IRTCVideoEventHandler-onaudiodevicestatechanged) 回调，对应的错误码为 `MediaDeviceError.kMediaDeviceErrorDeviceNoPermission = 1`。
...
当所选的 [RoomProfileType](70098#RoomProfileType) 中的音频参数无法满足你的场景需求时，调用本接口切换的音质档位。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| video_capture_config | const VideoCaptureConfig & | 视频采集参数。参看: [VideoCaptureConfig](70098#VideoCaptureConfig)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| rotation | VideoRotation | 相机朝向角度，默认为 `VIDEO_ROTATION_0(0)`，无旋转角度。详见 [VideoRotation](70098#VideoRotation)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| max_solution | const VideoEncoderConfig & | 期望发布的最大分辨率视频流参数。参看 [VideoEncoderConfig](70098#VideoEncoderConfig)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| channel_solutions | const VideoEncoderConfig * | 要推送的多路视频流的参数。注意，所设置的分辨率是各路流的最大分辨率。参看 [VideoEncoderConfig](70098#VideoEncoderConfig)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| screen_solution | const ScreenVideoEncoderConfig & | 屏幕共享视频流参数。参看 [ScreenVideoEncoderConfig](70098#ScreenVideoEncoderConfig)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| index | StreamIndex | 视频流属性, 参看 [StreamIndex](70098#StreamIndex) |
| canvas | const VideoCanvas & | 视图信息和渲染模式，参看： [VideoCanvas](70098#VideoCanvas)。其中 [render_rotation](70098#VideoCanvas-render_rotation) 仅对远端视频有效，对本接口不生效。 |
...
| --- | --- | --- |
| index | StreamIndex | 视频流属性, 参看 [StreamIndex](70098#StreamIndex) |
| render_mode | const enum RenderMode | 渲染模式，参看 [RenderMode](70098#RenderMode) |
| background_color | const uint32_t | 背景颜色，参看 [VideoCanvas](70098#VideoCanvas).background_color |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_key | RemoteStreamKey | 参看 [RemoteStreamKey](70098#RemoteStreamKey) |
| canvas | const VideoCanvas & | 视图信息和渲染模式，参看： [VideoCanvas](70098#VideoCanvas) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| index | StreamIndex | 视频流属性。采集的视频流/屏幕视频流，参看 [StreamIndex](70098#StreamIndex)。 |
| video_sink | IVideoSink * | 自定义视频渲染器，参看 [IVideoSink](70098#IVideoSink)。 |
| render_config | LocalVideoSinkConfig & | 本地视频帧回调配置，参看 [LocalVideoSinkConfig](70098#LocalVideoSinkConfig)。 |
...
- 一般在收到 [onFirstLocalVideoFrameCaptured](70096#IRTCVideoEventHandler-onfirstlocalvideoframecaptured) 回调通知完成本地视频首帧采集后，调用此方法为视频流绑定自定义渲染器；然后加入房间。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_key | RemoteStreamKey | 远端流信息，用于指定需要渲染的视频流来源及属性，参看 [RemoteStreamKey](70098#RemoteStreamKey)。 |
| video_sink | IVideoSink * | 自定义视频渲染器，参看 [IVideoSink](70098#IVideoSink)。 |
| config | RemoteVideoSinkConfig & | 远端视频帧回调配置，参看 [RemoteVideoSinkConfig](70098#RemoteVideoSinkConfig)。 |
...
- RTC SDK 默认使用自带的渲染器（内部渲染器）进行视频渲染。
- 该方法进房前后均可以调用。若想在进房前调用，你需要在加入房间前获取远端流信息；若无法预先获取远端流信息，你可以在加入房间并通过 [onUserPublishStream](70096#IRTCRoomEventHandler-onuserpublishstream) 回调获取到远端流信息之后，再调用该方法。
- 退房时将清除绑定状态。
- 如果需要解除绑定，你必须将 video_sink 设置为 null。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| frame | IVideoFrame * | 设置屏幕视频帧，详见： [IVideoFrame](70098#IVideoFrame)。 |
...
- 调用此接口后，你将收到回调 [onExternalScreenFrameUpdate](70096#IRTCVideoEventHandler-onexternalscreenframeupdate)，根据推荐的帧率和分辨率积重新采集。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| region_rect | const Rectangle & | 采集区域。参见 [Rectangle](70098#Rectangle)<br>此参数描述了调用此接口后的采集区域，和 [startScreenVideoCapture](#IRTCVideo-startscreenvideocapture) 中 `source_info` 设定区域的相对关系。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| source_info | const ScreenCaptureSourceInfo & | 待共享的屏幕源，参看 [ScreenCaptureSourceInfo](70098#ScreenCaptureSourceInfo)。 <br>你可以调用 [getScreenCaptureSourceList](#IRTCVideo-getscreencapturesourcelist) 获得所有可以共享的屏幕源。 |
| capture_params | const ScreenCaptureParameters & | 共享参数。参看 [ScreenCaptureParameters](70098#ScreenCaptureParameters)。 |
...
- 调用本接口时，采集模式应为内部模式。在外部采集模式下调用无效，并将触发 [onVideoDeviceWarning](70096#IRTCVideoEventHandler-onvideodevicewarning) 回调。
...
- 本地用户通过 [onVideoDeviceStateChanged](70096#IRTCVideoEventHandler-onvideodevicestatechanged) 的回调获取屏幕采集状态，包括开始、暂停、恢复、错误等。
- 调用成功后，本端会收到 [onFirstLocalVideoFrameCaptured](70096#IRTCVideoEventHandler-onfirstlocalvideoframecaptured) 回调。
- 调用此接口前，你可以调用 [setScreenVideoEncoderConfig](#IRTCVideo-setscreenvideoencoderconfig) 设置屏幕视频流的采集帧率和编码分辨率。
- 在收到 [onFirstLocalVideoFrameCaptured](70096#IRTCVideoEventHandler-onfirstlocalvideoframecaptured) 回调后通过调用 [setLocalVideoCanvas](#IRTCVideo-setlocalvideocanvas) 或 [setLocalVideoSink](#IRTCVideo-setlocalvideosink) 函数设置本地屏幕共享视图。
- 可以调用 [setLocalVideoSink](#IRTCVideo-setlocalvideosink) 将本地视频流与自定义渲染器绑定，通过回调 [onFrame](70098#IVideoSink-onframe) 获取采集成功的本地视频帧。
...
- 调用后，本地用户会收到 [onVideoDeviceStateChanged](70096#IRTCVideoEventHandler-onvideodevicestatechanged) 的回调。
- 调用此接口不影响屏幕视频流发布。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| highlight_config | const HighlightConfig & | 边框高亮设置。参见 [HighlightConfig](70098#HighlightConfig) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| capture_mouse_cursor | MouseCursorCaptureState | 参看 [MouseCursorCaptureState](70098#MouseCursorCaptureState) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| filter_config | const ScreenFilterConfig & | 窗口过滤设置，参看 [ScreenFilterConfig](70098#ScreenFilterConfig) |
...
- 调用此接口前，必须已通过调用 [startScreenVideoCapture](#IRTCVideo-startscreenvideocapture) 开启了内部屏幕流采集。
- 本函数在屏幕源类别是屏幕而非应用窗体时才起作用。详见： [ScreenCaptureSourceType](70098#ScreenCaptureSourceType)。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | ScreenCaptureSourceType | 屏幕采集对象的类型。详见 [ScreenCaptureSourceType](70098#ScreenCaptureSourceType)。 |
...
共享对象缩略图，详见 [IVideoFrame](70098#IVideoFrame)。
...
详见 [IVideoFrame](70098#IVideoFrame)。当屏幕共享对象为应用窗体，且有图标时有效，否则返回 nullptr。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | StreamIndex | 视频流的属性，参看 [StreamIndex](70098#StreamIndex) |
| type | VideoSourceType | 视频输入源类型，参看 [VideoSourceType](70098#VideoSourceType) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| frame | IVideoFrame * | 设置视频帧，参看 [IVideoFrame](70098#IVideoFrame)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| option | PublishFallbackOption | 本地发布的音视频流回退选项，参看 [PublishFallbackOption](70098#PublishFallbackOption)。 |
...
- 调用该方法后，如因性能或网络不佳产生发布性能回退或恢复，本端会提前收到 [onPerformanceAlarms](70096#IRTCVideoEventHandler-onperformancealarms) 回调发出的告警，以便采集设备配合调整。
- 设置回退后，本地发布的音视频流发生回退或从回退中恢复后，远端会收到 [onSimulcastSubscribeFallback](70096#IRTCVideoEventHandler-onsimulcastsubscribefallback) 回调，通知该情况。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| option | SubscribeFallbackOption | 订阅的音视频流回退选项，参看 [SubscribeFallbackOption](70098#SubscribeFallbackOption)。 |
...
- 设置回退选项后，订阅的音视频流发生回退或从回退中恢复时，会收到 [onSimulcastSubscribeFallback](70096#IRTCVideoEventHandler-onsimulcastsubscribefallback) 和 [onRemoteVideoSizeChanged](70096#IRTCVideoEventHandler-onremotevideosizechanged) 回调通知。
...
| priority | RemoteUserPriority | 远端用户的需求优先级，详见枚举类型 [RemoteUserPriority](70098#RemoteUserPriority) |
...
- 0： 成功
- < 0： 失败，具体原因参照 [BusinessCheckCode](70098#BusinessCheckCode)。
- -6001： 用户已经在房间中。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| mirror_type | MirrorType | 镜像类型，参看 [MirrorType](70098#MirrorType) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_key | RemoteStreamKey | 远端流信息，用于指定需要镜像的视频流来源及属性，参看 [RemoteStreamKey](70098#RemoteStreamKey)。 |
| mirror_type | RemoteMirrorType | 远端流的镜像类型，参看 [RemoteMirrorType](70098#RemoteMirrorType)。 |


**返回值**

- 0: 调用成功。
- < 0: 调用失败，参看 [ReturnStatus](70098#ReturnStatus) 获得更多错误说明。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| beauty_mode | EffectBeautyMode | 基础美颜模式，参看 [EffectBeautyMode](70098#EffectBeautyMode)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| encrypt_type | EncryptType | 内置加密算法，详见 [EncryptType](70098#EncryptType) |
...
- 使用传输时内置加密时，使用此方法；如果需要使用传输时自定义加密，参看 [onEncryptData](70096#IEncryptHandler-onencryptdata)。 内置加密和自定义加密互斥，根据最后一个调用的方法确定传输是加密的方案。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| handler | IEncryptHandler * | 自定义加密 handler，需要实现 handler 的加密和解密方法。参看 [IEncryptHandler](70096#IEncryptHandler)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| method | AudioFrameCallbackMethod | 音频回调方法，参看 [AudioFrameCallbackMethod](70098#AudioFrameCallbackMethod)。<br>当音频回调方法设置为 `kRecord`、`kPlayback`、`kMixed`、`kRecordScreen`时，你需要在参数 `format` 中指定准确的采样率和声道，暂不支持设置为自动。<br>当音频回调方法设置为 `kRemoteUser`时，将 `format` 中的各个字段设置为默认值。 |
| format | AudioFormat | 音频参数格式，参看 [AudioFormat](70098#AudioFormat)。 |
...
开启音频回调并调用 [registerAudioFrameObserver](#IRTCVideo-registeraudioframeobserver) 后， [IAudioFrameObserver](70096#IAudioFrameObserver) 会收到对应的音频回调。两者调用顺序没有限制且相互独立。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| method | AudioFrameCallbackMethod | 音频回调方法，参看 [AudioFrameCallbackMethod](70098#AudioFrameCallbackMethod)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| observer | IAudioFrameObserver * | 音频数据观察者，参看 [IAudioFrameObserver](70096#IAudioFrameObserver)。如果传入 null，则取消注册。 |
...
注册音频数据回调观察者并调用 [enableAudioFrameCallback](#IRTCVideo-enableaudioframecallback) 后， [IAudioFrameObserver](70096#IAudioFrameObserver) 会收到对应的音频回调。对回调中收到的音频数据进行处理，不会影响 RTC 的编码发送或渲染。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| processor | IAudioFrameProcessor * | 自定义音频处理器，详见 [IAudioFrameProcessor](70096#IAudioFrameProcessor)。 <br>SDK 只持有 processor 的弱引用，你应保证其生命周期。需要取消注册时，设置此参数为 nullptr。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| method | AudioProcessorMethod | 音频帧类型，参看 [AudioProcessorMethod](70098#AudioProcessorMethod)。可多次调用此接口，处理不同类型的音频帧。<br>选择不同类型的音频帧将收到对应的回调：<br><ul><li>选择本地采集的音频时，会收到 [onProcessRecordAudioFrame](70096#IAudioFrameProcessor-onprocessrecordaudioframe)。</li><li>选择远端音频流的混音音频时，会收到 [onProcessPlayBackAudioFrame](70096#IAudioFrameProcessor-onprocessplaybackaudioframe)。</li><li>选择远端音频流时，会收到 [onProcessRemoteUserAudioFrame](70096#IAudioFrameProcessor-onprocessremoteuseraudioframe)。</li><li>选择屏幕共享音频流时，会收到 [onProcessScreenAudioFrame](70096#IAudioFrameProcessor-onprocessscreenaudioframe)。（Linux 不适用）</li></ul> |
| format | AudioFormat | 设定自定义处理时获取的音频帧格式，参看 [AudioFormat](70098#AudioFormat)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| method | AudioProcessorMethod | 音频帧类型，参看 [AudioProcessorMethod](70098#AudioProcessorMethod)。 |
...
| config | VideoPreprocessorConfig | 自定义视频前处理器适用的设置，详见 [VideoPreprocessorConfig](70098#VideoPreprocessorConfig)。 <br>当前，`config` 中的 `required_pixel_format` 仅支持：`kVideoPixelFormatI420` 和 `kVideoPixelFormatUnknown`： <br><ul><li>设置为 `kVideoPixelFormatUnknown` 时，RTC SDK 给出供 processor 处理的视频帧格式即采集的格式。 你可以通过 [VideoFrameType](70098#VideoFrameType) 和 [VideoPixelFormat](70098#VideoPixelFormat) 获取实际采集的视频帧格式和像素类型。<br></li><li>设置为 `kVideoPixelFormatI420` 时，RTC SDK 会将采集得到的视频帧转变为对应的格式，供前处理使用。</li><li>设置为其他值时，此方法调用失败。</li></ul> |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | ZoomConfigType | 数码变焦参数类型，缩放系数或移动步长。参看 [ZoomConfigType](70098#ZoomConfigType)。必填。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| direction | ZoomDirectionType | 数码变焦操作类型，参看 [ZoomDirectionType](70098#ZoomDirectionType)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | StreamIndex | 指定携带 SEI 数据的媒体流类型，参看 [StreamIndex](70098#StreamIndex)。<br>语音通话场景下，该值需设为 `kStreamIndexMain`，否则 SEI 数据会被丢弃从而无法送达远端。 |
...
| mode | SEICountPerFrame | SEI 发送模式，参看 [SEICountPerFrame](70098#SEICountPerFrame)。 |
...
- 消息发送成功后，远端会收到 [onSEIMessageReceived](70096#IRTCVideoEventHandler-onseimessagereceived) 回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | StreamIndex | 流属性，指定录制主流还是屏幕流，参看 [StreamIndex](70098#StreamIndex) |
| config | RecordingConfig | 本地录制参数配置，参看 [RecordingConfig](70098#RecordingConfig) |
| recording_type | RecordingType | 本地录制的媒体类型，参看 [RecordingType](70098#RecordingType) |
...
- 该方法需在进房后调用。
- 调用该方法后，你会收到 [onRecordingStateUpdate](70096#IRTCVideoEventHandler-onrecordingstateupdate) 回调。
- 如果录制正常，系统每秒钟会通过 [onRecordingProgressUpdate](70096#IRTCVideoEventHandler-onrecordingprogressupdate) 回调通知录制进度。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | StreamIndex | 流属性，指定停止主流或者屏幕流录制，参看 [StreamIndex](70098#StreamIndex) |
...
- 调用该方法后，你会收到 [onRecordingStateUpdate](70096#IRTCVideoEventHandler-onrecordingstateupdate) 回调提示录制结果。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| config | AudioRecordingConfig & | 参看 [AudioRecordingConfig](70098#AudioRecordingConfig) |
...
- 调用该方法后，你会收到 [onAudioRecordingStateUpdate](70096#IRTCVideoEventHandler-onaudiorecordingstateupdate) 回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | uint64_t | 预设问题列表，参看 [ProblemFeedbackOption](70098#ProblemFeedbackOption) |
| info | const ProblemFeedbackInfo * | 预设问题以外的其他问题的具体描述，房间信息。 参看 [ProblemFeedbackInfo](70098#ProblemFeedbackInfo) |
...
本地用户调用此方法登录后，会收到 [onLoginResult](70096#IRTCVideoEventHandler-onloginresult) 回调通知登录结果，远端用户不会收到通知。
...
- 本地用户调用此方法登出后，会收到 [onLogout](70096#IRTCVideoEventHandler-onlogout) 回调通知结果，远端用户不会收到通知。
...
调用 [login](#IRTCVideo-login) 方法登录时，如果使用了过期的 Token 将导致登录失败，并会收到 [onLoginResult](70096#IRTCVideoEventHandler-onloginresult) 回调通知，错误码为 kLoginErrorCodeInvalidToken。此时需要重新获取 Token，并调用此方法更新 Token。
...
- 调用本接口后，SDK 会使用 [onServerParamsSetResult](70096#IRTCVideoEventHandler-onserverparamssetresult) 返回相应结果。
...
- 调用本接口后，SDK 会使用 [onGetPeerOnlineStatus](70096#IRTCVideoEventHandler-ongetpeeronlinestatus) 回调通知查询结果。
...
| config | MessageConfig | 消息类型，参看 [MessageConfig](70098#MessageConfig)。 |
...
- 用户调用本接口发送文本信息后，会收到一次 [onUserMessageSendResultOutsideRoom](70096#IRTCVideoEventHandler-onusermessagesendresultoutsideroom) 回调，得知消息是否成功发送。
- 若文本消息发送成功，则 uid 所指定的用户会通过 [onUserMessageReceivedOutsideRoom](70096#IRTCVideoEventHandler-onusermessagereceivedoutsideroom) 回调收到该消息。
...
- 用户调用本接口发送二进制消息后，会收到一次 [onUserMessageSendResultOutsideRoom](70096#IRTCVideoEventHandler-onusermessagesendresultoutsideroom) 回调，通知消息是否发送成功。
- 若二进制消息发送成功，则 uid 所指定的用户会通过 [onUserBinaryMessageReceivedOutsideRoom](70096#IRTCVideoEventHandler-onuserbinarymessagereceivedoutsideroom) 回调收到该条消息。
...
- 调用本接口后，会收到一次 [onServerMessageSendResult](70096#IRTCVideoEventHandler-onservermessagesendresult) 回调，通知消息发送方是否发送成功。
...
- 调用本接口后，会收到一次 [onServerMessageSendResult](70096#IRTCVideoEventHandler-onservermessagesendresult) 回调，通知消息发送方发送成功或失败。
...
- 0: 调用成功。成功调用本接口后，会在 3s 内收到一次 [onNetworkDetectionResult](70096#IRTCVideoEventHandler-onnetworkdetectionresult) 回调，此后每 2s 会收到一次该回调，通知探测结果。
- < 0 : 调用失败。查看 [ReturnStatus](70098#ReturnStatus) 获得更多错误说明。
...
- 若探测停止，则会收到一次 [onNetworkDetectionStopped](70096#IRTCVideoEventHandler-onnetworkdetectionstopped) 通知探测停止。
...
- 调用本接口后，会收到一次 [onNetworkDetectionStopped](70096#IRTCVideoEventHandler-onnetworkdetectionstopped) 回调通知探测停止。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| source_type | AudioSourceType | 屏幕音频输入源类型, 参看 [AudioSourceType](70098#AudioSourceType) |
...
- 你应该在 [publishScreen](#IRTCRoom-publishscreen) 前，调用此方法。否则，你将收到 [onWarning](70096#IRTCVideoEventHandler-onwarning) 的报错：`kWarningCodeSetScreenAudioSourceTypeFailed`
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| index | StreamIndex | 混流方式，参看 [StreamIndex](70098#StreamIndex)<br><ul><li>`kStreamIndexMain`: 将屏幕音频流和麦克风采集到的音频流混流</li><li>`kStreamIndexScreen`: 默认值， 将屏幕音频流和麦克风采集到的音频流分为两路音频流</li></ul> |
...
你应该在 [publishScreen](#IRTCRoom-publishscreen) 之前，调用此方法。否则，你将收到 [onWarning](70096#IRTCVideoEventHandler-onwarning)`的报错：kWarningCodeSetScreenAudioStreamIndexFailed`
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| channel | AudioChannel | 声道数，参看 [AudioChannel](70098#AudioChannel) |
...
- 本接口仅对内部采集生效，RTC SDK 默认使用内部采集模块采集屏幕音频。若已调用 [setScreenAudioSourceType](#IRTCVideo-setscreenaudiosourcetype) 将音频输入源设置为 `kAudioSourceTypeExternal` 自定义采集，需先切换为 `kAudioSourceTypeInternal` 内部采集，否则该接口调用无效，并将触发 [onAudioDeviceWarning](70096#IRTCVideoEventHandler-onaudiodevicewarning) 回调。
...
- 调用本接口时，采集模式应为内部模式。在外部采集模式下调用无效，并将触发 [onAudioDeviceWarning](70096#IRTCVideoEventHandler-onaudiodevicewarning) 回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| frame | IAudioFrame * | 音频数据帧，参见 [IAudioFrame](70098#IAudioFrame)<br><ul><li>音频采样格式为 S16。音频缓冲区内的数据格式必须为 PCM 数据，其容量大小应该为 samples × frame.channel × 2。</li><li>必须指定具体的采样率和声道数，不支持设置为自动。</li></ul> |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_key | const RemoteStreamKey & | 作为对齐基准的远端音频流。参看 [RemoteStreamKey](70098#RemoteStreamKey)。 <br>一般选择主唱的音频流。 <br>你必须在收到 [onUserPublishStream](70096#IRTCRoomEventHandler-onuserpublishstream)，确认此音频流已发布后，调用此 API。 |
| mode | AudioAlignmentMode | 是否对齐，默认不对齐。参看 [AudioAlignmentMode](70098#AudioAlignmentMode)。 |
...
- 你必须在实时合唱场景下使用此功能。在加入房间时，所有人应设置 [RoomProfileType](70098#RoomProfileType) 为 `kRoomProfileTypeChorus`。
- 订阅的所有远端流必须通过 [startAudioMixing](#IAudioMixingManager-startaudiomixing) 开启了背景音乐混音，并将 [AudioMixingConfig](70098#AudioMixingConfig) 中的 `sync_progress_to_record_frame` 设置为 `true`。
...
| config | IMixedStreamConfig * | 转推直播配置参数。参看 [IMixedStreamConfig](70098#IMixedStreamConfig)。 |
| observer | IMixedStreamObserver * | 端云一体转推直播观察者。参看 [IMixedStreamObserver](70096#IMixedStreamObserver)。 <br>通过注册 observer 接收转推直播相关的回调。 |
...
- 调用该方法后，关于启动结果和推流过程中的错误，会收到 [onMixingEvent](70096#IMixedStreamObserver-onmixingevent) 回调。
...
更新合流转推直播参数，会收到 [onMixingEvent](70096#IMixedStreamObserver-onmixingevent) 回调。
...
| config | IMixedStreamConfig * | 转推直播配置参数，参看 [IMixedStreamConfig](70098#IMixedStreamConfig)。除特殊说明外，均支持过程中更新。 <br>调用时，结构体中没有传入值的属性，会被更新为默认值。 |
...
| param | PushSingleStreamParam & | 转推直播配置参数。详见 [PushSingleStreamParam](70098#pushsinglestreamparam)。 |
| observer | IPushSingleStreamToCDNObserver * | 单流转推直播观察者。详见 [IPushSingleStreamToCDNObserver](70096#IPushSingleStreamToCDNObserver)。 <br>通过注册 observer 接收单流转推直播相关的回调。 |
...
- 在调用该接口前，你需要在[控制台](https://console.volcengine.com/rtc/workplaceRTC)开启转推直播功能。
- 调用该方法后，关于启动结果和推流过程中的错误，会收到 [onStreamPushEvent](70096#IPushSingleStreamToCDNObserver-onstreampushevent) 回调。
...
| param | IPublicStreamParam * | 公共流参数。详见 [IPublicStreamParam](70098#IPublicStreamParam)。 <br>一路公共流可以包含多路房间内的媒体流，按照指定的布局方式进行聚合。 <br>如果指定的媒体流还未发布，则公共流将在指定流开始发布后实时更新。 |


**返回值**

- 0: 成功。同时将收到 [onPushPublicStreamResult](70096#IRTCVideoEventHandler-onpushpublicstreamresult) 回调。
...
| --- | --- | --- |
| public_stream_id | const char * | 公共流 ID |
| param | IPublicStreamParam * | 公共流参数。详见 [IPublicStreamParam](70098#IPublicStreamParam)。 <br>指定的流必须为当前用户所发布的。 |
...
调用本接口前需要通过 [onPushPublicStreamResult](70096#IRTCVideoEventHandler-onpushpublicstreamresult) 确认公共流是否已经成功启动。
...
```
启用音频信息提示。开启提示后，你会收到 [onLocalAudioPropertiesReport](70096#IRTCVideoEventHandler-onlocalaudiopropertiesreport)， [onRemoteAudioPropertiesReport](70096#IRTCVideoEventHandler-onremoteaudiopropertiesreport) 和 [onActiveSpeaker](70096#IRTCVideoEventHandler-onactivespeaker)。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| config | const AudioPropertiesConfig & | 详见 [AudioPropertiesConfig](70098#AudioPropertiesConfig) |
...
无论使用内部采集还是自定义采集，调用该方法后，SDK 每监测到一帧本地视频帧时，都会将视频帧信息通过 [onLocalEncodedVideoFrame](70096#ILocalEncodedVideoFrameObserver-onlocalencodedvideoframe) 回调给用户
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| observer | ILocalEncodedVideoFrameObserver * | 本地视频帧监测器，参看 [ILocalEncodedVideoFrameObserver](70096#ILocalEncodedVideoFrameObserver)。将参数设置为 nullptr 则取消注册。 |
...
完成注册后，当 SDK 监测到远端编码后视频帧时，会触发 [onRemoteEncodedVideoFrame](70096#IRemoteEncodedVideoFrameObserver-onremoteencodedvideoframe) 回调
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| observer | IRemoteEncodedVideoFrameObserver * | 远端编码后视频数据监测器，参看 [IRemoteEncodedVideoFrameObserver](70096#IRemoteEncodedVideoFrameObserver) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| encoder_handler | IExternalVideoEncoderEventHandler * | 自定义编码帧回调类，参看 [IExternalVideoEncoderEventHandler](70096#IExternalVideoEncoderEventHandler) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| index | StreamIndex | 需要推送的编码流的属性，参看 [StreamIndex](70098#StreamIndex) |
| video_index | int | 对应的编码流下标，从 0 开始，如果调用 [setVideoEncoderConfig](#IRTCVideo-setvideoencoderconfig) 设置了多路流，此处数量须与之保持一致 |
| video_stream | IEncodedVideoFrame * | 编码流视频帧信息，参看 [IEncodedVideoFrame](70098#IEncodedVideoFrame)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| key | RemoteStreamKey | 远端流信息，即对哪一路视频流进行解码方式设置，参看 [RemoteStreamKey](70098#RemoteStreamKey)。 |
| config | VideoDecoderConfig | 视频解码方式，参看 [VideoDecoderConfig](70098#VideoDecoderConfig)。 |
...
- 当你想要对远端流进行自定义解码时，你需要先调用 [registerRemoteEncodedVideoFrameObserver](#IRTCVideo-registerremoteencodedvideoframeobserver) 注册远端视频流监测器，然后再调用该接口将解码方式设置为自定义解码。监测到的视频数据会通过 [onRemoteEncodedVideoFrame](70096#IRemoteEncodedVideoFrameObserver-onremoteencodedvideoframe) 回调出来。
...
```
发送音频流同步信息。将消息通过音频流发送到远端，并实现与音频流同步，该接口调用成功后，远端用户会收到 [onStreamSyncInfoReceived](70096#IRTCVideoEventHandler-onstreamsyncinforeceived) 回调。
...
| config | const StreamSycnInfoConfig & | 媒体流信息同步的相关配置，详见 [StreamSycnInfoConfig](70098#StreamSycnInfoConfig) 。 |
...
- -4: 消息发送失败。通过用麦克风或自定义设备采集到的音频流进行消息同步时，此音频流还未发布，详见错误码 [ErrorCode](70097#ErrorCode)。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| pitch | int | 相对于语音原始音调的升高/降低值，取值范围[-12，12]，默认值为 0，即不做调整。<br>取值范围内每相邻两个值的音高距离相差半音，正值表示升调，负值表示降调，设置的绝对值越大表示音调升高或降低越多。<br>超出取值范围则设置失败，并且会触发 [onWarning](70096#IRTCVideoEventHandler-onwarning) 回调，提示 [WarningCode](70097#WarningCode) 错误码为 `WARNING_CODE_SET_SCREEN_STREAM_INVALID_VOICE_PITCH` 设置语音音调不合法 |
...
- 0: 成功。同时将收到 [onPlayPublicStreamResult](70096#IRTCVideoEventHandler-onplaypublicstreamresult) 回调。
- !0: 失败。当参数不合法或参数为空，调用失败。
...
- 调用本接口后，可以通过 [onFirstPublicStreamVideoFrameDecoded](70096#IRTCVideoEventHandler-onfirstpublicstreamvideoframedecoded) 和 [onFirstPublicStreamAudioFrame](70096#IRTCVideoEventHandler-onfirstpublicstreamaudioframe) 回调公共流的视频和音频首帧解码情况。
- 调用本接口后，可以通过 [onPublicStreamSEIMessageReceived](70096#IRTCVideoEventHandler-onpublicstreamseimessagereceived) 回调公共流中包含的 SEI 信息。
...
| --- | --- | --- |
| public_stream_id | const char * | 公共流 ID |
| canvas | const VideoCanvas & | 内部渲染视图，如果需要解除视频的绑定视图，把 videoCanvas 设置为空。详见 [VideoCanvas](70098#VideoCanvas)。 |
...
| --- | --- | --- |
| public_stream_id | const char * | 公共流 ID |
| video_sink | IVideoSink * | 自定义视频渲染器，自定义视频渲染器，需要释放渲染器资源时，将 videoSink 设置为 `null`。参看 [IVideoSink](70098#IVideoSink) |
| format | IVideoSink::PixelFormat | videoSink 适用的视频帧编码格式，参看 [PixelFormat](70098#PixelFormat) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | StreamIndex | 需要添加水印的视频流属性，参看 [StreamIndex](70098#StreamIndex)。 |
| image_path | const char * | 水印图片路径，仅支持本地文件绝对路径，长度限制为 512 字节。<br>水印图片为 PNG 或 JPG 格式。 |
| config | RTCWatermarkConfig | 水印参数，参看 [RTCWatermarkConfig](70098#RTCWatermarkConfig)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | StreamIndex | 需要移除水印的视频流属性，参看 [StreamIndex](70098#StreamIndex)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | const StreamIndex | 截图的视频流的属性，参看 [StreamIndex](70098#StreamIndex)。 |
| callback | ISnapshotResultCallback * | 本地截图的回调。参看 [ISnapshotResultCallback](70096#ISnapshotResultCallback)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_key | const RemoteStreamKey | 截图的视频流，参看 [RemoteStreamKey](70098#RemoteStreamKey)。 |
| callback | ISnapshotResultCallback * | 参看 [ISnapshotResultCallback](70096#ISnapshotResultCallback)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| configuration | const CloudProxyConfiguration & | 云代理服务器信息列表。参看 [CloudProxyConfiguration](70098#CloudProxyConfiguration)。 |
...
- 在加入房间前调用此接口
- 在开启云代理后，进行通话前网络探测
- 开启云代理后，并成功链接云代理服务器后，会收到 [onCloudProxyConnected](70096#IRTCVideoEventHandler-oncloudproxyconnected)。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| echo_test_config | EchoTestConfig | 回路测试参数设置，参看 [EchoTestConfig](70098#EchoTestConfig)。 |
...
- 在调用 [startEchoTest](#IRTCVideo-startechotest) 和 [stopEchoTest](#IRTCVideo-stopechotest) 之间调用的所有跟设备采集、流控制、进房相关的方法均不生效，并会收到 [onWarning](70096#IRTCVideoEventHandler-onwarning) 回调，提示警告码为 `kWarningCodeInEchoTestMode`。
- 音视频回路检测的结果会通过 [onEchoTestResult](70096#IRTCVideoEventHandler-onechotestresult) 回调通知。
...
网络时间。参看 [NetworkTimeInfo](70098#NetworkTimeInfo)。


**注意**

- 第一次调用此接口会启动网络时间同步功能，并返回 `0`。同步完成后，会收到 [onNetworkTimeSynchronized](70096#IRTCVideoEventHandler-onnetworktimesynchronized)，此后，再次调用此 API，即可获取准确的网络时间。
...
- 只有当 [RoomProfileType](70098#RoomProfileType) 为 `kRoomProfileTypeMeeting` 和 `kRoomProfileTypeMeetingRoom` 时支持开启本功能。
- 开启检测前，你需要向用户获取音频设备的使用权限。
- 开启检测前，请确保音频设备没有被静音，采集和播放音量正常。
- 调用本接口后监听 [onHardwareEchoDetectionResult](70096#IRTCVideoEventHandler-onhardwareechodetectionresult) 获取检测结果。
...
- 建议在收到 [onHardwareEchoDetectionResult](70096#IRTCVideoEventHandler-onhardwareechodetectionresult) 通知的检测结果后，调用本接口停止检测。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| configurations | const LocalProxyConfiguration * | 本地代理配置参数。参看 [LocalProxyConfiguration](70098#LocalProxyConfiguration)。 <br>你可以根据自己的需要选择同时设置 Http 隧道 和 Socks5 两类代理，或者单独设置其中一类代理。如果你同时设置了 Http 隧道 和 Socks5 两类代理，此时，媒体和信令采用 Socks5 代理， Http 请求采用 Http 隧道代理；如果只设置 Http 隧道 或 Socks5 一类代理，媒体、信令和 Http 请求均采用已设置的代理。 <br>调用此接口设置本地代理后，若想清空当前已有的代理设置，可再次调用此接口，选择不设置任何代理即可清空。 |
...
- 该方法需要在进房前调用。
- 调用该方法设置本地代理后，SDK 会触发 [onLocalProxyStateChanged](70096#IRTCVideoEventHandler-onlocalproxystatechanged)，返回代理连接的状态。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| mode | VideoEnhancementMode | 默认不开启。参看 [VideoEnhancementMode](70098#VideoEnhancementMode)。 |


**返回值**

- 0: API 调用成功。会立即生效，但需要等待下载和检测完成后才能看到增强后的效果。
- < 0: API 调用失败。查看 [ReturnStatus](70098#ReturnStatus) 获得更多错误说明。
...
| --- | --- | --- |
| index | StreamIndex | 视频流属性，参看 [StreamIndex](70098#StreamIndex)。 |
| solutions | const VideoSolution * | 要推送的多路视频流参数，参看 [VideoSolution](70098#VideoSolution)。 <br>最大分辨率为 4096px × 4096px，超过或设置的分辨率无法编码时，会导致编码推流失败。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_key | RemoteStreamKey | 远端流信息。参看 [RemoteStreamKey](70098#RemoteStreamKey)。 |
| render_mode | const enum RenderMode | 渲染模式，参看 [RenderMode](70098#RenderMode)。 |
| background_color | const uint32_t | 背景颜色，参看 [VideoCanvas](70098#VideoCanvas).background_color |
...
| index | StreamIndex | 视频流属性。采集的视频流/屏幕视频流，参看 [StreamIndex](70098#StreamIndex) |
| video_sink | IVideoSink * | 自定义视频渲染器，参看 [IVideoSink](70098#IVideoSink)。 |
| required_format | IVideoSink::PixelFormat | video_sink 适用的视频帧编码格式，参看 [PixelFormat](70098#PixelFormat) |
...
| --- | --- | --- |
| task_id | const char * | 转推直播任务 ID，长度不超过 126 字节。<br>你可以在同一房间内发起多个转推直播任务，并用不同的任务 ID 加以区分。当你需要发起多个转推直播任务时，应使用多个 ID；当你仅需发起一个转推直播任务时，建议使用空字符串。 |
| param | ITranscoderParam * | 转推直播配置参数。参看 [ITranscoderParam](70098#ITranscoderParam)。 |
| observer | ITranscoderObserver * | 端云一体转推直播观察者。参看 [ITranscoderObserver](70096#ITranscoderObserver)。 <br>通过注册 observer 接收转推直播相关的回调。 |
...
- 在调用该接口前，你需要在[控制台](https://console.volcengine.com/rtc/workplaceRTC)开启转推直播功能。
- 调用该方法后，关于启动结果和推流过程中的错误，会收到 [onStreamMixingEvent](70096#ITranscoderObserver-onstreammixingevent) 回调。
...
停止转推直播，会收到 [onStreamMixingEvent](70096#ITranscoderObserver-onstreammixingevent) 回调。
...
更新转推直播参数，会收到 [onStreamMixingEvent](70096#ITranscoderObserver-onstreammixingevent) 回调。
...
| --- | --- | --- |
| task_id | const char * | 转推直播任务 ID。指定想要更新参数设置的转推直播任务。 |
| param | ITranscoderParam * | 转推直播配置参数，参看 [ITranscoderParam](70098#ITranscoderParam)。除特殊说明外，均支持过程中更新。 <br>调用时，结构体中没有传入值的属性，会被更新为默认值。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| mute_state | MuteState | 播放状态，标识是否播放本地音频流，详见： [MuteState](70098#MuteState) |
...
| handler | ISingScoringEventHandler * | K 歌评分事件回调类，详见 [ISingScoringEventHandler](70096#ISingScoringEventHandler)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| config | const SingScoringConfig & | K 歌评分的各项参数，详见 [SingScoringConfig](70098#SingScoringConfig)。 |
...
- 调用该接口后，将会根据设置的回调时间间隔，收到评分结果 [onCurrentScoringInfo](70096#ISingScoringEventHandler-oncurrentscoringinfo) 回调。
- 如果调用 [startAudioMixing](#IAudioMixingManager-startaudiomixing) 接口播放音频文件，请在收到 [onAudioMixingStateChanged](70096#IRTCVideoEventHandler-onaudiomixingstatechanged)(AUDIO_MIXING_STATE_PLAYING(1)) 之后调用此接口。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| ktv_manager_event_handler | IKTVManagerEventHandler * | KTV 回调类，参看 [IKTVManagerEventHandler](70096#IKTVManagerEventHandler)。 |
...
| filters | int | 歌曲过滤方式，参看 [MusicFilterType](70098#MusicFilterType)。 |


**注意**

调用接口后，你会收到 [onMusicListResult](70096#IKTVManagerEventHandler-onmusiclistresult) 回调歌曲列表。
...
调用接口后，你会收到 [onSearchMusicResult](70096#IKTVManagerEventHandler-onsearchmusicresult) 回调歌曲列表。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| hot_types | int | 榜单类别，参看 [MusicHotType](70098#MusicHotType)。 |
| filters | int | 歌曲过滤方式，参看 [MusicFilterType](70098#MusicFilterType)。 |


**注意**

调用接口后，你会收到 [onHotMusicResult](70096#IKTVManagerEventHandler-onhotmusicresult) 回调歌曲列表。
...
调用接口后，你会收到 [onMusicDetailResult](70096#IKTVManagerEventHandler-onmusicdetailresult) 回调。
...
- 若音乐下载成功，你会收到 [onDownloadSuccess](70096#IKTVManagerEventHandler-ondownloadsuccess) 回调。
- 若音乐下载失败，你会收到 [onDownloadFailed](70096#IKTVManagerEventHandler-ondownloadfailed) 回调。
- 音乐下载进度更新时，你会收到 [onDownloadMusicProgress](70096#IKTVManagerEventHandler-ondownloadmusicprogress) 回调。
...
| --- | --- | --- |
| music_id | const char * | 音乐 ID。 |
| type | DownloadLyricType | 歌词文件类型，参看 [DownloadLyricType](70098#DownloadLyricType)。 |
...
- 若歌词下载成功，你会收到 [onDownloadSuccess](70096#IKTVManagerEventHandler-ondownloadsuccess) 回调。
- 若歌词下载失败，你会收到 [onDownloadFailed](70096#IKTVManagerEventHandler-ondownloadfailed) 回调。
...
- 若文件下载成功，你会收到 [onDownloadSuccess](70096#IKTVManagerEventHandler-ondownloadsuccess) 回调。
- 若文件下载失败，你会收到 [onDownloadFailed](70096#IKTVManagerEventHandler-ondownloadfailed) 回调。
...
| --- | --- | --- |
| index | int | 设备索引号，从 0 开始，注意需小于 [getCount](#IAudioDeviceCollection-getcount) 返回值。 |
| audio_device_info | AudioDeviceInfo * | 设备信息，详见 [AudioDeviceInfo](70098#AudioDeviceInfo) |
...
屏幕源类型信息，详见 [ScreenCaptureSourceInfo](70098#ScreenCaptureSourceInfo)。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| position_info | const PositionInfo & | 空间音频位置信息。参看 [PositionInfo](70098#PositionInfo)。 |
...
| position_info | const PositionInfo & | 远端用户的空间音频位置信息。参看 [PositionInfo](70098#PositionInfo)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| pos | const Position & | 三维坐标的值，默认为 [0, 0, 0]。参看 [Position](70098#Position)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| orientation | const HumanOrientation & | 参看 [HumanOrientation](70098#HumanOrientation)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| pos | const Position & | 空间直角坐标系下的坐标值。参看 [Position](70098#Position)。 <br>如果未调用此接口设定收听位置，那么默认值为通过 [updatePosition](#ISpatialAudio-updateposition) 设定的值。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| orientation | const HumanOrientation & | 自身朝向信息，参看 [HumanOrientation](70098#HumanOrientation)。 <br>如果未调用此接口设定收听朝向，那么默认值为通过 [updateSelfOrientation](#ISpatialAudio-updateselforientation) 设定的值。 |
...
| --- | --- | --- |
| index | int | 设备索引号，从 0 开始，注意需小于 [getCount](#IVideoDeviceCollection-getcount) 返回值。 |
| video_device_info | VideoDeviceInfo * | 设备信息，参看 [VideoDeviceInfo](70098#videodeviceinfo)。 |
...
- 设置用户可见性，会收到设置成功/失败回调 [onUserVisibilityChanged](70096#IRTCRoomEventHandler-onuservisibilitychanged)。（v3.54 新增） &#x0020; • 在加入房间前设置用户可见性，若设置的可见性与默认值不同，将在加入房间时触发本回调。

&#x0020; • 在加入房间后设置用户可见性，若可见性前后不同，会触发本回调。

&#x0020; • 在断网重连后，若可见性发生改变，会触发本回调。
- 在房间内，调用此方法成功切换用户可见性后，房间内其他用户会收到相应的回调。 &#x0020; • 从可见换至不可见时，房间内其他用户会收到 [onUserLeave](70096#IRTCRoomEventHandler-onuserleave)。

&#x0020; • 从不可见切换至可见时，房间内其他用户会收到 [onUserJoined](70096#IRTCRoomEventHandler-onuserjoined)。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| room_event_handler | IRTCRoomEventHandler * | 参见 [IRTCRoomEventHandler](70096#IRTCRoomEventHandler) |
...
| user_info | const UserInfo & | 用户信息，参看 [UserInfo](70098#UserInfo)。 |
| config | const RTCRoomConfig & | 房间参数配置，设置房间模式以及是否自动发布或订阅流。具体配置模式参看 [RTCRoomConfig](70098#RTCRoomConfig)。 |
...
方法调用结果。
- `0`: 调用成功。
- `-1`,`-2`,或 `-4`: 调用失败。查看 [ReturnStatus](70098#ReturnStatus) 获得错误说明。


**注意**

- 同一个 App ID 的同一个房间内，每个用户的用户 ID 必须是唯一的。如果两个用户的用户 ID 相同，则后进房的用户会将先进房的用户踢出房间，并且先进房的用户会收到 [onRoomStateChanged](70096#IRTCRoomEventHandler-onroomstatechanged) 回调通知，错误类型详见 [ErrorCode](70097#ErrorCode) 中的 kErrorCodeDuplicateLogin。
- 本地用户调用此方法加入房间成功后，会收到 [onRoomStateChanged](70096#IRTCRoomEventHandler-onroomstatechanged) 回调通知。若本地用户同时为可见用户，加入房间时远端用户会收到 [onUserJoined](70096#IRTCRoomEventHandler-onuserjoined) 回调通知。
- 房间内不可见用户的容量远远大于可见用户，而且用户默认可见，因此对于不参与互动的用户，你需要在其进房后调用 setUserVisibility(https://www.volcengine.com/docs/6348/257549)。用户加入房间成功后，在本地网络状况不佳的情况下，SDK 可能会与服务器失去连接，并触发 [onConnectionStateChanged](70096#IRTCVideoEventHandler-onconnectionstatechanged) 回调。此时 SDK 会自动重试，直到成功重连。重连成功后，本地会收到 [onRoomStateChanged](70096#IRTCRoomEventHandler-onroomstatechanged) 回调通知。
...
- 此方法是异步操作，调用返回时并没有真正退出房间。真正退出房间后，本地会收到 [onLeaveRoom](70096#IRTCRoomEventHandler-onleaveroom) 回调通知。你必须在收到 [onLeaveRoom](70096#IRTCRoomEventHandler-onleaveroom) 回调后，再销毁房间或引擎，或调用 [joinRoom](#IRTCRoom-joinroom) 再次加入房间。
- 调用 [setUserVisibility](#IRTCRoom-setuservisibility) 将自身设为可见的用户离开房间后，房间内其他用户会收到 [onUserLeave](70096#IRTCRoomEventHandler-onuserleave) 回调通知。
...
收到 [onTokenWillExpire](70096#IRTCRoomEventHandler-ontokenwillexpire)， [onPublishPrivilegeTokenWillExpire](70096#IRTCRoomEventHandler-onpublishprivilegetokenwillexpire)， 或 [onSubscribePrivilegeTokenWillExpire](70096#IRTCRoomEventHandler-onsubscribeprivilegetokenwillexpire) 时，你必须重新获取 Token，并调用此方法更新 Token，以保证通话的正常进行。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| token | const char * | 重新获取的有效 Token。<br>如果 Token 无效，你会收到 [onRoomStateChanged](70096#IRTCRoomEventHandler-onroomstatechanged)，错误码是 `-1010`。 |
...
- 调用该函数后会收到一次 [onUserMessageSendResult](70096#IRTCRoomEventHandler-onusermessagesendresult) 回调，通知消息发送方发送成功或失败；
- 若文本消息发送成功，则 uid 所指定的用户会收到 [onUserMessageReceived](70096#IRTCRoomEventHandler-onusermessagereceived) 回调。
...
- 调用该函数后会收到一次 [onUserMessageSendResult](70096#IRTCRoomEventHandler-onusermessagesendresult) 回调，通知消息发送方发送成功或失败；
- 若二进制消息发送成功，则 uid 所指定的用户会收到 [onUserBinaryMessageReceived](70096#IRTCRoomEventHandler-onuserbinarymessagereceived) 回调。
...
- 在发送房间内二进制消息前，必须先调用 [joinRoom](#IRTCRoom-joinroom) 加入房间。
- 调用该函数后，会收到一次 [onRoomMessageSendResult](70096#IRTCRoomEventHandler-onroommessagesendresult) 回调。
- 同一房间内的其他用户会收到 [onRoomMessageReceived](70096#IRTCRoomEventHandler-onroommessagereceived) 回调。
...
- 在发送房间内二进制消息前，必须先调用 [joinRoom](#IRTCRoom-joinroom) 加入房间。
- 调用该函数后，会收到一次 [onRoomMessageSendResult](70096#IRTCRoomEventHandler-onroommessagesendresult) 回调。
- 同一房间内的其他用户会收到 [onRoomBinaryMessageReceived](70096#IRTCRoomEventHandler-onroombinarymessagereceived) 回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | MediaStreamType | 媒体流类型，用于指定发布音频/视频，参看 [MediaStreamType](70098#MediaStreamType) |
...
- 调用此方法后，房间中的所有远端用户会收到 [onUserPublishStream](70096#IRTCRoomEventHandler-onuserpublishstream) 回调通知，其中成功收到了音频流的远端用户会收到 [onFirstRemoteAudioFrame](70096#IRTCVideoEventHandler-onfirstremoteaudioframe) 回调，订阅了视频流的远端用户会收到 [onFirstRemoteVideoFrameDecoded](70096#IRTCVideoEventHandler-onfirstremotevideoframedecoded) 回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | MediaStreamType | 媒体流类型，用于指定停止发布音频/视频，参看 [MediaStreamType](70098#MediaStreamType) |
...
- 调用此方法停止发布音视频流后，房间中的其他用户将会收到 [onUserUnpublishStream](70096#IRTCRoomEventHandler-onuserunpublishstream) 回调通知。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | MediaStreamType | 媒体流类型，用于指定发布屏幕音频/视频，参看 [MediaStreamType](70098#MediaStreamType)。 |
...
- 调用该方法后，房间中的所有远端用户会收到 [onUserPublishScreen](70096#IRTCRoomEventHandler-onuserpublishscreen) 回调，其中成功收到音频流的远端用户会收到 [onFirstRemoteAudioFrame](70096#IRTCVideoEventHandler-onfirstremoteaudioframe) 回调，订阅了视频流的远端用户会收到 [onFirstRemoteVideoFrameDecoded](70096#IRTCVideoEventHandler-onfirstremotevideoframedecoded) 回调。
- 调用该方法后，本地用户会收到 [onScreenVideoFrameSendStateChanged](70096#IRTCVideoEventHandler-onscreenvideoframesendstatechanged)。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | MediaStreamType | 媒体流类型，用于指定停止发布屏幕音频/视频，参看 [MediaStreamType](70098#MediaStreamType) |
...
- 调用此方法停止发布屏幕音视频流后，房间中的其他用户将会收到 [onUserUnpublishScreen](70096#IRTCRoomEventHandler-onuserunpublishscreen) 回调。
...
| remote_video_config | const RemoteVideoConfig & | 期望配置的远端视频流参数，参看 [RemoteVideoConfig](70098#RemoteVideoConfig)。 |
...
| type | MediaStreamType | 媒体流类型，用于指定订阅音频/视频。参看 [MediaStreamType](70098#MediaStreamType)。 |
...
- 你必须先通过 [onUserPublishStream](70096#IRTCRoomEventHandler-onuserpublishstream) 回调获取当前房间里的远端摄像头音视频流信息，然后调用本方法按需订阅。
- 调用该方法后，你会收到 [onStreamSubscribed](70096#IRTCRoomEventHandler-onstreamsubscribed) 通知方法调用结果。
- 成功订阅远端用户的媒体流后，订阅关系将持续到调用 [unsubscribeStream](#IRTCRoom-unsubscribestream) 取消订阅或本端用户退房。
- 关于其他调用异常，你会收到 [onStreamStateChanged](70096#IRTCRoomEventHandler-onstreamstatechanged) 回调通知，具体异常原因参看 [ErrorCode](70097#ErrorCode)。
...
| type | MediaStreamType | 媒体流类型，用于指定取消订阅音频/视频。参看 [MediaStreamType](70098#MediaStreamType)。 |
...
- 调用该方法后，你会收到 [onStreamSubscribed](70096#IRTCRoomEventHandler-onstreamsubscribed) 通知方法调用结果。
- 关于其他调用异常，你会收到 [onStreamStateChanged](70096#IRTCRoomEventHandler-onstreamstatechanged) 回调通知，具体失败原因参看 [ErrorCode](70097#ErrorCode)。
...
- 若当前用户在调用本接口时已经订阅该远端用户（手动订阅或自动订阅），则将根据本次传入的参数，更新订阅配置。
- 你必须先通过 [onUserPublishScreen](70096#IRTCRoomEventHandler-onuserpublishscreen)} 回调获取当前房间里的远端屏幕流信息，然后调用本方法按需订阅。
- 调用该方法后，你会收到 [onStreamSubscribed](70096#IRTCRoomEventHandler-onstreamsubscribed) 通知方法调用结果。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| media_type | PauseResumeControlMediaType | 媒体流类型，指定需要暂停接收音频还是视频流，参看 [PauseResumeControlMediaType](70098#PauseResumeControlMediaType)。 |
...
- 调用该接口后音画同步状态发生改变时，你会收到 [onAVSyncStateChange](70096#IRTCRoomEventHandler-onavsyncstatechange) 回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| configuration | const ForwardStreamConfiguration & | 跨房间媒体流转发指定房间的信息。参看 [ForwardStreamConfiguration](70098#ForwardStreamConfiguration)。 |
...
- 调用本方法后，将在本端触发 [onForwardStreamStateChanged](70096#IRTCRoomEventHandler-onforwardstreamstatechanged) 回调。
- 调用本方法后，你可以通过监听 [onForwardStreamEvent](70096#IRTCRoomEventHandler-onforwardstreamevent) 回调来获取各个目标房间在转发媒体流过程中的相关事件。
- 开始转发后，目标房间中的用户将接收到本地用户进房 [onUserJoined](70096#IRTCRoomEventHandler-onuserjoined) 和发流 [onUserPublishStream](70096#IRTCRoomEventHandler-onuserpublishstream)/ [onUserPublishScreen](70096#IRTCRoomEventHandler-onuserpublishscreen) 的回调。
...
调用本方法增加或删减房间后，将在本端触发 [onForwardStreamStateChanged](70096#IRTCRoomEventHandler-onforwardstreamstatechanged) 回调，包含发生了变动的目标房间中媒体流转发状态。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| configuration | const ForwardStreamConfiguration & | 跨房间媒体流转发目标房间信息。参看 [ForwardStreamConfiguration](70098#ForwardStreamConfiguration)。 |
...
- 增加目标房间后，新增目标房间中的用户将接收到本地用户进房 [onUserJoined](70096#IRTCRoomEventHandler-onuserjoined) 和发布 [onUserPublishStream](70096#IRTCRoomEventHandler-onuserpublishstream)/ [onUserPublishScreen](70096#IRTCRoomEventHandler-onuserpublishscreen) 的回调。
- 删减目标房间后，原目标房间中的用户将接收到本地用户停止发布 [onUserUnpublishStream](70096#IRTCRoomEventHandler-onuserunpublishstream)/ [onUserUnpublishScreen](70096#IRTCRoomEventHandler-onuserunpublishscreen) 和退房 [onUserLeave](70096#IRTCRoomEventHandler-onuserleave) 的回调。
...
- 调用本方法后，将在本端触发 [onForwardStreamStateChanged](70096#IRTCRoomEventHandler-onforwardstreamstatechanged) 回调。
- 调用本方法后，原目标房间中的用户将接收到本地用户停止发布 [onUserUnpublishStream](70096#IRTCRoomEventHandler-onuserunpublishstream)/ [onUserUnpublishScreen](70096#IRTCRoomEventHandler-onuserunpublishscreen) 和退房 [onUserLeave](70096#IRTCRoomEventHandler-onuserleave) 的回调。
...
调用本方法后，目标房间中的用户将接收到本地用户停止发布 [onUserUnpublishStream](70096#IRTCRoomEventHandler-onuserunpublishstream)/ [onUserUnpublishScreen](70096#IRTCRoomEventHandler-onuserunpublishscreen) 和退房 [onUserLeave](70096#IRTCRoomEventHandler-onuserleave) 的回调。
...
目标房间中的用户将接收到本地用户进房 [onUserJoined](70096#IRTCRoomEventHandler-onuserjoined) 和发布 [onUserPublishStream](70096#IRTCRoomEventHandler-onuserpublishstream)/ [onUserPublishScreen](70096#IRTCRoomEventHandler-onuserpublishscreen) 的回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audio_selection_priority | AudioSelectionPriority | 本端发布流在音频选路中的优先级，默认正常参与音频选路。参见 [AudioSelectionPriority](70098#AudioSelectionPriority)。 |
...
- <0: 方法调用失败，具体原因详见 [SetRoomExtraInfoResult](70098#SetRoomExtraInfoResult)。


**注意**

- 在设置房间附加信息前，必须先调用 [joinRoom](#IRTCRoom-joinroom) 加入房间。
- 调用该方法后，会收到一次 [onSetRoomExtraInfoResult](70096#IRTCRoomEventHandler-onsetroomextrainforesult) 回调，提示设置结果。
- 调用该方法成功设置附加信息后，同一房间内的其他用户会收到关于该信息的回调 [onRoomExtraInfoUpdate](70096#IRTCRoomEventHandler-onroomextrainfoupdate)。
...
调用该方法时，可以在 [SubtitleMode](70098#SubtitleMode) 中选择语音识别或翻译模式。如果选择识别模式，语音识别文本会通过 [onSubtitleMessageReceived](70096#IRTCRoomEventHandler-onsubtitlemessagereceived) 事件回调给你；

如果选择翻译模式，你会同时收到两个 [onSubtitleMessageReceived](70096#IRTCRoomEventHandler-onsubtitlemessagereceived) 回调，分别包含字幕原文及字幕译文。

调用该方法后，用户会收到 [onSubtitleStateChanged](70096#IRTCRoomEventHandler-onsubtitlestatechanged) 回调，通知字幕是否开启。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| subtitle_config | const SubtitleConfig & | 字幕配置信息。参看 [SubtitleConfig](70098#SubtitleConfig)。 |
...
调用该方法后，用户会收到 [onSubtitleStateChanged](70096#IRTCRoomEventHandler-onsubtitlestatechanged) 回调，通知字幕是否关闭。
...
| --- | --- | --- |
| user_id | const char * | 指定订阅的远端发布音视频流的用户 ID。 |
| stream_type | StreamIndex | 流属性，用于指定订阅主流/屏幕流，参看 [StreamIndex](70098#StreamIndex)。 |
| media_type | SubscribeMediaType | 媒体类型，用于指定订阅音/视频，参看 [SubscribeMediaType](70098#SubscribeMediaType)。 |
| video_config | const SubscribeVideoConfig & | 视频订阅配置，参看 [SubscribeVideoConfig](70098#SubscribeVideoConfig)。 |


**注意**

- 若订阅失败，你会收到 [onStreamStateChanged](70096#IRTCRoomEventHandler-onstreamstatechanged) 回调通知，具体失败原因参看 [ErrorCode](70097#ErrorCode)。
...
| event_handler | bytertc::IRTCVideoEventHandler * | SDK 回调给应用层的 Callback 对象，详见 [IRTCVideoEventHandler](70096#IRTCVideoEventHandler) 。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| log_config | const LogConfig & | 本地日志参数，参看 [LogConfig](70098#LogConfig)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| builder | const AudioFrameBuilder & | 音频帧构建实例，参看 [AudioFrameBuilder](70098#audioframebuilder) |


**返回值**

详见 [IAudioFrame](70098#IAudioFrame)
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| builder | const VideoFrameBuilder & | 视频帧构建实例，参看 [VideoFrameBuilder](70098#VideoFrameBuilder) |


**返回值**

[IVideoFrame](70098#IVideoFrame) 实例
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| builder | const EncodedVideoFrameBuilder & | 编码后的视频帧构建实例，参看 [EncodedVideoFrameBuilder](70098#EncodedVideoFrameBuilder) |


**返回值**

[IEncodedVideoFrame](70098#IEncodedVideoFrame) 实例
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| device_type | HWDeviceType | 参看 [HWDeviceType](70098#HWDeviceType)。 |


**返回值**

参看 [IHWDeviceContext](70098#IHWDeviceContext)。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| config | const streamingrtc::EngineConfig & | 设置引擎类型服务端或客户端，端口范围，和 appid。详见 [EngineConfig](70098#EngineConfig)。 |
| handler | IStreamingRTCEventHandler * | SDK 回调给应用层的回调对象，详见 [IStreamingRTCEventHandler](70096#IStreamingRTCEventHandler) 。 |
...
通过 [EngineConfig](70098#EngineConfig) 设置引擎类型服务端或客户端和端口号范围
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audio_frame | bytertc::IAudioFrame * | 接收到的音频帧，参看 [IAudioFrame](70098#IAudioFrame)<br>关于推送音频帧，参看 [pushAudioFrame](#IStreamingRTCEngine-pushaudioframe) |


**返回值**

参看 [ReturnStatus](70098#ReturnStatus)。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audio_frame | bytertc::IAudioFrame * | 发送的音频帧，参看 [IAudioFrame](70098#IAudioFrame)<br>创建音频帧，参看 [AudioFrameBuilder](70098#audioframebuilder) 和 [buildAudioFrame](#buildaudioframe)。 <br>关于接收音频帧，参看 [pullAudioFrame](#IStreamingRTCEngine-pullaudioframe) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | streamingrtc::StreamIndex | 视频流标识，参看 [StreamIndex](70098#StreamIndex-2)。 |
| solutions | const VideoSolution * | 要推送的多路视频流参数，参看 [VideoSolution](70098#VideoSolution)。 <br>最大分辨率为 4096 px × 4096 px，超过或设置的分辨率无法编码时，会导致编码推流失败。 |
| solution_num | int | 视频参数数组长度。<br>最多支持 4 路参数。当设置了多路参数时，分辨率必须是从大到小排列。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | streamingrtc::StreamIndex | 远端流信息，即对哪一路视频流进行解码方式设置，参看 [StreamIndex](70098#StreamIndex-2)。 |
| config | VideoDecoderConfig | 视频解码方式，参看 [VideoDecoderConfig](70098#VideoDecoderConfig)。 |


**注意**

当你想要对远端流进行自定义解码时，调用该接口将解码方式设置为自定义解码。监测到的视频数据会通过 [onP2PRemoteEncodedVideoFrame](70096#IStreamingRTCEventHandler-onp2premoteencodedvideoframe) 和/或 [onP2PRemoteVideoFrame](70096#IStreamingRTCEventHandler-onp2premotevideoframe) 回调出来。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | StreamIndex | 参看 [StreamIndex](70098#StreamIndex-2)。 |
| hw_context | IHWDeviceContext * | 参看 [IHWDeviceContext](70098#IHWDeviceContext)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | streamingrtc::StreamIndex | 视频流的属性，参看 [StreamIndex](70098#StreamIndex-2) |
| type | VideoSourceType | 视频输入源类型。参看 [VideoSourceType](70098#VideoSourceType)，暂不支持内部编码模式 `VideoSourceTypeInternal`。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | streamingrtc::StreamIndex | 选择作为哪一路流发送视频帧 参看 [StreamIndex](70098#StreamIndex-2) |
| frame | IVideoFrame * | 设置视频帧，暂时只支持 YUV420P 格式的视频帧。参看 [IVideoFrame](70098#IVideoFrame)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | streamingrtc::StreamIndex | 需要推送的编码流的属性，参看 [StreamIndex](70098#StreamIndex-2)。 |
...
| video_stream | IEncodedVideoFrame * | 编码流视频帧信息，仅支持推送 H264 和 ByteVC1 格式的视频帧。参看 [IEncodedVideoFrame](70098#IEncodedVideoFrame)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_index | streamingrtc::StreamIndex | 选择哪一路流请求视频关键帧 参看 [StreamIndex](70098#StreamIndex-2)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| handler | IKTVPlayerEventHandler * | KTV 播放器回调类，参看 [IKTVPlayerEventHandler](70096#IKTVPlayerEventHandler)。 |
...
| track_type | AudioTrackType | 原唱伴唱类型，参看 [AudioTrackType](70098#AudioTrackType)。 |
| play_type | AudioPlayType | 音乐播放类型。参看 [AudioPlayType](70098#AudioPlayType)。 |


**注意**

- 调用接口后，你会收到 [onPlayStateChanged](70096#IKTVPlayerEventHandler-onplaystatechanged) 回调歌曲播放状态。
- 若音乐 ID 错误，会触发 [onPlayStateChanged](70096#IKTVPlayerEventHandler-onplaystatechanged) 回调，error_code 为 –3023，play_state 为 4。
- 若未进房，会触发 [onPlayStateChanged](70096#IKTVPlayerEventHandler-onplaystatechanged) 回调，error_code 为 –3022，play_state 为 4。
- 若音乐文件不存在，会触发 [onPlayStateChanged](70096#IKTVPlayerEventHandler-onplaystatechanged) 回调，error_code 为 –3020，play_state 为 4。
...
| config | const MediaPlayerConfig & | 详见 [MediaPlayerConfig](70098#MediaPlayerConfig)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| source | const MediaPlayerCustomSource & | 数据源，详见 [MediaPlayerCustomSource](70098#MediaPlayerCustomSource) |
| config | const MediaPlayerConfig & | 详见 [MediaPlayerConfig](70098#MediaPlayerConfig) |
...
| type | AudioMixingType | 详见 [AudioMixingType](70098#AudioMixingType)。 |
...
```
设置音频文件混音时，收到 [onMediaPlayerPlayingProgress](70096#IMediaPlayerEventHandler-onmediaplayerplayingprogress) 的间隔。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| observer | IMediaPlayerAudioFrameObserver * | 参看 [IMediaPlayerAudioFrameObserver](70096#IMediaPlayerAudioFrameObserver)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audio_frame | IAudioFrame * | 音频帧，详见 [IAudioFrame](70098#IAudioFrame)。 <br><ul><li>音频采样格式必须为 S16。音频缓冲区内的数据格式必须为 PCM，其容量大小应该为 audioFrame.samples × audioFrame.channel × 2。</li><li>必须指定具体的采样率和声道数，不支持设置为自动。</li></ul> |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| handler | IMediaPlayerEventHandler * | 参看 [IMediaPlayerEventHandler](70096#IMediaPlayerEventHandler)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| effect_id | int | 音效 ID。用于标识音效，请保证音效 ID 唯一性。<br>如果使用相同的 ID 重复调用本方法后，上一个音效会停止，下一个音效开始，并收到 [onAudioEffectPlayerStateChanged](70096#IAudioEffectPlayerEventHandler-onaudioeffectplayerstatechanged)。 |
...
| config | const AudioEffectPlayerConfig & | 音效配置，详见 [AudioEffectPlayerConfig](70098#AudioEffectPlayerConfig)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| handler | IAudioEffectPlayerEventHandler * | 参看 [IAudioEffectPlayerEventHandler](70096#IAudioEffectPlayerEventHandler)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| src_frame | IVideoFrame * | RTC SDK 采集得到的视频帧，参看 [IVideoFrame](70098#IVideoFrame)。 |


**返回值**

经过视频前处理后的视频帧，返回给 RTC SDK 供编码和传输，参看 [IVideoFrame](70098#IVideoFrame)。
...
等待超时后会返回空列表。超时时间默认为 10 s。建议通过 [onAudioDeviceStateChanged](70096#IRTCVideoEventHandler-onaudiodevicestatechanged) 监听到 `kMediaDeviceListUpdated` 后，再次调用本接口获取。


**注意**

你可以在收到 [onAudioDeviceStateChanged](70096#IRTCVideoEventHandler-onaudiodevicestatechanged) 了解设备变更后，重新调用本接口以获得新的设备列表。
...
等待超时后会返回空列表。超时时间默认为 10 s。建议通过 [onVideoDeviceStateChanged](70096#IRTCVideoEventHandler-onvideodevicestatechanged) 监听到 `kMediaDeviceListUpdated` 后，再次调用本接口获取。


**注意**

你可以在收到 [onVideoDeviceStateChanged](70096#IRTCVideoEventHandler-onvideodevicestatechanged) 了解设备变更后，重新调用本接口以获得新的设备列表。
