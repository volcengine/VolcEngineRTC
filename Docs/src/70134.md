...
1. #### 创建引擎实例 [createRTCVideo](Android-api.md#RTCVideo-creatertcvideo)
...
2. #### 设置视频编码参数 [setVideoEncoderConfig](Android-api.md#RTCVideo-setvideoencoderconfig)
...
|**参数名** |**类型** |**说明** |
|---|---|---|
|max\_solution |**VideoEncoderConfig** |期望发布的最大分辨率视频流参数。参看 [VideoEncoderConfig](Android-keytype.md#videoencoderconfig)。 |


3. #### 设置本地视频渲染视图 [setLocalVideoCanvas](Android-api.md#RTCVideo-setlocalvideocanvas)
...
4. #### 开始视频采集 [startVideoCapture](Android-api.md#RTCVideo-startvideocapture)
...
5. #### 开始音频采集 [startAudioCapture](Android-api.md#RTCVideo-startaudiocapture)
...
6. #### 创建房间 [createRTCRoom](Android-api.md#RTCVideo-creatertcroom)
...
7. #### 监听房间回调事件 [setRTCRoomEventHandler](Android-api.md#RTCRoom-setrtcroomeventhandler)
...
8. #### 加入房间 [joinRoom](Android-api.md#RTCRoom-joinroom)
...
9. #### 处理房间状态改变回调 [onRoomStateChanged](Android-callback.md#IRTCRoomEventHandler-onroomstatechanged)
...
|roomId |**String** |房间 ID。 |
|uid |**String** |用户 ID。 |
|state |**int** |房间状态码。 • 0: 成功。 • !0: 失败，具体原因参看 [ErrorCode](Android-errorcode.md#errorcode) 及 [WarningCode](Android-errorcode.md#warningcode)。 |
|extraInfo |**String**|额外信息，如 `{"elapsed":1187,"join_type":0}`。 `join_type` 表示加入房间的类型，`0`为首次进房，`1`为重连进房。 `elapsed`表示加入房间耗时，即本地用户从调用 [joinRoom](Android-api.md#RTCRoom-joinroom) 到加入房间成功所经历的时间间隔，单位为 ms。|| |


10. #### 处理用户加入房间回调 [onUserJoined](Android-callback.md#IRTCRoomEventHandler-onuserjoined)
...
11. #### 处理远端视频首帧解码的回调 [onFirstRemoteVideoFrameDecoded](Android-callback.md#IRTCVideoEventHandler-onfirstremotevideoframedecoded)
...
12. #### 设置远端视图 [setRemoteVideoCanvas](Android-api.md#RTCVideo-setremotevideocanvas)
...
13. #### 处理用户离开房间回调 [onUserLeave](Android-callback.md#IRTCRoomEventHandler-onuserleave)
...
14. #### 离开房间 [leaveRoom](Android-api.md#RTCRoom-leaveroom)
...
15. #### 销毁引擎实例 [destroyRTCVideo](Android-api.md#RTCVideo-destroyrtcvideo)
