...
| 开启内部视频采集 | [startVideoCapture](iOS-api#startvideocapture) |
| 创建 RTC 房间 | [createRTCRoom:](iOS-api#creatertcroom) |
...
| 开始播放音频文件 | [open:config:](iOS-api#ByteRTCMediaPlayer-open-config)  |
...
| 订阅房间内指定的通过摄像头/麦克风采集的媒体流 | [subscribeStream:mediaStreamType:](iOS-api#subscribestream-mediastreamtype) |
| 取消订阅房间内指定的通过摄像头/麦克风采集的媒体流 | [unSubscribeStream:mediaStreamType:](iOS-api#unsubscribestream-mediastreamtype) |
| 启动 PCM 音频数据混音 | [openWithCustomSource:config:](iOS-api#openwithcustomsource-config) |
| 注册音频数据回调观察者。 | [registerAudioFrameObserver:](iOS-api#registeraudioframeobserver) |
| 开启音频回调 |[enableAudioFrameCallback:format:](iOS-api#enableaudioframecallback-format) |
...
| 打开/关闭耳返功能 |  [setEarMonitorMode:](iOS-api#setearmonitormode) |
| 设置耳返音量 |  [setEarMonitorVolume:](iOS-api#setearmonitorvolume) |
...
| 销毁 RTC 房间引擎 | [destroy](iOS-api#destroy) |
...
| 音频流同步信息回调 | [rtcEngine:onStreamSyncInfoReceived:streamType:data:](iOS-callback#rtcengine-onstreamsyncinforeceived-streamtype-data) |
| 返回远端单个用户的音频数据 | [onRemoteUserAudioFrame:audioFrame:](iOS-callback#onremoteuseraudioframe-audioframe) |
...
| 音频混音文件播放状态改变时回调 | [onMediaPlayerStateChanged:state:error: ](iOS-callback#onmediaplayerstatechanged-state-error)|
| 音频播放路由变化回调 |[rtcEngine:onAudioRouteChanged:](iOS-callback#rtcengine-onaudioroutechanged) |
| 混音音频文件播放进度回调 | [onMediaPlayerPlayingProgress:progress:](iOS-callback#onmediaplayerplayingprogress-progress) |
| 远端视频流的状态改变回调 | [rtcEngine:onRemoteVideoStateChanged:withVideoState:withVideoStateReason:](iOS-callback#rtcengine-onremotevideostatechanged-withvideostate-withvideostatereason) |
