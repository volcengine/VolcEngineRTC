...
| 方法 | 描述 |
| :-- | :-- |
| [createRTSRoom](#irts-creatertsroom) | 创建房间。<br>多次调用此方法以创建多个 [IRTSRoom](136656#irtsroom) 实例。分别调用各 IRTSRoom 实例中的 [joinRoom](136656#joinroom) 方法，同时加入多个房间。 |
| [login](#irts-login) | 登录即时消息服务器。<br>调用此方法登录后，可以向同 `appID` 下其他已登录用户发送文本或二进制消息。 |
...
| [setServerParams](#irts-setserverparams) | 设置应用服务器参数。<br>客户端调用 [sendServerMessage](136656#sendservermessage) 或 [sendServerBinaryMessage](136656#sendserverbinarymessage) 发送消息给应用服务器之前，必须需要设置有效签名和应用服务器地址。 |
...
创建房间。
多次调用此方法以创建多个 [IRTSRoom](136656#irtsroom) 实例。分别调用各 IRTSRoom 实例中的 [joinRoom](136656#joinroom) 方法，同时加入多个房间。
...
- **返回值**
  ...
  类型: <code>Promise<[USER_ONLINE_STATUS](136659#user_online_status)\></code>
...
- **注意**
  ...
  + 若文本消息发送成功，则 uid 所指定的用户会通过 [onMessageReceived](136657#onmessagereceived) 回调收到该消息。
...
- **注意**
  ...
  + 若二进制消息发送成功，则 uid 所指定的用户会通过 [onBinaryMessageReceived](136657#onbinarymessagereceived) 回调收到该条消息。
...
设置应用服务器参数。
客户端调用 [sendServerMessage](136656#sendservermessage) 或 [sendServerBinaryMessage](136656#sendserverbinarymessage) 发送消息给应用服务器之前，必须需要设置有效签名和应用服务器地址。
...
- **参数**
  ...
  - **type**
    ...
    需要监听的事件类型，参看 [IRTSEvents](136657#irtsevents)

  - **handler**

    类型: <code>[IRTSEvents](136657#irtsevents)[T]</code>
...
- **参数**
  ...
  - **type**
    ...
    需要取消监听的事件类型，参看 [IRTSEvents](136657#irtsevents)
...
| 方法 | 描述 |
| :-- | :-- |
| [joinRoom](#irtsroom-joinroom) | 加入房间。<br>调用 [createRTSRoom](136656#creatertsroom) 创建房间后，可调用该方法进房，在房间内收发广播消息。 |
...
| [destroy](#irtsroom-destroy) | 退出并销毁调用 [createRTSRoom](136656#creatertsroom) 所创建的房间。 |

### joinRoom <span id="irtsroom-joinroom"></span> 

加入房间。
调用 [createRTSRoom](136656#creatertsroom) 创建房间后，可调用该方法进房，在房间内收发广播消息。
...
- **注意**
  ...
  + 你需在调用 [login](#login) 登录 RTS 服务器后再加入房间。
...
- **参数**
  ...
  - **type**
    ...
    需要监听的事件类型，参看 [IRTSRoomEvents](136657#irtsroomevents)

  - **handler**

    类型: <code>[IRTSRoomEvents](136657#irtsroomevents)[T]</code>
...
- **参数**
  ...
  - **type**
    ...
    需要取消监听的事件类型，参看 [IRTSRoomEvents](136657#irtsroomevents)
...
退出并销毁调用 [createRTSRoom](136656#creatertsroom) 所创建的房间。
...
| 方法 | 描述 |
| :-- | :-- |
| [createRTS](#iverts-createrts) | 创建引擎对象。如果当前进程中未创建引擎实例，那么你必须先使用此方法，以使用 RTS 提供的各种消息能力。<br>如果当前进程中已创建了引擎实例，再次调用此方法时，会创建另一个独立的引擎实例。 |
| [destroyRTS](#iverts-destroyrts) | 销毁由 [createRTS](136656#createrts) 所创建引擎实例，并释放所有相关资源。 |
...
销毁由 [createRTS](136656#createrts) 所创建引擎实例，并释放所有相关资源。
...
- **注意**
  ...
  + 请确保和需要销毁的 [IRTS](136656#irts) 实例相关的业务场景全部结束后，才调用此方法。
  + 该方法在调用之后，会销毁所有和此 [IRTS](136656#irts) 实例相关的内存，并且停止与媒体服务器的任何交互
