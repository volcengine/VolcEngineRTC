...
| 类型 | 值 | 说明 |
| --- | --- | --- |
| kSEIMessageSourceTypeDefault | 0 | （默认值）用户自定义。 |
| kSEIMessageSourceTypeSystem | 1 | 系统定义，包含音量指示信息。 |
...
| 类型 | 值 | 说明 |
| --- | --- | --- |
| kRoomProfileTypeCommunication | 0 | 默认场景，通用模式<br><br/>与 `kRoomProfileTypeMeeting = 16` 配置相同。 |
...
| 类型 | 值 | 说明 |
| --- | --- | --- |
| kPublicStreamOK | 0 | 发布或订阅成功 |
| kPublicStreamPushInvalidParam | 1191 | 公共流的参数异常，请修改参数后重试 |
| kPublicStreamPushInvalidStatus | 1192 | 服务端状态异常，将自动重试 |
| kPublicStreamPushInternalError | 1193 | 内部错误，不可恢复，请重试。 |
| kPublicStreamPushFailed | 1195 | 推流失败，将自动重试，用户不需要处理 |
| kPublicStreamPushTimeout | 1196 | 推流失败，10s 后会重试，重试 3 次失败后自动停止。 |
