...
| **NSString*** | [[deprecated] getRoomId](#ByteRTCRoom-getroomid) |
...
- 从 NO 切换至 YES 时，房间内其他用户会收到 [rtcRoom:onUserJoined:elapsed:](macOS-callback.md#ByteRTCRoomDelegate-rtcroom-onuserjoined-elapsed) 回调通知；  
- 从 YES 切换至 NO 时，房间内其他用户会收到 [rtcRoom:onUserLeave:reason:](macOS-callback.md#ByteRTCRoomDelegate-rtcroom-onuserleave-reason) 回调通知。  
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| token | **NSString*** | 重新获取的有效 Token。<br/>如果传入的 Token 无效，回调错误码为 [ByteRTCErrorCode](macOS-errorcode.md#bytertcerrorcode) 中的 `-1010` 提示传入的 Token 无效。 |
...
2. 请勿同时调用 [updateToken:](#ByteRTCRoom-updatetoken) 和 [joinRoom:userInfo:roomConfig:](#ByteRTCRoom-joinroom-userinfo-roomconfig) 重新加入房间。若因 Token 过期或无效导致加入房间失败或已被移出房间，你应该在获取新的有效 Token 后调用 `joinRoom` 重新加入房间。
...
| **id<ByteRTCMonitorDelegate>** | [monitorDelegate](#ByteRTCVideo-monitordelegate) |
...
| **void** | [setOriginalScreenVideoInfo:withOriginalCaptureHeight:](#ByteRTCVideo-setoriginalscreenvideoinfo-withoriginalcaptureheight) |
...
| **void** | [startLiveTranscoding:transcoding:observer:](#ByteRTCVideo-startlivetranscoding-transcoding-observer) |
| **void** | [stopLiveTranscoding:](#ByteRTCVideo-stoplivetranscoding) |
...
| **void** | [setScreenAudioChannel:](#ByteRTCVideo-setscreenaudiochannel) |
...
<span id="ByteRTCVideo-monitordelegate"></span>
### monitorDelegate
```objectivec
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| screen_solution | **ByteRTCScreenVideoEncoderConfig*** | 屏幕共享视频流参数。参看 [ByteRTCScreenVideoEncoderConfig](macOS-keytype.md#bytertcscreenvideoencoderconfig)。 |
...
```objectivec
-(void)setOriginalScreenVideoInfo:(int) originalCaptureWidth withOriginalCaptureHeight:(int)originalCaptureHeight;
```
...
| repeatCount | **int** | 消息发送重复次数。取值范围是 [0, 30]。<br/>调用此接口后，SEI 数据会添加到从当前视频帧开始的连续 `repeatCount` 个视频帧中。 |
...
```
> Deprecated  since 3.38 and will be deleted in 3.51, use [setAudioRoute:](#setaudioroute) instead.

设置音频播放设备为扬声器或者听筒，默认使用扬声器。  <br>
音频播放设备发生变化时，会收到 [rtcEngine:onAudioPlaybackDeviceChanged:](macOS-callback.md#ByteRTCVideoDelegate-rtcengine-onaudioplaybackdevicechanged) 回调。  <br>

**传入参数**

| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audioPlaybackDevice | **ByteRTCAudioPlaybackDevice** | 音频播放设备（扬声器或者听筒），详见 [ByteRTCAudioPlaybackDevice](macOS-keytype.md#bytertcaudioplaybackdevice)。 |
...
+ 连接有线或者蓝牙音频播放设备后，音频路由将切换至有线耳机或者蓝牙耳机。不同音频场景中，音频路由和发布订阅状态到音量类型的映射关系详见 [ByteRTCAudioScenarioType](macOS-keytype.md#bytertcaudioscenariotype) 。  
...
+ 在调用该方法前，你需要在[控制台](https://console.volcengine.com/rtc/workplaceRTC)开启转推直播功能。
...
- (void)registerLocalAudioProcessor:(_Nullable id<ByteRTCAudioProcessor>) audioProcessor format:(ByteRTCAudioFormat* _Nullable)format;
```
...
+ 调用 [startAudioMixing:filePath:config:](#ByteRTCAudioMixingManager-startaudiomixing-filepath-config) 方法开始播放音频文件后，可以调用本方法停止播放所有音频文件。  
+ 调用本方法停止播放所有音频文件后，会收到 `onAudioMixingStateChanged` 回调，通知已停止播放。  
...
+ 调用 [startAudioMixing:filePath:config:](#ByteRTCAudioMixingManager-startaudiomixing-filepath-config)方法开始播放音频文件后，可以通过调用本方法暂停播放所有音频文件。  
+ 调用本方法暂停播放所有音频文件后，可调用 [resumeAllAudioMixing](#ByteRTCAudioMixingManager-resumeallaudiomixing) 方法恢复所有播放。  
...
+ 调用 [pauseAllAudioMixing](#ByteRTCAudioMixingManager-pauseallaudiomixing) 方法暂停所有正在播放音频文件后，可以通过调用本方法恢复播放。  
...
| **int** | [updateListenerPosition:](#ByteRTCSpatialAudio-updatelistenerposition) |
| **int** | [updateListenerOrientation:](#ByteRTCSpatialAudio-updatelistenerorientation) |
...
- (int) registerFaceDetectionObserver:(_Nullable id<ByteRTCFaceDetectionObserver>) observer withInterval:(NSInteger)interval;
```
