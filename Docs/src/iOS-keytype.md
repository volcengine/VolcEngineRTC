...
降噪模式。降噪算法受调用 [joinRoom:userInfo:roomConfig:](#joinroom-userinfo-roomconfig) 时设置的房间模式影响。
...
| 类型 | 值 | 说明 |
| --- | --- | --- |
| ByteRTCRoomProfileCommunication | 0 | 默认场景，通用模式<br><br/>与 `ByteRTCRoomProfileMeeting = 16` 配置相同。 |
...
| 类型 | 值 | 说明 |
| --- | --- | --- |
| ByteRTCSEIMessageSourceTypeDefault | 0 | （默认值）用户自定义。 |
| ByteRTCSEIMessageSourceTypeSystem | 1 | 系统定义，包含音量指示信息。 |
...
| ByteRTCVideoEncoderPreferenceMaintainFramerate | 1 | （默认值）帧率优先。分辨率保持不变。 |
...
屏幕流编码模式。默认采用清晰模式。若在 macOS 平台采集时调用 [updateScreenCaptureFilterConfig:](#updatescreencapturefilterconfig) 排除指定窗口，共享视频时帧率无法达到 30fps。
