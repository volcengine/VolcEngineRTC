...
若调用 [setUserVisibility:](70092#ByteRTCRoom-setuservisibility) 将自身可见性设为 false，无论是默认的自动发布流还是手动设置的自动发布流都不会进行发布，你需要将自身可见性设为 true 后方可发布。
...
| 类型 | 值 | 说明 |
| --- | --- | --- |
| ByteRTCSEIStreamEventTypeStreamAdd | 0 | 远端用户发布黑帧视频流。<br>纯语音通话场景下，远端用户调用 [sendSEIMessage:andMessage:andRepeatCount:andCountPerFrame:](70092#ByteRTCVideo-sendseimessage-andmessage-andrepeatcount-andcountperframe) 发送 SEI 数据时，SDK 会自动发布一路黑帧视频流，并触发该回调。 |
| ByteRTCSEIStreamEventTypeStreamRemove | 1 | 远端黑帧视频流移除。该回调的触发时机包括：<br><ul><li>远端用户开启摄像头采集，由语音通话切换至视频通话，黑帧视频流停止发布；</li></ul><ul><li>远端用户调用 [sendSEIMessage:andMessage:andRepeatCount:andCountPerFrame:](70092#ByteRTCVideo-sendseimessage-andmessage-andrepeatcount-andcountperframe) 后 1min 内未有 SEI 数据发送，黑帧视频流停止发布；</li></ul><ul><li>远端用户调用 [setVideoSourceType:WithStreamIndex:](70092#ByteRTCVideo-setvideosourcetype-withstreamindex) 切换至自定义视频采集时，黑帧视频流停止发布。</li></ul> |
...
本设置与 [setRemoteVideoMirrorType:withMirrorType:](70092#ByteRTCVideo-setremotevideomirrortype-withmirrortype)（适用于内部渲染）相互独立。
...
- true：是
  ...
  - 若使用自定义采集，此时你需调用 [pushExternalAudioFrame:](70092#ByteRTCVideo-pushexternalaudioframe) 将采集到的音频推送给 SDK
- false：否
...
- true：是
  ...
  - 若使用自定义采集，此时你需调用 [pushExternalVideoFrame:](70092#ByteRTCVideo-pushexternalvideoframe) 将采集到的视频推送给 SDK
...
当远端用户通过调用 [enableSimulcastMode:](70092#ByteRTCVideo-enablesimulcastmode) 方法启动发布多路不同分辨率的视频流时，本地用户需通过此参数指定希望订阅的流。
...
- 调用 [enableAudioPropertiesReport:](70092#ByteRTCVideo-enableaudiopropertiesreport)，并设置参数 enableVoicePitch 的值为 `true`。
...
当远端用户调用 [setVideoEncoderConfig:](70092#ByteRTCVideo-setvideoencoderconfig) 方法发布多个配置的视频流时，此处会包含该用户发布的所有视频流的属性信息。
...
| ByteRTCAudioMixingErrorLoadConflict | 6 | 播放的文件与预加载的文件不一致。请先使用 [unloadAudioMixing:](70092#ByteRTCAudioMixingManager-unloadaudiomixing) 卸载此前的文件。 |
...
```
降噪模式。降噪算法受调用 [joinRoom:userInfo:roomConfig:](70092#ByteRTCRoom-joinroom-userinfo-roomconfig) 时设置的房间模式影响。
...
用户可以通过调用 [setVideoEncoderConfig:](70092#ByteRTCVideo-setvideoencoderconfig) 方法在一路流中发布多个不同的视频分辨率。此参数即为流中所有分辨率的相关信息。
...
```
仅使用内存播放时，传入对应的 [ByteRTCMediaPlayerCustomSourceProvider](70093#ByteRTCMediaPlayerCustomSourceProvider) 实例。
...
@property(assign, nonatomic) ByteRTCNetworkQuality txQuality;
```
> Deprecated since 3.36 and will be deleted in 3.51, use [rtcRoom:onNetworkQuality:remoteQualities:](70093#ByteRTCRoomDelegate-rtcroom-onnetworkquality-remotequalities) instead
...
@property(assign, nonatomic) NSInteger callbackOnProgressInterval;
```
设置音频文件混音时，收到 [onMediaPlayerPlayingProgress:progress:](70093#ByteRTCMediaPlayerEventHandler-onmediaplayerplayingprogress-progress) 的间隔。单位毫秒。
...
```
是否自动播放。如果不自动播放，调用 [start](70092#ByteRTCMediaPlayer-start) 播放音乐文件。默认为 True。
...
```
[rtcEngine:onLocalAudioPropertiesReport:](70093#ByteRTCVideoDelegate-rtcengine-onlocalaudiopropertiesreport) 中包含的音频信息的范围。
...
```
[rtcEngine:onLocalAudioPropertiesReport:](70093#ByteRTCVideoDelegate-rtcengine-onlocalaudiopropertiesreport) 中包含音频数据的范围。参看 [ByteRTCAudioPropertiesMode](#ByteRTCAudioPropertiesMode)。
