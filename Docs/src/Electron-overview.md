...
| 方法 | 描述 |
| :-- | :-- |
| [createRTCRoom](Electron-api.md#rtcroom-creatertcroom) | 创建房间实例 |
...
| [updateToken](Electron-api.md#rtcroom-updatetoken) | 更新 Token。<br>Token 中同时包含进房、发布和订阅权限，各权限有一定的有效期，并且到期前 30 秒会触发回调，提示用户更新 Token 相关权限。此时需要重新获取 Token，并调用此方法更新 Token，以保证通话的正常进行。 |
...
| [startSubtitle](Electron-api#rtcroom-startsubtitle) | 识别或翻译房间内所有用户的语音，形成字幕。<br>语音识别或翻译的结果会通过 [onSubtitleMessageReceived](Electron-event.md#onsubtitlemessagereceived) 事件回调给你。<br>调用该方法后，你会收到 [onSubtitleStateChanged](Electron-event.md#onsubtitlestatechanged) 回调，通知字幕是否开启。 |
...
| [feedback](Electron-api.md#rtcvideo-feedback) | 本次通话质量打分评价 |
| [getErrorDescription](Electron-api.md#rtcvideo-geterrordescription) | 获取错误码的描述 |
...
| [enumerateAudioPlaybackDevices](Electron-api.md#rtcvideo-enumerateaudioplaybackdevices) | 获取当前系统内音频播放设备列表。如果后续设备有变更，你会收到 `onAudioDeviceStateChanged`] 回调通知，然后需要重新调用本接口以获得新的设备列表。 |
...
| [setPlaybackVolume](Electron-api.md#rtcvideo-setplaybackvolume) | 调节本地播放的所有远端用户混音后的音量。播放音频前或播放音频时，你都可以使用此接口设定播放音量。 |
...
| [setVideoEncoderConfig](Electron-api.md#rtcvideo-setvideoencoderconfig) | 视频发布端设置期望发布的最大分辨率视频流参数，包括分辨率、帧率、码率、缩放模式、网络不佳时的回退策略等。 |
...
| [setRemoteVideoSuperResolution](Electron-api.md#rtcvideo-setremotevideosuperresolution) | 设置远端视频超分模式。 |
...
| [login](Electron-api.md#rtcvideo-login) | 必须先登录，才能发送房间外点对点消息和向应用服务器发送消息 |
| [logout](Electron-api.md#rtcvideo-logout) | 调用本接口登出后，无法调用房间外消息以及端到服务器消息相关的方法或收到相关回调。 |
...
| [startAudioMixing](Electron-api.md#rtcvideo-startaudiomixing) | 开始播放音乐文件及混音 |
| [stopAudioMixing](Electron-api.md#rtcvideo-stopaudiomixing) | 停止播放音乐文件及混音 |
| [pauseAudioMixing](Electron-api.md#rtcvideo-pauseaudiomixing) | 暂停播放音乐文件及混音 |
| [resumeAudioMixing](Electron-api.md#rtcvideo-resumeaudiomixing) | 恢复播放音乐文件及混音 |
| [preloadAudioMixing](Electron-api.md#rtcvideo-preloadaudiomixing) | 预加载指定音乐文件到内存中，以避免频繁播放同一文件时的重复加载，减少 CPU 占用。 |
| [unloadAudioMixing](Electron-api.md#rtcvideo-unloadaudiomixing) | 卸载指定音乐文件 |
| [setAudioMixingVolume](Electron-api.md#rtcvideo-setaudiomixingvolume) | 调节音乐文件在本地和远端播放的音量大小 |
| [getAudioMixingDuration](Electron-api.md#rtcvideo-getaudiomixingduration) | 获取音乐文件时长 |
| [getAudioMixingCurrentPosition](Electron-api.md#rtcvideo-getaudiomixingcurrentposition) | 获取音乐文件播放进度 |
| [setAudioMixingPosition](Electron-api.md#rtcvideo-setaudiomixingposition) | 设置音乐文件的播放位置 |
| [getAudioMixingPlaybackDuration](Electron-api.md#rtcvideo-getaudiomixingplaybackduration) | 获取混音音频文件的实际播放时长（单位：毫秒）。 |
| [setAudioMixingDualMonoMode](Electron-api.md#rtcvideo-setaudiomixingdualmonomode) | 设置当前音频文件的声道模式 |
| [setAudioMixingPitch](Electron-api.md#rtcvideo-setaudiomixingpitch) | 开启本地播放音乐文件变调功能，多用于 K 歌场景。 |
| [setAudioMixingPlaybackSpeed](Electron-api.md#rtcvideo-setaudiomixingplaybackspeed) | 设置混音时音频文件的播放速度 |
| [setAudioMixingLoudness](Electron-api.md#rtcvideo-setaudiomixingloudness) | 如果你需要使用 enableVocalInstrumentBalance 对混音使音频文件/PCM 音频数据进行音量调整，你必须通过此接口传入其原始响度。 |
| [setAudioMixingProgressInterval](Electron-api.md#rtcvideo-setaudiomixingprogressinterval) | 设置混音时音频文件播放进度回调的间隔 |
| [getAudioTrackCount](Electron-api.md#rtcvideo-getaudiotrackcount) | 获取当前音频文件的音轨索引 |
| [selectAudioTrack](Electron-api.md#rtcvideo-selectaudiotrack) | 指定当前音频文件的播放音轨 |
...
| [setLocalVoiceReverbParam](Electron-api.md#rtcvideo-setlocalvoicereverbparam) | 设置本地采集语音的均衡效果。包含内部采集和外部采集，但不包含混音音频文件。 |
...
| [stopAllAudioMixing](Electron-api.md#rtcvideo-stopallaudiomixing) | 停止播放所有音频文件及混音。 |
| [pauseAllAudioMixing](Electron-api.md#rtcvideo-pauseallaudiomixing) | 暂停播放所有音频文件及混音。 |
| [resumeAllAudioMixing](Electron-api.md#rtcvideo-resumeallaudiomixing) | 恢复播放所有音频文件及混音。 |
| `static` [getSDKVersion](Electron-api.md#static-getsdkversion) | 获取当前 SDK 版本信息 |
| `static` [getCurrentProcessId](Electron-api.md#static-getcurrentprocessid) | 获取加载引擎进程的 ID 方便调试 |
...
| [onUserJoined](Electron-event.md#rtcroomcallback-onuserjoined) | 可见用户加入房间，或房内隐身用户切换为可见的回调。<br>1.远端可见用户调用 `setUserVisibility` 方法将自身设为可见后加入房间时，房间内其他用户将收到该事件。<br>2.远端可见用户断网后重新连入房间时，房间内其他用户将收到该事件。<br>3.房间内隐身远端用户调用 `setUserVisibility` 方法切换至可见时，房间内其他用户将收到该事件。<br>4.新进房用户也会收到进房前已在房内的可见角色用户的进房回调通知。 |
| [onUserLeave](Electron-event.md#rtcroomcallback-onuserleave) | 远端用户离开房间，或切至不可见时，本地用户会收到此事件 |
| [onTokenWillExpire](Electron-event.md#rtcroomcallback-ontokenwillexpire) | Token 过期前 30 秒将触发该回调。 |
...
| [onRemoteVideoSuperResolutionModeChanged](Electron-event.md#rtcvideocallback-onremotevideosuperresolutionmodechanged) | 远端视频流的超分状态发生改变时，房间内订阅此流的用户会收到该回调。 |
...
| [onPublicStreamDataMessageReceived](Electron-event#rtcvideocallback-onpublicstreamdatamessagereceived) | 回调公共流中包含的数据信息。<br>通过 [startPlayPublicStream](Electron-api.md#rtcvideo-startplaypublicstream) 开始播放公共流后，可以通过本回调获取发送端发送的非SEI消息。 |
...
| [onPublicStreamSEIMessageReceived](Electron-event.md#rtcvideocallback-onpublicstreamseimessagereceived) | 回调公共流中包含的 SEI 信息 |
...
| [onAudioMixingFinished](Electron-event.md#rtcvideocallback-onaudiomixingfinished) | 本地音乐文件播放已结束回调。 |
| [onAudioMixingStateChanged](Electron-event.md#rtcvideocallback-onaudiomixingstatechanged) | 音频混音文件播放状态改变时回调 |
| [onAudioMixingPlayingProgress](Electron-event.md#rtcvideocallback-onaudiomixingplayingprogress) | 混音音频文件播放进度回调 |
...
| [onPerformanceAlarms](Electron-event.md#rtcvideocallback-onperformancealarms) | 本地未开启发布性能回退，检测到设备性能不足时，收到此回调。本地开启发布性能回退，因设备性能/网络原因，造成发布性能回退/恢复时，收到此回调。 |
...
| [onLocalAudioStateChanged](Electron-event.md#rtcvideocallback-onlocalaudiostatechanged) | 本地音频的状态发生改变时，该回调通知当前的本地音频状态。 |
...
| [onASRSuccess](Electron-event.md#rtcvideocallback-onasrsuccess) | 语音识别服务开启成功回调 |
| [onMessage](Electron-event.md#rtcvideocallback-onmessage) | 语音转文字成功回调，该回调返回识别后的全量消息。 |
| [onASRError](Electron-event.md#rtcvideocallback-onasrerror) | 当语音识别服务内部发生错误事件时触发该回调。 |
...
| [onRemoteAudioPropertiesReport](Electron-event.md#rtcvideocallback-onremoteaudiopropertiesreport) | 远端用户进房后，本地调用 `enableAudioPropertiesReport`，根据设置的 interval 值，本地会周期性地收到此回调，了解订阅的远端用户的音频信息。 |
| [onEchoTestResult](Electron-event.md#rtcvideocallback-onechotestresult) | 通话前网络探测结束 |
| [onVideoDeviceStateChanged](Electron-event.md#rtcvideocallback-onvideodevicestatechanged) | 视频频设备状态回调。提示摄像头视频采集、屏幕视频采集等设备的状态。 |
