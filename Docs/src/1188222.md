...
在实时音视频通话场景中，若需调整音频切片任务的配置，你可通过调用此接口实现。
## 使用说明
### 接口行为
...
## 请求说明
- 请求方式：**POST**
- 请求地址：**https://rtc.volcengineapi.com?Action=UpdateSegment&Version=2023-11-01**
## 调试

<APILink link="https://api.volcengine.com/api-explorer/debug?action=UpdateSegment&serviceCode=rtc&version=2023-11-01&groupName=音频切片" />
...
const data = [
  ...
  },
  {
    "name": "Identifier",
    "key": "0-4",
    "title": "自定义文件前缀。<br>\n切片文件名由 <code>Identifier</code> + <code>UserId</code> + 时间戳 + 序号组成。默认情况下 <code>Identifier</code> 为 随机生成的 UUID。在自定义文件名时，<code>Identifier</code> 的命名规则符合正则表达式：<code>[a-zA-Z0-9_@\\-\\.]{1,128}</code>。<br>\n在自定义文件名时，你需确保 identifier 命名全局唯一，否则在 TOS 平台会因文件名重复被覆盖。",
  ...
  },
  {
    "name": "Handle",
    "key": "0-5",
    "title": "<p>是否在开启音视切片时，立刻开始切片。</p>\n<ul>\n<li><code>true</code>：立刻开启切片。</li>\n<li><code>false</code>：不切片。</li>\n</ul>\n默认值 <code>true</code>。",
  ...
  },
  {
    "name": "Duration",
    "key": "0-6",
    "title": "音频切片的时长。单位为秒。取值范围为 <code>[1, 600]</code>，默认值为 <code>20</code>。 更新该字段后，计时器会重新启动，当前切片立即停止，生成一个新切片。",
...
## 请求示例
```json
POST https://rtc.volcengineapi.com?Action=UpdateSegment&Version=2023-11-01
{
    "AppId": "661e****543cf",
    "BusinessId": "B****23",
    "RoomId": "Room1",
