...
```java
public enum com.ss.bytertc.engine.data.SEIMessageSourceType
```
...
| 类型 | 说明 |
| --- | --- |
| **SEI_MESSAGE_SOURCE_TYPE_DEFAULT(0)** | （默认值）用户自定义。 |
| **SEI_MESSAGE_SOURCE_TYPE_SYSTEM(1)** | 系统定义，包含音量指示信息。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| x | **float** | 在空间直角坐标系中的 x 坐标。 |
| y | **float** | 在空间直角坐标系中的 y 坐标。 |
| z | **float** | 在空间直角坐标系中的 z 坐标。 |
...
| 类型 | 说明 |
| --- | --- |
| **CHANNEL_PROFILE_COMMUNICATION(0)** | 普通音频通话，默认模式 <br><br/>与 `CHANNEL_PROFIEL_MEETING(16)` 配置相同。 |
...
| **MaintainFramerate(1)** | （默认值）帧率优先。分辨率保持不变。 |
...
| --- | --- |
| **Disabled(0)** | 无偏好。不降低帧率和分辨率。 |
| **MaintainFramerate(1)** | 帧率优先。分辨率保持不变。 |
| **MaintainQuality(2)** | 分辨率优先。 |
| **Balance(3)** | 平衡帧率与分辨率。 |