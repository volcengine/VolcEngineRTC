...
### getVideoFormat
```java
MixedStreamClientMixVideoFormat com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamClientMixConfig.getVideoFormat()
```
获取客户端合流回调视频格式。
...
### setUseAudioMixer
```java
MixedStreamClientMixConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamClientMixConfig.setUseAudioMixer(boolean useAudioMixer)
```
设置客户端合流是否使用混音。
...
### getUseAudioMixer
```java
boolean com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamClientMixConfig.getUseAudioMixer()
```
获取客户端合流是否使用混音的设置。


<span id="PublicStreaming"></span>
# PublicStreaming
```java
class com.ss.bytertc.engine.publicstream.PublicStreaming
```
公共流参数。
...
```java
static PublicStreaming com.ss.bytertc.engine.publicstream.PublicStreaming.getDefualtPublicStreaming()
```
获取默认的公共流配置属性。
...
### setRoomId
```java
void com.ss.bytertc.engine.publicstream.PublicStreaming.setRoomId(String roomId)
```
设置 roomId。
...
### setAction
```java
void com.ss.bytertc.engine.publicstream.PublicStreaming.setAction(String action)
```
设置公共流发布动作参数。
...
### setVideo
```java
void com.ss.bytertc.engine.publicstream.PublicStreaming.setVideo(VideoConfig video)
```
设置公共流视频参数。
...
### setLayout
```java
void com.ss.bytertc.engine.publicstream.PublicStreaming.setLayout(Layout layout)
```
设置公共流整体布局信息。
...
### getVideo
```java
VideoConfig com.ss.bytertc.engine.publicstream.PublicStreaming.getVideo()
```
获取公共流视频编码参数。
...
### getLayout
```java
Layout com.ss.bytertc.engine.publicstream.PublicStreaming.getLayout()
```
获取公共流整体布局信息。
...
### value
```java
int com.ss.bytertc.engine.data.RemoteAudioStateChangeReason.value()
```
获取当前值
...
# PlayerError
```java
public enum com.ss.bytertc.engine.data.PlayerError
```
播放错误码。
...
| --- | --- | --- |
| OK | 0 | 正常 |
| FORMAT_NOT_SUPPORT | 1 | 不支持此类型 |
| INVALID_PATH | 2 | 无效的播放路径 |
| INVALID_STATE | 3 | 未满足前序接口调用的要求。请查看具体接口文档。 |
| INVALID_POSITION | 4 | 设置播放位置出错。 |
| INVALID_VOLUME | 5 | 音量参数不合法。 |
| INVALID_PITCH | 6 | 音调参数设置不合法。 |
| INVALID_AUDIO_TRACK_INDEX | 7 | 音轨参数设置不合法。 |
...
```
屏幕流编码配置。参考 [屏幕共享](https://www.volcengine.com/docs/6348/124176)。
...
### encodePreference
```java
public EncoderPreference com.ss.bytertc.engine.ScreenVideoEncoderConfig.encodePreference = EncoderPreference.MaintainFramerate
```
屏幕流编码模式。默认为流畅模式。参看 [EncoderPreference](#EncoderPreference)。


<span id="MediaPlayerConfig"></span>
# MediaPlayerConfig
```java
public class com.ss.bytertc.engine.data.MediaPlayerConfig
```
混音配置
...
| int | [playCount](#MediaPlayerConfig-playcount) |
| int | [startPos](#MediaPlayerConfig-startpos) |
...
### type
```java
public AudioMixingType com.ss.bytertc.engine.data.MediaPlayerConfig.type = AudioMixingType.AUDIO_MIXING_TYPE_PLAYOUT_AND_PUBLISH
```
混音播放类型，详见 [AudioMixingType](#AudioMixingType)


<span id="MediaPlayerConfig-playcount"></span>
### playCount
```java
...
<span id="MediaPlayerConfig-startpos"></span>
### startPos
```java
public int com.ss.bytertc.engine.data.MediaPlayerConfig.startPos = 0
```
混音起始位置。默认值为 0，单位为毫秒。


<span id="MediaPlayerConfig-callbackonprogressinterval"></span>
### callbackOnProgressInterval
```java
...
<span id="MediaPlayerConfig-syncprogresstorecordframe"></span>
### syncProgressToRecordFrame
```java
...
<span id="MediaPlayerConfig-autoplay"></span>
### autoPlay
```java
...
| --- | --- | --- |
| UNKNOWN | 0 | 色彩空间未知 |
| BT601_LIMITED_RANGE | 1 | BT.601 数字编码标准，色彩空间[16-235] |
...
<span id="AudioPropertiesInfo-voicepitch"></span>
### voicePitch
```java
public double com.ss.bytertc.engine.data.AudioPropertiesInfo.voicePitch = 0
```
本地用户的人声基频，单位为赫兹。
...
### Original
```java
public int com.ss.bytertc.engine.video.IVideoSink.PixelFormat.Original = 0
```
原始视频帧格式
...
### I420
```java
public int com.ss.bytertc.engine.video.IVideoSink.PixelFormat.I420 = 1
```
I420 数据格式


<span id="PixelFormat-rgba"></span>
### RGBA
```java
public int com.ss.bytertc.engine.video.IVideoSink.PixelFormat.RGBA = 5
```
RGBA 格式, 一个像素占据 32 位, 字节序为 A8 B8 G8 R8
...
### SubscribeVideoConfig
```java
com.ss.bytertc.engine.SubscribeVideoConfig.SubscribeVideoConfig(
    int videoIndex,
    int priority
)
```
...
| NOT_JOIN_ROOM | -1 | 设置失败，尚未加入房间 |
| KEY_IS_NULL | -2 | 设置失败，key 指针为空 |
| VALUE_IS_NULL | -3 | 设置失败，value 指针为空 |
| UNKNOW | -99 | 设置失败，未知错误 |
| KEY_IS_EMPTY | -400 | 设置失败，key 长度为 0 |
| TOO_OFTEN | -406 | 调用 `setRoomExtraInfo` 过于频繁，建议不超过 10 次/秒。 |
| SILENT_USER | -412 | 设置失败，用户已调用 `setUserVisibility` 将自身设为隐身状态。 |
| KEY_TOO_LONG | -413 | 设置失败，Key 长度超过 10 字节 |
...
```
远端视频流参数，参看 [RemoteVideoConfig](#remotevideoconfig-2)。
...
### Rectangle
```java
com.ss.bytertc.engine.video.Rectangle.Rectangle(
    int x,
    int y,
    int w,
    int h
...
### toString
```java
String com.ss.bytertc.engine.video.Rectangle.toString()
```
...
### getValue
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamClientMixVideoFormat.getValue()
```
获取合流输出内容类型。
...
# PlayerState
```java
public enum com.ss.bytertc.engine.data.PlayerState
```
播放状态。
...
### getValue
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.MixedStreamVideoType.getValue()
```

<span id="MixedStreamConfig-MixedStreamLayoutRegionConfig"></span>
...
| int | [getLocationX](#MixedStreamLayoutRegionConfig-getlocationx) |
| MixedStreamLayoutRegionConfig | [setLocationY](#MixedStreamLayoutRegionConfig-setlocationy) |
| int | [getLocationY](#MixedStreamLayoutRegionConfig-getlocationy) |
| MixedStreamLayoutRegionConfig | [setWidth](#MixedStreamLayoutRegionConfig-setwidth) |
| int | [getWidth](#MixedStreamLayoutRegionConfig-getwidth) |
| MixedStreamLayoutRegionConfig | [setHeight](#MixedStreamLayoutRegionConfig-setheight) |
| double | [getHeight](#MixedStreamLayoutRegionConfig-getheight) |
...
| MixedStreamLayoutRegionConfig | [setAlternateImageFillMode](#MixedStreamLayoutRegionConfig-setalternateimagefillmode) |
| MixedStreamAlternateImageFillMode | [getAlternateImageFillMode](#MixedStreamLayoutRegionConfig-getalternateimagefillmode) |
| MixedStreamLayoutRegionConfig | [setAlternateImageURL](#MixedStreamLayoutRegionConfig-setalternateimageurl) |
| String | [getAlternateImageURL](#MixedStreamLayoutRegionConfig-getalternateimageurl) |
...
### setRoomID
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setRoomID(String roomID)
```
设置视频流发布用户的房间 ID。建议设置。
...
### getRoomID
```java
String com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getRoomID()
```
获取视频流发布用户的房间 ID。
...
### setUserID
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setUserID(String userID)
```
设置视频流发布用户的用户 ID。建议设置。
...
### getUserID
```java
String com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getUserID()
```
获取视频流发布用户的用户 ID。
...
### setLocationX
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setLocationX(int locationX)
```
> Available since 3.57

设置单个用户画面左上角在整个画布坐标系中的 X 坐标（pixel），即以画布左上角为原点，用户画面左上角相对于原点的横向位移。
...
### getLocationX
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getLocationX()
```
> Available since 3.57
...
### setLocationY
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setLocationY(int locationY)
```
> Available since 3.57

设置单个用户画面左上角在整个画布坐标系中的 Y 坐标（pixel），即以画布左上角为原点，用户画面左上角相对于原点的纵向位移。
...
### getLocationY
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getLocationY()
```
> Available since 3.57
...
<span id="MixedStreamLayoutRegionConfig-setwidth"></span>
### setWidth
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setWidth(int width)
```
> Available since 3.57
...
### getWidth
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getWidth()
```
> Available since 3.57
...
<span id="MixedStreamLayoutRegionConfig-setheight"></span>
### setHeight
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setHeight(int height)
```
> Available since 3.57
...
### getHeight
```java
double com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getHeight()
```
> Available since 3.57
...
### setZOrder
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setZOrder(int zOrder)
```
设置用户视频布局在画布中的层级。
...
### getZOrder
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getZOrder()
```
获取用户视频布局在画布中的层级。
...
### setAlpha
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setAlpha(double alpha)
```
（仅服务端合流支持设置）设置透明度。
...
### getAlpha
```java
double com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getAlpha()
```
获取透明度。
...
### setCornerRadius
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setCornerRadius(double cornerRadius)
```
（仅服务端合流支持设置）设置圆角半径。
...
### getCornerRadius
```java
double com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getCornerRadius()
```
获取圆角半径。
...
### setMediaType
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setMediaType(MixedStreamMediaType mediaType)
```
（仅服务端合流支持设置）设置合流内容类型。
...
### getMediaType
```java
MixedStreamMediaType com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getMediaType()
```
获取合流内容类型。
...
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setRegionContentType(MixedStreamLayoutRegionType regionContentType)
```
设置合流布局区域类型。
...
```java
MixedStreamLayoutRegionType com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getRegionContentType()
```
获取合流布局区域类型。
...
### setRenderMode
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setRenderMode(MixedStreamRenderMode renderMode)
```
设置图片或视频流渲染的缩放模式。建议设置。
...
### getRenderMode
```java
MixedStreamRenderMode com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getRenderMode()
```
获取图片或视频流渲染的缩放模式。
...
### getIsLocalUser
```java
boolean com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getIsLocalUser()
```
是否本地用户。
...
### setIsLocalUser
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setIsLocalUser(boolean islocalUser)
```
设置是否是本地用户。
...
### setStreamType
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setStreamType(MixedStreamVideoType streamType)
```
...
### getStreamType
```java
MixedStreamVideoType com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getStreamType()
```
获取 region 中的流类型。
...
### setImageWaterMark
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setImageWaterMark(byte[] imageWaterMark)
```
设置图片合流区域类型对应的数据。
...
### getImageWaterMark
```java
byte[] com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getImageWaterMark()
```
获取图片合流区域类型对应的数据。
...
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setImageWaterMarkConfig(MixedStreamLayoutRegionImageWaterMarkConfig imageWaterMarkConfig)
```
设置图片合流布局区域数据的对应参数。
...
```java
MixedStreamLayoutRegionImageWaterMarkConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getImageWaterMarkConfig()
```
获取图片合流布局区域数据的对应参数。


<span id="MixedStreamLayoutRegionConfig-setalternateimagefillmode"></span>
### setAlternateImageFillMode
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setAlternateImageFillMode(MixedStreamAlternateImageFillMode alternateImageFillMode)
```
> Available since 3.57
...
```java
MixedStreamAlternateImageFillMode com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getAlternateImageFillMode()
```
> Available since 3.57
...
<span id="MixedStreamLayoutRegionConfig-setalternateimageurl"></span>
### setAlternateImageURL
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setAlternateImageURL(String alternateImageUrl)
```
> Available since 3.57
...
```java
String com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getAlternateImageURL()
```
> Available since 3.57
...
### setSpatialPosition
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setSpatialPosition(Position spatialPosition)
```
设置当前 region 空间音频位置。
...
### getSpatialPosition
```java
Position com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getSpatialPosition()
```
获取当前 region 空间音频位置。
...
```java
MixedStreamLayoutRegionConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.setApplySpatialAudio(boolean applySpatialAudio)
```
设置某用户是否应用空间音频效果。
...
```java
boolean com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionConfig.getApplySpatialAudio()
```
获取某用户是否应用空间音频效果的设置。
...
| AUDIO_SCENARIO_HIGHQUALITY_COMMUNICATION | 1 | 高质量通话场景。兼顾外放/蓝牙耳机时的音频体验。<br>此场景适用于对音乐表现力有要求，但又希望能够使用蓝牙耳麦进行音频采集的场景。<br>注意：在此场景下，使用内置扬声器/听筒时，开关麦可能发生音量突变。使用 `AUDIO_SCENARIO_HIGHQUALITY_CHAT` 可以避免此种情况。<br>不同音频设备，开麦和闭麦状态下的使用的音量类型：<br><table><tr><th></th><th>闭麦</th><th>开麦</th></tr><tr><td>内置扬声器</td><td>媒体音量</td><td>通话音量</td></tr><tr><td>内置听筒</td><td>通话音量</td><td>通话音量</td></tr><tr><td>有线耳机/ USB 耳机/ 外置声卡</td><td>媒体音量</td><td>媒体音量</td></tr><tr><td>蓝牙耳机</td><td>通话音量</td><td>通话音量</td></tr></table> |
| AUDIO_SCENARIO_COMMUNICATION | 2 | 纯通话音量场景。<br>可以最大程度地消除回声，使通话清晰度达到最优。此场景下，无论客户端音频采集播放设备和采集播放状态，全程使用通话音量，不会有音量突变的听感。<br>适用于需要频繁上下麦的通话或会议场景。<br>但是，此场景会压低使用媒体音量进行播放的其他音频的音量，且音质会变差。 |
| AUDIO_SCENARIO_MEDIA | 3 | 媒体场景。<br>当设备是内置听筒时，使用通话音量。其余情况使用媒体音量。<br>注意：在此场景下，使用蓝牙耳麦时，会通过手机自带麦克风采集音频。因此，不适用“通话时手机放在包里”等类似场景。<br>外放通话时，可能出现回声和啸叫，请联系技术支持人员。 |
...
| double | [cpuAppUsage](#SysStats-cpuappusage) |
| double | [cpuTotalUsage](#SysStats-cputotalusage) |
| double | [memoryUsage](#SysStats-memoryusage) |
| long | [fullMemory](#SysStats-fullmemory) |
| long | [totalMemoryUsage](#SysStats-totalmemoryusage) |
...
### cpuCores
```java
public int com.ss.bytertc.engine.SysStats.cpuCores
```
设备的 CPU 核数


<span id="SysStats-cpuappusage"></span>
### cpuAppUsage
```java
public double com.ss.bytertc.engine.SysStats.cpuAppUsage
```
应用的 CPU 使用率，取值范围为 [0, 1]。


<span id="SysStats-cputotalusage"></span>
### cpuTotalUsage
```java
public double com.ss.bytertc.engine.SysStats.cpuTotalUsage
```
系统的 CPU 使用率，取值范围为 [0, 1]。


<span id="SysStats-memoryusage"></span>
### memoryUsage
```java
public double com.ss.bytertc.engine.SysStats.memoryUsage
```
应用的内存占用大小（单位 MB）


<span id="SysStats-fullmemory"></span>
### fullMemory
```java
public long com.ss.bytertc.engine.SysStats.fullMemory
```
设备的内存大小 单位：MB


<span id="SysStats-totalmemoryusage"></span>
### totalMemoryUsage
```java
public long com.ss.bytertc.engine.SysStats.totalMemoryUsage
```
系统已使用内存 MB


<span id="SysStats-freememory"></span>
### freeMemory
```java
public long com.ss.bytertc.engine.SysStats.freeMemory
```
系统当前空闲内存（MB）


<span id="SysStats-memoryratio"></span>
### memoryRatio
```java
public double com.ss.bytertc.engine.SysStats.memoryRatio
```
当前应用的内存使用率（单位 %）


<span id="SysStats-totalmemoryratio"></span>
### totalMemoryRatio
```java
public double com.ss.bytertc.engine.SysStats.totalMemoryRatio
```
系统内存使用率（单位 %）
...
<span id="SubscribeConfig-subwidth"></span>
### subWidth
```java
public int com.ss.bytertc.engine.SubscribeConfig.subWidth = 0
```
视频宽度，单位：px


<span id="SubscribeConfig-subheight"></span>
### subHeight
```java
public int com.ss.bytertc.engine.SubscribeConfig.subHeight = 0
```
视频高度，单位：px
...
### getEchoRenderView
```java
View com.ss.bytertc.engine.data.EchoTestConfig.getEchoRenderView()
```

<span id="EchoTestConfig-getechouid"></span>
### getEchoUid
```java
String com.ss.bytertc.engine.data.EchoTestConfig.getEchoUid()
```

<span id="EchoTestConfig-getechoroomid"></span>
### getEchoRoomId
```java
String com.ss.bytertc.engine.data.EchoTestConfig.getEchoRoomId()
```

<span id="EchoTestConfig-getechotoken"></span>
### getEchoToken
```java
String com.ss.bytertc.engine.data.EchoTestConfig.getEchoToken()
```

<span id="EchoTestConfig-getechoenabledaudio"></span>
### getEchoEnabledAudio
```java
boolean com.ss.bytertc.engine.data.EchoTestConfig.getEchoEnabledAudio()
```

<span id="EchoTestConfig-getechoenabledvideo"></span>
### getEchoEnabledVideo
```java
boolean com.ss.bytertc.engine.data.EchoTestConfig.getEchoEnabledVideo()
```

<span id="EchoTestConfig-getaudioreportinterval"></span>
### getAudioReportInterval
```java
int com.ss.bytertc.engine.data.EchoTestConfig.getAudioReportInterval()
```
...
# MusicInfo
```java
public class com.ss.bytertc.ktv.data.MusicInfo
```
歌曲数据。
...
| String | [vendorId](#MusicInfo-vendorid) |
| String | [vendorName](#MusicInfo-vendorname) |
| long | [updateTimestamp](#MusicInfo-updatetimestamp) |
| String | [posterUrl](#MusicInfo-posterurl) |
| LyricStatus | [lyricStatus](#MusicInfo-lyricstatus) |
...
### musicId
```java
public String com.ss.bytertc.ktv.data.MusicInfo.musicId
```
音乐 ID。


<span id="MusicInfo-musicname"></span>
### musicName
```java
public String com.ss.bytertc.ktv.data.MusicInfo.musicName
```
音乐名称。
...
### singer
```java
public String com.ss.bytertc.ktv.data.MusicInfo.singer
```
歌手。


<span id="MusicInfo-vendorid"></span>
### vendorId
```java
public String com.ss.bytertc.ktv.data.MusicInfo.vendorId
```
版权商 ID。


<span id="MusicInfo-vendorname"></span>
### vendorName
```java
public String com.ss.bytertc.ktv.data.MusicInfo.vendorName
```
版权商名称。


<span id="MusicInfo-updatetimestamp"></span>
### updateTimestamp
```java
public long com.ss.bytertc.ktv.data.MusicInfo.updateTimestamp
```
最新更新时间戳，单位为毫秒。


<span id="MusicInfo-posterurl"></span>
### posterUrl
```java
public String com.ss.bytertc.ktv.data.MusicInfo.posterUrl
```
封面地址。


<span id="MusicInfo-lyricstatus"></span>
### lyricStatus
```java
public LyricStatus com.ss.bytertc.ktv.data.MusicInfo.lyricStatus
```
...
### duration
```java
public int com.ss.bytertc.ktv.data.MusicInfo.duration
```
歌曲长度，单位为毫秒。


<span id="MusicInfo-enablescore"></span>
### enableScore
```java
public boolean com.ss.bytertc.ktv.data.MusicInfo.enableScore
```
歌曲是否支持打分。


<span id="MusicInfo-climaxstarttime"></span>
### climaxStartTime
```java
public int com.ss.bytertc.ktv.data.MusicInfo.climaxStartTime
```
歌曲高潮片段开始时间，单位为毫秒。


<span id="MusicInfo-climaxendtime"></span>
### climaxEndTime
```java
public int com.ss.bytertc.ktv.data.MusicInfo.climaxEndTime
```
歌曲高潮片段停止时间，单位为毫秒。
...
### getValue
```java
String com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.MixedStreamVideoCodecType.getValue()
```
获取视频编码格式。
...
### ByteRTCStreamMixingType
```java
com.ss.bytertc.engine.live.ByteRTCStreamMixingType.ByteRTCStreamMixingType(int value)
```
...
<span id="CpuBufferVideoFrameBuilder"></span>
...
### setTimeStampUs
```java
CpuBufferVideoFrameBuilder com.ss.bytertc.engine.video.builder.CpuBufferVideoFrameBuilder.setTimeStampUs(long timeStampUs)
```
设置视频帧的时间戳
...
### setWidth
```java
CpuBufferVideoFrameBuilder com.ss.bytertc.engine.video.builder.CpuBufferVideoFrameBuilder.setWidth(int width)
```
设置视频帧宽度
...
### setHeight
```java
CpuBufferVideoFrameBuilder com.ss.bytertc.engine.video.builder.CpuBufferVideoFrameBuilder.setHeight(int height)
```
设置视频帧高度
...
### setRotation
```java
CpuBufferVideoFrameBuilder com.ss.bytertc.engine.video.builder.CpuBufferVideoFrameBuilder.setRotation(VideoRotation rotation)
```
设置视频帧旋转角度
...
### setColorSpace
```java
CpuBufferVideoFrameBuilder com.ss.bytertc.engine.video.builder.CpuBufferVideoFrameBuilder.setColorSpace(ColorSpace colorSpace)
```
设置视频帧色彩空间
...
### setPlaneData
```java
CpuBufferVideoFrameBuilder com.ss.bytertc.engine.video.builder.CpuBufferVideoFrameBuilder.setPlaneData(
    int planeIndex,
    ByteBuffer buffer
)
```
...
### setPlaneStride
```java
CpuBufferVideoFrameBuilder com.ss.bytertc.engine.video.builder.CpuBufferVideoFrameBuilder.setPlaneStride(
    int planeIndex,
    int stride
)
```
...
### setExternalDataInfo
```java
CpuBufferVideoFrameBuilder com.ss.bytertc.engine.video.builder.CpuBufferVideoFrameBuilder.setExternalDataInfo(ByteBuffer externalDataInfo)
```
设置 SEI 数据
...
```java
CpuBufferVideoFrameBuilder com.ss.bytertc.engine.video.builder.CpuBufferVideoFrameBuilder.setSupplementaryInfo(ByteBuffer supplementaryInfo)
```
设置补充数据指针
...
### setReleaseCallback
```java
CpuBufferVideoFrameBuilder com.ss.bytertc.engine.video.builder.CpuBufferVideoFrameBuilder.setReleaseCallback(Runnable releaseCallback)
```
...
### build
```java
VideoFrame com.ss.bytertc.engine.video.builder.CpuBufferVideoFrameBuilder.build()
```
获取到创建好的视频帧
...
| STREAM_REMOVE_REASON_KEEP_LIVE_FAILED | 2 | 媒体服务器 10s 没收到客户端的媒体数据。 |
...
### jitter
```java
public int com.ss.bytertc.engine.type.LocalVideoStats.jitter
```
视频上行网络抖动，单位为 ms 。
...
### value
```java
int com.ss.bytertc.engine.data.AudioPlaybackDevice.value()
```
获取当前值
...
### requiredPixelFormat
```java
public VideoPixelFormat com.ss.bytertc.engine.video.VideoPreprocessorConfig.requiredPixelFormat = VideoPixelFormat.UNKNOWN
```
...
```
本地代理类型，参看 [LocalProxyType](#localproxytype-2)。
...
# AudioDumpStatus
```java
public enum com.ss.bytertc.engine.type.AudioDumpStatus
```
音频 dump 状态
...
| AUDIO_DUMP_START_SUCCESS | 1 | 音频 dump 启动成功 |
| AUDIO_DUMP_STOP_FAILURE | 2 | 音频 dump 停止失败 |
...
# VideoConfig
```java
static class com.ss.bytertc.engine.publicstream.PublicStreaming.VideoConfig
```
公共流的视频编码参数
...
### setFps
```java
VideoConfig com.ss.bytertc.engine.publicstream.PublicStreaming.VideoConfig.setFps(int fps)
```
设置公共流视频帧率。
...
<span id="VideoConfig-setkbitrate-2"></span>
### setKBitRate
```java
VideoConfig com.ss.bytertc.engine.publicstream.PublicStreaming.VideoConfig.setKBitRate(int kBitRate)
```
设置公共流视频码率
...
<span id="VideoConfig-setwidth-2"></span>
### setWidth
```java
VideoConfig com.ss.bytertc.engine.publicstream.PublicStreaming.VideoConfig.setWidth(int width)
```
设置公共流视频宽度。
...
<span id="VideoConfig-setheight-2"></span>
### setHeight
```java
VideoConfig com.ss.bytertc.engine.publicstream.PublicStreaming.VideoConfig.setHeight(int height)
```
设置公共流视频高度。
...
<span id="LiveTranscoding-Region"></span>
# Region
```java
...
### uid
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.uid(String uid)
```
设置视频流发布用户的用户 ID。
...
### roomId
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.roomId(String roomId)
```
设置视频流发布用户的房间 ID。必填。
...
### zorder
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.zorder(int zorder)
```
设置用户视频布局在画布中的层级。
...
### alpha
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.alpha(double alpha)
```
（仅服务端合流支持设置）设置透明度。
...
### setCornerRadius
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.setCornerRadius(double radius)
```
（仅服务端合流支持设置）设置圆角半径。
...
### contentControl
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.contentControl(TranscoderContentControlType contentControl)
```
（仅服务端合流支持设置）设置合流内容类型。
...
### renderMode
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.renderMode(TranscoderRenderMode renderMode)
```
设置图片或视频流渲染的缩放模式。
...
### position
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.position(
    double x,
    double y
)
```
...
### size
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.size(
    double width,
    double height
)
```
...
### isScreenStream
```java
boolean com.ss.bytertc.engine.live.LiveTranscoding.Region.isScreenStream()
```
是否是屏幕流。
...
### setScreenStream
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.setScreenStream(boolean screenStream)
```
...
### isLocalUser
```java
boolean com.ss.bytertc.engine.live.LiveTranscoding.Region.isLocalUser()
```
是否本地用户。
...
### setLocalUser
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.setLocalUser(boolean localUser)
```
设置是否是本地用户。
...
### type
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.type(TranscoderLayoutRegionType type)
```
设置合流布局区域类型。
...
### data
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.data(byte[] data)
```
设置图片合流区域类型对应的数据。
...
### dataParam
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.dataParam(Region.DataParam dataParam)
```
设置合流布局区域数据的对应参数。
...
### spatialPosition
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.spatialPosition(
    float x,
    float y,
    float z
)
...
### applySpatialAudio
```java
Region com.ss.bytertc.engine.live.LiveTranscoding.Region.applySpatialAudio(boolean applySpatialAudio)
```
设置某用户是否应用空间音频效果。
...
### getValue
```java
int com.ss.bytertc.engine.live.LiveTranscoding.TranscoderContentControlType.getValue()
```
获取合流输出内容类型。
...
| CHANNEL_PROFILE_GAME | 2 | 游戏语音模式，低功耗、低流量消耗。<br>低端机在此模式下运行时，进行了额外的性能优化：<br><ul><li>部分低端机型配置编码帧长 40/60</li></ul><ul><li>部分低端机型关闭软件 3A 音频处理 增强对 iOS 其他屏幕录制进行的兼容性，避免音频录制被 RTC 打断。</li></ul> |
| CHANNEL_PROFILE_CLOUD_GAME | 3 | 云游戏模式。<br>如果你的游戏场景需要低延迟的配置，使用此设置。<br>此设置下，弱网抗性较差。 |
| CHANNEL_PROFILE_LOW_LATENCY | 4 | 云渲染模式。超低延时配置。<br>如果你的应用并非游戏但又对延时要求较高时，选用此模式<br>该模式下，音视频通话延时会明显降低，但同时弱网抗性、通话音质等均会受到一定影响。 |
| CHANNEL_PROFILE_CHAT | 5 | 适用于 1 vs 1 音视频通话 |
| CHANNEL_PROFILE_CHAT_ROOM | 6 | 适用于 3 人及以上纯语音通话。<br>通话中，闭麦时为是媒体模式，上麦后切换为通话模式。 |
| CHANNEL_PROFILE_LW_TOGETHER | 7 | 实现多端同步播放音视频，适用于 “一起看” 或 “一起听” 场景。<br>该场景中，使用 RTC 信令同步播放进度，共享的音频内容不通过 RTC 进行传输。 |
| CHANNEL_PROFILE_GAME_HD | 8 | 适用于对音质要求较高的游戏场景，优化音频 3A 策略，只通过媒体模式播放音频 |
| CHANNEL_PROFILE_CO_HOST | 9 | 适用于直播中主播之间连麦的业务场景。<br>该场景中，直播时通过 CDN，发起连麦 PK 时使用 RTC。 |
| CHANNEL_PROFILE_INTERACTIVE_PODCAST | 10 | 适用于单主播和观众进行音视频互动的直播。通话模式，上下麦不会有模式切换，避免音量突变现象 |
| CHANNEL_PROFILE_KTV | 11 | 线上 KTV 场景，音乐音质，低延迟<br>使用 RTC 传输伴奏音乐，混音后的歌声，适合独唱或单通合唱 |
...
| CHANNEL_PROFILE_LAN_LIVE_STREAMING | 15 | 适用于局域网的 1 对多视频直播，最高支持 8K， 60 帧/秒， 100 Mbps 码率<br>需要在局域网配置私有化部署媒体服务器。 |
...
| MixedStreamLayoutConfig | [setBackgroundImageURL](#MixedStreamLayoutConfig-setbackgroundimageurl) |
| String | [getBackgroundImageURL](#MixedStreamLayoutConfig-getbackgroundimageurl) |
...
### setBackgroundColor
```java
MixedStreamLayoutConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutConfig.setBackgroundColor(String backgroundColor)
```
设置画布的背景颜色。值不合法或未设置时，自动使用默认值。建议设置。
...
### getBackgroundColor
```java
String com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutConfig.getBackgroundColor()
```
获取画布的背景颜色。


<span id="MixedStreamLayoutConfig-setbackgroundimageurl"></span>
### setBackgroundImageURL
```java
MixedStreamLayoutConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutConfig.setBackgroundImageURL(String backgroundImageURL)
```
> Available since 3.57
...
```java
String com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutConfig.getBackgroundImageURL()
```
> Available since 3.57
...
### setRegions
```java
MixedStreamLayoutConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutConfig.setRegions(MixedStreamLayoutRegionConfig[] regions)
```
设置用户布局信息列表。建议设置。
...
### getRegions
```java
MixedStreamLayoutRegionConfig[] com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutConfig.getRegions()
```
获取用户布局信息列表。
...
```java
MixedStreamLayoutConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutConfig.setUserConfigExtraInfo(String userConfigExtraInfo)
```
设置透传的 App 数据
...
```java
String com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutConfig.getUserConfigExtraInfo()
```
获取透传的 App 数据。
...
### width
```java
public int com.ss.bytertc.engine.data.RemoteVideoConfig.width = 0
```
视频宽度，单位：px


<span id="RemoteVideoConfig-height"></span>
### height
```java
public int com.ss.bytertc.engine.data.RemoteVideoConfig.height = 0
```
视频高度，单位：px
...
### framerate
```java
public int com.ss.bytertc.engine.data.RemoteVideoConfig.framerate = 0
```
...
### RemoteVideoConfig
```java
com.ss.bytertc.engine.data.RemoteVideoConfig.RemoteVideoConfig(
    int width,
    int height,
    int framerate
)
...
| int | [totalRtt](#RemoteAudioStats-totalrtt) |
...
<span id="RemoteAudioStats-totalrtt"></span>
### totalRtt
```java
public int com.ss.bytertc.engine.type.RemoteAudioStats.totalRtt
```
...
```
远端用户发送的音频流质量。值含义参考 [NetworkQuality](#NetworkQuality)。
...
<span id="ScreenSharingParameters"></span>
...
### value
```java
int com.ss.bytertc.engine.data.AudioRoute.value()
```
获取当前值
...
```java
public enum com.ss.bytertc.engine.data.MediaPlayerCustomSourceSeekWhence
```
音频数据的起始读取位置。
...
### getValue
```java
String com.ss.bytertc.engine.live.LiveTranscoding.AudioConfig.AudioCodecType.getValue()
```
获取音频编码格式。
...
### getValue
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamLayoutRegionType.getValue()
```
获取合流布局区域类型。
...
# LogoutReason
```java
public class com.ss.bytertc.engine.type.LogoutReason
```
用户登出的原因
...
### LOGOUT_REASON_LOGOUT
```java
public static final int com.ss.bytertc.engine.type.LogoutReason.LOGOUT_REASON_LOGOUT = 0
```
用户主动退出
...
<span id="LogoutReason-logout_reason_duplicate_login"></span>
### LOGOUT_REASON_DUPLICATE_LOGIN
```java
public static final int com.ss.bytertc.engine.type.LogoutReason.LOGOUT_REASON_DUPLICATE_LOGIN = 1
```
用户被动退出
...
```java
public enum com.ss.bytertc.engine.video.VideoRenderMirrorType
```
是否将视频帧镜像。
...
# SourceCrop
```java
static class com.ss.bytertc.engine.publicstream.PublicStreaming.SourceCrop
```
公共流布局中的视频裁剪参数。
...
### setLocationX
```java
SourceCrop com.ss.bytertc.engine.publicstream.PublicStreaming.SourceCrop.setLocationX(double x)
```
设置左上角横坐标。
...
### setLocationY
```java
SourceCrop com.ss.bytertc.engine.publicstream.PublicStreaming.SourceCrop.setLocationY(double y)
```
设置左上角纵坐标。
...
### setWidthProportion
```java
SourceCrop com.ss.bytertc.engine.publicstream.PublicStreaming.SourceCrop.setWidthProportion(double w)
```
设置视频宽度。
...
### setHeightProportion
```java
SourceCrop com.ss.bytertc.engine.publicstream.PublicStreaming.SourceCrop.setHeightProportion(double h)
```
设置视频高度。
...
```java
static LiveTranscoding com.ss.bytertc.engine.live.LiveTranscoding.getDefualtLiveTranscode()
```
获取默认的转推直播配置参数。
...
```java
boolean com.ss.bytertc.engine.live.LiveTranscoding.getFieldVideoConfigBFrame()
```

<span id="LiveTranscoding-seturl"></span>
### setUrl
```java
void com.ss.bytertc.engine.live.LiveTranscoding.setUrl(String url)
```
...
### setRoomId
```java
void com.ss.bytertc.engine.live.LiveTranscoding.setRoomId(String roomId)
```
...
### setUserId
```java
void com.ss.bytertc.engine.live.LiveTranscoding.setUserId(String userId)
```
...
### setVideo
```java
void com.ss.bytertc.engine.live.LiveTranscoding.setVideo(VideoConfig video)
```
设置视频转码配置参数。
...
### setAudio
```java
void com.ss.bytertc.engine.live.LiveTranscoding.setAudio(AudioConfig audio)
```
设置音频转码配置参数。
...
### setSpatialConfig
```java
void com.ss.bytertc.engine.live.LiveTranscoding.setSpatialConfig(SpatialConfig spatialConfig)
```
设置推流 CDN 的空间音频参数。
...
### setClientMixParam
```java
void com.ss.bytertc.engine.live.LiveTranscoding.setClientMixParam(ClientMixParam param)
```
设置客户端合流配置参数。
...
### setLayout
```java
void com.ss.bytertc.engine.live.LiveTranscoding.setLayout(Layout layout)
```
设置视频流合流整体布局信息。
...
### getMixType
```java
ByteRTCStreamMixingType com.ss.bytertc.engine.live.LiveTranscoding.getMixType()
```
获取合流类型。
...
### setMixType
```java
void com.ss.bytertc.engine.live.LiveTranscoding.setMixType(ByteRTCStreamMixingType mixType)
```
设置合流类型。
...
### getUrl
```java
String com.ss.bytertc.engine.live.LiveTranscoding.getUrl()
```
获取推流的 CDN 地址。
...
### getVideo
```java
VideoConfig com.ss.bytertc.engine.live.LiveTranscoding.getVideo()
```
获取视频转码配置参数。
...
当前视频转码配置参数。详见 [VideoConfig](#LiveTranscoding-VideoConfig) 数据类型。


<span id="LiveTranscoding-getaudio"></span>
### getAudio
```java
AudioConfig com.ss.bytertc.engine.live.LiveTranscoding.getAudio()
```
获取音频转码配置参数。
...
当前音频转码配置参数，参看 [AudioConfig](#LiveTranscoding-AudioConfig)。


<span id="LiveTranscoding-getspatialconfig"></span>
### getSpatialConfig
```java
SpatialConfig com.ss.bytertc.engine.live.LiveTranscoding.getSpatialConfig()
```
获取空间音频配置参数。
...
当前空间音频配置参数。详见 [SpatialConfig](#LiveTranscoding-SpatialConfig) 数据类型。


<span id="LiveTranscoding-getclientmixparam"></span>
### getClientMixParam
```java
ClientMixParam com.ss.bytertc.engine.live.LiveTranscoding.getClientMixParam()
```
获取客户端合流配置参数。
...
当前客户端合流配置参数。详见 [ClientMixParam](#LiveTranscoding-ClientMixParam) 数据类型。


<span id="LiveTranscoding-getlayout"></span>
### getLayout
```java
Layout com.ss.bytertc.engine.live.LiveTranscoding.getLayout()
```
获取视频流合流整体布局信息。
...
<span id="LiveTranscoding-SpatialConfig"></span>
# SpatialConfig
```java
...
```java
SpatialConfig com.ss.bytertc.engine.live.LiveTranscoding.SpatialConfig.setAudienceSpatialPosition(Position pos)
```
...
```java
SpatialConfig com.ss.bytertc.engine.live.LiveTranscoding.SpatialConfig.setAudienceSpatialOrientation(HumanOrientation orientation)
```
...
```java
SpatialConfig com.ss.bytertc.engine.live.LiveTranscoding.SpatialConfig.setEnableSpatialRender(boolean enableSpatialRender)
```
...
```java
public enum com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamSEIContentMode
```
服务端合流转推 SEI 内容。
...
### getValue
```java
String com.ss.bytertc.engine.live.LiveTranscoding.AACProfile.getValue()
```
获取 AAC 编码规格。
...
# Layout
```java
static class com.ss.bytertc.engine.publicstream.PublicStreaming.Layout
```
公共流的合流布局
...
| void | [setRegions](#Layout-setregions-2) |
...
### getRegions
```java
Region[] com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.getRegions()
```
获取当前公共流的用户布局信息列表。
...
<span id="Layout-setregions-2"></span>
### setRegions
```java
void com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.setRegions(Region[] regions)
```
设置合成公共流的各路流的布局信息。
...
```java
int com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.getInterpolationMode()
```
获取插帧模式。
...
```java
void com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.setInterpolationMode(int interMode)
```
设置插帧模式。
...
### getLayoutMode
```java
int com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.getLayoutMode()
```
获取当前公共流的布局模式。
...
### setLayoutMode
```java
void com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.setLayoutMode(int layoutMode)
```
设置公共流的布局模式。
...
### getBackgroundImage
```java
String com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.getBackgroundImage()
```
获取视频的背景图片链接。
...
### setBackgroundImage
```java
void com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.setBackgroundImage(String bgImage)
```
设置背景图片链接。
...
### getBackgroundColor
```java
String com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.getBackgroundColor()
```
获取画布的背景颜色。
...
<span id="Layout-setbackgroundcolor-2"></span>
### setBackgroundColor
```java
void com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.setBackgroundColor(String backgroundColor)
```
设置画布的背景颜色。
...
<span id="FaceDetectionResult"></span>
# FaceDetectionResult
```java
...
### setReleaseCallback
```java
synchronized void com.ss.bytertc.engine.video.VideoFrame.setReleaseCallback(Runnable releaseCallback)
```
...
### hasReleaseCallback
```java
synchronized boolean com.ss.bytertc.engine.video.VideoFrame.hasReleaseCallback()
```
...
### getFrameType
```java
abstract VideoFrameType com.ss.bytertc.engine.video.VideoFrame.getFrameType()
```
...
### getPixelFormat
```java
abstract VideoPixelFormat com.ss.bytertc.engine.video.VideoFrame.getPixelFormat()
```
...
### getContentType
```java
abstract VideoContentType com.ss.bytertc.engine.video.VideoFrame.getContentType()
```
获取视频内容类型
...
### getTimeStampUs
```java
abstract long com.ss.bytertc.engine.video.VideoFrame.getTimeStampUs()
```
获取视频帧时间戳，单位：微秒
...
### getWidth
```java
abstract int com.ss.bytertc.engine.video.VideoFrame.getWidth()
```
获取视频帧宽度，单位：px
...
### getHeight
```java
abstract int com.ss.bytertc.engine.video.VideoFrame.getHeight()
```
获取视频帧高度，单位：px
...
### getRotation
```java
abstract VideoRotation com.ss.bytertc.engine.video.VideoFrame.getRotation()
```
获取视频帧旋转角度
...
### getColorSpace
```java
abstract ColorSpace com.ss.bytertc.engine.video.VideoFrame.getColorSpace()
```
...
### getCameraId
```java
abstract CameraId com.ss.bytertc.engine.video.VideoFrame.getCameraId()
```
...
### getNumberOfPlanes
```java
abstract int com.ss.bytertc.engine.video.VideoFrame.getNumberOfPlanes()
```
获取视频帧像素平面数量
...
### getPlaneData
```java
abstract ByteBuffer com.ss.bytertc.engine.video.VideoFrame.getPlaneData(int planeIndex)
```
获取 plane 数据
...
### getPlaneStride
```java
abstract int com.ss.bytertc.engine.video.VideoFrame.getPlaneStride(int planeIndex)
```
获取指定 plane 中数据行的长度
...
### getExternalDataInfo
```java
abstract ByteBuffer com.ss.bytertc.engine.video.VideoFrame.getExternalDataInfo()
```
获取 SEI 数据
...
```java
abstract ByteBuffer com.ss.bytertc.engine.video.VideoFrame.getSupplementaryInfo()
```
获取补充数据指针
...
### getTextureID
```java
abstract int com.ss.bytertc.engine.video.VideoFrame.getTextureID()
```
获取纹理 ID
...
### getTextureMatrix
```java
abstract float[] com.ss.bytertc.engine.video.VideoFrame.getTextureMatrix()
```
获取纹理矩阵
...
### getEGLContext
```java
abstract EGLContext com.ss.bytertc.engine.video.VideoFrame.getEGLContext()
```
获取纹理使用的 EGLContext
...
### retain
```java
synchronized void com.ss.bytertc.engine.video.VideoFrame.retain()
```
对当前的视频帧进行一次引用计数
...
### release
```java
synchronized void com.ss.bytertc.engine.video.VideoFrame.release()
```
对当前的视频帧减少一次引用计数
...
### roomId
```java
public String com.ss.bytertc.engine.data.RemoteStreamKey.roomId
```
房间 ID


<span id="RemoteStreamKey-userid"></span>
### userId
```java
public String com.ss.bytertc.engine.data.RemoteStreamKey.userId
```
用户 ID


<span id="RemoteStreamKey-streamindex"></span>
### streamIndex
```java
public StreamIndex com.ss.bytertc.engine.data.RemoteStreamKey.streamIndex
```
流属性，包括主流、屏幕流。参看 [StreamIndex](#StreamIndex)
...
### getRoomId
```java
String com.ss.bytertc.engine.data.RemoteStreamKey.getRoomId()
```
获取房间 ID
...
### getUserId
```java
String com.ss.bytertc.engine.data.RemoteStreamKey.getUserId()
```
获取用户 ID
...
### getStreamIndex
```java
StreamIndex com.ss.bytertc.engine.data.RemoteStreamKey.getStreamIndex()
```
获取流属性，包括主流、屏幕流。参看 [StreamIndex](#StreamIndex)


<span id="RemoteStreamKey-hasnullproperty"></span>
### hasNullProperty
```java
boolean com.ss.bytertc.engine.data.RemoteStreamKey.hasNullProperty()
```
检查当前类中是否有为空的字段
...
```java
public enum com.ss.bytertc.engine.data.MediaPlayerCustomSourceMode
```
自定义音频源模式
...
| --- | --- | --- |
| OK | 0 | 成功。 |
| FILE_NOT_EXIST | -3020 | 播放错误，请下载后播放。 |
| FILE_ERROR | -3021 | 播放错误，请确认文件播放格式。 |
| NOT_JOIN_ROOM | -3022 | 播放错误，未进入房间。 |
| PARAM | -3023 | 参数错误。 |
| START_ERROR | -3024 | 播放失败，找不到文件或文件打开失败。 |
| MIX_ID_ERROR | -3025 | 混音 ID 异常。 |
| POSITION_ERROR | -3026 | 设置播放位置出错。 |
| AUDIO_VOLUME_ERROR | -3027 | 音量参数不合法，可设置的取值范围为 [0,400]。 |
| TYPE_ERROR | -3028 | 不支持此混音类型。 |
| PITCH_ERROR | -3029 | 音调文件不合法。 |
...
<span id="HotMusicInfo-musicinfos"></span>
### musicInfos
```java
public MusicInfo [] com.ss.bytertc.ktv.data.HotMusicInfo.musicInfos
```
...
### getValue
```java
int com.ss.bytertc.engine.live.LiveTranscoding.TranscoderRenderMode.getValue()
```
获取视频内容的缩放模式。
...
<span id="MixedStreamConfig-MixedStreamServerControlConfig"></span>
# MixedStreamServerControlConfig
```java
public static class com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig
```
服务端合流控制参数。
...
| int | [getSeiPayloadType](#MixedStreamServerControlConfig-getseipayloadtype) |
| MixedStreamServerControlConfig | [setSeiPayloadUuid](#MixedStreamServerControlConfig-setseipayloaduuid) |
| String | [getSeiPayloadUuid](#MixedStreamServerControlConfig-getseipayloaduuid) |
| MixedStreamServerControlConfig | [setVolumeIndicationInterval](#MixedStreamServerControlConfig-setvolumeindicationinterval) |
| float | [getVolumeIndicationInterval](#MixedStreamServerControlConfig-getvolumeindicationinterval) |
| MixedStreamServerControlConfig | [setEnableVolumeIndication](#MixedStreamServerControlConfig-setenablevolumeindication) |
| boolean | [getEnableVolumeIndication](#MixedStreamServerControlConfig-getenablevolumeindication) |
| MixedStreamServerControlConfig | [setIsAddVolumeValue](#MixedStreamServerControlConfig-setisaddvolumevalue) |
| boolean | [getIsAddVolumeValue](#MixedStreamServerControlConfig-getisaddvolumevalue) |
| MixedStreamServerControlConfig | [setSeiContentMode](#MixedStreamServerControlConfig-setseicontentmode) |
| MixedStreamSEIContentMode | [getSeiContentMode](#MixedStreamServerControlConfig-getseicontentmode) |
| MixedStreamServerControlConfig | [setTalkVolume](#MixedStreamServerControlConfig-settalkvolume) |
| int | [getTalkVolume](#MixedStreamServerControlConfig-gettalkvolume) |
| MixedStreamServerControlConfig | [setPushStreamMode](#MixedStreamServerControlConfig-setpushstreammode) |
| MixedStreamPushMode | [getPushStreamMode](#MixedStreamServerControlConfig-getpushstreammode) |
...
### setSeiPayloadType
```java
MixedStreamServerControlConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.setSeiPayloadType(int seiPayloadType)
```
> Available since 3.56

设置合流转推 SEI 信息的 payload type。
...
MixedStreamServerControlConfig，参看 MixedStreamServerControlConfig{ MixedStreamServerControlConfig}。
...
<span id="MixedStreamServerControlConfig-getseipayloadtype"></span>
### getSeiPayloadType
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.getSeiPayloadType()
```
> Available since 3.56

获取合流转推 SEI 信息的 payload type。


<span id="MixedStreamServerControlConfig-setseipayloaduuid"></span>
### setSeiPayloadUuid
```java
MixedStreamServerControlConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.setSeiPayloadUuid(String seiPayloadUuid)
```
> Available since 3.56

设置合流转推 SEI 信息的 Payload UUID。
...
<span id="MixedStreamServerControlConfig-getseipayloaduuid"></span>
### getSeiPayloadUuid
```java
String com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.getSeiPayloadUuid()
```
> Available since 3.56

获取合流转推 SEI 信息的 Payload UUID。


<span id="MixedStreamServerControlConfig-setvolumeindicationinterval"></span>
### setVolumeIndicationInterval
```java
MixedStreamServerControlConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.setVolumeIndicationInterval(float volumeIndicationInterval)
```
> Available since 3.56
...
```java
float com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.getVolumeIndicationInterval()
```
> Available since 3.56
...
<span id="MixedStreamServerControlConfig-setenablevolumeindication"></span>
### setEnableVolumeIndication
```java
MixedStreamServerControlConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.setEnableVolumeIndication(boolean enableVolumeIndication)
```
> Available since 3.56

设置是否开启单独发送声音提示 SEI 的功能。
...
开启后，你可以通过 [setSeiContentMode](#MixedStreamServerControlConfig-setseicontentmode) 控制 SEI 的内容是否只携带声音信息。

<span id="MixedStreamServerControlConfig-getenablevolumeindication"></span>
### getEnableVolumeIndication
```java
boolean com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.getEnableVolumeIndication()
```
> Available since 3.56

获取是否开启单独发送声音提示 SEI 功能的设置。


<span id="MixedStreamServerControlConfig-setisaddvolumevalue"></span>
### setIsAddVolumeValue
```java
MixedStreamServerControlConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.setIsAddVolumeValue(boolean addVolumeValue)
```
> Available since 3.56
...
### getIsAddVolumeValue
```java
boolean com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.getIsAddVolumeValue()
```
> Available since 3.56

获取声音信息 SEI 是否包含音量值的设置。


<span id="MixedStreamServerControlConfig-setseicontentmode"></span>
### setSeiContentMode
```java
MixedStreamServerControlConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.setSeiContentMode(MixedStreamSEIContentMode seiContentMode)
```
> Available since 3.56
...
### getSeiContentMode
```java
MixedStreamSEIContentMode com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.getSeiContentMode()
```
> Available since 3.56
...
<span id="MixedStreamServerControlConfig-settalkvolume"></span>
### setTalkVolume
```java
MixedStreamServerControlConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.setTalkVolume(int talkVolume)
```
> Available since 3.56
...
### getTalkVolume
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.getTalkVolume()
```
> Available since 3.56
...
<span id="MixedStreamServerControlConfig-setpushstreammode"></span>
### setPushStreamMode
```java
MixedStreamServerControlConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.setPushStreamMode(MixedStreamPushMode pushStreamMode)
```
> Available since 3.57
...
### getPushStreamMode
```java
MixedStreamPushMode com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.getPushStreamMode()
```
> Available since 3.57
...
<span id="MixedStreamServerControlConfig-setmediatype"></span>
### setMediaType
```java
MixedStreamServerControlConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.setMediaType(MixedStreamMediaType mediaType)
```
> Available since 3.57
...
### getMediaType
```java
MixedStreamMediaType com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamServerControlConfig.getMediaType()
```
> Available since 3.57

获取合流推到 CDN 时输出的媒体流类型设置。
...
本地用户发布音频流成功后，SDK 会周期性地通过 `onLocalStreamStats` 通知用户发布的音频流在此次统计周期内的发送状况。此数据结构即为回调给用户的参数类型。
...
### jitter
```java
public int com.ss.bytertc.engine.type.LocalAudioStats.jitter
```
音频上行网络抖动，单位为 ms 。


<span id="MixedStreamConfig-MixedStreamVideoConfig"></span>
...
### setFps
```java
MixedStreamVideoConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.setFps(int fps)
```
设置合流视频帧率。建议设置。
...
### getFps
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.getFps()
```
获取合流视频帧率。
...
### setGop
```java
MixedStreamVideoConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.setGop(int gop)
```
设置视频 I 帧时间间隔。建议设置。
...
### getGop
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.getGop()
```
获取视频 I 帧时间间隔。
...
### setBitrate
```java
MixedStreamVideoConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.setBitrate(int bitrate)
```
设置合流视频码率。建议设置。
...
### getBitrate
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.getBitrate()
```
获取合流视频码率。
...
### setWidth
```java
MixedStreamVideoConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.setWidth(int width)
```
设置合流视频宽度。建议设置。
...
### getWidth
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.getWidth()
```
获取合流视频宽度。
...
### setHeight
```java
MixedStreamVideoConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.setHeight(int height)
```
设置合流视频高度。建议设置。
...
### getHeight
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.getHeight()
```
获取合流视频高度。
...
### setEnableBframe
```java
MixedStreamVideoConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.setEnableBframe(boolean enableBframe)
```
设置是否使用 B 帧（仅在服务端合流生效）。
...
### getEnableBframe
```java
boolean com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.getEnableBframe()
```
获取是否使用 B 帧（仅在服务端合流生效）的设置。


<span id="MixedStreamVideoConfig-setvideocodec"></span>
### setVideoCodec
```java
MixedStreamVideoConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.setVideoCodec(MixedStreamVideoCodecType videoCodec)
```
设置视频编码格式。建议设置。
...
### getVideoCodec
```java
MixedStreamVideoCodecType com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamVideoConfig.getVideoCodec()
```
获取视频编码格式。
...
```java
public class com.ss.bytertc.engine.data.MediaPlayerCustomSource
```
音频源
...
### mode
```java
public MediaPlayerCustomSourceMode com.ss.bytertc.engine.data.MediaPlayerCustomSource.mode = MediaPlayerCustomSourceMode.PUSH
```
...
### type
```java
public MediaPlayerCustomSourceStreamType com.ss.bytertc.engine.data.MediaPlayerCustomSource.type = MediaPlayerCustomSourceStreamType.RAW
```
...
```java
public enum com.ss.bytertc.engine.video.LocalVideoRenderPosition
```
本地视频帧回调位置。
...
# RTCLogConfig
```java
public class com.ss.bytertc.engine.data.RTCLogConfig
```
本地日志参数。
...
### logPath
```java
public String com.ss.bytertc.engine.data.RTCLogConfig.logPath
```
日志存储路径，必填。


<span id="RTCLogConfig-logfilesize"></span>
### logFileSize
```java
public int com.ss.bytertc.engine.data.RTCLogConfig.logFileSize = 10
```
...
<span id="RTCLogConfig-loglevel"></span>
### logLevel
```java
public LocalLogLevel com.ss.bytertc.engine.data.RTCLogConfig.logLevel = LocalLogLevel.WARNING
```
日志等级，参看 [LocalLogLevel](#LocalLogLevel)，默认为警告级别，选填。


<span id="RTCLogConfig-logfilenameprefix"></span>
### logFilenamePrefix
```java
public String com.ss.bytertc.engine.data.RTCLogConfig.logFilenamePrefix
```
...
<span id="AudioFormat-samplespercall"></span>
### samplesPerCall
```java
public int com.ss.bytertc.engine.data.AudioFormat.samplesPerCall
```
...
### encodePreference
```java
public EncoderPreference com.ss.bytertc.engine.VideoEncoderConfig.encodePreference = EncoderPreference.MAINTAIN_FRAMERATE
```
编码策略偏好，默认为帧率优先。参看 [EncoderPreference](#EncoderPreference-3)。
...
```java
public class com.ss.bytertc.engine.video.RemoteVideoSinkConfig
```
远端视频帧回调配置。
...
### position
```java
public RemoteVideoRenderPosition com.ss.bytertc.engine.video.RemoteVideoSinkConfig.position = RemoteVideoRenderPosition.AFTER_POST_PROCESS
```
远端视频帧回调位置，参看 [RemoteVideoRenderPosition](#RemoteVideoRenderPosition)，默认回调后处理后的视频帧。


<span id="RemoteVideoSinkConfig-pixelformat"></span>
### pixelFormat
```java
public int com.ss.bytertc.engine.video.RemoteVideoSinkConfig.pixelFormat = 0
```
远端视频帧回调格式，参看 [VideoPixelFormat](#VideoPixelFormat)，默认值为 0。


<span id="RemoteVideoSinkConfig-applyrotation"></span>
### applyRotation
```java
public VideoApplyRotation com.ss.bytertc.engine.video.RemoteVideoSinkConfig.applyRotation = VideoApplyRotation.DEFAULT
```
是否将视频帧自动转正，参看 [VideoApplyRotation](#VideoApplyRotation)，默认为不旋转。


<span id="RemoteVideoSinkConfig-mirrortype"></span>
### mirrorType
```java
public VideoRenderMirrorType com.ss.bytertc.engine.video.RemoteVideoSinkConfig.mirrorType = VideoRenderMirrorType.OFF
```
...
| LOCAL_AUDIO_STREAM_STATE_RECORDING | 1 | 本地音频录制设备启动成功。<br>采集到音频首帧时回调该状态，对应错误码 [LocalAudioStreamError](#LocalAudioStreamError) 中的 kLocalAudioStreamErrorOk 。 |
...
### value
```java
int com.ss.bytertc.engine.data.LocalAudioStreamState.value()
```
获取当前值
...
### width
```java
public int com.ss.bytertc.engine.data.VideoFrameInfo.width = 0
```
宽（像素）
...
### height
```java
public int com.ss.bytertc.engine.data.VideoFrameInfo.height = 0
```
高（像素）
...
# EngineConfig
```java
public class com.ss.bytertc.engine.data.EngineConfig
```
引擎配置
...
| int | [ENGINE_CONFIG_INVALID_APP_ID](#EngineConfig-engine_config_invalid_app_id) |
| int | [ENGINE_CONFIG_INVALID_NATIVE_LIB_PATH](#EngineConfig-engine_config_invalid_native_lib_path) |
...
### ENGINE_CONFIG_VALID
```java
public static final int com.ss.bytertc.engine.data.EngineConfig.ENGINE_CONFIG_VALID = 0
```
EngineConfig 有效


<span id="EngineConfig-engine_config_invalid_app_id"></span>
### ENGINE_CONFIG_INVALID_APP_ID
```java
public static final int com.ss.bytertc.engine.data.EngineConfig.ENGINE_CONFIG_INVALID_APP_ID = 1
```
EngineConfig 无效，AppID 无效


<span id="EngineConfig-engine_config_invalid_native_lib_path"></span>
### ENGINE_CONFIG_INVALID_NATIVE_LIB_PATH
```java
public static final int com.ss.bytertc.engine.data.EngineConfig.ENGINE_CONFIG_INVALID_NATIVE_LIB_PATH = 2
```
EngineConfig 无效，nativeLoadPath 无效


<span id="EngineConfig-appid"></span>
### appID
```java
public String com.ss.bytertc.engine.data.EngineConfig.appID
```
必填，每个应用的唯一标识符，由 RTC 控制台随机生成的。不同的 AppId 生成的实例在 RTC 中进行音视频通话完全独立，无法互通。


<span id="EngineConfig-eglcontext"></span>
### eglContext
```java
public Object com.ss.bytertc.engine.data.EngineConfig.eglContext = null
```
选填，如果需要支持外部纹理硬编码，则需要以 `JObject` 方式传入 `eglContext`


<span id="EngineConfig-parameters"></span>
### parameters
```java
public JSONObject com.ss.bytertc.engine.data.EngineConfig.parameters = null
```
...
### getValue
```java
int com.ss.bytertc.engine.live.LiveTranscoding.TranscoderClientMixVideoFormat.getValue()
```
获取合流输出内容类型。
...
<span id="LiveTranscoding-VideoConfig"></span>
# VideoConfig
```java
...
### setCodec
```java
VideoConfig com.ss.bytertc.engine.live.LiveTranscoding.VideoConfig.setCodec(VideoCodecType codec)
```
设置视频编码格式。
...
### setFps
```java
VideoConfig com.ss.bytertc.engine.live.LiveTranscoding.VideoConfig.setFps(int fps)
```
设置合流视频帧率。
...
### setGop
```java
VideoConfig com.ss.bytertc.engine.live.LiveTranscoding.VideoConfig.setGop(int gop)
```
设置视频 I 帧时间间隔。
...
### setBFrame
```java
VideoConfig com.ss.bytertc.engine.live.LiveTranscoding.VideoConfig.setBFrame(boolean useBFrame)
```
设置是否使用 B 帧（仅在服务端合流生效）。
...
### setKBitRate
```java
VideoConfig com.ss.bytertc.engine.live.LiveTranscoding.VideoConfig.setKBitRate(int kBitRate)
```
设置合流视频码率。
...
### setWidth
```java
VideoConfig com.ss.bytertc.engine.live.LiveTranscoding.VideoConfig.setWidth(int width)
```
设置合流视频宽度。
...
### setHeight
```java
VideoConfig com.ss.bytertc.engine.live.LiveTranscoding.VideoConfig.setHeight(int height)
```
设置合流视频高度。
...
| int | [beforeVideoIndex](#RemoteStreamSwitch-beforevideoindex) |
| int | [afterVideoIndex](#RemoteStreamSwitch-aftervideoindex) |
...
<span id="RemoteStreamSwitch-beforevideoindex"></span>
### beforeVideoIndex
```java
public int com.ss.bytertc.engine.type.RemoteStreamSwitch.beforeVideoIndex
```
流切换前订阅视频流的分辨率对应的索引


<span id="RemoteStreamSwitch-aftervideoindex"></span>
### afterVideoIndex
```java
public int com.ss.bytertc.engine.type.RemoteStreamSwitch.afterVideoIndex
```
流切换后订阅视频流的分辨率对应的索引


<span id="RemoteStreamSwitch-beforeenable"></span>
### beforeEnable
```java
public boolean com.ss.bytertc.engine.type.RemoteStreamSwitch.beforeEnable
```
流切换前是否有视频流


<span id="RemoteStreamSwitch-afterenable"></span>
### afterEnable
```java
public boolean com.ss.bytertc.engine.type.RemoteStreamSwitch.afterEnable
```
流切换后是否有视频流
...
<span id="MixedStreamConfig-MixedStreamSpatialConfig"></span>
...
### setAudienceSpatialPosition
```java
MixedStreamSpatialConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamSpatialConfig.setAudienceSpatialPosition(Position pos)
```
设置听众的空间位置。听众指收听来自 CDN 的音频流的用户。
...
```java
Position com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamSpatialConfig.getAudienceSpatialPosition()
```
获取听众的空间位置。
...
### setAudienceSpatialOrientation
```java
MixedStreamSpatialConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamSpatialConfig.setAudienceSpatialOrientation(HumanOrientation orientation)
```
设置听众的空间朝向。听众指收听来自 CDN 的音频流的用户。
...
```java
HumanOrientation com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamSpatialConfig.getAudienceSpatialOrientation()
```
获取听众的空间朝向。
...
### setEnableSpatialRender
```java
MixedStreamSpatialConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamSpatialConfig.setEnableSpatialRender(boolean enableSpatialRender)
```
设置是否开启推流 CDN 时的空间音频效果。
...
```java
boolean com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamSpatialConfig.getEnableSpatialRender()
```
...
| Surface | [renderSurface](#VideoCanvas-rendersurface) |
...
<span id="VideoCanvas-rendersurface"></span>
### renderSurface
```java
public Surface com.ss.bytertc.engine.VideoCanvas.renderSurface
```
本地 surface 句柄。
...
<span id="VideoCanvas-backgroundcolor"></span>
### backgroundColor
```java
public int com.ss.bytertc.engine.VideoCanvas.backgroundColor
```
用于填充画布空白部分的背景颜色。取值范围是 `[0x00000000, 0xFFFFFFFF]`。默认值是 `0x00000000`。其中，透明度设置无效。


<span id="VideoCanvas-renderrotation"></span>
### renderRotation
```java
public VideoRotation com.ss.bytertc.engine.VideoCanvas.renderRotation
```
...
### jitter
```java
public int com.ss.bytertc.engine.type.RemoteVideoStats.jitter
```
视频下行网络抖动，单位为 ms。
...
# VideoApplyRotation
```java
public enum com.ss.bytertc.engine.video.VideoApplyRotation
```
是否将视频帧自动转正。
...
# VideoDeviceInfo
```java
public class com.ss.bytertc.engine.video.VideoDeviceInfo
```
> Available since 3.56
...
### deviceId
```java
public String com.ss.bytertc.engine.video.VideoDeviceInfo.deviceId
```
设备 ID


<span id="VideoDeviceInfo-devicename"></span>
### deviceName
```java
public String com.ss.bytertc.engine.video.VideoDeviceInfo.deviceName
```
设备名称


<span id="VideoDeviceInfo-devicefacing"></span>
### deviceFacing
```java
public VideoDeviceFacing com.ss.bytertc.engine.video.VideoDeviceInfo.deviceFacing
```
视频设备朝向信息，参看 [VideoDeviceFacing](#VideoDeviceFacing)。
...
### onFrame
```java
void com.ss.bytertc.engine.video.IVideoSink.onFrame(VideoFrame frame)
```
视频帧回调
...
```java
public class com.ss.bytertc.engine.video.LocalVideoSinkConfig
```
本地视频帧回调配置。
...
### position
```java
public LocalVideoRenderPosition com.ss.bytertc.engine.video.LocalVideoSinkConfig.position = LocalVideoRenderPosition.AFTER_CAPTURE
```
本地视频帧回调位置，参看 [LocalVideoRenderPosition](#LocalVideoRenderPosition)，默认回调前处理后的视频帧。


<span id="LocalVideoSinkConfig-pixelformat"></span>
### pixelFormat
```java
public int com.ss.bytertc.engine.video.LocalVideoSinkConfig.pixelFormat = 0
```
...
# RenderError
```java
public enum com.ss.bytertc.engine.type.RenderError
```
渲染错误码
...
```java
public class com.ss.bytertc.engine.video.ExpressionDetectInfo
```
人像属性检测信息
...
| float | [boyProb](#ExpressionDetectInfo-boyprob) |
| float | [attractive](#ExpressionDetectInfo-attractive) |
| float | [happyScore](#ExpressionDetectInfo-happyscore) |
| float | [sadScore](#ExpressionDetectInfo-sadscore) |
| float | [angryScore](#ExpressionDetectInfo-angryscore) |
...
### age
```java
public float com.ss.bytertc.engine.video.ExpressionDetectInfo.age
```
预测年龄，取值范围 (0, 100)。


<span id="ExpressionDetectInfo-boyprob"></span>
### boyProb
```java
public float com.ss.bytertc.engine.video.ExpressionDetectInfo.boyProb
```
预测为男性的概率，取值范围 (0.0, 1.0)。


<span id="ExpressionDetectInfo-attractive"></span>
### attractive
```java
public float com.ss.bytertc.engine.video.ExpressionDetectInfo.attractive
```
预测的吸引力分数，取值范围 (0, 100)。


<span id="ExpressionDetectInfo-happyscore"></span>
### happyScore
```java
public float com.ss.bytertc.engine.video.ExpressionDetectInfo.happyScore
```
预测的微笑程度，取值范围 (0, 100)。


<span id="ExpressionDetectInfo-sadscore"></span>
### sadScore
```java
public float com.ss.bytertc.engine.video.ExpressionDetectInfo.sadScore
```
预测的伤心程度，取值范围 (0, 100)。


<span id="ExpressionDetectInfo-angryscore"></span>
### angryScore
```java
public float com.ss.bytertc.engine.video.ExpressionDetectInfo.angryScore
```
预测的生气程度，取值范围 (0, 100)。


<span id="ExpressionDetectInfo-surprisescore"></span>
### surpriseScore
```java
public float com.ss.bytertc.engine.video.ExpressionDetectInfo.surpriseScore
```
预测的吃惊程度，取值范围 (0, 100)。


<span id="ExpressionDetectInfo-arousal"></span>
### arousal
```java
public float com.ss.bytertc.engine.video.ExpressionDetectInfo.arousal
```
预测的情绪激动程度，取值范围 (0, 100)。


<span id="ExpressionDetectInfo-valence"></span>
### valence
```java
public float com.ss.bytertc.engine.video.ExpressionDetectInfo.valence
```
...
### value
```java
int com.ss.bytertc.engine.data.RemoteAudioState.value()
```
获取当前值
...
# FrameUpdateInfo
```java
public class com.ss.bytertc.engine.data.FrameUpdateInfo
```
...
### pixel
```java
public int com.ss.bytertc.engine.data.FrameUpdateInfo.pixel
```
分辨率积（宽*高）。
...
### frameRate
```java
public int com.ss.bytertc.engine.data.FrameUpdateInfo.frameRate
```
帧率。
...
| boolean | [enableSpectrum](#AudioPropertiesConfig-enablespectrum) |
| boolean | [enableVad](#AudioPropertiesConfig-enablevad) |
| AudioReportMode | [localMainReportMode](#AudioPropertiesConfig-localmainreportmode) |
| AudioPropertiesMode | [audioReportMode](#AudioPropertiesConfig-audioreportmode) |
...
<span id="AudioPropertiesConfig-enablespectrum"></span>
### enableSpectrum
```java
public boolean com.ss.bytertc.engine.data.AudioPropertiesConfig.enableSpectrum = false
```
是否开启音频频谱检测


<span id="AudioPropertiesConfig-enablevad"></span>
### enableVad
```java
public boolean com.ss.bytertc.engine.data.AudioPropertiesConfig.enableVad = false
```
是否开启人声检测 (VAD)


<span id="AudioPropertiesConfig-localmainreportmode"></span>
### localMainReportMode
```java
public AudioReportMode com.ss.bytertc.engine.data.AudioPropertiesConfig.localMainReportMode = AudioReportMode.AUDIO_REPORT_MODE_NORMAL
```
音量回调模式。详见 [AudioReportMode](#AudioReportMode)。


<span id="AudioPropertiesConfig-audioreportmode"></span>
### audioReportMode
```java
...
<span id="AudioPropertiesConfig-enablevoicepitch"></span>
### enableVoicePitch
```java
public boolean com.ss.bytertc.engine.data.AudioPropertiesConfig.enableVoicePitch = false
```
是否回调本地用户的人声基频。
...
# Region
```java
static class com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region
```
合成公共流的每条流的布局信息
...
| Region | [roomId](#Region-roomid-2) |
| Region | [zorder](#Region-zorder-2) |
| Region | [alpha](#Region-alpha-2) |
| Region | [mediaType](#Region-mediatype) |
| Region | [streamType](#Region-streamtype) |
| Region | [renderMode](#Region-rendermode-2) |
| Region | [position](#Region-position-2) |
...
### userId
```java
Region com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region.userId(String uid)
```
...
<span id="Region-roomid-2"></span>
### roomId
```java
Region com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region.roomId(String roomId)
```
...
<span id="Region-zorder-2"></span>
### zorder
```java
Region com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region.zorder(int zorder)
```
...
<span id="Region-alpha-2"></span>
### alpha
```java
Region com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region.alpha(double alpha)
```
设置透明度。
...
### mediaType
```java
Region com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region.mediaType(int mediaType)
```
设置公共流内容类型。
...
### streamType
```java
Region com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region.streamType(int streamType)
```
设置公共流媒体类型。
...
<span id="Region-rendermode-2"></span>
### renderMode
```java
Region com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region.renderMode(int renderMode)
```
设置视频显示模式。
...
<span id="Region-position-2"></span>
### position
```java
Region com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region.position(
    double x,
    double y
)
```
...
<span id="Region-size-2"></span>
### size
```java
Region com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region.size(
    double width,
    double height
)
```
...
### sourceCropPosition
```java
Region com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region.sourceCropPosition(
    double x,
    double y
)
```
...
### sourceCropSize
```java
Region com.ss.bytertc.engine.publicstream.PublicStreaming.Layout.Region.sourceCropSize(
    double width,
    double height
)
```
...
### value
```java
int com.ss.bytertc.engine.data.AudioFrameType.value()
```
获取当前值
...
### load
```java
boolean com.ss.bytertc.engine.loader.RTCNativeLibraryLoader.load(String libraryName)
```
SDK 在需要加载动态库时通过该方法回调
...
### getValue
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamRenderMode.getValue()
```
获取视频内容的缩放模式。
...
<span id="LiveTranscoding-ClientMixParam"></span>
# ClientMixParam
```java
...
```java
ClientMixParam com.ss.bytertc.engine.live.LiveTranscoding.ClientMixParam.setClientMixUseAudioMixer(Boolean enable)
```
...
```java
ClientMixParam com.ss.bytertc.engine.live.LiveTranscoding.ClientMixParam.setClientMixUseOriginalFrame(Boolean enable)
```
> Available since 3.50
...
```java
void com.ss.bytertc.engine.live.LiveTranscoding.ClientMixParam.setClientMixVideoFormat(TranscoderClientMixVideoFormat format)
```
> Available since 3.50
...
# VideoDeviceFacing
```java
public enum com.ss.bytertc.engine.type.VideoDeviceFacing
```
> Available since 3.56
...
### fromId
```java
static SubscribeFallbackOptions com.ss.bytertc.engine.type.SubscribeFallbackOptions.fromId(int id)
```
...
### setChannels
```java
MixedStreamAudioConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamAudioConfig.setChannels(int channels)
```
设置声道数。建议设置。
...
### getChannels
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamAudioConfig.getChannels()
```
获取声道数。
...
### setSampleRate
```java
MixedStreamAudioConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamAudioConfig.setSampleRate(int sampleRate)
```
设置音频采样率。建议设置。
...
### getSampleRate
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamAudioConfig.getSampleRate()
```
获取音频采样率。
...
### setBitrate
```java
MixedStreamAudioConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamAudioConfig.setBitrate(int bitrate)
```
设置音频码率。建议设置。
...
### getBitrate
```java
int com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamAudioConfig.getBitrate()
```
获取音频码率。
...
### setAudioCodec
```java
MixedStreamAudioConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamAudioConfig.setAudioCodec(MixedStreamAudioCodecType audioCodec)
```
设置音频编码格式。建议设置。
...
### getAudioCodec
```java
MixedStreamAudioCodecType com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamAudioConfig.getAudioCodec()
```
获取音频编码格式。
...
### setAudioProfile
```java
MixedStreamAudioConfig com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamAudioConfig.setAudioProfile(MixedStreamAudioProfile audioProfile)
```
设置 AAC 编码规格。建议设置。
...
### getAudioProfile
```java
MixedStreamAudioProfile com.ss.bytertc.engine.live.MixedStreamConfig.MixedStreamAudioConfig.getAudioProfile()
```
获取 AAC 编码规格。
...
### value
```java
int com.ss.bytertc.engine.type.BackgroundMode.value()
```
...
<span id="GLTextureVideoFrameBuilder"></span>
...
### setTimeStampUs
```java
GLTextureVideoFrameBuilder com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.setTimeStampUs(long timeStampUs)
```
设置视频帧的时间戳
...
### setWidth
```java
GLTextureVideoFrameBuilder com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.setWidth(int width)
```
设置视频帧宽度
...
### setHeight
```java
GLTextureVideoFrameBuilder com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.setHeight(int height)
```
设置视频帧高度
...
### setRotation
```java
GLTextureVideoFrameBuilder com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.setRotation(VideoRotation rotation)
```
设置视频帧旋转角度
...
### setColorSpace
```java
GLTextureVideoFrameBuilder com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.setColorSpace(ColorSpace colorSpace)
```
设置视频帧色彩空间
...
### setExternalDataInfo
```java
GLTextureVideoFrameBuilder com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.setExternalDataInfo(ByteBuffer externalDataInfo)
```
设置 SEI 数据
...
### setSupplementaryInfo
```java
GLTextureVideoFrameBuilder com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.setSupplementaryInfo(ByteBuffer supplementaryInfo)
```
设置补充数据指针
...
### setTextureID
```java
GLTextureVideoFrameBuilder com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.setTextureID(int textureID)
```
设置视频帧的纹理 ID
...
### setTextureMatrix
```java
GLTextureVideoFrameBuilder com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.setTextureMatrix(float[] textureMatrix)
```
设置视频帧纹理矩阵
...
### setEGLContext
```java
GLTextureVideoFrameBuilder com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.setEGLContext(EGLContext eglContext)
```
设置视频帧的 EGLContext
...
### setReleaseCallback
```java
GLTextureVideoFrameBuilder com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.setReleaseCallback(Runnable releaseCallback)
```
设置视频帧的释放回调，当视频帧的引用计数归 0 时，会执行 releaseCallback
...
### build
```java
VideoFrame com.ss.bytertc.engine.video.builder.GLTextureVideoFrameBuilder.build()
```
获取到创建好的视频帧
...
```java
public enum com.ss.bytertc.engine.data.MediaPlayerCustomSourceStreamType
```
自定义音频流类型
...
# AlphaLayout
```java
public enum com.ss.bytertc.engine.data.AlphaLayout
```
...
| MixedStreamConfig | [setServerControlConfig](#MixedStreamConfig-setservercontrolconfig) |
| MixedStreamServerControlConfig | [getServerControlConfig](#MixedStreamConfig-getservercontrolconfig) |
...
```java
static MixedStreamConfig com.ss.bytertc.engine.live.MixedStreamConfig.defaultMixedStreamConfig()
```
获取默认的转推直播配置参数。
...
### setUserID
```java
MixedStreamConfig com.ss.bytertc.engine.live.MixedStreamConfig.setUserID(String userID)
```
设置推流用户 ID。本参数不支持过程中更新。建议设置。
...
### getUserID
```java
String com.ss.bytertc.engine.live.MixedStreamConfig.getUserID()
```
获取推流用户 ID。
...
### setRoomID
```java
MixedStreamConfig com.ss.bytertc.engine.live.MixedStreamConfig.setRoomID(String roomID)
```
设置推流用户所在的房间 ID。本参数不支持过程中更新。建议设置。
...
### getRoomID
```java
String com.ss.bytertc.engine.live.MixedStreamConfig.getRoomID()
```
获取推流用户所在的房间 ID。
...
### setPushURL
```java
MixedStreamConfig com.ss.bytertc.engine.live.MixedStreamConfig.setPushURL(String pushURL)
```
...
### getPushURL
```java
String com.ss.bytertc.engine.live.MixedStreamConfig.getPushURL()
```
获取推流 CDN 地址。
...
```java
MixedStreamConfig com.ss.bytertc.engine.live.MixedStreamConfig.setExpectedMixingType(ByteRTCStreamMixingType expectedMixingType)
```
设置合流类型。建议设置。
...
```java
ByteRTCStreamMixingType com.ss.bytertc.engine.live.MixedStreamConfig.getExpectedMixingType()
```
获取合流类型。
...
### setAudioConfig
```java
MixedStreamConfig com.ss.bytertc.engine.live.MixedStreamConfig.setAudioConfig(MixedStreamAudioConfig audioConfig)
```
设置音频转码配置参数。建议设置。
...
### getAudioConfig
```java
MixedStreamAudioConfig com.ss.bytertc.engine.live.MixedStreamConfig.getAudioConfig()
```
获取音频转码配置参数。


<span id="MixedStreamConfig-setservercontrolconfig"></span>
### setServerControlConfig
```java
MixedStreamConfig com.ss.bytertc.engine.live.MixedStreamConfig.setServerControlConfig(MixedStreamServerControlConfig serverControlConfig)
```
> Available since 3.56
...
```java
MixedStreamServerControlConfig com.ss.bytertc.engine.live.MixedStreamConfig.getServerControlConfig()
```
> Available since 3.56
...
### setVideoConfig
```java
MixedStreamConfig com.ss.bytertc.engine.live.MixedStreamConfig.setVideoConfig(MixedStreamVideoConfig videoConfig)
```
设置视频转码配置参数。建议设置。
...
### getVideoConfig
```java
MixedStreamVideoConfig com.ss.bytertc.engine.live.MixedStreamConfig.getVideoConfig()
```
获取视频转码配置参数。
...
### setClientMixConfig
```java
MixedStreamConfig com.ss.bytertc.engine.live.MixedStreamConfig.setClientMixConfig(MixedStreamClientMixConfig clientMixConfig)
```
设置客户端合流配置参数。建议设置。
...
### setSpatialConfig
```java
MixedStreamConfig com.ss.bytertc.engine.live.MixedStreamConfig.setSpatialConfig(MixedStreamSpatialConfig spatialConfig)
```
设置推流 CDN 的空间音频参数。
...
### getSpatialConfig
```java
MixedStreamSpatialConfig com.ss.bytertc.engine.live.MixedStreamConfig.getSpatialConfig()
```
获取推流 CDN 的空间音频参数。
...
### setLayout
```java
MixedStreamConfig com.ss.bytertc.engine.live.MixedStreamConfig.setLayout(MixedStreamLayoutConfig layout)
```
设置视频流合流整体布局信息。建议设置。
...
### getLayout
```java
MixedStreamLayoutConfig com.ss.bytertc.engine.live.MixedStreamConfig.getLayout()
```
获取视频流合流整体布局信息。
...
```java
boolean com.ss.bytertc.engine.live.MixedStreamConfig.getMixedStreamVideoConfigBFrame()
```
