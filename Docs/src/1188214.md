...
调用此接口时，需确保云端录制功能已启用且当前处于合流录制的自定义布局模式，即 `RecordMode=0` 且 `Layout.LayoutMode=2`。
## 使用说明
### 接口行为
...
<APILink link="https://api.volcengine.com/api-explorer/debug?action=UpdateRecord&serviceCode=rtc&version=2023-11-01&groupName=云端录制" />
