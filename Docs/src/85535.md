...
设置音频文件混音时，收到 [`onMediaPlayerPlayingProgress`](85533#rtcmediaplayercallback-onmediaplayerplayingprogress) 的间隔。单位毫秒。
...
是否自动播放。如果不自动播放，调用 [`startAudioMixing`](85532#rtcvideo-startaudiomixing) 播放音乐文件。
...
+ true：是  
    ...
    - 若使用自定义采集，此时你需调用 [`pushExternalAudioFrame`](85532#rtcvideo-pushexternalaudioframe) 将采集到的音频推送给 SDK  
+ false：否
...
+ true：是  
    ...
    - 若使用自定义采集，此时你需调用 [`pushExternalVideoFrame`](85532#rtcvideo-pushexternalvideoframe) 将采集到的视频推送给 SDK  
+ false：否  
...
设置 [`onLocalAudioPropertiesReport`](85533#onlocalaudiopropertiesreport) 回调中是否包含本地混音音频数据。
...
屏幕共享对象信息
[`startScreenVideoCapture`](85532#rtcvideo-startscreenvideocapture) 接口的中的参数类型
...
屏幕共享对象缩略图
调用 [getThumbnail](85532#rtcvideo-getthumbnail) 后返回本对象
...
共享窗体的图标信息
调用 [getWindowAppIcon](85532#rtcvideo-getwindowappicon) 后返回本对象
...
- **成员**
  ...
  | kAutoPerformance | `2` | 采集参数与编码参数一致，即在 [setVideoEncoderConfig](85532#setvideoencoderconfig) 中设置的参数。 |
...
- **成员**
  ...
  | 属性 | 值 | 描述 |
  | :-- | :-- | :-- |
  | kRecord | `0` | 本地麦克风录制的音频数据回调 [`onRecordAudioFrame`](85533#rtcvideocallback-onrecordaudioframe) |
  | kPlayback | `1` | 订阅的远端所有用户混音后的音频数据回调  [`onPlaybackAudioFrame`](85533#rtcvideocallback-onplaybackaudioframe) |
  | kMixed | `2` | 本地麦克风录制和订阅的远端所有用户混音后的音频数据回调 [`onMixedAudioFrame`](85533#rtcvideocallback-onmixedaudioframe) |
  | kRemoteUser | `3` | 订阅的远端每个用户混音前的音频数据回调 [`onRemoteUserAudioFrame`](85533#rtcvideocallback-onremoteuseraudioframe) |
  | kRecordScreen | `4` | 本地屏幕录制的音频数据回调 [`onRecordScreenAudioFrame`](85533#rtcvideocallback-onrecordscreenaudioframe) |
...
- **成员**
  ...
  | extra_info | string \| undefined | 用户的额外信息，最大长度为 200 字节。会在 [`onUserJoined`](85533#rtcroomcallback-onuserjoined) 中回调给远端用户。 |
...
若调用 [`setUserVisibility`](85532#rtcroom-setuservisibility) 将自身可见性设为 false，无论是默认的自动发布流还是手动设置的自动发布流都不会进行发布，你需要将自身可见性设为 true 后方可发布。
...
进房后，你可以调用 [`subscribeStream`](85532#rtcroom-subscribestream) 修改订阅设置。
...
用户离线原因。
房间内的远端用户离开房间时，本端用户会收到 [`onUserLeave`](85533#rtcroomcallback-onuserleave) 回调通知，此枚举类型为回调的用户离线原因。

- **成员**

  | 属性 | 值 | 描述 |
  | :-- | :-- | :-- |
  | kUserOfflineReasonQuit | `0` | 远端用户调用 [`leaveRoom`](85532#rtcroom-leaveroom) 方法退出房间。 |
...
本地音频流统计信息，统计周期为 2s 。  本地用户发布音频流成功后，SDK 会周期性地通过 [`onLocalStreamStats`](85533#rtcroomcallback-onlocalstreamstats) 通知用户发布的音频流在此次统计周期内的发送状况。此数据结构即为回调给用户的参数类型。
...
本地视频流统计信息，统计周期为 2s 。
本地用户发布视频流成功后，SDK 会周期性地通过 [`onLocalStreamStats`](85533#rtcroomcallback-onlocalstreamstats)  通知用户发布的视频流在此次统计周期内的发送状况。此数据结构即为回调给用户的参数类型。
...
订阅远端用户发布音/视频流成功后，SDK 会周期性地通过 [onRemoteStreamStats](85533#rtcroomcallback-onremotestreamstats)
...
远端音频流统计信息，统计周期为 2s。
本地用户订阅远端音频流成功后，SDK 会周期性地通过 [`onRemoteStreamStats`](85533#rtcroomcallback-onremotestreamstats) 通知本地用户订阅的音频流在此次统计周期内的接收状况。此数据结构即为回调给本地用户的参数类型。
...
远端音频流统计信息，统计周期为 2s 。
本地用户订阅远端音频流成功后，SDK 会周期性地通过 [onRemoteStreamStats](85533#rtcroomcallback-onremotestreamstats)
...
类型: [ForwardStreamError](85534#forwardstreamerror)
...
- **成员**
  ...
  | kForwardStreamStateFailure | `2` | 转发失败，失败详情参考 [ForwardStreamError](85534#forwardstreamerror)<br>调用 `startForwardStreamToRooms` 或 `updateForwardStreamToRooms` 后，如遇转发失败，返回此状态。 |
...
[`onPerformanceAlarms`](85533#rtcvideocallback-onperformancealarms) 告警的原因
...
- **成员**
  ...
  | kLocalAudioStreamStateMute | `4` | 本地音频静音成功后回调该状态。<br>调用 [`setAudioCaptureDeviceMute`](85532#rtcvideo-setaudiocapturedevicemute) 成功后回调，对应错误码 [`LocalAudioStreamError`](85534#localaudiostreamerror) 中的 kLocalAudioStreamErrorOk 。 |
  | kLocalAudioStreamStateUnmute | `5` | 本地音频解除静音成功后回调该状态。<br>调用 [`setAudioCaptureDeviceMute`](85532#rtcvideo-setaudiocapturedevicemute) 成功后回调，对应错误码 [`LocalAudioStreamError`](85534#localaudiostreamerror) 中的 kLocalAudioStreamErrorOk 。 |
...
+ 调用 [`enableAudioPropertiesReport`](85532#rtcvideo-enableaudiopropertiesreport)，并设置参数 `enable_voice_pitch` 的值为 `true`。 
+ 本地采集的音频中包含本地用户的人声。 
其他情况下返回 0。
...
- **成员**
  ...
  | kMediaDeviceWarningDetectLeakEcho | `11` | 通话中出现回声现象。<br>当 [`RoomProfileType`](85535#roomprofiletype) 为 `kRoomProfileTypeMeeting` 和<br>`kRoomProfileTypeMeetingRoom`，且 AEC 关闭时，SDK 自动启动回声检测，如果检测到回声问题，将通过<br>`onAudioDeviceWarning` 返回本枚举值。 |
...
- **成员**
  ...
  | 属性 | 值 | 描述 |
  | :-- | :-- | :-- |
  | kSEIStreamEventTypeStreamAdd | `0` | 远端用户发布黑帧视频流。<br>纯语音通话场景下，远端用户调用 [`sendSEIMessage`](85532#rtcvideo-sendseimessage) 发送 SEI 数据时，SDK 会自动发布一路黑帧视频流，并触发该回调。 |
  | kSEIStreamEventTypeStreamRemove | `1` | 远端黑帧视频流移除。该回调的触发时机包括：<br>+ 远端用户开启摄像头采集，由语音通话切换至视频通话，黑帧视频流停止发布；<br>+ 远端用户调用 [`sendSEIMessage`](85532#rtcvideo-sendseimessage) 后 1min 内未有 SEI 数据发送，黑帧视频流停止发布；<br>+ 远端用户切换至自定义视频采集时，黑帧视频流停止发布。 |
