...
| uid | **NSString*** | 用户 ID。 |
| state | **NSInteger** | 房间状态码。  <br/>• 0: 加入房间成功。  <br/>• !0: 加入房间失败、异常退房、发生房间相关的警告或错误。具体原因参看 [ByteRTCErrorCode](70085.md#bytertcerrorcode) 及 [ByteRTCWarningCode](70085.md#bytertcwarningcode)。 |
| extraInfo | **NSString*** | 额外信息，如 `{"elapsed":1187,"join_type":0}`。<br/>`join_type`表示加入房间的类型，`0`为首次进房，`1`为重连进房。<br/>`elapsed`表示加入房间耗时，即本地用户从调用 [joinRoom:userInfo:roomConfig:](70086.md#ByteRTCRoom-joinroom-userinfo-roomconfig) 到加入房间成功所经历的时间间隔，单位为 ms。 |
...
| --- | --- | --- |
| rtcRoom | **ByteRTCRoom*** | ByteRTCRoom 对象。 |
| warningCode | **ByteRTCWarningCode** | 警告码，详见枚举类型 [ByteRTCWarningCode](70085.md#bytertcwarningcode) 。 |
...
| --- | --- | --- |
| rtcRoom | **ByteRTCRoom*** | ByteRTCRoom 对象。 |
| errorCode | **ByteRTCErrorCode** | 错误码，详见枚举类型 [ByteRTCErrorCode](70085.md#bytertcerrorcode) 。 |
...
统计信息通过 [ByteRTCLocalStreamStats](70088.md#bytertclocalstreamstats) 类型的回调参数传递给用户，其中包括发送音视频比特率、发送帧率、编码帧率，网络质量等。
...
收到该回调后，你需调用 [updateToken:](70086.md#ByteRTCRoom-updatetoken) 更新 Token 进房权限。
...
收到该回调后，你需调用 [updateToken:](70086.md#ByteRTCRoom-updatetoken) 更新 Token 发布权限。
...
收到该回调后，你需调用 [updateToken:](70086.md#ByteRTCRoom-updatetoken) 更新 Token 订阅权限有效期。
...
| --- | --- | --- |
| rtcRoom | **ByteRTCRoom*** | ByteRTCRoom 对象。 |
| stream | **id<ByteRTCStream>** | 流属性，参看 [ByteRTCStream](70088.md#bytertcstream) 。 |
...
| stream | **id<ByteRTCStream>** | 流的属性，参看 [ByteRTCStream](70088.md#bytertcstream)。 |
...
```
收到房间内广播二进制消息的回调。<br>
房间内其他用户调用 [sendRoomBinaryMessage:](70086.md#ByteRTCRoom-sendroombinarymessage) 发送的广播二进制消息时，收到此回调。
...
```
收到来自房间中其他用户通过 [sendUserMessage:message:config:](70086.md#ByteRTCRoom-sendusermessage-message-config) 发来的点对点文本消息时，会收到此回调。
...
```
收到来自房间中其他用户通过 [sendUserBinaryMessage:message:config:](70086.md#ByteRTCRoom-senduserbinarymessage-message-config) 发来的点对点二进制消息时，会收到此回调。
...
| error | **ByteRTCUserMessageSendResult** | 消息发送结果，详见 [ByteRTCUserMessageSendResult](70085.md#bytertcusermessagesendresult) |
...
```
调用 [sendRoomMessage:](70086.md#ByteRTCRoom-sendroommessage) 或 [sendRoomBinaryMessage:](70086.md#ByteRTCRoom-sendroombinarymessage) 向房间内群发文本或二进制消息后，消息发送方会收到该消息发送结果回调。
...
| rtcRoom | **ByteRTCRoom*** | ByteRTCRoom 对象。 |
| msgid | **NSInteger** | 本条消息的 ID。 |
| error | **ByteRTCRoomMessageSendResult** | 消息发送结果，详见 [ByteRTCRoomMessageSendResult](70085.md#bytertcroommessagesendresult) |
...
调用 [setRoomExtraInfo:value:](70086.md#ByteRTCRoom-setroomextrainfo-value) 结果回调。
...
| error | **ByteRTCSetRoomExtraInfoResult** | 设置房间附加信息结果，详见 [ByteRTCSetRoomExtraInfoResult](70088.md#bytertcsetroomextrainforesult) |
...
接收到房间附加信息的回调。<br>
房间内其他用户调用 [setRoomExtraInfo:value:](70086.md#ByteRTCRoom-setroomextrainfo-value) 设置房间附加信息，收到此回调。<br>
...
| remoteQualities | **NSArray<ByteRTCNetworkQualityStats*>*** | 已订阅用户的网络质量，详见 [ByteRTCNetworkQualityStats](70088.md#bytertcnetworkqualitystats)。 |
...
```
字幕状态发生改变回调。 <br>
当用户调用 [startSubtitle:](70086.md#ByteRTCRoom-startsubtitle) 和 [stopSubtitle](70086.md#ByteRTCRoom-stopsubtitle) 使字幕状态发生改变或出现错误时，触发该回调。

**传入参数**
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| rtcRoom | **ByteRTCRoom*** | ByteRTCRoom 实例。 |
| state | **ByteRTCSubtitleState** | 字幕状态。参看 [ByteRTCSubtitleState](70088.md#bytertcsubtitlestate)。 |
| errorCode | **ByteRTCSubtitleErrorCode** | 字幕任务错误码。参看 [ByteRTCSubtitleErrorCode](70088.md#bytertcsubtitleerrorcode)。 |
...
```
字幕相关内容回调。 <br>
当用户调用 [startSubtitle:](70086.md#ByteRTCRoom-startsubtitle) 后会收到此回调，通知字幕的相关信息。

**传入参数**
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| rtcRoom | **ByteRTCRoom*** | ByteRTCRoom 实例。 |
| subtitles | **NSArray<ByteRTCSubtitleMessage*>*** | 字幕消息内容。参看 [ByteRTCSubtitleMessage](70088.md#bytertcsubtitlemessage)。 |
...
| muteState | **ByteRTCMuteState** | 发送状态，参看 [ByteRTCMuteState](70088.md#bytertcmutestate) |
...
| muteState | **ByteRTCMuteState** | 视频流的发送状态。参看 [ByteRTCMuteState](70088.md#bytertcmutestate)。 |
...
| device_type | **ByteRTCMediaDeviceType** | 设备类型，参看 [ByteRTCMediaDeviceType](70088.md#bytertcmediadevicetype)。 |
...
| deviceType | **ByteRTCMediaDeviceType** | 参看 [ByteRTCMediaDeviceType](70088.md#bytertcmediadevicetype) |
...
| --- | --- | --- |
| engine | **ByteRTCVideo*** | ByteRTCVideo 实例。 |
| device | **ByteRTCAudioPlaybackDevice** | 新的音频设备，详见 [ByteRTCAudioPlaybackDevice](70088.md#bytertcaudioplaybackdevice) |

**注意**
音频设备设置与音频路由策略，详见 [setAudioPlaybackDevice:](70086.md#ByteRTCVideo-setaudioplaybackdevice)
...
```
收到通过调用 [sendSEIMessage:andMessage:andRepeatCount:andCountPerFrame:](70086.md#ByteRTCVideo-sendseimessage-andmessage-andrepeatcount-andcountperframe) 发送带有 SEI 消息的视频帧时，收到此回调。
...
```
黑帧视频流发布状态回调。  <br>
在语音通话场景下，本地用户调用 [sendSEIMessage:andMessage:andRepeatCount:andCountPerFrame:](70086.md#ByteRTCVideo-sendseimessage-andmessage-andrepeatcount-andcountperframe) 通过黑帧视频流发送 SEI 数据时，流的发送状态会通过该回调通知远端用户。  <br>
...
| --- | --- | --- |
| engine | **ByteRTCVideo*** | 当前 ByteRTCVideo 实例。 |
| remoteStreamKey | **ByteRTCRemoteStreamKey*** | 远端流信息，参看 [ByteRTCRemoteStreamKey](70088.md#bytertcremotestreamkey)。 |
| eventType | **ByteSEIStreamEventType** | 黑帧视频流状态，参看 [ByteSEIStreamEventType](70088.md#byteseistreameventtype) |
...
| --- | --- | --- |
| engine | **ByteRTCVideo*** | ByteRTCVideo 对象 |
| errorCode | **NSInteger** | 设置结果  <br/>• 返回 200，设置成功  <br/>• 返回其他，设置失败，详见 [ByteRTCUserMessageSendResult](70085.md#bytertcusermessagesendresult) |
...
| error | **ByteRTCUserMessageSendResult** | 消息发送结果  <br/>详见 [ByteRTCUserMessageSendResult](70085.md#bytertcusermessagesendresult)。 |
...
```
调用 [startAudioRecording:](70086.md#ByteRTCVideo-startaudiorecording) 或者 [stopAudioRecording](70086.md#ByteRTCVideo-stopaudiorecording) 改变音频文件录制状态时，收到此回调。

**传入参数**
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | **ByteRTCAudioRecordingState** | 录制状态，参看 [ByteRTCAudioRecordingState](70088.md#bytertcaudiorecordingstate) |
| error_code | **ByteRTCAudioRecordingErrorCode** | 录制错误码，参看 [ByteRTCAudioRecordingErrorCode](70085.md#bytertcaudiorecordingerrorcode) |
...
| errorCode | **ByteRTCPublicStreamErrorCode** | 公共流发布结果状态码。 详见 [ByteRTCPublicStreamErrorCode](70085.md#bytertcpublicstreamerrorcode)。 |
...
| errorCode | **ByteRTCPublicStreamErrorCode** | 公共流订阅结果状态码。 详见 [ByteRTCPublicStreamErrorCode](70085.md#bytertcpublicstreamerrorcode)。 |
...
```
回调公共流中包含的 SEI 信息。
调用 [startPlayPublicStream:](70086.md#ByteRTCVideo-startplaypublicstream) 接口启动拉公共流功能后，通过此回调收到公共流中的 SEI 消息。
...
| message | **NSData*** | 收到的 SEI 消息内容。<br/>本回调可以获取通过调用客户端 [sendSEIMessage:andMessage:andRepeatCount:andCountPerFrame:](70086.md#ByteRTCVideo-sendseimessage-andmessage-andrepeatcount-andcountperframe) 插入的 SEI 信息。<br/>当公共流中的多路视频流均包含有 SEI 信息：SEI 不互相冲突时，将通过多次回调分别发送；SEI 在同一帧有冲突时，则只有一条流中的 SEI 信息被透传并融合到公共流中。 |
| sourceType | **ByteRTCDataMessageSourceType** | SEI 消息类型，自 3.52.1 版本后固定为 `0`，自定义消息。参看 [ByteRTCDataMessageSourceType](70088.md#bytertcdatamessagesourcetype)。 |
...
回调公共流中包含的数据信息。<br>
通过 [startPlayPublicStream:](70086.md#ByteRTCVideo-startplaypublicstream) 开始播放公共流后，可以通过本回调获取发送端发送的非SEI消息。
...
| sourceType | **ByteRTCDataMessageSourceType** | 数据消息来源，参看 [ByteRTCDataMessageSourceType](70088.md#bytertcdatamessagesourcetype)。 |
...
```
首次调用 [getNetworkTimeInfo](70086.md#ByteRTCVideo-getnetworktimeinfo) 后，SDK 内部启动网络时间同步，同步完成时会触发此回调。

**传入参数**
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| engine | **ByteRTCVideo*** | [ByteRTCVideo](70086.md#bytertcvideo) 对象 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| result | **ByteRTCHardwareEchoDetectionResult** | 参见 [ByteRTCHardwareEchoDetectionResult](70088.md#bytertchardwareechodetectionresult)。 |

**注意**
+ 通话前调用 [startHardwareEchoDetection:](70086.md#ByteRTCVideo-starthardwareechodetection) 后，将触发本回调返回检测结果。
+ 建议在收到检测结果后，调用 [stopHardwareEchoDetection](70086.md#ByteRTCVideo-stophardwareechodetection) 停止检测，释放对音频设备的占用。
...
```
本地代理状态发生改变回调。调用 [setLocalProxy:](70086.md#ByteRTCVideo-setlocalproxy) 设置本地代理后，SDK 会触发此回调，返回代理连接的状态。  <br>

**传入参数**
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | **ByteRTCLocalProxyType** | 本地代理类型。参看 [ByteRTCLocalProxyType](70088.md#bytertclocalproxytype) 。 |
| state | **ByteRTCLocalProxyState** | 本地代理状态。参看 [ByteRTCLocalProxyState](70088.md#bytertclocalproxystate)。 |
| error | **ByteRTCLocalProxyError** | 本地代理错误。参看 [ByteRTCLocalProxyError](70088.md#bytertclocalproxyerror)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audioFrame | **ByteRTCAudioFrame*** | RTC SDK 采集到的音频帧，自定义处理可直接对音频 buffer 中的数据进行修改。参看 [ByteRTCAudioFrame](70088.md#bytertcaudioframe)。 |
**返回值**
...
**注意**
+ 此数据处理只影响软件耳返音频数据。  
+ 要启用此回调，必须调用 [enableAudioProcessor:audioFormat:](70086.md#ByteRTCVideo-enableaudioprocessor-audioformat)，并选择耳返音频。每 10 ms 收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| errorCode | **NSInteger** | 错误码  <br/>• < 0: 参数错误或 API 调用顺序错误，参看 [ByteRTCASRErrorCode](70088.md#bytertcasrerrorcode)；  <br/>• >0: 参看 [语音识别服务错误码](https://www.volcengine.com/docs/6561/80818#_3-3-%E9%94%99%E8%AF%AF%E7%A0%81) |
...
| --- | --- | --- |
| mix_id | **int** | 混音 ID。 |
| audio_frame | **ByteRTCAudioFrame*** | 参看 [ByteRTCAudioFrame](70088.md#bytertcaudioframe)。 |
...
| event | **ByteRTCStreamMixingEvent** | 转推直播任务状态，参看 [ByteRTCStreamMixingEvent](70088.md#bytertcstreammixingevent)。 |
| taskId | **NSString*** | 转推直播任务 ID。 |
| Code | **ByteRTCStreamMixingErrorCode** | 转推直播错误码，参看 [ByteRTCStreamMixingErrorCode](70085.md#bytertcstreammixingerrorcode) 和 [ByteRTCStreamMixingErrorCode](70085.md#bytertcstreammixingerrorcode)。 |
| mixType | **ByteRTCMixedStreamType** | 转推直播类型，参看 [ByteRTCMixedStreamType](70088.md#bytertcmixedstreamtype)。 |
...
| Code | **NSInteger** | 错误码，参看 [ByteRtcTranscoderErrorCode](70085.md#bytertctranscodererrorcode)。 |
...
调用 [enableFaceDetection:withInterval:withModelPath:](70086.md#ByteRTCVideoEffect-enablefacedetection-withinterval-withmodelpath) 注册了 [ByteRTCFaceDetectionObserver](#bytertcfacedetectionobserver) ，并使用 RTC SDK 中包含的特效 SDK 进行视频特效处理时，你会收到此回调。
