...
用户调用 [leaveRoom](Android-api#RTCRoom-leaveroom) 方法后，SDK 会停止所有的发布订阅流，并在释放所有通话相关的音视频资源后，通过此回调通知用户离开房间成功。
...
- 用户调用 [leaveRoom](Android-api#RTCRoom-leaveroom) 方法离开房间后，如果立即调用 [destroy](Android-api#RTCRoom-destroy) 销毁房间实例或 [destroyRTCVideo](Android-api#RTCVideo-destroyrtcvideo) 方法销毁 RTC 引擎，则将无法收到此回调事件。
...
| roomId | String | 房间 ID。 |
| uid | String | 用户 ID。 |
| state | int | 房间状态码。<br><ul><li>0: 加入房间成功。</li><li>!0: 加入房间失败、异常退房、发生房间相关的警告或错误。具体原因参看 [ErrorCode](Android-errorcode#ErrorCode) 及 [WarningCode](Android-errorcode#WarningCode)。</li></ul> |
| extraInfo | String | 额外信息，如 `{"elapsed":1187,"join_type":0}`。<br>`join_type` 表示加入房间的类型，`0`为首次进房，`1`为重连进房。<br>`elapsed` 表示加入房间耗时，即本地用户从调用 [joinRoom](Android-api#RTCRoom-joinroom) 到加入房间成功所经历的时间间隔，单位为 ms。 |
...
| roomId | String | 房间 ID。 |
| uid | String | 用户 ID。 |
| state | int | 流状态码，参看 [ErrorCode](Android-errorcode#ErrorCode) 及 [WarningCode](Android-errorcode#WarningCode)。 |
...
```
发布端调用 [setMultiDeviceAVSync](Android-api#RTCRoom-setmultideviceavsync) 后音视频同步状态发生改变时，会收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | AVSyncState | 音视频同步状态，参看 [AVSyncState](Android-keytype#AVSyncState)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stats | RTCRoomStats | 房间内的汇总统计数据。详见 [RTCRoomStats](Android-keytype#RTCRoomStats)。 |
...
1.远端用户调用 [setUserVisibility](Android-api#RTCRoom-setuservisibility) 方法将自身设为可见后加入房间时，房间内其他用户将收到该事件。

2.远端可见用户断网后重新连入房间时，房间内其他用户将收到该事件。

3.房间内隐身远端用户调用 [setUserVisibility](Android-api#RTCRoom-setuservisibility) 方法切换至可见时，房间内其他用户将收到该事件。
...
| reason | int | 用户离开房间的原因：<br><ul><li>0: 远端用户调用 [leaveRoom](Android-api#RTCRoom-leaveroom) 主动退出房间。</li><li>1: 远端用户因 Token 过期或网络原因等掉线。详细信息请参看[连接状态提示](https://www.volcengine.com/docs/6348/95376)</li><li>2: 远端用户调用 [setUserVisibility](Android-api#RTCRoom-setuservisibility) 切换至不可见状态。</li><li>3: 服务端调用 OpenAPI 将该远端用户踢出房间。</li></ul> |
...
收到该回调后，你需调用 [updateToken](Android-api#RTCRoom-updatetoken) 更新 Token 进房权限。
...
收到该回调后，你需调用 [updateToken](Android-api#RTCRoom-updatetoken) 更新 Token 发布权限。
...
收到该回调后，你需调用 [updateToken](Android-api#RTCRoom-updatetoken) 更新 Token 订阅权限有效期。
...
| type | MediaStreamType | 远端媒体流的类型，参看 [MediaStreamType](Android-keytype#MediaStreamType)。 |


**注意**

当房间内的远端用户调用 [publishStream](Android-api#RTCRoom-publishstream) 成功发布由摄像头/麦克风采集的媒体流时，本地用户会收到该回调，此时本地用户可以自行选择是否调用 [subscribeStream](Android-api#RTCRoom-subscribestream) 订阅此流。
...
| type | MediaStreamType | 移除的远端流类型，参看 [MediaStreamType](Android-keytype#MediaStreamType)。 |
| reason | StreamRemoveReason | 远端流移除的原因，参看 [StreamRemoveReason](Android-keytype#StreamRemoveReason)。 |


**注意**

收到该回调通知后，你可以自行选择是否调用 [unsubscribeStream](Android-api#RTCRoom-unsubscribestream) 取消订阅此流。
...
当房间内的远端用户调用 [publishScreen](Android-api#RTCRoom-publishscreen) 成功发布来自屏幕共享的音视频流时，本地用户会收到该回调，此时本地用户可以自行选择是否调用 [subscribeScreen](Android-api#RTCRoom-subscribescreen) 订阅此流。
...
收到该回调通知后，你可以自行选择是否调用 [unsubscribeScreen](Android-api#RTCRoom-unsubscribescreen) 取消订阅此流。
...
统计信息通过 [LocalStreamStats](Android-keytype#LocalStreamStats) 类型的回调参数传递给用户，其中包括发送音视频比特率、发送帧率、编码帧率，网络质量等。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stats | LocalStreamStats | 音视频流以及网络状况统计信息。参见 [LocalStreamStats](Android-keytype#LocalStreamStats)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stats | RemoteStreamStats | 音视频流以及网络状况统计信息。参见 [RemoteStreamStats](Android-keytype#RemoteStreamStats)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stateCode | int | 订阅媒体流状态，参看 [SubscribeState](Android-keytype#SubscribeState) |
| userId | String | 流发布用户的用户 ID |
| info | SubscribeConfig | 流的属性，参看 [SubscribeConfig](Android-keytype#SubscribeConfig) |


**注意**

本地用户收到该回调的时机包括：
- 调用 [subscribeStream](Android-api#RTCRoom-subscribestream) 或 [unsubscribeStream](Android-api#RTCRoom-unsubscribestream) 订阅/取消订阅指定远端摄像头音视频流后；
- 调用 [subscribeScreen](Android-api#RTCRoom-subscribescreen) 或 [unsubscribeScreen](Android-api#RTCRoom-unsubscribescreen) 订阅/取消订阅指定远端屏幕共享流后。
...
房间内其他用户调用 [sendRoomMessage](Android-api#RTCRoom-sendroommessage) 发送广播消息时，收到此回调。
...
房间内其他用户调用 [sendRoomBinaryMessage](Android-api#RTCRoom-sendroombinarymessage) 发送广播二进制消息时，收到此回调。
...
```
收到来自房间中其他用户通过 [sendUserMessage](Android-api#RTCRoom-sendusermessage) 发来的点对点文本消息时，会收到此回调。
...
```
收到来自房间中其他用户通过 [sendUserBinaryMessage](Android-api#RTCRoom-senduserbinarymessage) 发来的点对点二进制消息时，会收到此回调。
...
| --- | --- | --- |
| msgid | long | 本条消息的 ID。 |
| error | int | 文本或二进制消息发送结果，详见 [UserMessageSendResult](Android-errorcode#UserMessageSendResult) |


**注意**

调用 [sendUserMessage](Android-api#RTCRoom-sendusermessage) 或 [sendUserBinaryMessage](Android-api#RTCRoom-senduserbinarymessage) 接口，才能收到此回调。
...
```
调用 [sendRoomMessage](Android-api#RTCRoom-sendroommessage) 或 [sendRoomBinaryMessage](Android-api#RTCRoom-sendroombinarymessage) 向房间内群发文本或二进制消息后，消息发送方会收到该消息发送结果回调。
...
| --- | --- | --- |
| msgid | long | 本条消息的 ID。 |
| error | int | 消息发送结果，详见 [RoomMessageSendResult](Android-errorcode#RoomMessageSendResult) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stateInfos | ForwardStreamStateInfo[] | 跨房间媒体流转发目标房间信息数组，详见 [ForwardStreamStateInfo](Android-keytype#ForwardStreamStateInfo) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| eventInfos | ForwardStreamEventInfo[] | 跨房间媒体流转发目标房间事件数组，详见 [ForwardStreamEventInfo](Android-keytype#ForwardStreamEventInfo) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| localQuality | NetworkQualityStats | 本地网络质量，详见 [NetworkQualityStats](Android-keytype#NetworkQualityStats)。 |
| remoteQualities | NetworkQualityStats[] | 已订阅用户的网络质量，详见 [NetworkQualityStats](Android-keytype#NetworkQualityStats)。 |
...
调用 [setRoomExtraInfo](Android-api#RTCRoom-setroomextrainfo) 设置房间附加信息结果的回调。
...
| error | SetRoomExtraInfoResult | 设置房间附加信息的结果，详见 [SetRoomExtraInfoResult](Android-keytype#SetRoomExtraInfoResult) |
...
接收同一房间内，其他用户调用 [setRoomExtraInfo](Android-api#RTCRoom-setroomextrainfo) 设置的房间附加信息的回调。
...
用户调用 [setUserVisibility](Android-api#RTCRoom-setuservisibility) 设置用户可见性的回调。
...
| errorCode | UserVisibilityChangeError | 设置用户可见性错误码，参看 [UserVisibilityChangeError](Android-keytype#UserVisibilityChangeError)。 |
...
当用户调用 [startSubtitle](Android-api#RTCRoom-startsubtitle) 和 [stopSubtitle](Android-api#RTCRoom-stopsubtitle) 使字幕状态发生改变或字幕任务出现错误时，触发该回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | SubtitleState | 字幕状态。参看 [SubtitleState](Android-keytype#SubtitleState)。 |
| errorCode | SubtitleErrorCode | 字幕任务错误码。参看 [SubtitleErrorCode](Android-errorcode#SubtitleErrorCode)。 |
...
当用户成功调用 [startSubtitle](Android-api#RTCRoom-startsubtitle) 后会收到此回调，通知字幕的相关信息。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| subtitles | SubtitleMessage[] | 字幕消息内容。参看 [SubtitleMessage](Android-keytype#SubtitleMessage)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| warn | int | 警告代码，参见 [WarningCode](Android-errorcode#WarningCode) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| err | int | 错误代码，详情定义见: [ErrorCode](Android-errorcode#ErrorCode) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream | RTCStream | 流的属性，参看 [RTCStream](Android-keytype#RTCStream)。 |
| reason | StreamRemoveReason | 远端流移除的原因，参看 [StreamRemoveReason](Android-keytype#StreamRemoveReason)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream | RTCStream | 流的属性，详见 [RTCStream](Android-keytype#RTCStream) 数据类型。 |
...
| --- | --- | --- |
| musicId | @NonNull String | 音乐 ID。 |
| playState | @NonNull PlayState | 音乐播放状态，参看 [PlayState](Android-keytype#PlayState)。 |
| errorCode | KTVPlayerErrorCode | 错误码，参看 [KTVPlayerErrorCode](Android-keytype#KTVPlayerErrorCode)。 |


**注意**

此回调被触发的时机汇总如下：
- 调用 [playMusic](Android-api#IKTVPlayer-playmusic) 成功后，会触发 playState 值为 PLAY_STATE_PLAYING 的回调；否则会触发 playState 值为 PLAY_STATE_FAILED 的回调。
- 使用相同的音乐 ID 重复调用 [playMusic](Android-api#IKTVPlayer-playmusic) 后，后一次播放会覆盖前一次，且会触发 playState 值为 PLAY_STATE_PLAYING 的回调，表示后一次音乐播放已开始。
- 调用 [pauseMusic](Android-api#IKTVPlayer-pausemusic) 方法暂停播放成功后，会触发 playState 值为 PLAY_STATE_PAUSED 的回调；否则触发 playState 值为 PLAY_STATE_FAILED 的回调。
- 调用 [resumeMusic](Android-api#IKTVPlayer-resumemusic) 方法恢复播放成功后，会触发 playState 值为 PLAY_STATE_PLAYING 的回调；否则触发 playState 值为 PLAY_STATE_FAILED 的回调。
- 调用 [stopMusic](Android-api#IKTVPlayer-stopmusic) 方法停止播放成功后，会触发 playState 值为 PLAY_STATE_STOPPED 的回调；否则触发 playState 值为 PLAY_STATE_FAILED 的回调。
...
```
[IAudioEffectPlayer](Android-api#IAudioEffectPlayer) 对应的回调句柄。你必须调用 [setEventHandler](Android-api#IAudioEffectPlayer-seteventhandler) 完成设置后，才能收到对应回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| effectId | int | [IAudioEffectPlayer](Android-api#IAudioEffectPlayer) 的 ID。通过 [getAudioEffectPlayer](Android-api#RTCVideo-getaudioeffectplayer) 设置。 |
| state | PlayerState | 混音状态。参考 [PlayerState](Android-keytype#PlayerState)。 |
| error | PlayerError | 错误码。参考 [PlayerError](Android-keytype#PlayerError)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| eventType | ByteRTCStreamSinglePushEvent | 任务状态, 参看 [ByteRTCStreamSinglePushEvent](Android-errorcode#ByteRTCStreamSinglePushEvent) |
| taskId | String | 任务 ID |
| error | int | 错误码，参看 [ByteRTCTranscoderErrorCode](Android-errorcode#ByteRTCTranscoderErrorCode) |
...
```
调用 [registerRemoteEncodedVideoFrameObserver](Android-api#RTCVideo-registerremoteencodedvideoframeobserver) 后，SDK 监测到远端编码后视频数据时，触发该回调
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamKey | RemoteStreamKey | 收到的远端流信息，参看 [RemoteStreamKey](Android-keytype#RemoteStreamKey) |
| encodedVideoFrame | RTCEncodedVideoFrame | 收到的远端视频帧信息，参看 [RTCEncodedVideoFrame](Android-keytype#RTCEncodedVideoFrame) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| musicInfos | MusicInfo[] | 歌曲数据数组，参看 [MusicInfo](Android-keytype#MusicInfo)。 |
| totalSize | int | 数据条目总数。 |
| errorCode | KTVErrorCode | 错误码，成功时返回 0，其余值参看 [KTVErrorCode](Android-errorcode#KTVErrorCode)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| hotMusics | HotMusicInfo[] | 热榜歌曲数据数组，参看 [HotMusicInfo](Android-keytype#HotMusicInfo)。 |
| errorCode | KTVErrorCode | 错误码，成功时返回 0，其余值参看 [KTVErrorCode](Android-errorcode#KTVErrorCode)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| musicInfo | MusicInfo | 歌曲数据，参看 [MusicInfo](Android-keytype#MusicInfo)。 |
| errorCode | KTVErrorCode | 错误码，成功时返回 0，其余值参看 [KTVErrorCode](Android-errorcode#KTVErrorCode)。 |
...
| result | DownloadResult | 下载信息，参看 [DownloadResult](Android-keytype#DownloadResult)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| errorCode | KTVErrorCode | 错误码，非 0 为失败，参看 [KTVErrorCode](Android-errorcode#KTVErrorCode)。 |
...
```
实时评分信息回调。调用 [startSingScoring](Android-api#ISingScoringManager-startsingscoring) 后，会收到该回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| info | SingScoringRealtimeInfo | 实时评分信息。详见 [SingScoringRealtimeInfo](Android-keytype#SingScoringRealtimeInfo)。 |
...
调用 [enableFaceDetection](Android-api#IVideoEffect-enablefacedetection) 注册了 [IFaceDetectionObserver](#IFaceDetectionObserver)，并使用 RTC SDK 中包含的特效 SDK 进行视频特效处理时，你会收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| result | FaceDetectionResult | 人脸检测结果, 参看 [FaceDetectionResult](Android-keytype#FaceDetectionResult)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| eventType | ByteRTCStreamMixingEvent | 转推直播任务状态, 参看 [ByteRTCStreamMixingEvent](Android-keytype#ByteRTCStreamMixingEvent) |
| taskId | String | 转推直播任务 ID |
| error | ByteRTCTranscoderErrorCode | 转推直播错误码，参看 [ByteRTCTranscoderErrorCode](Android-errorcode#ByteRTCTranscoderErrorCode) |
| mixType | MixedStreamType | 转推直播类型，参看 [MixedStreamType](Android-keytype#MixedStreamType) |
...
| --- | --- | --- |
| taskId | String | 转推直播任务 ID |
| videoFrame | VideoFrame | YUV 合流视频数据帧，参看 [VideoFrame](Android-keytype#VideoFrame) |
...
| --- | --- | --- |
| playerId | int | - |
| frame | IAudioFrame | 参看 [IAudioFrame](Android-keytype#IAudioFrame)。 |
...
| --- | --- | --- |
| mixID | int | 混音 ID。 |
| audioFrame | IAudioFrame | 参看 [IAudioFrame](Android-keytype#IAudioFrame)。 |
...
| taskId | String | 转推直播任务 ID |
| error | ByteRTCTranscoderErrorCode | 转推直播错误码，参看 [ByteRTCTranscoderErrorCode](Android-errorcode#ByteRTCTranscoderErrorCode) |
| mixType | ByteRTCStreamMixingType | 转推直播类型，参看 [ByteRTCStreamMixingType](Android-keytype#ByteRTCStreamMixingType) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| errorCode | int | 错误码<br><ul><li>< 0: 参数错误或 API 调用顺序错误，参看 [RTCASRErrorCode](Android-keytype#RTCASRErrorCode)；</li><li>>0: 参看 [语音识别服务错误码](https://www.volcengine.com/docs/6561/80818#_3-3-%E9%94%99%E8%AF%AF%E7%A0%81)</li></ul> |
...
```
调用 [registerLocalEncodedVideoFrameObserver](Android-api#RTCVideo-registerlocalencodedvideoframeobserver) 后，SDK 每次使用内部采集，采集到一帧视频帧，或收到一帧外部视频帧时，都会回调该事件。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamIndex | StreamIndex | 本地视频帧类型，参看 [StreamIndex](Android-keytype#StreamIndex-2) |
| encodedVideoFrame | RTCEncodedVideoFrame | 本地视频帧信息，参看 [RTCEncodedVideoFrame](Android-keytype#RTCEncodedVideoFrame) |
...
收到该回调后，你即可调用 [pushExternalEncodedVideoFrame](Android-api#RTCVideo-pushexternalencodedvideoframe) 向 SDK 推送自定义编码视频帧
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| index | StreamIndex | 可以推送的编码流的属性，参看 [StreamIndex](Android-keytype#streamindex-2) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| index | StreamIndex | 需停止推送的编码流的属性，参看 [StreamIndex](Android-keytype#streamindex-2) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamIndex | StreamIndex | 发生变化的编码流的属性，参看 [StreamIndex](Android-keytype#streamindex-2) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamIndex | StreamIndex | 远端编码流的属性，参看 [StreamIndex](Android-keytype#streamindex-2) |
| videoIndex | int | 对应编码流的下标 |
...
要收到此回调，必须调用 [setVideoSourceType](Android-api#RTCVideo-setvideosourcetype) 设置视频源是自定义编码，且通过 [setExternalVideoEncoderEventHandler](Android-api#RTCVideo-setexternalvideoencodereventhandler) 设置了回调句柄。
...
```
[IMediaPlayer](Android-api#IMediaPlayer) 对应的回调句柄。你必须调用 [setEventHandler](Android-api#IMediaPlayer-seteventhandler) 完成设置后，才能收到对应回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| playerId | int | [IMediaPlayer](Android-api#IMediaPlayer) 的 ID。通过 [getMediaPlayer](Android-api#RTCVideo-getmediaplayer) 设置。 |
| state | PlayerState | 混音状态。参考 [PlayerState](Android-keytype#PlayerState)。 |
| error | PlayerError | 错误码。参考 [PlayerError](Android-keytype#PlayerError)。 |
...
```
播放进度周期性回调。回调周期通过 [setProgressInterval](Android-api#IMediaPlayer-setprogressinterval) 设置。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audioFrame | IAudioFrame | 音频数据, 参看 [IAudioFrame](Android-keytype#IAudioFrame)。 |
...
调用 [openWithCustomSource](Android-api#IMediaPlayer-openwithcustomsource) 接口播放用户传入的内存音频数据时，会触发此回调，用户需要写入音频数据。
...
若 [openWithCustomSource](Android-api#IMediaPlayer-openwithcustomsource) 接口调用失败，请在 buffer 和 bufferSize 两个参数中填入 0。 此时 SDK 会停止调用此接口。
...
在调用 [openWithCustomSource](Android-api#IMediaPlayer-openwithcustomsource) 接口传入内存音频数据，或者调用 [setPosition](Android-api#IMediaPlayer-setposition) 设置了音频数据的起始播放位置后，SDK 会对音频数据进行读取和分析，此时会触发该回调，你需要根据参数中设置的起始读取位置和偏移量进行操作。
...
| whence | MediaPlayerCustomSourceSeekWhence | 音频数据的起始读取位置。参看 [MediaPlayerCustomSourceSeekWhence](Android-keytype#MediaPlayerCustomSourceSeekWhence) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stats | SysStats | cpu，内存信息。详见 [SysStats](Android-keytype#SysStats) 数据类型。 |
...
```
房间内的可见用户调用 [startVideoCapture](Android-api#RTCVideo-startvideocapture) 开启内部视频采集时，房间内其他用户会收到此回调。
...
```
房间内的可见用户调用 [stopVideoCapture](Android-api#RTCVideo-stopvideocapture) 关闭内部视频采集时，房间内其他用户会收到此回调。
...
| --- | --- | --- |
| roomId | String | 房间 ID。 |
| errorCode | int | 创建房间错误码，具体原因参看 [ErrorCode](Android-errorcode#ErrorCode)。 |
...
```
房间内的用户调用 [startAudioCapture](Android-api#RTCVideo-startaudiocapture) 开启音频采集时，房间内其他用户会收到此回调。
...
```
房间内的用户调用 [stopAudioCapture](Android-api#RTCVideo-stopaudiocapture) 关闭音频采集时，房间内其他用户会收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | LocalAudioStreamState | 本地音频设备的状态，详见 [LocalAudioStreamState](Android-keytype#LocalAudioStreamState) |
| error | LocalAudioStreamError | 本地音频流状态改变时的错误码，详见 [LocalAudioStreamError](Android-keytype#LocalAudioStreamError) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| key | RemoteStreamKey | 远端流信息, 详见 [RemoteStreamKey](Android-keytype#RemoteStreamKey) |
| state | RemoteAudioState | 远端音频流状态，详见 [RemoteAudioState](Android-keytype#RemoteAudioState) |
| reason | RemoteAudioStateChangeReason | 远端音频流状态改变的原因，详见 [RemoteAudioStateChangeReason](Android-keytype#RemoteAudioStateChangeReason) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamIndex | StreamIndex | 流属性，参看 [StreamIndex](Android-keytype#streamindex-2) |
| state | LocalVideoStreamState | 本地视频流状态，参看 [LocalVideoStreamState](Android-keytype#LocalVideoStreamState) |
| error | LocalVideoStreamError | 本地视频状态改变时的错误码，参看 [LocalVideoStreamError](Android-keytype#LocalVideoStreamError) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamKey | RemoteStreamKey | 远端视频流信息，房间、用户 ID、流属性等，参看 [RemoteStreamKey](Android-keytype#RemoteStreamKey) |
| videoState | RemoteVideoState | 远端视频流状态，参看 [RemoteVideoState](Android-keytype#RemoteVideoState) |
| videoStateReason | RemoteVideoStateChangeReason | 远端视频流状态改变原因，参看 [RemoteVideoStateChangeReason](Android-keytype#RemoteVideoStateChangeReason) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| remoteStreamKey | RemoteStreamKey | 远端流信息，参看 [RemoteStreamKey](Android-keytype#RemoteStreamKey) |
| frameInfo | VideoFrameInfo | 视频帧信息，参看 [VideoFrameInfo](Android-keytype#VideoFrameInfo) |
...
| --- | --- | --- |
| streamIndex | StreamIndex | 流属性，参看 [StreamIndex](Android-keytype#streamindex-2) |
| frameInfo | VideoFrameInfo | 视频信息，参看 [VideoFrameInfo](Android-keytype#VideoFrameInfo) |


**注意**

对于采集到的本地视频帧，你可以调用 [setLocalVideoCanvas](Android-api#RTCVideo-setlocalvideocanvas) 或 [setLocalVideoSink](Android-api#RTCVideo-setlocalvideosink) 在本地渲染。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | int | <br>当前 SDK 与信令服务器连接状态。 详细定义参见 [ConnectionState](Android-keytype#ConnectionState) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| route | AudioRoute | 新的音频播放路由，详见 [AudioRoute](Android-keytype#AudioRoute) |


**注意**

插拔音频外设，或调用 [setAudioRoute](Android-api#RTCVideo-setaudioroute) 都可能触发音频路由切换，详见[移动端设置音频路由](https://www.volcengine.com/docs/6348/117836) 。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamIndex | StreamIndex | 音频流属性, 详见 [StreamIndex](Android-keytype#streamindex-2) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| remoteStreamKey | RemoteStreamKey | 远端音频流信息, 详见 [RemoteStreamKey](Android-keytype#RemoteStreamKey) |
...
```
收到通过调用 [sendSEIMessage](Android-api#RTCVideo-sendseimessage) 发送带有 SEI 消息的视频帧时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| remoteStreamKey | RemoteStreamKey | 包含 SEI 发送者的用户名，所在的房间名和媒体流，详见 [RemoteStreamKey](Android-keytype#RemoteStreamKey) |
...
在语音通话场景下，本地用户调用 [sendSEIMessage](Android-api#RTCVideo-sendseimessage) 通过黑帧视频流发送 SEI 数据时，流的发送状态会通过该回调通知远端用户。
...
| --- | --- | --- |
| remoteStreamKey | RemoteStreamKey | 远端流信息，参看 [RemoteStreamKey](Android-keytype#RemoteStreamKey)。 |
| event | SEIStreamUpdateEvent | 黑帧视频流状态，参看 [SEIStreamUpdateEvent](Android-keytype#SEIStreamUpdateEvent) |
...
| errorCode | int | <br>登录结果<br>详见 [LoginErrorCode](Android-errorcode#LoginErrorCode)。 |
| elapsed | int | <br>从调用 [login](Android-api#RTCVideo-login) 接口开始到返回结果所用时长。<br>单位为 ms。 |


**注意**

调用 [login](Android-api#RTCVideo-login) 后，会收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| reason | int | 用户登出的原因，参看 [LogoutReason](Android-keytype#LogoutReason) |


**注意**

在以下两种情况下会收到此回调：调用 [logout](Android-api#RTCVideo-logout) 接口主动退出；或其他用户以相同 UserId 进行 `login` 导致本地用户被动登出。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| error | int | <br>设置结果<br><ul><li>返回 200，设置成功</li><li>返回其他，设置失败，详见 [UserMessageSendResult](Android-errorcode#UserMessageSendResult) </li></ul> |


**注意**

调用 [setServerParams](Android-api#RTCVideo-setserverparams) 后，会收到此回调。
...
| status | int | <br>查询的用户登录状态<br>详见 [UserOnlineStatus](Android-keytype#UserOnlineStatus). |


**注意**

必须先调用 [getPeerOnlineStatus](Android-api#RTCVideo-getpeeronlinestatus)，才能收到此回调。
...
```
收到房间外用户调用 [sendUserMessageOutsideRoom](Android-api#RTCVideo-sendusermessageoutsideroom) 发来的文本消息时，会收到此回调
...
```
收到房间外用户调用 [sendUserBinaryMessageOutsideRoom](Android-api#RTCVideo-senduserbinarymessageoutsideroom) 发来的二进制消息时，会收到此回调
...
| error | int | <br>消息发送结果<br>详见 [UserMessageSendResult](Android-errorcode#UserMessageSendResult)。 |


**注意**

当调用 [sendUserMessageOutsideRoom](Android-api#RTCVideo-sendusermessageoutsideroom) 或 [sendUserBinaryMessageOutsideRoom](Android-api#RTCVideo-senduserbinarymessageoutsideroom) 发送消息后，会收到此回调。
...
| error | int | 消息发送结果，详见 [UserMessageSendResult](Android-errorcode#UserMessageSendResult)。 |
| message | ByteBuffer | 应用服务器收到 HTTP 请求后，在 ACK 中返回的信息。消息不超过 64 KB。 |


**注意**

本回调为异步回调。当调用 [sendServerMessage](Android-api#RTCVideo-sendservermessage) 或 [sendServerBinaryMessage](Android-api#RTCVideo-sendserverbinarymessage) 接口发送消息后，会收到此回调。
...
成功调用 [startNetworkDetection](Android-api#RTCVideo-startnetworkdetection) 接口开始探测后，会在 3s 内首次收到该回调，之后每 2s 收到一次该回调。
...
| quality | int | 探测网络的质量，参看 [NetworkQuality](Android-keytype#NetworkQuality)。 |
...
1. 当调用 [stopNetworkDetection](Android-api#RTCVideo-stopnetworkdetection) 接口停止探测后，会收到一次该回调；
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| reason | NetworkDetectionStopReason | <br>停止探测的原因类型,参考 [NetworkDetectionStopReason](Android-keytype#NetworkDetectionStopReason) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| event | RemoteStreamSwitch | 音视频流发生变化的信息。参看 [RemoteStreamSwitch](Android-keytype#RemoteStreamSwitch)。 |
...
```
本地未通过 [setPublishFallbackOption](Android-api#RTCVideo-setpublishfallbackoption) 开启发布性能回退，检测到设备性能不足时，收到此回调。

本地通过 [setPublishFallbackOption](Android-api#RTCVideo-setpublishfallbackoption) 开启发布性能回退，因设备性能/网络原因，造成发布性能回退/恢复时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| mode | PerformanceAlarmMode | 指示本地是否开启发布回退功能。参看 [PerformanceAlarmMode](Android-keytype#PerformanceAlarmMode)<br><ul><li>当发布端未开启发布性能回退时，mode 值为 NORMAL。</li><li>当发布端开启发布性能回退时，mode 值为 SIMULCAST。</li></ul> |
| roomId | String | <ul><li>未开启发布性能回退时，roomId 为空</li><li>开启发布性能回退时，roomId 是告警影响的房间 ID。</li></ul> |
| reason | PerformanceAlarmReason | 告警原因，参看 [PerformanceAlarmReason](Android-keytype#PerformanceAlarmReason) |
| data | SourceWantedData | 性能回退相关数据，详见 [SourceWantedData](Android-keytype#SourceWantedData)。 |
...
| user | RtcUser | 本地用户信息，详见 [RtcUser](Android-keytype#RtcUser) |
| state | FirstFrameSendState | 首帧发送状态，详见 [FirstFrameSendState](Android-keytype#FirstFrameSendState) |
...
| --- | --- | --- |
| roomId | String | 音频发布用户所在的房间 ID |
| user | RtcUser | 远端用户信息，详见 [RtcUser](Android-keytype#RtcUser) |
| state | FirstFramePlayState | 首帧播放状态，详见 [FirstFramePlayState](Android-keytype#FirstFramePlayState) |
...
| deviceType | AudioDeviceType | 设备类型，详见 [AudioDeviceType](Android-keytype#AudioDeviceType)。 |
| deviceState | int | 设备状态，详见 [MediaDeviceState](Android-keytype#MediaDeviceState)。 |
| deviceError | int | 设备错误类型，详见 [MediaDeviceError](Android-keytype#MediaDeviceError)。 |
...
| --- | --- | --- |
| deviceID | String | 设备 ID |
| deviceType | VideoDeviceType | 设备类型，详见 [VideoDeviceType](Android-keytype#VideoDeviceType)。 |
| deviceState | int | 设备状态，详见 [MediaDeviceState](Android-keytype#MediaDeviceState)。 |
| deviceError | int | 设备错误类型，详见 [MediaDeviceError](Android-keytype#MediaDeviceError)。 |
...
| --- | --- | --- |
| deviceID | String | 设备 ID |
| deviceType | AudioDeviceType | 参看 [AudioDeviceType](Android-keytype#AudioDeviceType) |
| deviceWarning | int | 参看 [MediaDeviceWarning](Android-keytype#MediaDeviceWarning) |
...
| --- | --- | --- |
| deviceID | String | 设备 ID |
| deviceType | VideoDeviceType | 参看 [VideoDeviceType](Android-keytype#VideoDeviceType) |
| deviceWarning | int | 参看 [MediaDeviceWarning](Android-keytype#MediaDeviceWarning) |
...
该回调由 [startFileRecording](Android-api#RTCVideo-startfilerecording) 或 [stopFileRecording](Android-api#RTCVideo-stopfilerecording) 触发。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | StreamIndex | 录制流的流属性，参看 [StreamIndex](Android-keytype#streamindex-2) |
| state | RecordingState | 录制状态，参看 [RecordingState](Android-keytype#RecordingState) |
| errorCode | RecordingErrorCode | 录制错误码，参看 [RecordingErrorCode](Android-keytype#RecordingErrorCode) |
| info | RecordingInfo | 录制文件的详细信息，参看 [RecordingInfo](Android-keytype#RecordingInfo) |
...
该回调由 [startFileRecording](Android-api#RTCVideo-startfilerecording) 触发，录制状态正常时，系统每秒钟都会通过该回调提示录制进度。
...
| --- | --- | --- |
| type | StreamIndex | 录制流的流属性，参看 [StreamIndex](Android-keytype#streamindex-2) |
| progress | RecordingProgress | 录制进度，参看 [RecordingProgress](Android-keytype#RecordingProgress) |
| info | RecordingInfo | 录制文件的详细信息，参看 [RecordingInfo](Android-keytype#RecordingInfo) |
...
```
调用 [startAudioRecording](Android-api#RTCVideo-startaudiorecording) 或 [stopAudioRecording](Android-api#RTCVideo-stopaudiorecording) 改变音频文件录制状态时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | AudioRecordingState | 录制状态，参看 [AudioRecordingState](Android-keytype#AudioRecordingState) |
| errorCode | AudioRecordingErrorCode | 录制错误码，参看 [AudioRecordingErrorCode](Android-errorcode#AudioRecordingErrorCode) |
...
调用 [setAudioMixingProgressInterval](Android-api#IAudioMixingManager-setaudiomixingprogressinterval) 将时间间隔设为大于 0 的值后，或调用 [startAudioMixing](Android-api#IAudioMixingManager-startaudiomixing) 将 [AudioMixingConfig](Android-keytype#AudioMixingConfig) 中的时间间隔设为大于 0 的值后，SDK 会按照设置的时间间隔回调该事件。
...
```
调用 [enableAudioPropertiesReport](Android-api#RTCVideo-enableaudiopropertiesreport) 后，你会周期性地收到此回调，了解本地音频的相关信息。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audioPropertiesInfos | LocalAudioPropertiesInfo[] | 本地音频信息，详见 [LocalAudioPropertiesInfo](Android-keytype#LocalAudioPropertiesInfo)。 |
...
调用 [startAudioPlaybackDeviceTest](Android-api#IRTCAudioDeviceManager-startaudioplaybackdevicetest) 或 [startAudioDeviceRecordTest](Android-api#IRTCAudioDeviceManager-startaudiodevicerecordtest)，开始播放音频文件或录音时，将开启该回调。本回调为周期性回调，回调周期由上述接口的 `interval` 参数指定。
...
```
远端用户进房后，本地调用 [enableAudioPropertiesReport](Android-api#RTCVideo-enableaudiopropertiesreport) ，根据设置的 interval 值，本地会周期性地收到此回调，了解订阅的远端用户的瞬时音频信息。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audioPropertiesInfos | RemoteAudioPropertiesInfo[] | 远端音频信息，其中包含音频流属性、房间 ID、用户 ID ，详见 [RemoteAudioPropertiesInfo](Android-keytype#RemoteAudioPropertiesInfo)。 |
...
```
调用 [enableAudioPropertiesReport](Android-api#RTCVideo-enableaudiopropertiesreport) 后，根据设置的 `AudioPropertiesConfig.interval`，你会周期性地收到此回调，获取房间内的最活跃用户信息。
...
```
音频流同步信息回调。可以通过此回调，在远端用户调用 [sendStreamSyncInfo](Android-api#RTCVideo-sendstreamsyncinfo) 发送音频流同步消息后，收到远端发送的音频流同步信息。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamKey | RemoteStreamKey | 远端流信息，详见 [RemoteStreamKey](Android-keytype#RemoteStreamKey)。 |
| streamType | StreamSycnInfoConfig.SyncInfoStreamType | 媒体流类型，详见 [SyncInfoStreamType](Android-keytype#SyncInfoStreamType)。 |
...
调用 [startPushPublicStream](Android-api#RTCVideo-startpushpublicstream) 接口发布公共流后，启动结果通过此回调方法通知用户。
...
| error | PublicStreamErrorCode | 公共流发布结果状态码。详见 [PublicStreamErrorCode](Android-errorcode#PublicStreamErrorCode)。 |
...
调用 [startPlayPublicStream](Android-api#RTCVideo-startplaypublicstream) 接口启动拉公共流功能后，通过此回调收到启动结果和拉流过程中的错误。
...
| --- | --- | --- |
| publicStreamId | String | 公共流 ID。 |
| error | PublicStreamErrorCode | 公共流订阅结果状态码。详见 [PublicStreamErrorCode](Android-errorcode#PublicStreamErrorCode)。 |
...
调用 [startPlayPublicStream](Android-api#RTCVideo-startplaypublicstream) 接口启动拉公共流功能后，通过此回调收到公共流中的 SEI 消息。
...
| --- | --- | --- |
| publicStreamId | String | 公共流 ID。 |
| message | ByteBuffer | 收到的 SEI 消息内容。<br>通过调用客户端 [sendSEIMessage](Android-api#RTCVideo-sendseimessage-2) 插入的 SEI 信息。<br>当公共流中的多路视频流均包含有 SEI 信息：SEI 不互相冲突时，将通过多次回调分别发送；SEI 在同一帧有冲突时，则只有一条流中的 SEI 信息被透传并融合到公共流中。 |
| sourceType | DataMessageSourceType | SEI 消息类型，自 3.52.1 版本后固定为 `0`，自定义消息。参看 [DataMessageSourceType](Android-keytype#DataMessageSourceType)。 |
...
调用 [startPlayPublicStream](Android-api#RTCVideo-startplaypublicstream) 订阅公共流后，通过监听本回调获取公共流中的数据消息，包括调用 Open API 发送的 SEI 消息和音量回调。
...
| sourceType | DataMessageSourceType | 数据消息来源，参看 [DataMessageSourceType](Android-keytype#DataMessageSourceType)。 |
...
关于订阅公共流，详见 [startPlayPublicStream](Android-api#RTCVideo-startplaypublicstream)。
...
| --- | --- | --- |
| publicStreamId | String | 公共流 ID |
| videoFrameInfo | VideoFrameInfo | 视频帧信息，参看 [VideoFrameInfo](Android-keytype#VideoFrameInfo) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| result | EchoTestResult | 测试结果，参看 [EchoTestResult](Android-keytype#EchoTestResult) |
...
```
调用 [startCloudProxy](Android-api#RTCVideo-startcloudproxy) 开启云代理，SDK 首次成功连接云代理服务器时，回调此事件。
...
```
首次调用 [getNetworkTimeInfo](Android-api#RTCVideo-getnetworktimeinfo) 后，SDK 内部启动网络时间同步，同步完成时会触发此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| hardwareEchoDetectionResult | HardwareEchoDetectionResult | 参见 [HardwareEchoDetectionResult](Android-keytype#HardwareEchoDetectionResult) |


**注意**

- 通话前调用 [startHardwareEchoDetection](Android-api#RTCVideo-starthardwareechodetection) 后，将触发本回调返回检测结果。
- 建议在收到检测结果后，调用 [stopHardwareEchoDetection](Android-api#RTCVideo-stophardwareechodetection) 停止检测，释放对音频设备的占用。
...
```
本地代理状态发生改变回调。调用 [setLocalProxy](Android-api#RTCVideo-setlocalproxy) 设置本地代理后，SDK 会触发此回调，通知代理连接的状态。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| localProxyType | LocalProxyType | 本地代理类型。参看 [LocalProxyType](Android-keytype#LocalProxyType)。 |
| localProxyState | LocalProxyState | 本地代理状态。参看 [LocalProxyState](Android-keytype#LocalProxyState)。 |
| localProxyError | LocalProxyError | 本地代理错误。参看 [LocalProxyError](Android-keytype#LocalProxyError)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| info | FrameUpdateInfo | RTC 智能决策后合适的帧率和分辨率积（宽*高）。参看 [FrameUpdateInfo](Android-keytype#FrameUpdateInfo)。 |
...
| muteState | MuteState | 发送状态，参看 [MuteState](Android-keytype#MuteState) |
...
| muteState | MuteState | 视频流的发送状态。参看 [MuteState](Android-keytype#MuteState)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| device | AudioPlaybackDevice | 新的音频设备，详见 [AudioPlaybackDevice](Android-keytype#AudioPlaybackDevice) |


**注意**

音频设备设置与音频路由策略，详见 [setAudioPlaybackDevice](Android-api#RTCVideo-setaudioplaybackdevice)
...
| --- | --- | --- |
| deviceID | String | 设备 ID |
| deviceType | int | 设备类型，详见 [MediaDeviceType](Android-keytype#MediaDeviceType)。 |
| deviceState | int | 设备状态，详见 [MediaDeviceState](Android-keytype#MediaDeviceState)。 |
| deviceError | int | 设备错误类型，详见 [MediaDeviceError](Android-keytype#MediaDeviceError)。 |
...
| --- | --- | --- |
| deviceID | String | 设备 ID |
| deviceType | int | 参看 [MediaDeviceType](Android-keytype#MediaDeviceType) |
| deviceWarning | int | 参看 [MediaDeviceWarning](Android-keytype#MediaDeviceWarning) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| mixId | int | <br>混音 ID<br>使用 [IAudioMixingManager](Android-api#IAudioMixingManager) 相关接口时传入的唯一 ID。 |
| state | AudioMixingState | <br>混音状态<br>其混音状态可参考： [AudioMixingState](Android-keytype#AudioMixingState)。 |
| error | AudioMixingError | <br>错误码<br>详见 [AudioMixingError](Android-keytype#AudioMixingError)。 |


**注意**

此回调会被触发的时机汇总如下：
- 当调用 [startAudioMixing](Android-api#IAudioMixingManager-startaudiomixing) 方法成功后，会触发 state 值为 AUDIO_MIXING_STATE_PLAYING 回调；否则触发 state 值为 AUDIO_MIXING_STATE_FAILED 的回调。
- 当使用相同的 ID 重复调用 [startAudioMixing](Android-api#IAudioMixingManager-startaudiomixing) 后，后一次会覆盖前一次，且本回调会以 AUDIO_MIXING_STATE_STOPPED 通知前一次混音已停止。
- 当调用 [pauseAudioMixing](Android-api#IAudioMixingManager-pauseaudiomixing) 方法暂停播放成功后， 会触发 state 值为 AUDIO_MIXING_STATE_PAUSED 回调；否则触发 state 值为 AUDIO_MIXING_STATE_FAILED 的回调。
- 当调用 [resumeAudioMixing](Android-api#IAudioMixingManager-resumeaudiomixing) 方法恢复播放成功后，会触发 state 值为 AUDIO_MIXING_STATE_PLAYING 回调；否则触发 state 值为 AUDIO_MIXING_STATE_FAILED 的回调。
- 当调用 [stopAudioMixing](Android-api#IAudioMixingManager-stopaudiomixing) 方法暂停止播放成功后， 会触发 state 值为 AUDIO_MIXING_STATE_STOPPED 回调；否则触发 state 值为 AUDIO_MIXING_STATE_FAILED 的回调。
...
```
调用 [takeLocalSnapshot](Android-api#RTCVideo-takelocalsnapshot) 截取视频画面时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| taskId | long | 本地截图任务的编号。和 [takeLocalSnapshot](Android-api#RTCVideo-takelocalsnapshot) 的返回值一致。 |
| streamIndex | StreamIndex | 截图的视频流的属性，参看 [StreamIndex](Android-keytype#StreamIndex)。 |
...
```
调用 [takeRemoteSnapshot](Android-api#RTCVideo-takeremotesnapshot) 截取视频画面时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| taskId | long | 远端截图任务的编号。和 [takeRemoteSnapshot](Android-api#RTCVideo-takeremotesnapshot) 的返回值一致。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audioFrame | IAudioFrame | 音频帧地址，参看 [IAudioFrame](Android-keytype#IAudioFrame)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| streamKey | RemoteStreamKey | 音频流信息，参看 [RemoteStreamKey](Android-keytype#RemoteStreamKey) |
| audioFrame | IAudioFrame | 音频帧地址，参看 [IAudioFrame](Android-keytype#IAudioFrame) |
