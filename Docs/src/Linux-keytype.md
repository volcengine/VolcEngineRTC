...
**注意:**
如果录制时设置的的音频声道数与采集时的音频声道数不同：+ 如果采集的声道数为 1，录制的声道数为 2，那么，录制的音频为经过单声道数据拷贝后的双声道数据，而不是立体声。
...
# SEIMessageSourceType
```cpp
enum bytertc::SEIMessageSourceType
```
...
| 类型 | 说明 |
| --- | --- |
| **kSEIMessageSourceTypeDefault** | （默认值）用户自定义。 |
| **kSEIMessageSourceTypeSystem** | 系统定义，包含音量指示信息。 |
...
| 类型 | 说明 |
| --- | --- |
| **kRoomProfileTypeCommunication** | 默认场景，通用模式<br><br/>与 `kRoomProfileTypeMeeting = 16` 配置相同。 |
...
```
使用转发目标房间 roomID 和 UserID 生成 Token。不得使用空指针。
...
| 类型 | 说明 |
| --- | --- |
| **kPublicStreamOK** | 发布或订阅成功 |
| **kPublicStreamPushInvalidParam** | 公共流的参数异常，请修改参数后重试 |
| **kPublicStreamPushInvalidStatus** | 服务端状态异常，将自动重试 |
| **kPublicStreamPushInternalError** | 内部错误，不可恢复，请重试。 |
| **kPublicStreamPushFailed** | 推流失败，将自动重试，用户不需要处理 |
| **kPublicStreamPushTimeout** | 推流失败，10s 后会重试，重试 3 次失败后自动停止。 |
