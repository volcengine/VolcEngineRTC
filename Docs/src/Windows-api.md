...
+ 在 Windows 和 Linux 平台上，要使用此能力，你必须额外接入[音频技术](https://www.volcengine.com/docs/6489/71986)。使用本接口前，请联系技术支持。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| observer | **IRemoteAudioFrameObserver*** | 音频数据回调观察者，详见 [IRemoteAudioFrameObserver](Windows-callback.md#iremoteaudioframeobserver) |
...
+ 调用 [startAudioMixing](#IAudioMixingManager-startaudiomixing) 方法开始播放音频文件后，可以调用本方法停止播放所有音频文件。  
...
+ 调用 [startAudioMixing](#IAudioMixingManager-startaudiomixing) 方法开始播放音频文件后，可以通过调用本方法暂停播放所有音频文件。  
+ 调用本方法暂停播放所有音频文件后，可调用 [resumeAllAudioMixing](#IAudioMixingManager-resumeallaudiomixing) 方法恢复所有播放。  
...
+ 调用 [pauseAllAudioMixing](#IAudioMixingManager-pauseallaudiomixing) 方法暂停所有正在播放音频文件后，可以通过调用本方法恢复播放。  
...
| **virtual int** | [updateListenerPosition](#ISpatialAudio-updatelistenerposition) |
| **virtual int** | [updateListenerOrientation](#ISpatialAudio-updatelistenerorientation) |
...
```cpp
virtual int bytertc::ISpatialAudio::updateListenerPosition(
    const Position &pos)
```
更新在房间内收听音频时的位置。<br>
...
```cpp
virtual int bytertc::ISpatialAudio::updateListenerOrientation(
    const HumanOrientation& orientation)
```
更新在房间内收听音频时的朝向。<br>
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| device_id | **char** | 视频设备 ID。同时是入参和出参。 |
