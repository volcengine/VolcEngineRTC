...
| **kErrorCodeJoinRoomWithoutLicenseAuthenticateSDK** | 加入房间错误。通过 [onRoomStateChanged](Linux-callback.md#IRTCRoomEventHandler-onroomstatechanged) 回调。<br/>进房时, LICENSE 计费账号未使用 LICENSE_AUTHENTICATE SDK，加入房间错误。 |
...
| **kErrorCodeJoinRoomServerLicenseExpired** | 服务端license过期，拒绝进房。通过 [onRoomStateChanged](Linux-callback.md#IRTCRoomEventHandler-onroomstatechanged) 回调。 |
| **kErrorCodeJoinRoomExceedsTheUpperLimit** | 超过服务端license许可的并发量上限，拒绝进房。通过 [onRoomStateChanged](Linux-callback.md#IRTCRoomEventHandler-onroomstatechanged) 回调。 |
| **kErrorCodeJoinRoomLicenseParameterError** | license参数错误，拒绝进房。通过 [onRoomStateChanged](Linux-callback.md#IRTCRoomEventHandler-onroomstatechanged) 回调。 |
| **kErrorCodeJoinRoomLicenseFilePathError** | license证书路径错误。通过 [onRoomStateChanged](Linux-callback.md#IRTCRoomEventHandler-onroomstatechanged) 回调。 |
| **kErrorCodeJoinRoomLicenseIllegal** | license证书不合法。通过 [onRoomStateChanged](Linux-callback.md#IRTCRoomEventHandler-onroomstatechanged) 回调。 |
| **kErrorCodeJoinRoomLicenseExpired** | license证书已经过期，拒绝进房。通过 [onRoomStateChanged](Linux-callback.md#IRTCRoomEventHandler-onroomstatechanged) 回调。 |
| **kErrorCodeJoinRoomLicenseInformationNotMatch** | license证书内容不匹配。通过 [onRoomStateChanged](Linux-callback.md#IRTCRoomEventHandler-onroomstatechanged) 回调。 |
| **kErrorCodeJoinRoomLicenseNotMatchWithCache** | license当前证书与缓存证书不匹配。通过 [onRoomStateChanged](Linux-callback.md#IRTCRoomEventHandler-onroomstatechanged) 回调。 |
...
| **kErrorCodeJoinRoomLicenseFunctionNotFound** | license 计费方法没有加载成功。可能是因为 license 相关插件未正确集成。通过 [onRoomStateChanged](Linux-callback.md#IRTCRoomEventHandler-onroomstatechanged) 回调。 |
...
| **kWarningLicenseFileExpired** | 鉴权文件失效，当检查鉴权文件状态时，本地文件与远端文件不一致会触发次警告。通过 [onRoomStateChanged](Linux-callback.md#IRTCRoomEventHandler-onroomstatechanged) 回调。 |
