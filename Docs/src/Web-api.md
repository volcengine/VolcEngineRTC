...
- **注意**
  ...
  + 功能兼容性说明：<br/>
     ...
     - iOS 设备不支持使用 SEI 功能。
  + 语音通话场景下收发 SEI 消息，仍然需要发布和订阅视频流，而非音频流。<br/>
     - 发布端进房时设置自动发布，或调用 [publishStream](#publishstream)，媒体流类型选择 `AUDIO_AND_VIDEO`。
     - 订阅端进房时设置自动订阅视频流，或调用 [subscribeStream](#subscribestream)，媒体流类型选择 `AUDIO_AND_VIDEO`。
