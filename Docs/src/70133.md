...
#### 3\. 创建引擎 [createRTCVideo](iOS-api.md#ByteRTCVideo-creatertcvideo-delegate-parameters)
```objectivec
...
| delegate | id<ByteRTCEngineDelegate> | SDK 回调给应用层的 delegate，详见 [ByteRTCVideoDelegate](iOS-callback.md#bytertcvideodelegate) |
| parameters | NSDictionary\* | 用以覆盖默认参数的本引擎实例参数。JSON 字符串格式。 |


#### 4\. 设置视频编码参数 [setMaxVideoEncoderConfig](iOS-api.md#ByteRTCVideo-setmaxvideoencoderconfig)
```objectivec
...
#### 5\. 设置本地视图 [setLocalVideoCanvas](iOS-api.md#ByteRTCVideo-setlocalvideocanvas-withcanvas)
```objectivec
...
#### 6\. 开始视频采集 [startVideoCapture](iOS-api.md#ByteRTCVideo-startvideocapture)
...
#### 7\. 开始音频采集 [startAudioCapture](iOS-api.md#ByteRTCVideo-startaudiocapture)
...
#### 8\. 创建房间实例 [createRTCRoom](iOS-api.md#ByteRTCVideo-creatertcroom)
```objectivec
...
#### 9\. 设置房间回调事件 handler [setRtcRoomDelegate](iOS-api.md#ByteRTCRoom-setrtcroomdelegate)
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| roomDelegate | id | 参看 [ByteRTCRoomDelegate](iOS-callback.md#bytertcroomdelegate)。 |


#### 10\. 加入房间 [joinRoom:userInfo:roomConfig:](iOS-api.md#ByteRTCRoom-joinroom-userinfo-roomconfig)
```objectivec
...
#### 11\. 处理房间状态改变回调 [onRoomStateChanged](iOS-callback.md#ByteRTCRoomDelegate-rtcroom-onroomstatechanged-withuid-state-extrainfo)
```objectivec
...
#### 12\. 处理远端用户加入房间的回调 [onUserJoined](iOS-callback.md#ByteRTCRoomDelegate-rtcroom-onuserjoined-elapsed)
```objectivec
...
#### 13\. 处理远端视频首帧解码的回调 [onFirstRemoteVideoFrameDecoded](iOS-callback.md#ByteRTCVideoDelegate-rtcengine-onfirstremotevideoframedecoded-withframeinfo)
```objectivec
...
#### 14\. 设置远端视图 [setRemoteVideoCanvas:withCanvas:](iOS-api.md#ByteRTCVideo-setremotevideocanvas-withcanvas)
```objectivec
...
| streamIndex | ByteRTCStreamIndex | 视频流属性，参看 [ByteRTCStreamIndex](https://www.volcengine.com/docs/6348/iOS-keytype#bytertcstreamindex) |
| canvas | ByteRTCVideoCanvas\* | 视图信息和渲染模式，参看 [ByteRTCVideoCanvas](https://www.volcengine.com/docs/6348/iOS-keytype#bytertcvideocanvas) |


#### 15\. 处理远端用户离开房间的回调 [onUserLeave](iOS-callback.md#ByteRTCRoomDelegate-rtcroom-onuserleave-reason)
```objectivec
...
| reason | ByteRTCUserOfflineReason | 用户离开房间的原因，参看 [ByteRTCUserOfflineReason](iOS-keytype.md#bytertcuserofflinereason)。 |



#### 16\. 离开房间 [leaveRoom](iOS-api.md#ByteRTCRoom-leaveroom)
```objectivec
...
#### 17\. 销毁引擎实例 [destroyRTCVideo](iOS-api.md#ByteRTCVideo-destroyrtcvideo)
