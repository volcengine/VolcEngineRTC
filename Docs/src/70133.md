...
#### 3\. 创建引擎 [createRTCVideo](70086#ByteRTCVideo-creatertcvideo-delegate-parameters)
```objectivec
...
| delegate | id<ByteRTCEngineDelegate> | SDK 回调给应用层的 delegate，详见 [ByteRTCVideoDelegate](70087#bytertcvideodelegate) |
| parameters | NSDictionary\* | 用以覆盖默认参数的本引擎实例参数。JSON 字符串格式。 |

#### 4\. 设置视频编码参数 [setMaxVideoEncoderConfig](70086#ByteRTCVideo-setmaxvideoencoderconfig)
```objectivec
...
#### 5\. 设置本地视图 [setLocalVideoCanvas](70086#ByteRTCVideo-setlocalvideocanvas-withcanvas)
```objectivec
...
#### 6\. 开始视频采集 [startVideoCapture](70086#ByteRTCVideo-startvideocapture)
...
#### 7\. 开始音频采集 [startAudioCapture](70086#ByteRTCVideo-startaudiocapture)
...
#### 8\. 创建房间实例 [createRTCRoom](70086#ByteRTCVideo-creatertcroom)
```objectivec
...
#### 9\. 设置房间回调事件 handler [setRtcRoomDelegate](70086#ByteRTCRoom-setrtcroomdelegate)
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| roomDelegate | id | 参看 [ByteRTCRoomDelegate](70087#bytertcroomdelegate)。 |

#### 10\. 加入房间 [joinRoom:userInfo:roomConfig:](70086#ByteRTCRoom-joinroom-userinfo-roomconfig)
```objectivec
...
#### 11\. 处理房间状态改变回调 [onRoomStateChanged](70087#ByteRTCRoomDelegate-rtcroom-onroomstatechanged-withuid-state-extrainfo)
```objectivec
...
#### 12\. 处理远端用户加入房间的回调 [onUserJoined](70087#ByteRTCRoomDelegate-rtcroom-onuserjoined-elapsed)
```objectivec
...
#### 13\. 处理远端视频首帧解码的回调 [onFirstRemoteVideoFrameDecoded](70087#ByteRTCVideoDelegate-rtcengine-onfirstremotevideoframedecoded-withframeinfo)
```objectivec
...
#### 14\. 设置远端视图 [setRemoteVideoCanvas:withCanvas:](70086#ByteRTCVideo-setremotevideocanvas-withcanvas)
```objectivec
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| key | **ByteRTCRemoteStreamKey*** | 远端流信息, 详见 [ByteRTCRemoteStreamKey](https://www.volcengine.com/docs/6348/70088#bytertcremotestreamkey) |
| canvas | ByteRTCVideoCanvas\* | 视图信息和渲染模式，参看 [ByteRTCVideoCanvas](https://www.volcengine.com/docs/6348/70088#bytertcvideocanvas) |

#### 15\. 处理远端用户离开房间的回调 [onUserLeave](70087#ByteRTCRoomDelegate-rtcroom-onuserleave-reason)
```objectivec
...
| reason | ByteRTCUserOfflineReason | 用户离开房间的原因，参看 [ByteRTCUserOfflineReason](70088#bytertcuserofflinereason)。 |


#### 16\. 离开房间 [leaveRoom](70086#ByteRTCRoom-leaveroom)
```objectivec
- (int)leaveRoom;
```
...
#### 17\. 销毁引擎实例 [destroyRTCVideo](70086#ByteRTCVideo-destroyrtcvideo)
