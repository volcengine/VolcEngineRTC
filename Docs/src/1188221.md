...
在实时音视频通话场景中，若需终止正在进行的音频切片任务，你可通过调用此接口实现。

通过指定应用标志、业务标识、房间 ID以及任务 ID，你可以结束特定的音频切片任务。请求成功后，接口将返回操作结果，确认任务是否成功关闭。
## 注意事项
请求频率：QPS 不得超过 150。
## 请求说明
- 请求方式：**POST**
- 请求地址：**https://rtc.volcengineapi.com?Action=StopSegment&Version=2023-11-01**
## 调试

<APILink link="https://api.volcengine.com/api-explorer/debug?action=StopSegment&serviceCode=rtc&version=2023-11-01&groupName=音频切片" />
...
## 请求示例
```json
POST https://rtc.volcengineapi.com?Action=StopSegment&Version=2023-11-01
{
    "AppId": "661e****543cf",
    "BusinessId": "B****23",
    "RoomId": "Room1",
