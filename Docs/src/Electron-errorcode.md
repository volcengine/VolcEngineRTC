...
- **成员**
  ...
  | kWarningCodePublishStreamForbiden | `-2009` | 当调用 [`setUserVisibility`](Electron-api.md#rtcroom-setuservisibility) 将自身可见性设置为 false 后，再尝试发布流会触发此警告。 |
...
回调错误码。  SDK 内部遇到不可恢复的错误时，会通过 [onError](Electron-event.md#rtcvideocallback-onerror) 回调通知用户。
