...
| 变更详情 | 功能模块 | 方法 | 说明 |
| --- | --- | --- | --- |
| 参数变更 | 引擎管理 | [`feedback`](Electron-api.md#rtcvideo-feedback) | 参数数据类型变更 |
| 接口名称变更 | CDN 推流 | `startLiveTranscoding` | 用 [`startPushMixedStreamToCDN`](Electron-api.md#rtcvideo-startpushmixedstreamtocdn) 替代 |
| ^^ | ^^ | `stopLiveTranscoding` | 用 [`stopPushStreamToCDN`](Electron-api.md#rtcvideo-stoppushstreamtocdn) 替代 |
| ^^ | ^^ | `updateLiveTranscoding` | 用 [`updatePushMixedStreamToCDN`](Electron-api.md#rtcvideo-updatepushmixedstreamtocdn) 替代 |
| 接口废弃 | 高级功能 | `updatePosition` | 用 [`updateSelfPosition()`](Electron-api.md#updateselfposition) 替代，新接口支持本地用户同时设置自己的发声位置和发声朝向。 |\
||| `updateSelfOrientation` |
| ^^ | ^^ | `updateListenerPosition` | 用 [`updateRemotePosition()`](Electron-api.md#updateremoteposition)替代，新接口支持本地用户设置远端用户的发声位置和发声朝向。 |\
...
||| 拆分为： |\
||||\
||| - [`onPublicStreamSEIMessageReceived`](Electron-event.md#rtcroomcallback-onpublicstreamseimessagereceived)：接收客户端插入的 SEI 消息。 |\
||| - [`onPublicStreamDataMessageReceived`](Electron-event.md#rtcroomcallback-onpublicstreamdatamessagereceived)：接收服务端插入的 SEI 消息和其他数据信息，例如音量信息。 |
...
| **变更详情** | **错误码** | **说明** |
| --- | --- | --- |
| 枚举值变更 | [`PublicStreamErrorCode`](Electron-keytype.md#publicstreamerrorcode)` | 新增枚举值： |\
||||\
...
| ^^ | [`ErrorCode`](Electron-errorcode.md#errorcode) | 新增错误码枚举值：`kErrorCodeJoinRoomRoomForbidden=-1025` 和 `kErrorCodeJoinRoomUserForbidden=-1026`。当服务端在指定房间封禁指定用户，而此用户加入房间失败，或从房间中被踢出时收到。 |
