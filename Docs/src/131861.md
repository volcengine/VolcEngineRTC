...
- **参数**
  ...
  - **event**
    ...
    类型：<code>{ errorCode: [[ErrorCode](131863#errorcode)](131863#errorcode); message?: string | undefined; }</code>
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>{ state: [PlaybackState](131863#playbackstate); reason: [PlaybackReason](131863#playbackreason); }</code>
...
白板创建回调。调用 [createWhiteBoard](131860#createwhiteboard) 成功创建白板后，房间内所有用户收到本回调。
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>[CreateWhiteBoardResult](131863#createwhiteboardresult)</code>
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>{ state: [ConnectionState](131863#connectionstate); }</code>
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>[SwitchBoardResult](131863#switchboardresult)</code>
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>[RemoveWhiteBoardResult](131863#removewhiteboardresult)</code>
...
- **注意**
  ...
  + 调用 [setBoardBackground](131860#iwhiteboard-setboardbackground) 后，房间中所有用户将收到本回调。
  + 默认背景将应用在所有页面上。调用  [setPageBackground](131860#setpagebackground) 可以设置单个白板页面背景。
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>[SetBoardBackgroundResult](131863#setboardbackgroundresult)</code>
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>{ lock: boolean; reason: [LockBoardReason](131863#lockboardreason); }</code>
...
- **注意**
  ...
  调用 [createPages](131860#createpages) 后，房间中所有用户将收到本回调。
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>[CreatePageProgressResult](131863#createpageprogressresult)</code>
...
- **注意**
  ...
  调用 [removePages](131860#removepages) 后，房间中所有用户将收到本回调。
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>[RemovePagesResult](131863#removepagesresult)</code>
...
- **注意**
  ...
  调用 [clearPage](131860#clearpage) 后，房间中所有用户将收到本回调。
...
- **注意**
  ...
  成功调用 [flipPage](131860#flippage)、[flipNextPage](131860#flipnextpage)、[flipPrevPage](131860#flipprevpage) 或 [removePages](131860#removepages) 后，在本端和远端触发本回调。
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>[FlipPageResult](131863#flippageresult)</code>
...
- **注意**
  ...
  成功调用 [createPages](131860#createpages) 或 [removePages](131860#removepages) 后，在本端和远端触发本回调。
...
- **注意**
  ...
  调用 [setPageBackground](131860#setpagebackground) 后，房间中所有用户将收到本回调。
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>[SetPageBackgroundResult](131863#setpagebackgroundresult)</code>
...
- **注意**
  ...
  用户对白板进行编辑，也包括调用 [redo](131860#redo) 和 [undo](131860#undo) 接口，本端会收到本回调。
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>{ status: [ImageLoadStatus](131863#imageloadstatus); pageId: string; boardId: number; imageUrl: string; }</code>
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>[ZoomInfoChangedEvent](131863#zoominfochangedevent)</code>
...
- **注意**
  ...
  成功调用 [pptNextStep](131860#iwhiteboard-pptnextstep) 或 [pptPrevStep](131860#iwhiteboard-pptprevstep) 后，在本端和远端触发一次本回调。
...
- **参数**
  ...
  - **event**
    ...
    类型：<code>[PptStepChangedEvent](131863#pptstepchangedevent)</code>
