...
| [onPlayStateChange](#rtcvideocallback-onplaystatechange) | 音乐播放状态改变回调。 |
...
| [onStreamMixingEvent](#rtcvideocallback-onstreammixingevent) | 通知转推直播关键事件 |
...
- **类型**
  ...
  ```ts
  (error_code: KTVErrorCode, total_musics_size: number, music_infos: MusicInfo[]) => void
  ```
...
- **类型**
  ...
  ```ts
  (error_code: KTVErrorCode, hot_infos: HotMusicInfo[]) => void
  ```
...
- **类型**
  ...
  ```ts
  (error_code: KTVErrorCode, music_info: MusicInfo) => void
  ```
...
- **类型**
  ...
  ```ts
  (error_code: KTVErrorCode, download_id: number) => void
  ```
...
### onPlayStateChange <span id="rtcvideocallback-onplaystatechange"></span> 
...
- **类型**
  ...
  ```ts
  (error_code: KTVPlayerErrorCode, music_id: string, play_state: PlayState) => void
  ```
...
- **类型**
  ...
  ```ts
  (task_id: string, video_frame: IVideoFrame) => this
  ```
...
- **类型**
  ...
  ```ts
  (task_id: string, audio_frame: IAudioFrame) => this
  ```
...
- **类型**
  ...
  ```ts
  (task_id: string, data_frame: IDataFrame) => this
  ```
...
- **参数**
  ...
  - **error_code**
    ...
    类型: [PublicStreamErrorCode](85535.md#publicstreamerrorcode)
...
- **类型**
  ...
  ```ts
  (public_stream_id: string, message: string, source_type: SEIMessageSourceType) => void
  ```
...
- **参数**
  ...
  - **source_type**
    ...
    类型: [SEIMessageSourceType](85535.md#seimessagesourcetype)
...
### onStreamMixingEvent <span id="rtcvideocallback-onstreammixingevent"></span> 
...
+ `1101`: 服务端未知错误。
  ...
  - **mix_type**
    ...
    类型: [StreamMixingType](85535.md#streammixingtype)
