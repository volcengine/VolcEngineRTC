...
| [updateToken](#rtcroom-updatetoken) | 更新 Token。<br>Token 中同时包含进房、发布和订阅权限，各权限有一定的有效期，并且到期前 30 秒会触发回调，提示用户更新 Token 相关权限。此时需要重新获取 Token，并调用此方法更新 Token，以保证通话的正常进行。 |
...
- **注意**
  ...
  + 本地用户调用此方法加入房间成功后，会收到 [onRoomStateChanged](85533.md#onroomstatechanged) 回调通知。若本地用户同时为可见用户，加入房间时远端用户会收到 [onUserJoined](85533.md#onuserjoined) 回调通知。关于可见性设置参看 [setUserVisibility](#setuservisibility)。
  + 用户加入房间成功后，在本地网络状况不佳的情况下，SDK 可能会与服务器失去连接，并触发 [onConnectionStateChanged](85533.md#onconnectionstatechanged) 回调。此时 SDK 会自动重试，直到成功重连。重连成功后，本地会收到 [`onRoomStateChanged`](85533.md#onroomstatechanged) 回调通知。
...
| [startLiveTranscoding](#rtcvideo-startlivetranscoding) | 开启转推直播，并设置合流的视频视图布局和音频属性。 |
| [stopLiveTranscoding](#rtcvideo-stoplivetranscoding) | 停止转推直播。 |
| [updateLiveTranscoding](#rtcvideo-updatelivetranscoding) | 更新转推直播参数。 |
...
| `static` [getSDKVersion](#getsdkversion) | 获取当前 SDK 版本信息 |
| `static` [getCurrentProcessId](#getcurrentprocessid) | 获取加载引擎进程的 ID 方便调试 |
...
- **参数**
  ...
  - **problem_desc**
    ...
    上报的数据。JSON 格式，必须包含以下 4 个字段: <li>types: 预设的问题类型集合，可多选。具体的类型参考 [ProblemFeedbackOption](Windows-keytype.md#problemfeedbackoption) </li><li>problem_desc: 预设问题以外的其他问题的具体描述 </li><li>os_version: 系统版本  </li><li>network_type: 网络类型：包括 WiFi, 2g, 3g, 4g, 5g。如果是台式机，填写 pc 。</li>
...
### startLiveTranscoding <span id="rtcvideo-startlivetranscoding"></span> 
...
- **类型**
  ...
  ```ts
  (task_id: string, param: ITranscoderParam) => number
  ```
...
- **参数**
  ...
  - **task_id**
    ...
    转推直播任务 ID。你可以在同一房间内发起多个转推直播任务，并用不同的任务 ID 加以区。当你需要发起多个转推直播任务时，应使用多个 ID；当你仅需发起一个转推直播任务时，建议使用空字符串。

  - **param**

    类型: [ITranscoderParam](85535.md#itranscoderparam)
...
### stopLiveTranscoding <span id="rtcvideo-stoplivetranscoding"></span> 
...
### updateLiveTranscoding <span id="rtcvideo-updatelivetranscoding"></span> 
...
- **注意**
  ...
  关于启动转推直播，参看 [`startPushStreamToCDN`](#startpushstreamtocdn) 和 [`startPushSingleStreamToCDN`](#startpushsinglestreamtocdn)。
...
- **类型**
  ...
  ```ts
  (bg_sticker_path: string, source: VirtualBackgroundSource) => any
  ```
...
- **注意**
  ...
  + 调用接口后，你会收到 [onPlayStateChange](85533.md#onplaystatechange) 回调歌曲播放状态。
  + 若音乐 ID 错误，会触发 [onPlayStateChange](85533.md#onplaystatechange) 回调，error_code 为 –3023，play_state 为 4。
  + 若未进房，会触发 [onPlayStateChange](85533.md#onplaystatechange) 回调，error_code 为 –3022，play_state 为 4。
  + 若音乐文件不存在，会触发 [onPlayStateChange](85533.md#onplaystatechange) 回调，error_code 为 –3020，play_state 为 4。
