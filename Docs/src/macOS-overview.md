...
| [setVideoCaptureRotation:](macOS-api.md#ByteRTCVideo-setvideocapturerotation) | 设置本端采集的视频帧的旋转角度。<br/>当摄像头倒置或者倾斜安装时，可调用本接口进行调整。对于手机等普通设备，可调用 [setVideoRotationMode:](#setvideorotationmode) 实现旋转。 |
