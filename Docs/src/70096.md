...
1. 远端用户调用 [setUserVisibility](70095#IRTCRoom-setuservisibility) 方法将自身设为可见后加入房间时，房间内其他用户将收到该事件。
...
调用 [setRoomExtraInfo](70095#IRTCRoom-setroomextrainfo) 设置房间附加信息结果的回调。
...
| --- | --- | --- |
| mix_id | int | 混音 ID。 |
| audio_frame | const IAudioFrame & | 参看 [IAudioFrame](70098#IAudioFrame)。 |
...
```
获得单个流的音频数据，此回调通过调用 [registerAudioFrameObserver](70095#IRTCVideo-registeraudioframeobserver) 触发。
...
| --- | --- | --- |
| audio_frame | const IAudioFrame & | 音频数据, 详见： [IAudioFrame](70098#IAudioFrame) |
| stream_info | const RemoteStreamKey & | 该音频流的业务信息, 详见： [RemoteStreamKey](70098#RemoteStreamKey) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audio_frame | IAudioFrame & | 音频帧地址，参看 [IAudioFrame](70098#IAudioFrame) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_info | const RemoteStreamKey & | 音频流信息，参看 [RemoteStreamKey](70098#RemoteStreamKey) |
| audio_frame | IAudioFrame & | 音频帧地址，参看 [IAudioFrame](70098#IAudioFrame) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| warn | int | 警告标识码，详见: [WarningCode](70097#WarningCode) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| err | int | 错误标识码，参看 [ErrorCode](70097#ErrorCode) |
...
调用 [setAudioMixingProgressInterval](70095#IAudioMixingManager-setaudiomixingprogressinterval) 将时间间隔设为大于 0 的值后，或调用 [startAudioMixing](70095#IAudioMixingManager-startaudiomixing) 将 [AudioMixingConfig](70098#AudioMixingConfig) 中的时间间隔设为大于 0 的值后，SDK 会按照设置的时间间隔回调该事件。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | bytertc::ConnectionState | 当前 SDK 与信令服务器的连接状态，详见 [ConnectionState](70098#ConnectionState)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | bytertc::NetworkType | <br>SDK 当前的网络连接类型，详见： [NetworkType](70098#NetworkType)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| event | const RemoteStreamSwitch & | 音视频流发生变化的信息。参看 [RemoteStreamSwitch](70098#RemoteStreamSwitch)。 |
...
```
本地未通过 [setPublishFallbackOption](70095#IRTCVideo-setpublishfallbackoption) 开启发布性能回退，检测到设备性能不足时，收到此回调。

本地通过 [setPublishFallbackOption](70095#IRTCVideo-setpublishfallbackoption) 开启发布性能回退，因设备性能/网络原因，造成发布性能回退/恢复时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| mode | bytertc::PerformanceAlarmMode | 指示本地是否开启发布回退功能。参看 [PerformanceAlarmMode](70098#PerformanceAlarmMode)<br><ul><li>当发布端未开启发布性能回退时，mode 值为 kPerformanceAlarmModeNormal。</li><li>当发布端开启发布性能回退时，mode 值为 kPerformanceAlarmModeSimulcast。</li></ul> |
| room_id | const char * | <ul><li>未开启发布性能回退时，room_id 为空</li><li>开启发布性能回退时，room_id 是告警影响的房间 ID。</li></ul> |
| reason | bytertc::PerformanceAlarmReason | 告警原因，参看 [PerformanceAlarmReason](70098#PerformanceAlarmReason) |
| data | const bytertc::SourceWantedData & | 性能回退相关数据，详见 [SourceWantedData](70098#SourceWantedData)。 |
...
| device_type | bytertc::RTCAudioDeviceType | 设备类型，详见 [RTCAudioDeviceType](70098#RTCAudioDeviceType) |
| device_state | bytertc::MediaDeviceState | 设备状态，详见 [MediaDeviceState](70098#MediaDeviceState) |
| device_error | bytertc::MediaDeviceError | 设备错误类型，详见 [MediaDeviceError](70098#MediaDeviceError) |
...
| device_type | bytertc::RTCVideoDeviceType | 设备类型，详见 [RTCVideoDeviceType](70098#RTCVideoDeviceType) |
| device_state | bytertc::MediaDeviceState | 设备状态，详见 [MediaDeviceState](70098#MediaDeviceState) |
| device_error | bytertc::MediaDeviceError | 设备错误类型，详见 [MediaDeviceError](70098#MediaDeviceError) |
...
| --- | --- | --- |
| device_id | const char * | 设备 ID |
| device_type | bytertc::RTCAudioDeviceType | 详见 [RTCAudioDeviceType](70098#RTCAudioDeviceType) |
| device_warning | bytertc::MediaDeviceWarning | 详见 [MediaDeviceWarning](70098#MediaDeviceWarning) |
...
| --- | --- | --- |
| device_id | const char * | 设备 ID |
| device_type | bytertc::RTCVideoDeviceType | 详见 [RTCVideoDeviceType](70098#RTCVideoDeviceType) |
| device_warning | bytertc::MediaDeviceWarning | 详见 [MediaDeviceWarning](70098#MediaDeviceWarning) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stats | const bytertc::SysStats & | 包含当前 CPU 与内存相关信息的结构体。详见 [SysStats](70098#SysStats)。 |
...
| --- | --- | --- |
| room_id | const char * | 房间 ID。 |
| error_code | int | 创建房间错误码，具体原因参看 [ErrorCode](70097#ErrorCode)。 |
...
该回调由 [startFileRecording](70095#IRTCVideo-startfilerecording) 或 [stopFileRecording](70095#IRTCVideo-stopfilerecording) 触发。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | StreamIndex | 录制流的流属性，参看 [StreamIndex](70098#StreamIndex) |
| state | RecordingState | 录制状态，参看 [RecordingState](70098#RecordingState) |
| error_code | RecordingErrorCode | 录制错误码，参看 [RecordingErrorCode](70098#RecordingErrorCode) |
| info | RecordingInfo | 录制文件的详细信息，参看 [RecordingInfo](70098#RecordingInfo) |
...
该回调由 [startFileRecording](70095#IRTCVideo-startfilerecording) 触发，录制状态正常时，系统每秒钟都会通过该回调提示录制进度。
...
| --- | --- | --- |
| type | StreamIndex | 录制流的流属性，参看 [StreamIndex](70098#StreamIndex) |
| process | RecordingProgress | 录制进度，参看 [RecordingProgress](70098#RecordingProgress) |
| info | RecordingInfo | 录制文件的详细信息，参看 [RecordingInfo](70098#RecordingInfo) |
...
```
调用 [startAudioRecording](70095#IRTCVideo-startaudiorecording) 或 [stopAudioRecording](70095#IRTCVideo-stopaudiorecording) 改变音频文件录制状态时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | AudioRecordingState | 录制状态，参看 [AudioRecordingState](70098#AudioRecordingState) |
| error_code | AudioRecordingErrorCode | 录制错误码，参看 [AudioRecordingErrorCode](70097#AudioRecordingErrorCode) |
...
| error_code | int | <br>登录结果<br>详见 [LoginErrorCode](70097#LoginErrorCode)。 |
| elapsed | int | <br>从调用 [login](70095#IRTCVideo-login) 接口开始到返回结果所用时长<br>单位为 ms。 |


**注意**

调用 [login](70095#IRTCVideo-login) 后，会收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| reason | LogoutReason | 用户登出的原因，参看 [LogoutReason](70098#LogoutReason) |


**注意**

在以下两种情况下会收到此回调：调用 [logout](70095#IRTCVideo-logout) 接口主动退出；或其他用户以相同 UserId 进行 `login` 导致本地用户被动登出。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| error | int | <br>设置结果<br><ul><li>返回 200，设置成功</li><li>返回其他，设置失败，详见 [UserMessageSendResult](70097#UserMessageSendResult) </li></ul> |


**注意**

调用 [setServerParams](70095#IRTCVideo-setserverparams) 后，会收到此回调。
...
| status | int | <br>查询的用户登录状态<br>详见 [UserOnlineStatus](70098#UserOnlineStatus). |


**注意**

必须先调用 [getPeerOnlineStatus](70095#IRTCVideo-getpeeronlinestatus)，才能收到此回调。
...
```
收到房间外用户调用 [sendUserMessageOutsideRoom](70095#IRTCVideo-sendusermessageoutsideroom) 发来的文本消息时，会收到此回调
...
```
收到房间外用户调用 [sendUserBinaryMessageOutsideRoom](70095#IRTCVideo-senduserbinarymessageoutsideroom) 发来的二进制消息时，会收到此回调
...
| error | int | <br>消息发送结果<br>详见 [UserMessageSendResult](70097#UserMessageSendResult)。 |


**注意**

当调用 [sendUserMessageOutsideRoom](70095#IRTCVideo-sendusermessageoutsideroom) 或 [sendUserBinaryMessageOutsideRoom](70095#IRTCVideo-senduserbinarymessageoutsideroom) 发送消息后，会收到此回调。
...
| error | int | 消息发送结果，详见 [UserMessageSendResult](70097#UserMessageSendResult)。 |
| msg | const bytertc::ServerACKMsg & | 应用服务器收到 HTTP 请求后，在 ACK 中返回的信息。消息不超过 64 KB。 |


**注意**

本回调为异步回调。当调用 [sendServerMessage](70095#IRTCVideo-sendservermessage) 或 [sendServerBinaryMessage](70095#IRTCVideo-sendserverbinarymessage) 接口发送消息后，会收到此回调。
...
```
收到通过调用 [sendSEIMessage](70095#IRTCVideo-sendseimessage) 发送带有 SEI 消息的视频帧时，收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_key | RemoteStreamKey | 包含 SEI 发送者的用户名，所在的房间名和媒体流，参看 [RemoteStreamKey](70098#RemoteStreamKey) |
...
在语音通话场景下，本地用户调用 [sendSEIMessage](70095#IRTCVideo-sendseimessage) 通过黑帧视频流发送 SEI 数据时，流的发送状态会通过该回调通知远端用户。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| key | const RemoteStreamKey & | 远端流信息，参看 [RemoteStreamKey](70098#RemoteStreamKey)。 |
| type | SEIStreamEventType | 黑帧视频流状态，参看 [SEIStreamEventType](70098#SEIStreamEventType) |
...
```
远端用户进房后，本地调用 [enableAudioPropertiesReport](70095#IRTCVideo-enableaudiopropertiesreport)，根据设置的 interval 值，本地会周期性地收到此回调，了解订阅的远端用户的瞬时音频信息。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audio_properties_infos | const RemoteAudioPropertiesInfo * | 远端音频信息，其中包含音频流属性、房间 ID、用户 ID ，详见 [RemoteAudioPropertiesInfo](70098#RemoteAudioPropertiesInfo)。 |
...
```
调用 [enableAudioPropertiesReport](70095#IRTCVideo-enableaudiopropertiesreport) 后，根据设置的 interval 值，你会周期性地收到此回调，了解本地音频的相关信息。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| audio_properties_infos | const LocalAudioPropertiesInfo * | 本地音频信息，详见 [LocalAudioPropertiesInfo](70098#LocalAudioPropertiesInfo) 。 |
| audio_properties_info_number | int | 数组长度 |
...
调用 [startAudioPlaybackDeviceTest](70095#IAudioDeviceManager-startaudioplaybackdevicetest) 或 [startAudioDeviceRecordTest](70095#IAudioDeviceManager-startaudiodevicerecordtest)，开始播放音频文件或录音时，将开启该回调。本回调为周期性回调，回调周期由上述接口的 `interval` 参数指定。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| device_type | bytertc::RTCAudioDeviceType | 设备类型，包括麦克风和扬声器，参阅 [RTCAudioDeviceType](70098#RTCAudioDeviceType)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | LocalAudioStreamState | 本地音频设备的状态，详见： [LocalAudioStreamState](70098#LocalAudioStreamState) |
| error | LocalAudioStreamError | 本地音频状态改变时的错误码，详见： [LocalAudioStreamError](70098#LocalAudioStreamError) |
...
```
房间内的用户调用 [startAudioCapture](70095#IRTCVideo-startaudiocapture) 开启音频采集时，房间内其他用户会收到此回调。
...
```
房间内的用户调用 [stopAudioCapture](70095#IRTCVideo-stopaudiocapture) 关闭音频采集时，房间内其他用户会收到此回调。
...
```
调用 [enableAudioPropertiesReport](70095#IRTCVideo-enableaudiopropertiesreport) 后，根据设置的 `config.interval`，你会周期性地收到此回调，获取房间内的最活跃用户信息。
...
```
音频流同步信息回调。可以通过此回调，在远端用户调用 [sendStreamSyncInfo](70095#IRTCVideo-sendstreamsyncinfo) 发送音频流同步消息后，收到远端发送的音频流同步信息。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| stream_key | RemoteStreamKey | 远端流信息，详见 [RemoteStreamKey](70098#RemoteStreamKey) 。 |
| stream_type | SyncInfoStreamType | 媒体流类型，详见 [SyncInfoStreamType](70098#SyncInfoStreamType)。 |
...
成功调用 [startNetworkDetection](70095#IRTCVideo-startnetworkdetection) 接口开始探测后，会在 3s 内首次收到该回调，之后每 2s 收到一次该回调。
...
| quality | NetworkQuality | 探测网络的质量，参看 [NetworkQuality](70098#NetworkQuality)。 |
...
1. 当调用 [stopNetworkDetection](70095#IRTCVideo-stopnetworkdetection) 接口停止探测后，会收到一次该回调；
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| reason | NetworkDetectionStopReason | <br>停止探测的原因类型,参考 [NetworkDetectionStopReason](70098#NetworkDetectionStopReason) |
...
通过 [startPlayPublicStream](70095#IRTCVideo-startplaypublicstream) 订阅公共流后，可以通过本回调获取订阅结果。
...
| error_code | PublicStreamErrorCode | 公共流订阅结果状态码。详见 [PublicStreamErrorCode](70097#PublicStreamErrorCode)。 |
...
调用 [startPlayPublicStream](70095#IRTCVideo-startplaypublicstream) 接口启动拉公共流功能后，通过此回调收到公共流中的 SEI 消息。
...
| source_type | DataMessageSourceType | SEI 消息类型，自 3.52.1 版本后固定为 `0`，自定义消息。参看 [DataMessageSourceType](70098#DataMessageSourceType)。 |
...
通过 [startPlayPublicStream](70095#IRTCVideo-startplaypublicstream) 订阅公共流后，通过监听本回调获取公共流中的数据消息，包括调用 Open API 发送的 SEI 消息和音量回调。
...
| source_type | DataMessageSourceType | 数据消息来源，参看 [DataMessageSourceType](70098#DataMessageSourceType)。 |
...
关于订阅公共流，详见 [startPlayPublicStream](70095#IRTCVideo-startplaypublicstream)。
...
| --- | --- | --- |
| public_stream_id | const char * | 公共流 ID |
| info | const VideoFrameInfo & | 视频帧信息。详见 [VideoFrameInfo](70098#VideoFrameInfo)。 |
...
```
房间内的可见用户调用 [startVideoCapture](70095#IRTCVideo-startvideocapture) 开启内部视频采集时，房间内其他用户会收到此回调。
...
```
房间内的可见用户调用 [stopVideoCapture](70095#IRTCVideo-stopvideocapture) 关闭内部视频采集时，房间内其他用户会收到此回调。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| index | StreamIndex | 流属性。参看 [StreamIndex](70098#StreamIndex)。 |
| info | const VideoFrameInfo & | 视频帧信息。参看 [VideoFrameInfo](70098#VideoFrameInfo)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| key | RemoteStreamKey | 远端流信息。参看 [RemoteStreamKey](70098#RemoteStreamKey)。 |
| info | const VideoFrameInfo & | 视频帧信息。参看 [VideoFrameInfo](70098#VideoFrameInfo)。 |
...
| --- | --- | --- |
| index | StreamIndex | 流属性。参看 [StreamIndex](70098#StreamIndex)。 |
| info | VideoFrameInfo | 视频信息。参看 [VideoFrameInfo](70098#VideoFrameInfo)。 |


**注意**

对于采集到的本地视频帧，你可以调用 [setLocalVideoCanvas](70095#IRTCVideo-setlocalvideocanvas) 或 [setLocalVideoSink](70095#IRTCVideo-setlocalvideosink) 在本地渲染。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| key | const RemoteStreamKey & | 远端音频流信息, 详见 [RemoteStreamKey](70098#RemoteStreamKey) |
...
| --- | --- | --- |
| key | const RemoteStreamKey & | 远端流信息, 参看 [RemoteStreamKey](70098#RemoteStreamKey) |
| state | RemoteAudioState | 远端音频流状态，参看 [RemoteAudioState](70098#RemoteAudioState) |
| reason | RemoteAudioStateChangeReason | 远端音频流状态改变的原因，参看 [RemoteAudioStateChangeReason](70098#RemoteAudioStateChangeReason) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| index | StreamIndex | 音/视频属性，参看 [StreamIndex](70098#StreamIndex) |
| state | LocalVideoStreamState | 本地视频流状态，参看 [LocalVideoStreamState](70098#LocalVideoStreamState) |
| error | LocalVideoStreamError | 本地视频状态改变时的错误码，参看 [LocalVideoStreamError](70097#LocalVideoStreamError) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| key | RemoteStreamKey | 远端视频流的信息，房间、用户 ID、流属性等。参看 [RemoteStreamKey](70098#RemoteStreamKey)。 |
| state | RemoteVideoState | 远端视频流状态，参看 [RemoteVideoState](70098#RemoteVideoState)。 |
| reason | RemoteVideoStateChangeReason | 远端视频流状态改变的原因，参看 [RemoteVideoStateChangeReason](70098#RemoteVideoStateChangeReason)。 |
...
| user | const RtcUser & | 本地用户信息，详见 [RtcUser](70098#RtcUser) |
| state | FirstFrameSendState | 首帧发送状态，详见 [FirstFrameSendState](70098#FirstFrameSendState) |
...
| user | const RtcUser & | 远端用户信息，详见 [RtcUser](70098#RtcUser) |
| state | FirstFramePlayState | 首帧播放状态，详见 [FirstFramePlayState](70098#FirstFramePlayState) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| index | StreamIndex | 音频流属性, 参看 [StreamIndex](70098#StreamIndex) |
...
调用 [startPushPublicStream](70095#IRTCVideo-startpushpublicstream) 接口发布公共流后，启动结果通过此回调方法通知用户。
...
| error_code | PublicStreamErrorCode | 公共流发布结果状态码。详见 [PublicStreamErrorCode](70097#PublicStreamErrorCode) |
...
```
调用 [startCloudProxy](70095#IRTCVideo-startcloudproxy) 开启云代理，SDK 首次成功连接云代理服务器时，回调此事件。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| result | EchoTestResult | 测试结果，参看 [EchoTestResult](70098#EchoTestResult) |
...
```
首次调用 [getNetworkTimeInfo](70095#IRTCVideo-getnetworktimeinfo) 后，SDK 内部启动网络时间同步，同步完成时会触发此回调。
...
```
外部采集时，调用 [setOriginalScreenVideoInfo](70095#IRTCVideo-setoriginalscreenvideoinfo) 设置屏幕或窗口大小改变前的分辨率后，若屏幕采集模式为智能模式，你将收到此回调，根据 RTC 智能决策合适的帧率和分辨率积（宽*高）重新采集。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| frame_update_info | FrameUpdateInfo | RTC 智能决策后合适的帧率和分辨率积（宽*高）。参看 [FrameUpdateInfo](70098#FrameUpdateInfo)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| hardware_echo_detection_result | HardwareEchoDetectionResult | 参见 [HardwareEchoDetectionResult](70098#HardwareEchoDetectionResult) |


**注意**

- 通话前调用 [startHardwareEchoDetection](70095#IRTCVideo-starthardwareechodetection) 后，将触发本回调返回检测结果。
- 建议在收到检测结果后，调用 [stopHardwareEchoDetection](70095#IRTCVideo-stophardwareechodetection) 停止检测，释放对音频设备的占用。
...
```
本地代理状态发生改变回调。调用 [setLocalProxy](70095#IRTCVideo-setlocalproxy) 设置本地代理后，SDK 会触发此回调，返回代理连接的状态。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| local_proxy_type | LocalProxyType | 本地代理类型。参看 [LocalProxyType](70098#LocalProxyType)。 |
| local_proxy_state | LocalProxyState | 本地代理状态。参看 [LocalProxyState](70098#LocalProxyState)。 |
| local_proxy_error | LocalProxyError | 本地代理错误。参看 [LocalProxyError](70098#LocalProxyError)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| mix_id | int | 混音 ID <br>使用 [IAudioMixingManager](70095#IAudioMixingManager) 相关接口时传入的唯一 ID。 |
| state | bytertc::AudioMixingState | <br>混音状态<br>其混音状态可参考： [AudioMixingState](70098#AudioMixingState)。 |
| error | bytertc::AudioMixingError | <br>错误码<br>详见 [AudioMixingError](70098#AudioMixingError) |
...
此回调会被触发的时机汇总如下：
- 1. 音乐文件类型：
- 当调用 [startAudioMixing](70095#IAudioMixingManager-startaudiomixing) 方法成功后，会触发 state 值为 kAudioMixingStatePlaying 回调；否则触发 state 值为 kAudioMixingStateFailed 的回调。
- 当使用相同的 ID 重复调用 [startAudioMixing](70095#IAudioMixingManager-startaudiomixing) 后，后一次会覆盖前一次，且本回调会以 kAudioMixingStateStopped 通知前一次混音已停止。
- 当调用 [pauseAudioMixing](70095#IAudioMixingManager-pauseaudiomixing) 方法暂停播放成功后，会触发 state 值为 kAudioMixingStatePaused 回调；否则触发 state 值为 kAudioMixingStateFailed 的回调。
- 当调用 [resumeAudioMixing](70095#IAudioMixingManager-resumeaudiomixing) 方法恢复播放成功后，会触发 state 值为 kAudioMixingStatePlaying 回调；否则触发 state 值为 kAudioMixingStateFailed 的回调。
- 当调用 [stopAudioMixing](70095#IAudioMixingManager-stopaudiomixing) 方法暂停止播放成功后，会触发 state 值为 kAudioMixingStateStopped 回调；否则触发 state 值为 kAudioMixingStateFailed 的回调。
- 播放结束会触发 state 值为 kAudioMixingStateFinished 回调。
- 2. PCM 流类型：
- 当调用 [enableAudioMixingFrame](70095#IAudioMixingManager-enableaudiomixingframe) 方法成功后，会触发 state 值为 kAudioMixingStatePCMEnabled 回调；否则触发 state 值为 kAudioMixingStateFailed 的回调。（Linux 不支持）
- 当调用 [disableAudioMixingFrame](70095#IAudioMixingManager-disableaudiomixingframe) 方法成功后，会触发 state 值为 kAudioMixingStatePCMDisabled 回调；否则触发 state 值为 kAudioMixingStateFailed 的回调。
...
| device_type | bytertc::MediaDeviceType | 设备类型，详见 [MediaDeviceType](70098#MediaDeviceType) |
| device_state | bytertc::MediaDeviceState | 设备状态，详见 [MediaDeviceState](70098#MediaDeviceState) |
| device_error | bytertc::MediaDeviceError | 设备错误类型，详见 [MediaDeviceError](70098#MediaDeviceError) |
...
| --- | --- | --- |
| device_id | const char * | 设备 ID |
| device_type | bytertc::MediaDeviceType | 详见 [MediaDeviceType](70098#MediaDeviceType) |
| device_warning | bytertc::MediaDeviceWarning | 详见 [MediaDeviceWarning](70098#MediaDeviceWarning) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | int | 当前 HTTP 代理连接状态，详见 [HttpProxyState](70098#HttpProxyState) |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| state | int | 当前 HTTPS 代理连接状态，详见 [HttpProxyState](70098#HttpProxyState) |
...
| mute_state | MuteState | 发送状态，参看 [MuteState](70098#MuteState) |
...
| mute | MuteState | 视频流的发送状态。参看 [MuteState](70098#MuteState)。 |
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| event | SingleStreamPushEvent | 任务状态, 参看 [SingleStreamPushEvent](70097#SingleStreamPushEvent) |
| task_id | const char * | 任务 ID |
| error | int | 错误码，参看 [StreamMixingErrorCode](70097#StreamMixingErrorCode) |
...
```
调用 [registerLocalEncodedVideoFrameObserver](70095#IRTCVideo-registerlocalencodedvideoframeobserver) 后，SDK 每次使用内部采集，采集到一帧视频帧，或收到一帧外部视频帧时，都会回调该事件。
...
| 参数名 | 类型 | 说明 |
| --- | --- | --- |
| type | StreamIndex | 本地视频帧类型，参看 [StreamIndex](70098#StreamIndex) |
| video_stream | const IEncodedVideoFrame & | 本地视频帧信息，参看 [IEncodedVideoFrame](70098#IEncodedVideoFrame) |
