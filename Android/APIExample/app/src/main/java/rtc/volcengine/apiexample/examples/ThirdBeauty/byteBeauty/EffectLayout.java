...
public class EffectLayout extends ConstraintLayout {
    ...
    private EffectNode.EffectNodeCallback mEffectNodeCallback;
    ...
    private final INodeClick mINodeClick = new INodeClick() {
        ...
        public void onClick(EffectNode node) {
            for (EffectNode temp : mEffectNodeList) {
                if (node.type == EffectNode.EffectType.sticker || node.type == EffectNode.EffectType.virtualBackground) {
                    temp.selected = TextUtils.equals(temp.key, node.key) && temp.selected;
                } else {
                    temp.selected = TextUtils.equals(temp.key, node.key);
                }

                if (!temp.selected && node.type == EffectNode.EffectType.filter) {
                    temp.value = 0;
                }
            }
            ...
            EffectNode.EffectNodeCallback callback = mEffectNodeCallback;
            if (callback != null) {
    ...
    public void setEffectSection(EffectSection effectSection, EffectNode.EffectNodeCallback effectNodeCallback) {
