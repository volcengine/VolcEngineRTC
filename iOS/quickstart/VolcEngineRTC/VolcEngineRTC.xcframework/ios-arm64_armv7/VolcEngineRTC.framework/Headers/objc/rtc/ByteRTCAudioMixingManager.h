 ...
 * @author majun.lvhiei
 * @brief 开始播放音频文件及混音
 * @param id 混音 ID。用于标识混音，请保证混音 ID 唯一性。  <br>
 ...
 * @param filePath 用于混音文件路径。可以是本地文件的绝对路径或在线音频的 url。<br>
 *        不同平台支持的音频文件格式: <br>
 *        <table>
 ...
 * @notes  <br>
 *       + 如果已经通过 preloadAudioMixing:filePath:{@link #preloadAudioMixing:filePath:} 将音效加载至内存，确保此处的 ID 与之设置的 ID 相同。  <br>
 ...
 * @region 混音
 * @author majun.lvhiei
 * @brief 调节音频文件在本地和远端播放的音量大小
 * @param id  <br>
 *        混音 ID
 * @param volume  <br>
 *        音频文件播放音量范围为 0~400。  <br>
 *        + 0：静音  <br>
 *        + 100：原始音量（默认值）  <br>
 *        + 400: 最大可调音量 (自带溢出保护)
 * @param type  <br>
 *        混音播放类型  <br>
 *        设置音频文件是否本地播放、以及是否发送到远端，由此控制音频文件本地或远端播放的音量，详见 ByteRTCAudioMixingType{@link #ByteRTCAudioMixingType}。
 * @notes  <br>
 ...
 *        参数为整数，单位为毫秒。
 * @notes  <br>
 *       + 该方法可以设置音频文件的播放位置，这样你可以根据实际情况从指定的位置播放音频文件，无需从头到尾完整播放一个音频文件。  <br>
 *       + 调用本方法设置音频文件的播放位置前，需要先调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:} 开始播放音频文件。
 */
 ...
 *        + 此方法对 enableAudioMixingFrame:{@link #enableAudioMixingFrame:} 播放的音乐无效。
 */
-(void) setAudioMixingDualMonoMode:(int)id mode:(ByteRTCAudioMixingDualMonoMode)mode;
