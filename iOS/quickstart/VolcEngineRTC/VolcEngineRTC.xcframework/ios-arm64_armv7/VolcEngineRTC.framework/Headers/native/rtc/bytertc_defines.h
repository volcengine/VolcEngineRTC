...
enum ErrorCode {
    /** 
     * @brief Token 无效。
     *        调用 JoinRoom{@link #IRtcEngine#JoinRoom} 方法时使用的 Token 无效或过期失效。需要用户重新获取 Token，并调用 UpdateToken{@link #IRtcRoom#UpdateToken} 方法更新 Token。
     */
    kErrorCodeInvalidToken = -1000,
    /** 
     * @brief 加入房间错误。
     *        调用 JoinRoom{@link #IRtcEngine#JoinRoom} 方法时发生未知错误导致加入房间失败。需要用户重新加入房间。
     */
...
enum WarningCode {
   ...
   /** 
    ...
    /** 
     * @brief 不支持在 PublishScreen{@link #IRtcRoom#PublishScreen} 之后设置屏幕音频采集类型
     *        SetScreenAudioSourceType{@link #IRtcEngineLite#SetScreenAudioSourceType}，请在 PublishScreen 之前设置
     */
...
struct AudioMixingConfig {
    ...
    int play_count;
     ...
     int position;
     /** 
      * @brief 设置音频文件播放进度回调的时间间隔，单位毫秒，并按照设置的值触发 OnAudioMixingPlayingProgress{@link #IRtcEngineLiteEventHandler#OnAudioMixingPlayingProgress} 回调，默认不回调。  <br>
...
enum ForwardStreamState {
    /** 
     * @brief 空闲状态
     *        + 成功调用 StopForwardStreamToRooms{@link #IRtcRoom#StopForwardStreamToRooms} 后，所有目标房间为空闲状态。
     *        + 成功调用 UpdateForwardStreamToRooms{@link #IRtcRoom#UpdateForwardStreamToRooms} 减少目标房间后，本次减少的目标房间为空闲状态。
     */
    kForwardStreamStateIdle = 0,
    /** 
     * @brief 开始转发
     *        + 调用 StartForwardStreamToRooms{@link #IRtcRoom#StartForwardStreamToRooms} 成功向所有房间开始转发媒体流后，返回此状态。
     *        + 调用 UpdateForwardStreamToRooms{@link #IRtcRoom#UpdateForwardStreamToRooms} 后，成功向新增目标房间开始转发媒体流后，返回此状态。
     */
    kForwardStreamStateSuccess = 1,
    /** 
     * @brief 转发失败，失败详情参考 ForwardStreamError{@link #ForwardStreamError}
     *        调用 StartForwardStreamToRooms{@link #IRtcRoom#StartForwardStreamToRooms} 或 UpdateForwardStreamToRooms{@link #IRtcRoom#UpdateForwardStreamToRooms} 后，如遇转发失败，返回此状态。
     */
...
enum ForwardStreamEvent {
    ...
    /** 
     * @brief 目标房间已更新，由 UpdateForwardStreamToRooms{@link #IRtcRoom#UpdateForwardStreamToRooms} 触发。
     */
    kForwardStreamEventDstRoomUpdated = 3,
    /** 
     * @brief API 调用时序错误。例如，在调用 StartForwardStreamToRooms{@link #IRtcRoom#StartForwardStreamToRooms} 之前调用 UpdateForwardStreamToRooms{@link #IRtcRoom#UpdateForwardStreamToRooms} 。
     */
