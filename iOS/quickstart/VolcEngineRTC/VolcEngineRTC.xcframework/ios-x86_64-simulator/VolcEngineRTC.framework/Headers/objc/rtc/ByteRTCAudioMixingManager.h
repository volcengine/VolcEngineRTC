 ...
 *       + 调用本方法停止播放音频文件后，SDK 会向本地回调通知已停止混音，见 rtcEngine:onAudioMixingStateChanged:state:error:{@link #ByteRTCEngineDelegate#rtcEngine:onAudioMixingStateChanged:state:error:}。  <br>
 ...
 *       + 调用本方法暂停播放音频文件后，SDK 会向本地回调通知已暂停混音，见 rtcEngine:onAudioMixingStateChanged:state:error:{@link #ByteRTCEngineDelegate#rtcEngine:onAudioMixingStateChanged:state:error:}。
 */
 ...
 * @param mixId  <br>
 *        混音 ID
 * @notes 不论音频文件是否播放，调用本方法卸载该文件后，SDK 会回调通知混音已停止，见 rtcEngine:onAudioMixingStateChanged:state:error:{@link #ByteRTCEngineDelegate#rtcEngine:onAudioMixingStateChanged:state:error:}。
 */
 ...
 *        超出取值范围则设置失败，并且会触发 rtcEngine:onAudioMixingStateChanged:state:error:{@link #ByteRTCEngineDelegate#rtcEngine:onAudioMixingStateChanged:state:error:} 回调，提示 ByteRTCAudioMixingState{@link #ByteRTCAudioMixingState} 状态为 `AUDIO_MIXING_STATE_FAILED` 混音播放失败，ByteRTCAudioMixingError{@link #ByteRTCAudioMixingError} 错误码为 `AUDIO_MIXING_ERROR_ID_TYPE_INVALID_PITCH` 设置混音文件音调不合法。
 * @notes 本方法需要在调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:} 开始播放音频文件后、调用 stopAudioMixing:{@link #ByteRTCAudioMixingManager#stopAudioMixing:} 停止播放音频文件前使用，否则会触发 rtcEngine:onAudioMixingStateChanged:state:error:{@link #ByteRTCEngineDelegate#rtcEngine:onAudioMixingStateChanged:state:error:} 回调报错
 */
 ...
 * @region 混音
 * @author majun.lvhiei
 * @brief 如果你需要使用 enableVocalInstrumentBalance:{@link #ByteRTCEngineKit#enableVocalInstrumentBalance:} 对混音使音频文件/PCM 音频数据进行音量调整，你必须通过此接口传入其原始响度。
 * @param mixId 混音 ID
 ...
 *       + interval 的值为大于 0 的 10 的倍数，当设置的值不能被 10 整除时，则默认向上取整 10，如设为 52ms 时会默认调整为 60ms。设置完成后 SDK 将会按照设置的时间间隔触发 rtcEngine:onAudioMixingPlayingProgress:progress:{@link #ByteRTCEngineDelegate#rtcEngine:onAudioMixingPlayingProgress:progress:} 回调。  <br>
 *       + interval 的值小于等于 0 时，不会触发进度回调。  <br>
 * @notes 本方法需要在调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:} 开始播放音频文件后、调用 stopAudioMixing:{@link #ByteRTCAudioMixingManager#stopAudioMixing:} 停止播放音频文件前使用，否则会触发 rtcEngine:onAudioMixingStateChanged:state:error:{@link #ByteRTCEngineDelegate#rtcEngine:onAudioMixingStateChanged:state:error:} 回调报错。  <br>
...
-(void) setAudioMixingProgressInterval:(int)mixId interval:(int64_t) interval;
  ...
  *        如果使用相同的 ID 重复调用本方法后，前一次混音会停止，后一次混音开始，会收到 rtcEngine:onAudioMixingStateChanged:state:error:{@link #ByteRTCEngineDelegate#rtcEngine:onAudioMixingStateChanged:state:error:} 通知前一次混音已停止。
  * @param type 混音类型。是否本地播放、以及是否发送到远端，详见 ByteRTCAudioMixingType{@link #ByteRTCAudioMixingType}。
  * @notes  <br>
