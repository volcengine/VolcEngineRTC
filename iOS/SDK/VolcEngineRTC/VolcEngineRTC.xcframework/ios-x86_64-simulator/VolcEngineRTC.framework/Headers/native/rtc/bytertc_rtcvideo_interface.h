...
public:
     ...
     *        调用该方法后，本地用户会收到 onMediaDeviceStateChanged{@link #IRtcEngineEventHandler#onMediaDeviceStateChanged} 的回调。  <br>
   ...
   virtual int setVideoCaptureConfig(const VideoCaptureConfig& videoCaptureConfig) = 0;
     ...
     *       + 调用 stopScreenVideoCapture{@link #IRtcEngine#stopScreenVideoCapture} 关闭屏幕视频源采集，。  <br>
     ...
     *       + 调用此接口前，你可以调用 setScreenVideoEncoderConfig{@link #IRtcEngine#setScreenVideoEncoderConfig} 设置屏幕视频流的采集帧率和编码分辨率。  <br>
   ...
   virtual void disableAudioProcessor(AudioProcessorMethod method) = 0;
     ...
     * @notes  <br>
     *       + 在向应用服务器发送文本消息前，必须先调用 login{@link #IRtcEngine#login} 完成登录，随后调用 setServerParams{@link #IRtcEngine#setServerParams} 设置应用服务器。  <br>
     ...
     *        + -1：发送失败，RtcEngine 实例未创建
     * @notes  <br>
     *       + 在向应用服务器发送二进制消息前，必须先调用 login{@link #IRtcEngine#login} 完成登录，随后调用 setServerParams{@link #IRtcEngine#setServerParams} 设置应用服务器。  <br>
