...
public:
     ...
     * @param jitter  <br>
     *        探测网络的抖动,单位：ms  <br>
     * @notes 成功调用 startNetworkDetection{@link #IRTCAudio#startNetworkDetection} 接口开始探测后，会每 2s 收到一次此回调。
     */
     ...
     * @type callback
     * @region 音频事件回调
     * @brief 房间内的用户调用 startAudioCapture{@link #IRTCAudio#startAudioCapture}
     * 开启音频采集时，房间内其他用户会收到此回调。
     ...
     * @type callback
     * @region 音频事件回调
     * @brief 房间内的用户调用 stopAudioCapture{@link #IRTCAudio#stopAudioCapture}
     * 关闭音频采集时，房间内其他用户会收到此回调。
