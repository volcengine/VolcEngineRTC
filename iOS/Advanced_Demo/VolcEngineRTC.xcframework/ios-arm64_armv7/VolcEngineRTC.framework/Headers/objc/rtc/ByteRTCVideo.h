 ...
 * @region 媒体流管理
 * @author shenpengliang
 * @brief 房间内用户暂停/恢复发送音频流时，房间内其他用户会收到此回调。参看 muteLocalAudio:{@link #RTCVideo#muteLocalAudio:}。
 * @param engine RTCVideo 实例
...
- (void)rtcEngine:(RTCVideo * _Nonnull)engine onFirstRemoteAudioFrame:(ByteRTCRemoteStreamKey * _Nonnull)key;
  ...
  * @region 音频管理
  * @author wangjunzheng
  * @brief 调用 enableAudioPropertiesReport:{@link #RTCVideo#enableAudioPropertiesReport:} 后，根据设置的 interval 值，你会周期性地收到此回调，了解本地音频的相关信息。  <br>
  ...
  * @notes 调用 startAudioPlaybackDeviceTest{@link #startAudioPlaybackDeviceTest} 或 startAudioDeviceRecordTest{@link #startAudioDeviceRecordTest}，开始播放音频文件或录音时，将开启该回调。本回调为周期性回调，回调周期由上述接口的 `interval` 参数指定。
  */
  ...
  * @region 音频管理
  * @author wangjunzheng
  * @brief 远端用户进房后，本地调用 enableAudioPropertiesReport:{@link #RTCVideo#enableAudioPropertiesReport:} ，根据设置的 interval 值，本地会周期性地收到此回调，了解订阅的远端用户的音频信息。<br>
 ...
 * @region 音频管理
 * @author zhangcaining
 * @brief 调用 enableAudioPropertiesReport:{@link #RTCVideo#enableAudioPropertiesReport:}  后，根据设置的 `config.interval`，你会周期性地收到此回调，获取房间内的最活跃用户信息。
 * @param roomId 房间 ID
 ...
 * @author shenpengliang
 * @param engine RTCVideo 实例
 * @brief 房间内用户暂停/恢复发送视频流时，房间内其他用户会收到此回调。参看 muteLocalVideo:{@link #RTCVideo#muteLocalVideo:}。
 ...
 * @param engine ByteRTCEngineKit 实例
 * @param uid 调用 startVideoCapture{@link #ByteRTCEngineKit#startVideoCapture} 接口的远端用户 ID
 */
 ...
 * @author zhushufan.ref
 * @brief 远端视频流状态发生改变回调。
 * @param engine RTCVideo 对象，参考: RTCVideo{@link #RTCVideo}
 ...
 *  @notes 音频设备设置与音频路由策略，详见 setAudioPlaybackDevice:{@link #RTCVideo#setAudioPlaybackDevice:}
 */
 ...
 * @brief 收到通过调用 sendSEIMessage:andMessage:andRepeatCount:{@link #RTCVideo#sendSEIMessage:andMessage:andRepeatCount:} 带有 SEI 消息的视频帧时，收到此回调
 ...
 *        在语音通话场景下，本地用户调用 sendSEIMessage:andMessage:andRepeatCount:{@link #RTCVideo#sendSEIMessage:andMessage:andRepeatCount:} 通过黑帧视频流发送 SEI 数据时，流的发送状态会通过该回调通知远端用户。  <br>
 ...
 * @region 音频管理
 * @author wangjunzheng
 * @brief 音频流同步信息回调。可以通过此回调，在远端用户调用 sendStreamSyncInfo:config:{@link #RTCVideo#sendStreamSyncInfo:config:} 发送音频流同步消息后，收到远端发送的音频流同步信息。  <br>
 * @param engine 当前 RTCEngine 实例。
 ...
 * @param elapsed  <br>
 *        从调用 login:uid:{@link #RTCVideo#login:uid:} 接口开始到返回结果所用时长。  <br>
 *        单位为 ms。
 * @notes 调用 login:uid:{@link #RTCVideo#login:uid:} 后，会收到此回调。
 */
 ...
 * @brief 登出结果回调
 * @param engine RTCVideo 对象
 * @notes 调用 logout{@link #RTCVideo#logout} 后，会收到此回调。
 */
 ...
 *        + 返回 200，设置成功  <br>
 *        + 返回其他，设置失败
 * @notes 调用 setServerParams:url:{@link #RTCVideo#setServerParams:url:} 后，会收到此回调。
 */
 ...
 * @notes 必须先调用 getPeerOnlineStatus:{@link #RTCVideo#getPeerOnlineStatus:}，才能收到此回调。
 */
 ...
 * @region 实时消息通信
 * @author hanchenchen.c
 * @brief 收到房间外用户调用 sendUserMessageOutsideRoom:message:{@link #RTCVideo#sendUserMessageOutsideRoom:message:} 发来的文本消息时，会收到此回调
 * @param engine RTCVideo 对象
 * @param uid  <br>
 *        消息发送者 ID
 * @param message  <br>
 ...
 * @region 实时消息通信
 * @author hanchenchen.c
 * @brief 收到房间外用户调用 sendUserBinaryMessageOutsideRoom:message:{@link #RTCVideo#sendUserBinaryMessageOutsideRoom:message:} 发来的二进制消息时，会收到此回调。
 * @param engine RTCVideo 对象
 * @param  uid  <br>
 *         消息发送者 ID
 * @param  message  <br>
 ...
 * @notes 当调用 sendUserMessageOutsideRoom:message:{@link #RTCVideo#sendUserMessageOutsideRoom:message:} 或 sendUserBinaryMessageOutsideRoom:message:{@link #RTCVideo#sendUserBinaryMessageOutsideRoom:message:} 发送消息后，会收到此回调。
 */
 ...
 * @notes 当调用 sendServerMessage:{@link #RTCVideo#sendServerMessage:} 或 sendServerBinaryMessage:{@link #RTCVideo#sendServerBinaryMessage:} 接口发送消息后，会收到此回调。
 */
 ...
 *        探测网络类型为上行/下行  <br>
 * @param quality  <br>
 *        探测网络的质量，值含义参考 ByteRTCNetworkQuality{@link #ByteRTCNetworkQuality} <br>
 * @param rtt  <br>
 *        探测网络的 RTT，单位：ms  <br>
 * @param lost_rate  <br>
 *        探测网络的丢包率  <br>
 ...
 * @param jitter  <br>
 *        探测网络的抖动,单位：ms  <br>
 * @notes 成功调用 startNetworkDetection:uplinkBandwidth:downlink:downlinkBandwidth:{@link #RTCVideo#startNetworkDetection:uplinkBandwidth:downlink:downlinkBandwidth:} 接口开始探测后，会每 2s 收到一次此回调。
 */
 ...
 *        1. 当调用 stopNetworkDetection{@link #RTCVideo#stopNetworkDetection} 接口停止探测后，会收到一次该回调；
 ...
 * @region 本地录制
 * @brief 获取本地录制状态回调。  <br>
 *        该回调由 startFileRecording:withRecordingConfig:type:{@link #RTCVideo#startFileRecording:withRecordingConfig:type:} 或 stopFileRecording:{@link #RTCVideo#stopFileRecording:} 触发。
 * @param engine RTCVideo 对象
 ...
 * @region 本地录制
 * @brief 本地录制进度回调。  <br>
 *        该回调由 startFileRecording:withRecordingConfig:type:{@link #RTCVideo#startFileRecording:withRecordingConfig:type:} 触发，录制状态正常时，系统每秒钟都会通过该回调提示录制进度。
 * @param engine RTCVideo 对象
 * @param type 录制流的流属性，参看 ByteRTCStreamIndex{@link #ByteRTCStreamIndex}
 ...
 *        通过 startPlayPublicStream:{@link #RTCVideo#startPlayPublicStream:} 开始播放公共流后，可以通过本回调获取公共流中包含的 SEI 信息。
 * @param engine RTCVideo 实例。
 * @param publicStreamId 公共流ID
 * @param message 收到的 SEI 消息内容
 ...
 *        关于订阅公共流，详见 startPlayPublicStream:{@link #RTCVideo#startPlayPublicStream:}。
 * @param engine RTCVideo 对象
 ...
 * @notes  <br>
 *      + 进房前后调用此方法均有效。<br>
 *      + 如果你调用此方法由内部采集切换至自定义采集，SDK 会自动关闭内部采集。然后，调用 pushExternalAudioFrame:{@link #RTCVideo#pushExternalAudioFrame:} 推送自定义采集的音频数据到 RTC SDK 用于传输。 <br>
 *      + 如果你调用此方法由自定义采集切换至内部采集，你必须再调用 startAudioCapture{@link #RTCVideo#startAudioCapture} 手动开启内部采集。
 */
 ...
 * @notes  <br>
 *      + 进房前后调用此方法均有效。<br>
 *      + 如果你调用此方法切换至自定义渲染，调用 pullExternalAudioFrame:{@link #RTCVideo#pullExternalAudioFrame:} 获取音频数据。 <br>
 */
 ...
 *       + 调用 stopAudioCapture{@link #RTCVideo#stopAudioCapture} 可以关闭音频采集设备，否则，SDK 只会在销毁引擎的时候自动关闭设备。  <br>
...
- (void)startAudioCapture;
  ...
  * @notes  <br>
  *       + 调用 startAudioCapture{@link #RTCVideo#startAudioCapture} 可以开启音频采集设备。  <br>
 ...
 *        + 开启推送多路视频流模式后，你可以调用 SetVideoEncoderConfig:{@link #RTCVideo#SetVideoEncoderConfig:} 为多路视频流分别设置编码参数。  <br>
 ...
 *        该接口支持设置一路视频流参数，设置多路参数请使用 SetVideoEncoderConfig:{@link #RTCVideo#SetVideoEncoderConfig:}。
 * @param max_solution 期望发布的最大分辨率视频流参数。参看 ByteRTCVideoEncoderConfig{@link #ByteRTCVideoEncoderConfig}。
 * @return 方法调用结果： <br>
 *        + 0：成功  <br>
 *        + !0：失败  <br>
 * @notes  <br>
 *        + 若调用该方法设置了期望发布的最大分辨率的流参数之前，已调用 enableSimulcastMode:{@link #RTCVideo#enableSimulcastMode:} 开启发布多路视频流的模式，SDK 会根据订阅端的设置自动调整发布的流数以及各路流的参数，其中最大分辨率为设置的分辨率，流数最多 4 条，具体参看[推送多路流](https://www.volcengine.com/docs/6348/70139)文档；否则仅发布该条最大分辨率的视频流。 <br>
 *        + 调用该方法设置多路视频流参数前，SDK 默认仅发布一条分辨率为 640px × 360px，帧率为 15fps 的视频流。  <br>
 *        + 该方法适用于摄像头采集的视频流，设置屏幕共享视频流参数参看 SetScreenVideoEncoderConfig:{@link #RTCVideo#SetScreenVideoEncoderConfig:}。
 */
/**
 * {en}
 * @type api
 ...
 *        + !0：失败  <br>
 * @notes  <br>
 *        + 该方法是否生效取决于是否同时调用了 enableSimulcastMode:{@link #RTCVideo#enableSimulcastMode:} 开启发布多路参数不同的视频流模式。若未开启推送多路流模式，但调用本方法设置了多个分辨率，SDK 默认发布分辨率最大的一条流，多个分辨率的设置会在开启推送多路流模式之后生效。  <br>
 *        + 调用该方法设置多路视频流参数前，SDK 默认仅发布一条分辨率为 640px × 360px，帧率为 15fps 的视频流。  <br>
 ...
 *        + < 0: 失败；  <br>
 * @notes  <br>
 * + 本接口在引擎创建后可调用，调用后立即生效。建议在调用 startVideoCapture{@link #RTCVideo#startVideoCapture} 前调用本接口。
...
- (int)updateRemoteVideoCanvas:(NSString * _Nonnull)roomid
  ...
  * @notes  <br>
  *       + 调用 stopVideoCapture{@link #RTCVideo#stopVideoCapture} 可以停止内部视频采集。否则，只有当销毁引擎实例时，内部视频采集才会停止。  <br>
  *       + 创建引擎后，无论是否发布视频数据，你都可以调用该方法开启内部视频采集。只有当（内部或外部）视频采集开始以后视频流才会发布。  <br>
  *       + 如果需要从自定义视频采集切换为内部视频采集，你必须先停止发布流，关闭自定义采集，再调用此方法手动开启内部采集。<br>
  *       + 内部视频采集使用的摄像头由 switchCamera:{@link #RTCVideo#switchCamera:} 接口指定。<br>
  ...
  * @notes  <br>
  *       + 调用 startVideoCapture{@link #RTCVideo#startVideoCapture} 可以开启内部视频采集。  <br>
 ...
 * @return  <br>
  ...
  *      + 1001: 特效 SDK 不支持该功能。  <br>
  *      + < 0: 调用失败。具体错误码，参看 [错误码表](https://www.volcengine.com/docs/5889/61813)。  <br>
 * @notes 在调用这个方法之前，你须先调用 enableVideoEffect:{@link #RTCVideo#enableVideoEffect:} 。
 */
 ...
 *      + 1001: 特效 SDK 不支持该功能。  <br>
 *      + < 0: 调用失败。具体错误码，参看 [错误码表](https://www.volcengine.com/docs/5889/61813)。  <br>
 * @notes  该接口会在 setVideoEffectNodes:{@link #RTCVideo#setVideoEffectNodes:} 设置的特效基础上叠加特效。
 */
 ...
 *      + 1001: 特效 SDK 不支持该功能。  <br>
 *      + < 0: 调用失败。具体错误码，参看 [错误码表](https://www.volcengine.com/docs/5889/61813)。  <br>
 * @notes 移除 setVideoEffectNodes:{@link #RTCVideo#setVideoEffectNodes:} 或 appendVideoEffectNodes:{@link #RTCVideo#appendVideoEffectNodes:} 设置的视频特效资源。
 */
...
- (int) setVideoEffectColorFilterIntensity:(float) intensity;
  ...
  *        + -1: 调用失败，未调用 initVirtualBackground:withModel:{@link #RTCVideo#initVirtualBackground:withModel:} 或调用失败，导致 license 验证失败。  <br>
  *        + -4: 调用失败，自定义背景图片打开失败。  <br>
  *        + -37: 调用失败，自定义背景图片解码失败。  <br>
  *        + -38: 调用失败，图片格式不支持。支持的图片格式是 jpg、jpeg、和 png。  <br>
  * @notes  <br>
  *       + 调用此接口前，需要先调用 initVirtualBackground:withModel:{@link #RTCVideo#initVirtualBackground:withModel:} 初始化 CV 环境；  <br>
 ...
 * @notes <br>
 *        + 本方法不能与高级视频特效接口共用。如已购买高级视频特效，建议调用 enableVideoEffect:{@link #RTCVideo#enableVideoEffect:} 使用高级特效、贴纸功能等。 <br>
 *        + 使用此功能需要集成特效 SDK，建议使用特效 SDK V4.3.1+ 版本。 <br>
 *        + 调用 setBeautyIntensity:withIntensity:{@link #RTCVideo#setBeautyIntensity:withIntensity:} 设置基础美颜强度。若在调用本方法前没有设置美颜强度，则初始美白、磨皮、锐化强度均为 0.5。 <br>
 ...
 *        + <0: 调用失败，特效 SDK 内部错误，具体错误码请参考[错误码表](https://www.volcengine.com/docs/5889/61813)。  <br>
 * @notes <br>
 *        + 若在调用 enableVideoEffect:{@link #RTCVideo#enableVideoEffect:} 前设置美颜强度，则对应美颜功能的强度初始值会根据设置更新。 <br>
 ...
 *        + < 0： 失败。
 * @notes <br>
 *        + 必须已调用 startVideoCapture{@link #RTCVideo#startVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能设置摄像头变焦倍数。<br>
 *        + 设置结果在调用 stopVideoCapture{@link #RTCVideo#stopVideoCapture} 关闭内部采集后失效。
 */
 ...
 * @return 最大变焦倍数
 * @notes 必须已调用 startVideoCapture{@link #RTCVideo#startVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能检测摄像头最大变焦倍数。
 */
 ...
 *        + true: 支持 <br>
 *        + false: 不支持
 * @notes 必须已调用 startVideoCapture{@link #RTCVideo#startVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能检测摄像头变焦能力。
 */
 ...
 *        + true: 支持 <br>
 *        + false: 不支持
 * @notes 必须已调用 startVideoCapture{@link #RTCVideo#startVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能检测闪光能力。
 */
 ...
 *        + < 0： 失败。
 * @notes <br>
 *        + 必须已调用 startVideoCapture{@link #RTCVideo#startVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能设置闪光灯。<br>
 *        + 设置结果在调用 stopVideoCapture{@link #RTCVideo#stopVideoCapture} 关闭内部采集后失效。
 */
 ...
 *        + true: 支持。 <br>
 *        + false: 不支持。
 * @notes 必须已调用 startVideoCapture{@link #RTCVideo#startVideoCapture} 使用 SDK 内部采集模块进行视频采集，才能检查摄像头是否支持手动对焦。
 */
 ...
 *        + < 0: 失败。
 * @notes <br>
 *        + 必须已调用 startVideoCapture{@link #RTCVideo#startVideoCapture} 使用 SDK 内部采集模块进行视频采集，并且使用 SDK 内部渲染时，才能设置对焦点。  <br>
 *        + 对焦点设置为画布中央（即`x`和`y`均取 0.5）时，恢复系统默认值。  <br>
 *        + 调用 stopVideoCapture{@link #RTCVideo#stopVideoCapture} 关闭内部采集后，设置的对焦点失效。
 */
 ...
 *        + true: 支持。 <br>
 *        + false: 不支持。
 * @notes 必须已调用 startVideoCapture{@link #RTCVideo#startVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能检查曝光点设置能力。
 */
 ...
 *        + < 0: 失败。
 * @notes <br>
 *        + 必须已调用 startVideoCapture{@link #RTCVideo#startVideoCapture} 使用 SDK 内部采集模块进行视频采集，并且使用 SDK 内部渲染时，才能设置曝光点。  <br>
 *        + 曝光点设置为画布中央（即`x`和`y`均取 0.5）时，恢复系统默认值。  <br>
 *        + 调用 stopVideoCapture{@link #RTCVideo#stopVideoCapture} 关闭内部采集后，设置的曝光点失效。
 */
 ...
 *        + < 0: 失败。
 * @notes <br>
 *        + 必须已调用 startVideoCapture{@link #RTCEngine#startVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能设置曝光补偿。  <br>
 *        + 调用 stopVideoCapture{@link #RTCVideo#stopVideoCapture} 关闭内部采集后，设置的曝光补偿失效。
 */
 ...
 *        + 消息发送成功后，远端会收到 rtcEngine:onSEIMessageReceived:andMessage:{@link #RTCVideoDelagate#rtcEngine:onSEIMessageReceived:andMessage:} 回调。
 ...
 *        + YES： 设置成功 <br>
 *        + NO： 设置失败
 * @notes 该方法可以在任意时间调用，建议在 joinRoomByKey:roomId:userInfo:rtcRoomConfig:{@link #RTCVideo#joinRoomByKey:roomId:userInfo:rtcRoomConfig:} 前。
 */
 ...
 * @region 音频管理
 * @author liuxiaowu
 * @brief 设置当前音频播放路由。默认使用 setDefaultAudioRoute:{@link #RTCVideo#setDefaultAudioRoute:} 中设置的音频路由。  <br>
 *        音频播放路由发生变化时，会收到 rtcEngine:onAudioRouteChanged:{@link #RTCVideoDelegate#rtcEngine:onAudioRouteChanged:} 回调。  <br>
 ...
 * @notes 要设置音频路由，详见 setAudioRoute:{@link #RTCVideo#setAudioRoute:}。
 */
 ...
 *       + 调用 stopLiveTranscoding:{@link #RTCVideo#stopLiveTranscoding:} 停止转推直播
 */
 ...
 * @author qipengxiang
 * @brief 发布一路公共流<br>
 *        公共流是指不属于任何房间，也不属于任何用户的媒体流。使用同一 `appID` 的用户，可以调用 startPlayPublicStream:{@link #RTCVideo#startPlayPublicStream:} 获取和播放指定的公共流。
 ...
 *        关于发布公共流，查看 startPushPublicStream:withLayout:{@link #RTCVideo#startPushPublicStream:withLayout:}。
 * @param publicStreamId 公共流 ID<br>
 *                  指定的流必须为当前用户所发布。
 * @return
 *        + 0: 成功<br>
 ...
 *              - 调用 setPublicStreamVideoCanvas:withCanvas:{@link #RTCVideo#setPublicStreamVideoCanvas:withCanvas:} 绑定内部渲染视图：<br>
 *              - 调用 setPublicStreamVideoSink:withSink:withPixelFormat:{@link #RTCVideo#setPublicStreamVideoSink:withSink:withPixelFormat:} 绑定自定义渲染视图：<br>
 ...
 *        + 订阅公共流之后，可以通过调用 stopPlayPublicStream:{@link RTCVideo#stopPlayPublicStream:} 接口取消订阅公共流。
 */
 ...
 * @author qipengxiang
 * @brief 取消订阅指定公共流<br>
 *        关于订阅公共流，查看 startPlayPublicStream:{@link #RTCVideo#startPlayPublicStream:}。
 * @param publicStreamId 公共流ID
 * @return  <br>
 *        + 0：成功  <br>
 *        + !0：失败  <br>
 ...
 *         + YES 推送成功  <br>
 *         + NO 推送失败  <br>
 * @notes 推送外部视频帧前，必须调用 setVideoSourceType:WithStreamIndex:{@link #RTCVideo#setVideoSourceType:WithStreamIndex:} 开启外部视频源采集。
 */
 ...
 * @notes 开启音频回调并调用 setAudioFrameObserver:{@link #RTCVideo#setAudioFrameObserver:} 后，ByteRTCAudioFrameObserver{@link #ByteRTCAudioFrameObserver} 会收到对应的音频回调。两者调用顺序没有限制且相互独立。  <br>
 */
 ...
 * @notes 该方法需要在调用 enableAudioFrameCallback:format:{@link #RTCVideo#enableAudioFrameCallback:format:} 之后调用。
 */
 ...
 *        + 0: 方法调用成功 <br>
 *        + < 0: 方法调用失败 <br>
 * @notes 注册音频数据回调观察者并调用 enableAudioFrameCallback:format:{@link #RTCVideo#enableAudioFrameCallback:format:} 后，ByteRTCAudioFrameObserver{@link #ByteRTCAudioFrameObserver} 会收到对应的音频回调。
 */
...
* @author majun.lvhiei
* @brief 注册自定义音频处理器。<br>
*        注册完成后，你可以调用 enableAudioProcessor:audioFormat:{@link #RTCVideo#enableAudioProcessor:audioFormat:}，对本地采集或接收到的远端音频进行处理。
...
* @notes <br>
*        + 在调用此接口前，你需要调用 registerAudioProcessor:{@link #RTCVideo#registerAudioProcessor:} 注册自定义音频处理器。<br>
*        + 要关闭音频自定义处理，调用 disableAudioProcessor:{@link #RTCVideo#disableAudioProcessor:}。
*/
 ...
 *          + < 0: 设置失败  <br>
 * @notes  <br>
 *       + 推送外部音频数据前，必须先调用 setAudioSourceType:{@link #RTCVideo#setAudioSourceType:} 开启自定义采集。<br>
 *       + 你必须每隔 10 毫秒推送一次外部采集的音频数据。单次推送的 samples (音频采样点个数）应该为 audioFrame.sampleRate / 100。比如设置采样率为 48000 时， 每次应该推送 480 个采样点。  <br>
 *       + 本方法中设置的 sampleRate（音频采样率） 和 channel（音频声道） 必须与 setAudioSourceType:{@link #RTCVideo#setAudioSourceType:} 中设置的一致。  <br>
 ...
 *        + < 0: 设置失败  <br>
 * @notes  <br>
 *       + 拉取外部音频数据前，必须先调用 setAudioRenderType:{@link #RTCVideo#setAudioRenderType:} 开启自定义渲染。<br>
 *       + 你应当每隔 10 毫秒拉取一次音频数据。samples (音频采样点个数）应该为 audioFrame.sampleRate / 100。如设置采样率为 48000 时， 每次应该拉取 480 个采样点。  <br>
 *       + 本方法中设置的 sampleRate（音频采样率） 和 channel（音频声道） 必须与 setAudioRenderType:{@link #RTCVideo#setAudioRenderType:} 中设置的一致，和远端音频采集设置无关。  <br>
 *       + 音频采样格式为 S16。音频缓冲区内的数据格式必须为 PCM 数据，其容量大小应该为 audioFrame.samples × audioFrame.channel × 2。  <br>
 *       + 该函数运行在用户级线程内。若同时运行其他进程，将导致本进程中断。  <br>
 */
 ...
 * @notes  <br>
 *        + 需要在调用 joinRoomByKey:roomId:userInfo:rtcRoomConfig:{@link #RTCVideo#joinRoomByKey:roomId:userInfo:rtcRoomConfig:} 之前调用，joinRoomByKey:roomId:userInfo:rtcRoomConfig:{@link #RTCVideo#joinRoomByKey:roomId:userInfo:rtcRoomConfig:}之后调用该方法无效。
 */
 ...
 *        + 必须在创建 RtcEngine 实例 initWithAppId:delegate:parameters:{@link #RTCVideo#initWithAppId:delegate:parameters:} 前设置。  <br>
 ...
 *        + < 0：失败
 * @notes  <br>
 *        + 该方法仅在调用 enableSimulcastMode:{@link #RTCVideo#enableSimulcastMode:} 开启了发送多路视频流的情况下生效。  <br>
 ...
 *       + 该方法必须在调用 joinRoomByKey:roomId:userInfo:rtcRoomConfig:{@link #RTCVideo#joinRoomByKey:roomId:userInfo:rtcRoomConfig:}
 ...
 * @notes 必须在调用 initWithAppId:delegate:parameters:{@link #RTCVideo#initWithAppId:delegate:parameters:} 之后立即调用此方法。在引擎实例的生命周期中，此方法只需要调用一次。  <br>
 */
 ...
 *      + 采集后，你还需要调用 publishScreen:{@link #RTCVideo#publishScreen:} 发布采集到的屏幕音视频。<br>
 ...
 * @param type 媒体类型，参看 ByteRTCScreenMediaType{@link #ByteRTCScreenMediaType}。
 * @notes <br>
 *      + 在 startScreenCapture:bundleId:{@link #RTCVideo#startScreenCapture:bundleId:} 后调用该方法。
 ...
 * @notes <br>
 *     + 本方法只会停止本地屏幕采集，并不会影响屏幕流的发布状态，再次调用 startScreenCapture:bundleId:{@link #RTCVideo#startScreenCapture:bundleId:} 开始屏幕音视频内部采集。
 ...
 * @notes 在 startScreenCapture:bundleId:{@link #RTCVideo#startScreenCapture:bundleId:} 后调用该方法。
 */
 ...
 * @notes  <br>
 *        + 调用 startFileRecording:withRecordingConfig:type:{@link #RTCVideo#startFileRecording:withRecordingConfig:type:} 开启本地录制后，你必须调用该方法停止录制。  <br>
 ...
 * @notes  <br>
 *       + 调用本接口登出前，必须先调用 login:uid:{@link #RTCVideo#login:uid:} 登录  <br>
 ...
 *        调用 login:uid:{@link #RTCVideo#login:uid:} 方法登录时，如果使用了过期的 Token 将导致登录失败，并会收到 rtcEngine:onLoginResult:errorCode:elapsed:{@link #ByteRTCEngineDelegate#rtcEngine:onLoginResult:errorCode:elapsed:} 回调通知，错误码为 ByteRTCLoginErrorCodeInvalidToken。此时需要重新获取 Token，并调用此方法更新 Token。
 * @param token  <br>
 *        更新的动态密钥  <br>
 * @notes  <br>
 *       + 如果 Token 无效导致登录失败，则调用此方法更新 Token 后，SDK 会自动重新登录，而用户不需要自己调用 login:uid:{@link #RTCVideo#login:uid:} 方法。  <br>
 ...
 * @author hanchenchen.c
 * @brief 设置应用服务器参数  <br>
 *        客户端调用 sendServerMessage:{@link #RTCVideo#sendServerMessage:} 或 sendServerBinaryMessage:{@link #RTCVideo#sendServerBinaryMessage:} 发送消息给应用服务器之前，必须需要设置有效签名和应用服务器地址。
 * @param signature  <br>
 *        动态签名  <br>
 ...
 *        应用服务器的地址
 * @notes  <br>
 *       + 用户必须调用 login:uid:{@link #RTCVideo#login:uid:} 登录后，才能调用本接口。  <br>
 ...
 *        需要查询的用户 ID
 * @notes  <br>
 *       + 必须调用 login:uid:{@link #RTCVideo#login:uid:} 登录后，才能调用本接口。  <br>
 ...
 *        + -2：发送失败，userId 为空
 * @notes  <br>
 *       + 在发送房间外文本消息前，必须先调用 login:uid:{@link #RTCVideo#login:uid:} 完成登录。  <br>
 ...
 *        + -2：发送失败，userId 为空
 * @notes  <br>
 *       + 在发送房间外二进制消息前，必须先调用 login:uid:{@link #RTCVideo#login:uid:} 完成登录。  <br>
 ...
 * @notes  <br>
 *       + 在向应用服务器发送文本消息前，必须先调用 login:uid:{@link #RTCVideo#login:uid:} 完成登录，随后调用 setServerParams:url:{@link #RTCVideo#setServerParams:url:} 设置应用服务器。  <br>
 ...
 *       + 若文本消息发送成功，则之前调用 setServerParams:url:{@link #RTCVideo#setServerParams:url:} 设置的应用服务器会收到该条消息。
 */
 ...
 *        + -1：发送失败，RtcEngine 实例未创建
 * @notes  <br>
 *       + 在向应用服务器发送二进制消息前，先调用 login:uid:{@link #RTCVideo#login:uid:} 完成登录，随后调用 setServerParams:url:{@link #RTCVideo#setServerParams:url:} 设置应用服务器。  <br>
 ...
 *       + 若二进制消息发送成功，则之前调用 setServerParams:url:{@link #RTCVideo#setServerParams:url:} 设置的应用服务器会收到该条消息。
 */
 ...
 *      + 你应该在 publishScreen:{@link #RTCVideo#publishScreen:} 前，调用此方法。否则，你将收到 rtcEngine:onWarning:{@link #ByteRTCEngineDelegate#rtcEngine:onWarning:} 的报错：`ByteRTCWarningSetScreenAudioSourceTypeFailed`。 <br>
 *      + 如果设定为内部采集，你必须再调用 startScreenCapture:bundleId:{@link #RTCVideo#startScreenCapture:bundleId:} 开始采集。
 *      + 如果设定为自定义采集，你必须再调用 pushScreenAudioFrame:{@link #RTCVideo#pushScreenAudioFrame:} 将自定义采集到的屏幕音频帧推送到 RTC SDK。<br>
 *      + 无论是内部采集还是自定义采集，你都必须调用 publishScreen:{@link #RTCVideo#publishScreen:} 将采集到的屏幕音频推送到远端。
 */
 ...
 * @notes 你应该在 publishScreen:{@link #RTCVideo#publishScreen:} 之前，调用此方法。否则，你将收到 rtcEngine:onWarning:{@link #ByteRTCEngineDelegate#rtcEngine:onWarning:} 的报错：`ByteRTCWarningSetScreenAudioStreamIndexFailed`
 */
 ...
 *        + < 0: 设置失败  <br>
 * @notes  <br>
 *        + 调用此接口推送屏幕共享时的自定义采集的音频数据前，必须调用 setScreenAudioSourceType:{@link #RTCVideo#setScreenAudioSourceType:} 开启屏幕音频自定义采集。  <br>
 ...
 *        + 调用此接口将自定义采集的音频帧推送到 RTC SDK 后，你必须调用 publishScreen:{@link #RTCVideo#publishScreen:} 将采集到的屏幕音频推送到远端。在调用 publishScreen:{@link #RTCVideo#publishScreen:} 前，推送到 RTC SDK 的音频帧信息会丢失。
 */
 ...
 * @param videoIndex 对应的编码流下标，从 0 开始，如果调用 SetVideoEncoderConfig:{@link #RTCVideo#SetVideoEncoderConfig:} 设置了多路流，此处数量须与之保持一致
 ...
 *        + 该函数运行在用户调用线程内  <br>
 *        + 推送自定义编码视频帧前，必须调用 setVideoSourceType:WithStreamIndex:{@link #RTCVideo#setVideoSourceType:WithStreamIndex:} 将视频输入源切换至自定义编码视频源。
 */
 ...
 *        + 当你想要对远端流进行自定义解码时，你需要先调用 registerRemoteEncodedVideoFrameObserver:{@link #RTCVideo#registerRemoteEncodedVideoFrameObserver:} 注册远端视频流监测器，然后再调用该接口将解码方式设置为自定义解码。监测到的视频数据会通过 onRemoteEncodedVideoFrame:withEncodedVideoFrame:{@link #ByteRTCRemoteEncodedVideoFrameObserver#onRemoteEncodedVideoFrame:withEncodedVideoFrame:} 回调出来。
 */
 ...
 *        + 该方法适用于调用 setVideoDecoderConfig:withVideoDecoderConfig:{@link #RTCVideo#setVideoDecoderConfig:withVideoDecoderConfig:} 开启自定义解码功能后，并且自定义解码失败的情况下使用
 */
 ...
 * @notes  <br>
 *        + 调用该方法开始音视频回路检测后，你可以调用 stopEchoTest{@link #RTCVideoDelagate#stopEchoTest} 立即结束测试，也可等待测试 60s 后自动结束，以更换设备进行下一次测试，或进房。  <br>
 *        + 在该方法之前调用的所有跟设备控制、流控制相关的方法均在开始检测时失效，在结束检测后恢复生效。  <br>
 *        + 在调用 startEchoTest:playDelay:{@link #RTCVideoDelagate#startEchoTest:playDelay:} 和 stopEchoTest{@link #RTCVideoDelagate#stopEchoTest} 之间调用的所有跟设备采集、流控制、进房相关的方法均不生效，并会收到 rtcEngine:onWarning:{@link #RTCVideoDelagate#rtcEngine:onWarning:} 回调，提示警告码为 `ByteRTCWarningCodeInEchoTestMode`。  <br>
 *        + 音视频回路检测的结果会通过 rtcEngine:onEchoTestResult:{@link #RTCVideoDelagate#rtcEngine:onEchoTestResult:} 回调通知。
 */
 ...
 * @author qipengxiang
 * @brief 停止音视频回路测试。  <br>
 *        调用 startEchoTest:playDelay:{@link #RTCVideoDelagate#startEchoTest:playDelay:} 开启音视频回路检测后，你必须调用该方法停止检测。
 * @return 方法调用结果：  <br>
 *        + 0：成功  <br>
 *        + -1：失败，未开启回路检测
 ...
 *        + 要关闭云代理，调用 stopCloudProxy{@link #RTCVideo#stopCloudProxy}。
 */
 ...
 * @author daining.nemo
 * @brief 关闭云代理
 * @notes 要开启云代理，调用 startCloudProxy:{@link #RTCVideo#startCloudProxy:}
 */
- (void)stopCloudProxy;
@end
