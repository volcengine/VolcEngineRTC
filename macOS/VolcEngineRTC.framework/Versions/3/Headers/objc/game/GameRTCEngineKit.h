 ...
 *        + 0: 成功  <br>
 *        + !0: 失败  <br>
 *        + 具体原因参看错误码 GameRTCJoinRoomErrorCode{@link #GameRTCJoinRoomErrorCode} 以及警告码 GameRTCEngineWarningCode{@link #GameRTCEngineWarningCode} 
 * @param isRejoined 是否为重连进房：<br>
 *        + true: 是重连进房  <br>
 *        + false：是初次进房
 * @param elapsed 加入房间耗时。用户从调用 joinRoom:uid:token:config:{@link #GameRTCEngineKit#joinRoom:uid:token:config:} 方法到加入房间成功所经历的时间间隔，单位为 ms。
 */
- (void)rtcEngine:(GameRTCEngineKit *)engine onJoinRoomResult:(NSString *)roomId uid:(NSString *)uid errorCode:(GameRTCJoinRoomErrorCode)errorCode isRejoined:(BOOL)isRejoined elapsed:(int)elapsed;
 ...
 * @region 房间管理
 * @brief 更新进房 token。
 *        在进房时如果收到错误码为 GameRTCJoinRoomErrorCodeJoinRoomInvalidToken 的 rtcEngine:onJoinRoomResult:uid:errorCode:isRejoined:elapsed:{@link #GameRTCEngineDelegate#rtcEngine:onJoinRoomResult:uid:errorCode:isRejoined:elapsed:} 回调，表示 token 过期导致进房失败。
 ...
 *        - 关于范围语音，参看 enableRangeAudio{@link #enableRangeAudio}。
 */
 ...
 *         + -1：失败。引擎已经被销毁或者未创建，或者是房间 ID 为空
 */
- (int)updateOrientation:(GameRTCOrientationInfo *)orientation room:(NSString *)roomId;
