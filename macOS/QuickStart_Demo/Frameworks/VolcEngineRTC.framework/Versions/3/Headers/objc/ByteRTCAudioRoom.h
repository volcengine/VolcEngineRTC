...
#import "rtc/ByteRTCCommonDefines.h"
...
- (void)rtcRoom:(ByteRTCAudioRoom *_Nonnull)rtcRoom onLeaveRoom:(ByteRTCRoomStats * _Nonnull)stats;
  ...
  *        调用 updateToken:{@link #ByteRTCAudioRoom#updateToken:} 更新 Token。否则 Token 过期后，用户将被移出房间无法继续进行音频通话。
  */
 ...
 * @author shenpengliang
 * @brief 更新 Token。  <br>
 *        用于加入房间的 Token 有一定的有效期。在 Token 过期前 30 秒，会收到 onTokenWillExpire:{@link #ByteRTCAudioRoomDelegate#onTokenWillExpire:} 回调，此时需要重新获取 Token，并调用此方法更新 Token，否则用户将因为 Token 过期被移出房间。 <br>
 *        用户调用 joinRoom:userInfo:roomConfig:{@link #ByteRTCAudioRoom#joinRoom:userInfo:roomConfig:} 方法加入房间或断网重连进入房间时，如果 Token 过期或无效，将导致加入房间失败，并会收到 rtcRoom:onRoomStateChanged:withUid:state:extraInfo:{@link #ByteRTCAudioRoomDelegate#rtcRoom:onRoomStateChanged:withUid:state:extraInfo:} 回调通知，
 ...
 *         +  0: 方法调用成功。  <br>
 *         + < 0: 方法调用失败。  <br>
 * @notes 当 Token 过期时，用户将被移出房间并将收到 rtcEngine:onError:{@link #ByteRTCAudioDelegate#rtcEngine:onError:} 回调， `ERROR_CODE_EXPIRED_TOKEN`，此时需要重新获取 Token，并调用 joinRoom:userInfo:roomConfig:{@link #ByteRTCAudioRoom#joinRoom:userInfo:roomConfig:} 重新加入房间。
 */
 ...
 *        + -1: 发布失败
 * @notes <br>
 *        + 调用 setUserVisibility:{@link #ByteRTCAudioRoom#setUserVisibility:} 方法将自身设置为不可见后无法调用该方法，需将自身切换至可见后方可调用该方法发布摄像头音频流。 <br>
 ...
 */
- (void)unsubscribeStream:(NSString *_Nonnull)userId;
/** 
 * @type api
 * @region 多房间
 * @author shenpengliang
 ...
 *        + 开始转发后，目标房间中的用户将接收到本地用户进房 rtcRoom:onUserJoined:elapsed:{@link #ByteRTCAudioRoomDelegate#rtcRoom:onUserJoined:elapsed:} 和发流 rtcRoom:onUserJoined:elapsed:{@link #ByteRTCAudioRoomDelegate#rtcRoom:onUserJoined:elapsed:} 的回调。
