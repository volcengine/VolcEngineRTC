 ...
 * @notes  <br>
 *       + 如果已经通过 preloadAudioMixing:filePath:{@link #ByteRTCAudioMixingManager#preloadAudioMixing:filePath:} 将文件加载至内存，确保此处的 ID
 * 与预加载时设置的 ID 相同。  <br>
 ...
 *       + 本方法的混音数据来源于外部文件，而 enableAudioMixingFrame:type:{@link #ByteRTCAudioMixingManager#enableAudioMixingFrame:type:}
 ...
 *        混音 ID
 * @notes  <br>
 *       + 调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:}
 * 方法开始播放音频文件及混音后，可以调用本方法停止播放音频文件及混音。  <br>
 ...
 * @notes  <br>
 *       + 调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:}
 * 方法开始播放音频文件及混音后，可以通过调用该方法暂停播放音频文件。  <br>
 ...
 *        如果先调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:}，再使用相同的 ID 调用本方法
 * ，会先回调 `onAudioMixingStateChanged` 通知上一个混音停止，然后加载后一个混音。  <br> 调用本方法预加载 A.mp3 后，如果需要使用相同的 ID 调用
 * startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:} 播放 B.mp3，请先调用 unloadAudioMixing:{@link
 * #ByteRTCAudioMixingManager#unloadAudioMixing:} 卸载 A.mp3。
 ...
 *        </table>
 * @notes  <br>
 *       + 本方法只是预加载指定音频文件，只有调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:}
 * 方法才开始播放指定音频文件。 <br>
 ...
 *       + < 0: 失败
 * @notes 调用本方法获取音频文件时长前，需要先调用 preloadAudioMixing:filePath:{@link #ByteRTCAudioMixingManager#preloadAudioMixing:filePath:} 或
 * startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:}。
 */
 ...
 *        + < 0: 失败
 * @notes 调用本方法获取音频文件播放进度前，需要先调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:}
 * 开始播放音频文件。
 */
 ...
 *        超出取值范围则设置失败，并且会触发 `onAudioMixingStateChanged` 回调，提示 ByteRTCAudioMixingState{@link #ByteRTCAudioMixingState} 状态为
 * `AUDIO_MIXING_STATE_FAILED` 混音播放失败，ByteRTCAudioMixingError{@link #ByteRTCAudioMixingError} 错误码为 `AUDIO_MIXING_ERROR_ID_TYPE_INVALID_PITCH`
 * 设置混音文件音调不合法。
 * @notes 本方法需要在调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:} 开始播放音频文件后、调用
 * stopAudioMixing:{@link #ByteRTCAudioMixingManager#stopAudioMixing:} 停止播放音频文件前使用，否则会触发 `onAudioMixingStateChanged` 回调报错
 */
 ...
 *        超出取值范围则设置失败，你会收到 `onAudioMixingStateChanged` 回调，提示 ByteRTCAudioMixingState{@link #ByteRTCAudioMixingState} 状态为
 * `ByteRTCAudioMixingStateFailed` 混音播放失败，ByteRTCAudioMixingError{@link #ByteRTCAudioMixingError} 错误码为 `ByteRTCAudioMixingErrorInValidPlaybackSpeed`
 * 设置混音文件的播放速度不合法。
 * @notes   <br>
 *        + 暂不支持对 PCM 音频数据进行变速调整。  <br>
 *        + 你需要在调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:}
 * 开始混音，并且收到`onAudioMixingStateChanged` 回调提示 ByteRTCAudioMixingState{@link #ByteRTCAudioMixingState} 状态为
 * `ByteRTCAudioMixingStatePlaying`，ByteRTCAudioMixingError{@link #ByteRTCAudioMixingError} 错误码为 `AUDIO_MIXING_ERROR_OK` 之后调用该方法。  <br>
 *        + 在 stopAudioMixing:{@link #ByteRTCAudioMixingManager#stopAudioMixing:} 停止混音或 unloadAudioMixing:{@link
 * #ByteRTCAudioMixingManager#unloadAudioMixing:} 卸载音频文件后调用该 API，会收到状态为 `ByteRTCAudioMixingStateFailed` 错误码为
 ...
 * @notes 建议在 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:}
 * 开始播放音频文件之前调用该接口，以免播放过程中的音量突变导致听感体验下降。
 */
 ...
 *       + interval 的值为大于 0 的 10 的倍数，当设置的值不能被 10 整除时，则默认向上取整 10，如设为 52ms 时会默认调整为 60ms。设置完成后 SDK
 * 将会按照设置的时间间隔触发 `onAudioMixingPlayingProgress` 回调。  <br>
 *       + interval 的值小于等于 0 时，不会触发进度回调。  <br>
 * @notes 本方法需要在调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:} 开始播放音频文件后、调用
 * stopAudioMixing:{@link #ByteRTCAudioMixingManager#stopAudioMixing:} 停止播放音频文件前使用，否则会触发 `onAudioMixingStateChanged` 回调报错。  <br>
 *        若想在音乐文件开始播放前设置播放进度回调间隔，你需调用 startAudioMixing:filePath:config:{@link
 * #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:} 在 ByteRTCAudioMixingConfig{@link #ByteRTCAudioMixingConfig}
 ...
 *        + < 0：方法调用失败。
 * @notes  <br>
 *       + 调用本方法获取音频文件的音轨前，需要先调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:}
 * 开始播放音频文件。<br>
 ...
 * @notes  <br>
 *       + 调用本方法设置音频文件的音轨前，需要先调用 startAudioMixing:filePath:config:{@link #ByteRTCAudioMixingManager#startAudioMixing:filePath:config:}
 * 开始播放音频文件。<br>
 *       + 此方法对 enableAudioMixingFrame:type:{@link #ByteRTCAudioMixingManager#enableAudioMixingFrame:type:} 播放的音乐无效。
 */
