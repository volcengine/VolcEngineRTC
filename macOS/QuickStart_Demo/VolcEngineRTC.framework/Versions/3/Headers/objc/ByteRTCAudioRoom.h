 ...
 */
- (void)rtcRoom:(ByteRTCAudioRoom * _Nonnull)rtcRoom onAudioStreamBanned:(NSString * _Nonnull)uid isBanned:(BOOL)banned;
 ...
 * @type callback
 * @region 多房间
 * @brief 调用 setRoomExtraInfo{@link #ByteRTCAudioRoom#setRoomExtraInfo} 结果回调。
 * @param rtcRoom ByteRTCAudioRoom 对象。
 * @param taskId 本次调用的任务编号。
 * @param error 设置房间附加信息结果，详见 SetRoomExtraInfoResult{@link #ByteRTCSetRoomExtraInfoResult}
 */
- (void)rtcRoom:(ByteRTCAudioRoom * _Nonnull)rtcRoom onSetRoomExtraInfoResult:(NSInteger)taskId
          error:(ByteRTCSetRoomExtraInfoResult)error;
 ...
 *        房间内其他用户调用 setRoomExtraInfo{@link #ByteRTCAudioRoom#setRoomExtraInfo} 设置房间附加信息，收到此回调。<br>
 ...
 */
- (void)rtcRoom:(ByteRTCAudioRoom * _Nonnull)rtcRoom onRoomExtraInfoUpdate:(NSString *_Nonnull)key
        value:(NSString *_Nonnull)value lastUpdateUserId:(NSString *_Nonnull)lastUpdateUserId lastUpdateTimeMs:(NSInteger)lastUpdateTimeMs;
 ...
 */
- (void)rtcRoom:(ByteRTCAudioRoom * _Nonnull)rtcRoom onForwardStreamStateChanged:(NSArray<ForwardStreamStateInfo *> * _Nonnull)infos;
/** 
 * @type callback
 * @region 多房间
 * @brief 跨房间音频流转发事件回调
 ...
 */
- (void)rtcRoom:(ByteRTCAudioRoom * _Nonnull)rtcRoom onForwardStreamEvent:(NSArray<ForwardStreamEventInfo *> * _Nonnull)infos;
/**  
 * @type callback
 * @region 字幕翻译服务
 * @brief  字幕状态发生改变回调。 <br>
 *         当用户调用 startSubtitle:{@link #ByteRTCAudioRoom#startSubtitle:} 和 stopSubtitle{@link #ByteRTCAudioRoom#stopSubtitle} 使字幕状态发生改变或出现错误时，触发该回调。 
 * @param rtcRoom ByteRTCAudioRoom 实例。
 ...
 */
- (void)rtcRoom:(ByteRTCAudioRoom *_Nonnull)rtcRoom onSubtitleStateChanged:(ByteRTCSubtitleState)state
 ...
 * @region 字幕翻译服务
 * @brief  字幕相关内容回调。 <br>
 *         当用户调用 startSubtitle:{@link #ByteRTCAudioRoom#startSubtitle:} 后会收到此回调，通知字幕的相关信息。
 * @param rtcRoom ByteRTCAudioRoom 实例。
 * @param subtitles  字幕消息内容。参看 ByteRTCSubtitleMessage{@link #ByteRTCSubtitleMessage}。
 */
- (void)rtcRoom:(ByteRTCAudioRoom *_Nonnull)rtcRoom onSubtitleMessageReceived:(NSArray<ByteRTCSubtitleMessage*> * _Nonnull)subtitles;
@end
 ...
 *        + YES: 可以在房间内发布音频流，房间中的其他用户将收到用户的行为通知，例如进房、开启音频采集和退房。  <br>
 *        + NO: 不可以在房间内发布音频流，房间中的其他用户不会收到用户的行为通知，例如进房、开启音频采集和退房。
 * @notes  <br>
 *       + 该方法在加入房间前后均可调用。 <br>
 *       + 在房间内调用此方法，房间内其他用户会收到相应的回调通知：<br>
 *            - 从 NO 切换至 YES 时，房间内其他用户会收到 rtcRoom:onUserJoined:elapsed:{@link #ByteRTCAudioRoomDelegate#rtcRoom:onUserJoined:elapsed:} 回调通知；  <br>
 *            - 从 YES 切换至 NO 时，房间内其他用户会收到 rtcRoom:onUserLeave:reason:{@link #ByteRTCAudioRoomDelegate#rtcRoom:onUserLeave:reason:} 回调通知。  <br>
 ...
 *      + 请勿同时调用 updateToken:{@link #ByteRTCAudioRoom#updateToken:} 和 joinRoom:userInfo:roomConfig:{@link #ByteRTCAudioRoom#joinRoom:userInfo:roomConfig:} 方法更新 Token。若因 Token 过期或无效导致加入房间失败或已被移出房间，你应该在获取新的有效 Token 后调用 joinRoom:userInfo:roomConfig:{@link #ByteRTCAudioRoom#joinRoom:userInfo:roomConfig:} 重新加入房间。
 */
 ...
 * @notes  <br>
 *       + 在设置房间附加信息前，必须先调用 joinRoom:userInfo:roomConfig:{@link #ByteRTCAudioRoom#joinRoom:userInfo:roomConfig:} 加入房间。  <br>
 *       + 调用该方法后，会收到一次 rtcRoom:onSetRoomExtraInfoResult:error:{@link #ByteRTCAudioRoomDelegate#rtcRoom:onSetRoomExtraInfoResult:error:} 回调，提示设置结果。  <br>
 *       + 调用该方法成功设置附加信息后，同一房间内的其他用户会收到关于该信息的回调 rtcRoom:onRoomExtraInfoUpdate:value:lastUpdateUserId:lastUpdateTimeMs:{@link #ByteRTCAudioRoomDelegate#rtcRoom:onRoomExtraInfoUpdate:value:lastUpdateUserId:lastUpdateTimeMs:}。<br>
