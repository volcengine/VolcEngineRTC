...
*/
- (void)rtcEngine:(ByteRTCAudio *_Nonnull)engine onExtensionAccessError:(NSString * _Nonnull)extensionName msg:(NSString * _Nonnull)msg;
/** 
 * @type callback
 * @region 引擎管理
...
#pragma mark - Audio Management Delegate Methods
  ...
  */
 - (void)rtcEngine:(ByteRTCAudio * _Nonnull)engine onLocalAudioPropertiesReport:(NSArray<ByteRTCLocalAudioPropertiesInfo *> * _Nonnull)audioPropertiesInfos;
 /** 
  * @type callback
  * @region 音频管理
  ...
  *       + [205,255] 为高音量。  <br>
  */
 - (void)rtcEngine:(ByteRTCAudio * _Nonnull)engine onRemoteAudioPropertiesReport:(NSArray<ByteRTCRemoteAudioPropertiesInfo *> * _Nonnull)audioPropertiesInfos
 ...
 * @param data 消息内容。
 */
- (void)rtcEngine:(ByteRTCAudio * _Nonnull)engine onStreamSyncInfoReceived:(ByteRTCRemoteStreamKey* _Nonnull)remoteStreamKey
 ...
 * @notes <br>
 *        + 通话前调用 startHardwareEchoDetection{@link #ByteRTCAudio#startHardwareEchoDetection} 后，将触发本回调返回检测结果。<br>
 *        + 建议在收到检测结果后，调用 stopHardwareEchoDetection{@link #ByteRTCAudio#stopHardwareEchoDetection} 停止检测，释放对音频设备的占用。<br>
 *        + 如果 SDK 在通话中检测到回声，将通过 rtcEngine:onAudioDeviceWarning:deviceType:deviceWarning:{@link #ByteRTCAudioDelegate#rtcEngine:onAudioDeviceWarning:deviceType:deviceWarning:} 回调 `kMediaDeviceWarningLeakEchoDetected`。
 */
- (void)rtcEngine:(ByteRTCAudio *_Nonnull)engine
    onHardwareEchoDetectionResult:(ByteRTCHardwareEchoDetectionResult)result NS_SWIFT_NAME(rtcEngine(_:onHardwareEchoDetectionResult:));
 ...
 */
- (void)rtcEngine:(ByteRTCAudio * _Nonnull)engine onNetworkDetectionStopped:(ByteRTCNetworkDetectionStopReason)err_code;
/** 
 * @type callback
...
/** 
 * @type callback
 * @deprecated 在3.52及之后废弃，将在3.57删除，使用 rtcEngine:onLocalProxyStateChanged:withProxyState:withProxyError:{@link #ByteRTCAudioDelegate#rtcEngine:onLocalProxyStateChanged:withProxyState:withProxyError:} 替换
 * @region 网络代理
 ...
 * @type callback
 * @region proxy
 * @brief 本地代理状态发生改变回调。调用 setLocalProxy:{@link #ByteRTCAudio#setLocalProxy:} 设置本地代理后，SDK 会触发此回调，返回代理连接的状态。  <br>
 ...
 */
- (void)rtcEngine:(ByteRTCAudio * _Nonnull)engine onLocalProxyStateChanged:(ByteRTCLocalProxyType)type withProxyState:(ByteRTCLocalProxyState)state 
 ...
 * @param recording_info 录制文件的详细信息，参看 ByteRTCRecordingInfo{@link #ByteRTCRecordingInfo}
 */
- (void)rtcEngine:(ByteRTCAudio* _Nonnull)engine onRecordingProgressUpdate:(ByteRTCRecordingProgress* _Nonnull)process
   recording_info:(ByteRTCRecordingInfo* _Nonnull)recording_info;
/** 
*  @type callback
 ...
 * @type api
 * @region 音频管理
 * @brief 启用音频信息提示。开启后，你可以收到 rtcEngine:onLocalAudioPropertiesReport:{@link #ByteRTCAudioDelegate#rtcEngine:onLocalAudioPropertiesReport:} 和 rtcEngine:onRemoteAudioPropertiesReport:totalRemoteVolume:{@link #ByteRTCAudioDelegate#rtcEngine:onRemoteAudioPropertiesReport:totalRemoteVolume:}。
 ...
 * @notes <br>
 *      + 本接口仅支持在 `ByteRTCAudioScenarioCommunication` 音频场景下使用。你可以通过调用 setAudioScenario:{@link #ByteRTCAudio#setAudioScenario:} 切换音频场景。
 *      + 连接/移除音频设备后，音频路由可能发生切换。切换逻辑参见[移动端设置音频路由](https://www.volcengine.com/docs/6348/117836)。
 ...
 *        在进房前后开启录制，如果未打开麦克风采集，录制任务正常进行，只是不会将数据写入生成的本地文件；只有调用 startAudioCapture{@link #ByteRTCAudio#startAudioCapture} 接口打开麦克风采集后，才会将录制数据写入本地文件。
 ...
 *        + 加入房间前后均可调用。在进房前调用该方法，退房之后，录制任务不会自动停止，需调用stopAudioRecording{@link #ByteRTCAudio#stopAudioRecording} 关闭录制。在进房后调用该方法，退房之后，录制任务会自动被停止。如果加入了多个房间，录制的文件中会包含各个房间的音频。<br>
 *        + 调用该方法后，你会收到 rtcEngine:onAudioRecordingStateUpdate:error_code:{@link #ByteRTCAudioDelegate#rtcEngine:onAudioRecordingStateUpdate:error_code:} 回调。  <br>
 */
 ...
 * @return 方法调用结果：  <br>
 *        + 0: 成功。<br>
 *        + -1：失败。上一次检测未结束，请先调用 stopHardwareEchoDetection{@link #ByteRTCAudio#stopHardwareEchoDetection} 停止检测 后重新调用本接口。
 ...
 *        + 调用本接口后监听 rtcEngine:onHardwareEchoDetectionResult:{@link #ByteRTCAudioDelegate#rtcEngine:onHardwareEchoDetectionResult:} 获取检测结果。<br>
 *        + 检测期间，进程将独占音频设备，无法使用其他音频设备测试接口： startEchoTest{@link #ByteRTCAudio#startEchoTest、startAudioDeviceRecordTest{@link #ByteRTCAudio#startAudioDeviceRecordTest 或 startAudioPlaybackDeviceTest{@link #ByteRTCAudio#startAudioPlaybackDeviceTest。 <br>
 *        + 调用  stopHardwareEchoDetection{@link #ByteRTCAudio#stopHardwareEchoDetection} 停止检测，释放对音频设备的占用。
*/
 ...
 *       + 建议在收到 rtcEngine:onHardwareEchoDetectionResult:{@link #ByteRTCAudioDelegate#rtcEngine:onHardwareEchoDetectionResult:} 通知的检测结果后，调用本接口停止检测。<br>
 ...
 * @param config 参看 AudioEnhancementConfig{@link #AudioEnhancementConfig}。
 * @notes 此功能默认不开启。
 */
- (void)setCellularEnhancement:(ByteRTCAudioEnhancementConfig* _Nonnull)config;
 ...
 * @notes <br>
 *       + 该方法需要在进房前调用。<br>
 *       + 调用该方法设置本地代理后，SDK 会触发 rtcEngine:onLocalProxyStateChanged:withProxyState:withProxyError:{@link #ByteRTCAudioDelegate#rtcEngine:onLocalProxyStateChanged:withProxyState:withProxyError:} ，返回代理连接的状态。
 */
