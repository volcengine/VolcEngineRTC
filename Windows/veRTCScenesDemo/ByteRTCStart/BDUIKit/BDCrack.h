...
// More benefit than BDMsg.h
// 
// Note about message maps with cracked handlers:
// you can use BEGIN_MSG_MAP for BDWndImpl derived classes,
// but must use BEGIN_MSG_MAP_EX for classes that don't derive from BDWndImpl.
//
//  Example:
//  BEGIN_MSG_MAP_EX(BDXXWindow)
//     MSG_WM_CREATE(xxxFunc)
//  END_MSG_MAP()
...
// BOOL OnInitDialog(DWnd wndFocus, LPARAM lInitParam)
...
// BOOL OnCopyData(DWnd wnd, PCOPYDATASTRUCT pCopyDataStruct)
...
#define MSG_WM_SIZE(func) \
    ...
    { \
        this->SetMsgHandled(TRUE); \
        func((UINT)wParam, BDSize(LOWORD(lParam), HIWORD(lParam))); \
        lResult = 0; \
        if(this->IsMsgHandled()) \
            return TRUE; \
    }

// void OnActivate(UINT nState, BOOL bMinimized, DWnd wndOther)
...
// void OnKillFocus(DWnd wndFocus)
...
// HBRUSH OnCtlColorEdit(DDCHandle dc, CEdit edit)
...
// HBRUSH OnCtlColorListBox(DDCHandle dc, CListBox listBox)
...
// HBRUSH OnCtlColorBtn(DDCHandle dc, CButton button)
...
// HBRUSH OnCtlColorDlg(DDCHandle dc, DWnd wnd)
...
// HBRUSH OnCtlColorScrollBar(DDCHandle dc, DScrollBar scrollBar)
...
// HBRUSH OnCtlColorStatic(DDCHandle dc, DStatic wndStatic)
...
// BOOL OnSetCursor(BDWnd wnd, UINT nHitTest, UINT message)
...
// int OnMouseActivate(BDWnd wndTopLevel, UINT nHitTest, UINT message)
...
// void OnIconEraseBkgnd(BDDCHandle dc)
...
#define MSG_WM_NCHITTEST(func) \
    ...
    { \
        this->SetMsgHandled(TRUE); \
        lResult = (LRESULT)func(BDPoint(MAKEPOINTS(lParam).x, MAKEPOINTS(lParam).y)); \
        if(this->IsMsgHandled()) \
            return TRUE; \
    }
...
// void OnNcMouseMove(UINT nHitTest, BDPoint point)
#define MSG_WM_NCMOUSEMOVE(func) \
    if (uMsg == WM_NCMOUSEMOVE) \
    { \
        this->SetMsgHandled(TRUE); \
        func((UINT)wParam, DPoint(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam))); \
        lResult = 0; \
        if(this->IsMsgHandled()) \
            return TRUE; \
    }

// void OnNcLButtonDown(UINT nHitTest, BDPoint point)
...
// void OnNcLButtonUp(UINT nHitTest, BDPoint point)
#define MSG_WM_NCLBUTTONUP(func) \
    if (uMsg == WM_NCLBUTTONUP) \
    { \
        this->SetMsgHandled(TRUE); \
        func((UINT)wParam, BDPoint(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam))); \
        lResult = 0; \
        if(this->IsMsgHandled()) \
            return TRUE; \
    }

// void OnNcLButtonDblClk(UINT nHitTest, BDPoint point)
...
// void OnNcRButtonDown(UINT nHitTest, BDPoint point)
...
// void OnNcRButtonUp(UINT nHitTest, BDPoint point)
...
// void OnNcRButtonDblClk(UINT nHitTest, BDPoint point)
...
// void OnNcMButtonDown(UINT nHitTest, BDPoint point)
...
// void OnNcMButtonUp(UINT nHitTest, BDPoint point)
...
// void OnNcMButtonDblClk(UINT nHitTest, DBPoint point)
...
// void OnHScroll(UINT nSBCode, UINT nPos, DScrollBar pScrollBar)
...
// void OnVScroll(UINT nSBCode, UINT nPos, DScrollBar pScrollBar)
...
// void OnEnterIdle(UINT nWhy, DWnd wndWho)
...
#define MSG_WM_MOUSEMOVE(func) \
    ...
    { \
        this->SetMsgHandled(TRUE); \
        func((UINT)wParam, BDPoint(LOWORD(lParam), HIWORD(lParam))); \
        lResult = 0; \
        if(this->IsMsgHandled()) \
            return TRUE; \
    }
...
#define MSG_WM_MOUSEWHEEL(func) \
    ...
    { \
        this->SetMsgHandled(TRUE); \
        lResult = (LRESULT)func((UINT)LOWORD(wParam), (short)HIWORD(wParam), BDPoint(((int)(short)LOWORD(lParam)), ((int)(short)HIWORD(lParam)))); \
        if(this->IsMsgHandled()) \
            return TRUE; \
    }
...
// void OnLButtonDblClk(UINT nFlags, BDPoint point)
...
// void OnRButtonDown(UINT nFlags, BDPoint point)
...
// void OnRButtonUp(UINT nFlags, BDPoint point)
...
// void OnRButtonDblClk(UINT nFlags, BDPoint point)
...
// void OnMButtonDown(UINT nFlags, DPoint point)
...
// void OnMButtonUp(UINT nFlags, DPoint point)
...
// void OnMButtonDblClk(UINT nFlags, DPoint point)
...
// void OnMDIActivate(DWnd wndActivate, DWnd wndDeactivate)
...
// void OnPaintClipboard(DWnd wndViewer, const LPPAINTSTRUCT lpPaintStruct)
...
// void OnVScrollClipboard(DWnd wndViewer, UINT nSBCode, UINT nPos)
...
// void OnContextMenu(DWnd wnd, DPoint point)
#define MSG_WM_CONTEXTMENU(func) \
    if (uMsg == WM_CONTEXTMENU) \
    { \
        this->SetMsgHandled(TRUE); \
        func((HWND)wParam, DPoint(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam))); \
        lResult = 0; \
        if(this->IsMsgHandled()) \
            return TRUE; \
    }

// void OnSizeClipboard(DWnd wndViewer, const LPRECT lpRect)
...
// void OnChangeCbChain(DWnd wndRemove, DWnd wndAfter)
...
// void OnHScrollClipboard(DWnd wndViewer, UINT nSBCode, UINT nPos)
...
// void OnPaletteChanged(DWnd wndFocus)
...
// void OnPaletteIsChanging(DWnd wndPalChg)
...
// void OnCaptureChanged(DWnd wnd)
...
// void OnCommand(UINT uNotifyCode, int nID, DWnd wndCtl)
...
#define MSG_WM_DISPLAYCHANGE(func) \
    ...
    { \
        this->SetMsgHandled(TRUE); \
        func((UINT)wParam, DSize(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam))); \
        lResult = 0; \
        if(this->IsMsgHandled()) \
            return TRUE; \
    }
...
// int OnNotifyFormat(DWnd wndFrom, int nCommand)
...
// void OnPrint(DDCHandle dc, UINT uFlags)
...
// void OnPrintClient(DDCHandle dc, UINT uFlags)
...
#define MSG_WM_MOUSEHOVER(func) \
    ...
    { \
        this->SetMsgHandled(TRUE); \
        func(wParam, DPoint(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam))); \
        lResult = 0; \
        if(this->IsMsgHandled()) \
            return TRUE; \
    }
...
// BOOL OnAppCommand(DWnd wndFocus, short cmd, WORD uDevice, int dwKeys)
...
#define MSG_WM_NCXBUTTONDOWN(func) \
    ...
    { \
        this->SetMsgHandled(TRUE); \
        func(GET_XBUTTON_WPARAM(wParam), GET_NCHITTEST_WPARAM(wParam), DPoint(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam))); \
...
#define MSG_WM_XBUTTONDOWN(func) \
    ...
    { \
        this->SetMsgHandled(TRUE); \
        func(GET_XBUTTON_WPARAM(wParam), GET_KEYSTATE_WPARAM(wParam), DPoint(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam))); \
        lResult = (LRESULT)TRUE; \
        if(this->IsMsgHandled()) \
            return TRUE; \
    }
...
// void OnReflectedCommand(UINT uNotifyCode, int nID, DWnd wndCtl)
...
// void OnReflectedHScroll(UINT nSBCode, UINT nPos, DScrollBar pScrollBar)
...
// void OnReflectedVScroll(UINT nSBCode, UINT nPos, DScrollBar pScrollBar)
...
// HBRUSH OnReflectedCtlColorEdit(DDCHandle dc, CEdit edit)
...
// HBRUSH OnReflectedCtlColorListBox(DDCHandle dc, CListBox listBox)
...
// HBRUSH OnReflectedCtlColorBtn(DDCHandle dc, CButton button)
...
// HBRUSH OnReflectedCtlColorDlg(DDCHandle dc, DWnd wnd)
...
// HBRUSH OnReflectedCtlColorScrollBar(DDCHandle dc, DScrollBar scrollBar)
...
// HBRUSH OnReflectedCtlColorStatic(DDCHandle dc, DStatic wndStatic)
...
// void OnCommandHandlerEX(UINT uNotifyCode, int nID, DWnd wndCtl)
...
// void OnCommandIDHandlerEX(UINT uNotifyCode, int nID, DWnd wndCtl)
...
// void OnCommandCodeHandlerEX(UINT uNotifyCode, int nID, DWnd wndCtl)
...
// void OnCommandRangeHandlerEX(UINT uNotifyCode, int nID, DWnd wndCtl)
...
// void OnCommandRangeCodeHandlerEX(UINT uNotifyCode, int nID, DWnd wndCtl)
...
// LRESULT OnReflectedCommandHandlerEX(UINT uNotifyCode, int nID, DWnd wndCtl)
...
// LRESULT OnReflectedCommandIDHandlerEX(UINT uNotifyCode, int nID, DWnd wndCtl)
...
// LRESULT OnReflectedCommandCodeHandlerEX(UINT uNotifyCode, int nID, DWnd wndCtl)
...
// void OnReflectedCommandRangeHandlerEX(UINT uNotifyCode, int nID, DWnd wndCtl)
...
// void OnReflectedCommandRangeCodeHandlerEX(UINT uNotifyCode, int nID, DWnd wndCtl)
...
// void OnAppCommandHandler(UINT uDevice, DWORD dwKeys, DWnd wndFocus)
