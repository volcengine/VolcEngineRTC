...
namespace sio
{
    ...
    void client_impl::connect(const string& uri, const std::string &path, const map<string,string>& query, const map<string, string>& headers)
    {
        if(m_reconn_timer)
        {
        ...
        this->reset_states();
        m_client.get_io_service().dispatch(std::bind(&client_impl::connect_impl,this,uri,path,m_query_string));
    ...
    void client_impl::connect_impl(const string& uri, const std::string &path, const string& queryString)
    {
        do{
    ...
    void client_impl::timeout_reconnect(asio::error_code const& ec)
    {
        ...
        if(m_con_state == con_closed)
        {
            ...
            m_client.get_io_service().dispatch(std::bind(&client_impl::connect_impl,this,m_base_url, m_base_path,m_query_string));
        }
    }
