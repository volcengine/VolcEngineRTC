 ...
 * @param alphaLayout 设置Alpha通道数据的布局位置，当前仅支持ByteRTCAlphaLayoutTop
 * @return  <br>
 *         + 0：成功。  <br>
 *         + !0：失败。
 * @notes <br>
 *        + 建议在发布视频流之前调用此接口。
 *        + 开启此接口后，需要配合自定义视频采集 pushExternalVideoFrame把RGBA相关视频帧推送至SDK。
 *        + 当前支持的视频帧格式有：ByteRTCVideoPixelFormatCVPixelBuffer(内部存储BGRA)
 */
