...
public:
    ...
    */
     ...
     * @type api
     * @region 视频管理
     * @brief 开启带有Alpha通道的视频帧编码支持。
     * @param [in] index 视频流属性，参看 StreamIndex{@link #StreamIndex}，当前仅支持kStreamIndexMain
     * @param [in] layout 设置Alpha通道数据的布局位置，参看 AlphaLayout{@link #AlphaLayout}，当前仅支持kAlphaLayoutTop
     * @return  <br>
     *        + 0: 调用成功。
     *        + < 0 : 调用失败。查看 ReturnStatus{@link #ReturnStatus} 获得更多错误说明
     * @notes <br>
     *        + 建议在发布视频流之前调用此接口。
     *        + 开启此接口后，需要配合自定义视频采集 pushExternalVideoFrame{@link #IRTCVideo#pushExternalVideoFrame} 把RGBA相关视频帧推送至SDK。
     *        + 当前支持的视频帧格式有：
     *        +   Windows: kVideoPixelFormatRGBA
     *        +   iOS: kVideoPixelFormatBGRA
     *        +   Android: kVideoPixelFormatTexture2D、kVideoPixelFormatTextureOES、kVideoPixelFormatRGBA
     */
    virtual int enableAlphaChannelVideoEncode(StreamIndex index, AlphaLayout layout) = 0;
     ...
     *        + 建议在停止发布视频流之后调用此接口。 
     */
    virtual int disableAlphaChannelVideoEncode(StreamIndex index) = 0;
