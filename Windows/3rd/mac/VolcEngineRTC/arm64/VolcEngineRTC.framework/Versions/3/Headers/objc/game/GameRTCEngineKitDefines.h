 ...
 * @brief 音量衰减模型
 */
typedef NS_ENUM(NSInteger, GameAttenuationType) {
    /**
     * @brief 启用范围语音但不根据距离做衰减
     *
     */
    GameAttenuationTypeNone = 0,
    /**
     * @brief 启用范围语音，根据距离做线性衰减
     *
     */
    GameAttenuationTypeLinear = 1,
    /**
     * @brief 启用范围语音，根据距离做非线性衰减
     *
     */
    GameAttenuationTypeExponential = 2,
};
 ...
 *        + true：开启（默认设置） <br>
 *        + false: 关闭 <br>
 * 参与通话的各端开启本选项后，将忽略远端用户的朝向，认为所有远端用户都面向本地用户。<br>
 * 如果此后更新本地用户朝向，远端用户无法感知这些变化，但本地用户接收音频时可以感知自身朝向改变带来的音频效果变化。
 */
@property (nonatomic, assign) BOOL enableRemoteOrient;
/**
 ...
 * @brief 描述空间语音中玩家的方位
 */
GAME_RTC_EXPORT @interface GameRTCOrientation : NSObject
/**
 * @brief 方向在 x 轴的投影
 */
...
typedef NS_ENUM(NSInteger, GameRTCConnectionState) {
     ...
     */
    GameRTCConnectionStateFailed = 6,
};
 ...
 * @brief 加入房间错误码
 */
typedef NS_ENUM(NSInteger, GameRTCRoomStateCode) {
    /**
     * @brief 加入房间成功
     */
 ...
 * @brief 流状态错误码
 */
typedef NS_ENUM(NSInteger, GameRTCStreamStateCode) {
    /**
     *  @brief 发布音视频流失败。  <br>
     *         用户在所在房间中发布音视频流时，由于服务器错误导致发布失败。SDK 会自动重试发布。
     */
    GameRTCStreamStateCodePublishStreamFailed       = -2002,
    /**
     *  @brief 订阅音视频流失败。  <br>
     *         当前房间中找不到订阅的音视频流导致订阅失败。建议用户退出重试。
     */
    GameRTCStreamStateCodeStreamFailed404           = -2003,
    /**
     *  @brief 订阅音视频流失败。  <br>
     *         用户订阅所在房间中的音视频流时，由于服务器错误导致订阅失败。SDK 会自动重试订阅。
     */
    GameRTCStreamStateCodeStreamFailed5xx           = -2004,
    /**
     *  @hidden
     *  @brief 调度异常，服务器返回的媒体服务器地址不可用。
    */
    GameRTCStreamStateCodeExpectMediaServerAddress  = -2007,
    /**
     * @brief 因没有发布音频流权限，导致在房间中发布音视频流失败。  <br>
     *        需要检查 appId 是否正确。
     */
    GameRTCStreamStateCodeNoPublishPermission         = -1002,
    /**
     * @brief 因没有订阅音频流权限，导致在房间中订阅音视频流失败。  <br>
     *        需要检查 appId 是否正确。
     */
    GameRTCStreamStateCodeNoSubscribePermission       = -1003,
};
