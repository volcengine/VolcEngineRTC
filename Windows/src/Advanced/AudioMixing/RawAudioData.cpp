...
void RawAudioData::updateAudioCaptureList()
{
    ...
    ui->widget_log->appendAPI("enumerateAudioCaptureDevices");
}
...
void RawAudioData::onComboDeviceIndexChanged(int index)
{
    ...
    ui->widget_log->appendAPI("setAudioCaptureDevice");
}
...
void RawAudioData::onCheckRecordChanged(int)
{
    ...
    if (ui->checkBox_record->checkState() == Qt::Checked) {
        ...
        int ret = m_video->enableAudioFrameCallback(bytertc::AudioFrameCallbackMethod::kAudioFrameCallbackRecord, format); //麦克风
        if (ret < 0) {
        ...
        ui->widget_log->appendAPI("enableAudioFrameCallback");
    } else {
        m_video->disableAudioFrameCallback(bytertc::AudioFrameCallbackMethod::kAudioFrameCallbackRecord);
        ui->widget_log->appendAPI("disableAudioFrameCallback");
    }
}
...
void RawAudioData::onCheckPlaybackChanged(int)
{
    ...
    if (ui->checkBox_playback->checkState() == Qt::Checked) {
        ...
        format.sample_rate = bytertc::kAudioSampleRate48000;
        format.samples_per_call = 480;
        int ret = m_video->enableAudioFrameCallback(bytertc::AudioFrameCallbackMethod::kAudioFrameCallbackPlayback, format);
        if (ret < 0) {
            QMessageBox box(QMessageBox::Warning, QStringLiteral("提示"), QString("enableAudioFrameCallback error:") + QString::number(ret), QMessageBox::Ok);
            box.exec();
        }
        ...
        m_video->disableAudioFrameCallback(bytertc::AudioFrameCallbackMethod::kAudioFrameCallbackPlayback);
        ui->widget_log->appendAPI("disableAudioFrameCallback");
    }
}
...
void RawAudioData::onCheckMixedChanged(int)
{
    ...
    if (ui->checkBox_mixed->checkState() == Qt::Checked) {
        ...
        format.sample_rate = bytertc::kAudioSampleRate48000;
        format.samples_per_call = 480;
        int ret = m_video->enableAudioFrameCallback(bytertc::AudioFrameCallbackMethod::kAudioFrameCallbackMixed, format); //麦克风
        if (ret < 0) {
            QMessageBox box(QMessageBox::Warning, QStringLiteral("提示"), QString("enableAudioFrameCallback error:") + QString::number(ret), QMessageBox::Ok);
            box.exec();
        }
        ...
        m_video->disableAudioFrameCallback(bytertc::AudioFrameCallbackMethod::kAudioFrameCallbackMixed);
        ui->widget_log->appendAPI("disableAudioFrameCallback");
    }
}
...
void RawAudioData::onCheckRemoteUserChanged(int)
{
    ...
    if (ui->checkBox_remote->checkState() == Qt::Checked) {
        ...
        int ret = m_video->enableAudioFrameCallback(bytertc::AudioFrameCallbackMethod::kAudioFrameCallbackRemoteUser, format); //麦克风
        if (ret < 0) {
            QMessageBox box(QMessageBox::Warning, QStringLiteral("提示"), QString("enableAudioFrameCallback error:") + QString::number(ret), QMessageBox::Ok);
            box.exec();
        }
        ...
        m_video->disableAudioFrameCallback(bytertc::AudioFrameCallbackMethod::kAudioFrameCallbackRemoteUser);
        ui->widget_log->appendAPI("disableAudioFrameCallback");
    }
}
