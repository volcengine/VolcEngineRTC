...
public:
     ...
     * @param effect_id  <br>
     *        音效ID  <br>
     *        使用 AudioEffectPlayer 相关API时传入的对应ID
     * @param state  <br>
     *        混音状态  <br>
...
public:
     ...
     * @param effect_id 混音 ID。用于标识混音，请保证混音 ID 唯一性。  <br>
     ...
     *        可以设置混音的播放次数、起始播放位置、音调值、是否本地播放混音、以及是否将混音发送至远端，详见 AudioEffectPlayerConfig{@link #AudioEffectPlayerConfig}
     * @notes  <br>
     ...
     * @param effect_id 需调节音量的混音 ID
     * @param volume 混音音量相对原音量的比值。范围为 `[0, 400]`，建议范围是 `[0, 100]`。  <br>
     *        + 0：静音  <br>
     *        + 100：原始音量（默认值）  <br>
     *        + 400: 最大可调音量 (自带溢出保护)
     * @notes 该方法仅对指定混音 ID 生效，若想调节全部混音文件播放音量，则需调用 setVolumeAll{@link #IAudioEffectPlayer#setVolumeAll}。该方法默认调节start{@link #IAudioEffectPlayer#start}时设置的AudioMixingType{@link #AudioMixingType}类型音量。
     */
     ...
     * @notes 该接口的优先级低于 setVolume{@link #IAudioEffectPlayer#setVolume}，即通过 setVolume{@link #IAudioEffectPlayer#setVolume} 单独设置了音量的混音 ID，不受该接口设置的影响。
